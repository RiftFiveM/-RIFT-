...
exports.runcmd = (fivemexports, client, message, params) => {
    ...
    fivemexports.ghmattimysql.execute("SELECT user_id FROM `Polar_verification` WHERE discord_id = ?", [user.id], (result) => {
        if (result.length > 0) {
            fivemexports.ghmattimysql.execute("SELECT * FROM Polar_user_moneys WHERE user_id = ?", [result[0].user_id], (result) => {
