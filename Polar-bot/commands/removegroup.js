...
exports.runcmd = (fivemexports, client, message, params) => {
    if (!params[0] || !parseInt(params[0]) || !params[1]) {
        return message.reply('Invalid args! Correct term is: ' + process.env.PREFIX + 'delgroup [permid] [group name]')
    }
    fivemexports.ghmattimysql.execute("SELECT * FROM `Polar_user_data` WHERE user_id = ?", [params[0]], (result) => {
        if (result.length > 0) {
            ...
            fivemexports.ghmattimysql.execute("UPDATE `Polar_user_data` SET dvalue = ? WHERE user_id = ?", [JSON.stringify(dvalue), params[0]])
        }
    })
    let embed = {
        "title": "Removed Group",
