...
exports.runcmd = (fivemexports, client, message, params) => {
    ...
    let count = 0
    fivemexports.ghmattimysql.execute("SELECT * FROM `Polar_user_vehicles` WHERE rentedid = ?", [params[0]], (result) => {
        if (result.length > 0) {
        var rentedvehicles = []
        ...
        for (i = 0; i < results; i++) {
            ...
            fivemexports.ghmattimysql.execute("SELECT * FROM `Polar_users` WHERE id = ?", [renter], (result) => {
                a = new Date()
