...
exports.runcmd = async(fivemexports, client, message, params) => {
    ...
    fivemexports.ghmattimysql.execute("SELECT * FROM `Polar_verification` WHERE code = ?", [params[0]], (code) => {
        if (code.length > 0) {
           if (code[0].discord_id === null ){
            fivemexports.ghmattimysql.execute("UPDATE `Polar_verification` SET discord_id = ?, verified = 1 WHERE code = ?", [message.author.id, params[0]], async (result) => {
                if (result) {
                    let embed = {
                        "title": "Verify",
