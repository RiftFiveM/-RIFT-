...
exports.runcmd = (fivemexports, client, message, params) => {
    ...
    let count = 0
    fivemexports.ghmattimysql.execute("SELECT * FROM `Polar_user_vehicles` WHERE vehicle = ?", [params[0]], (result) => {
        var owners = []
        for (i = 0; i < result.length; i++) { 
            ...
            fivemexports.ghmattimysql.execute("SELECT * FROM `Polar_users` WHERE id = ?", [user_id], (result) => {
