...
exports.runcmd = (fivemexports, client, message, params) => {
    if (!params[0]) {
        fivemexports.ghmattimysql.execute("SELECT user_id FROM `Polar_verification` WHERE discord_id = ?", [message.author.id], (discord) => {
            if (discord.length > 0) {
                fivemexports.ghmattimysql.execute("SELECT * FROM `Polar_user_data` WHERE user_id = ?", [discord[0].user_id], (result) => {
                    if (result.length > 0) {
                        let embed = {
