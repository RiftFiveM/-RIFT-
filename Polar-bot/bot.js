...
if (settingsjson.settings.StatusEnabled) {
    setInterval(() => {
        ...
        exports.ghmattimysql.execute("SELECT * FROM `Polar_user_moneys`", (result) => {
            playersSinceRelease = result.length
        });
        exports.Polar.Polarbot('getUsersByPermission', ['admin.tickets'], function(result) {
            if (!result.length)
                onlineStaff = 0
            else
                onlineStaff = result.length
        })
        exports.Polar.Polarbot('getUsersByPermission', ['police.onduty.permission'], function(result) {
            if (!result.length)
                onlinePD = 0
            else
                onlinePD = result.length
        })
        exports.Polar.Polarbot('getUsersByPermission', ['nhs.onduty.permission'], function(result) {
            if (!result.length)
                onlineNHS = 0
            else
                onlineNHS = result.length
        })
        exports.Polar.getServerStatus([], function(result) {
            serverStatus = result
        })
        channelid.fetchMessage(settingsjsons.messageid).then(msg => {
            let status = {
                ...
                "title": "<:Polar:1124621326074773554> Polar Server #1 Status",
                "footer": {
                    "text": `${currentFooterEmoji} Polar`
                },
                "timestamp": new Date()
            }
...
exports('dmUser', (source, args) => {
    ...
    try {
        let embed = {
            ...
            "color": settingsjson.settings.botColour,
            "thumbnail": {
                "url": "https://cdn.discordapp.com/icons/1167224647142621236/719d25a3f8b4852159905244bfed520b.webp?size=2048",
            },
        }
        member.send({embed})
    } catch (error) {}
