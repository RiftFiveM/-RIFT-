  ...
  getUser(username, password, function (user)
    ...
    MySQL.Async.insert("INSERT INTO twitter_tweets (`authorId`, `message`, `realUser`) VALUES(@authorId, @message, @realUser);", {
      ...
      MySQL.Async.fetchAll('SELECT * from twitter_tweets WHERE id = @id', {
        ...
        TriggerClientEvent('RIFT:twitter_newTweets', -1, tweet)
        TriggerEvent('RIFT:twitter_newTweets', tweet)
      end)
    end)
  end)
end
        ...
        if (row[1] == nil) then
          MySQL.Async.insert('INSERT INTO twitter_likes (`authorId`, `tweetId`) VALUES(@authorId, @tweetId)', {
            ...
            MySQL.Async.execute('UPDATE `twitter_tweets` SET `likes`= likes + 1 WHERE id = @id', {
              ...
              TriggerClientEvent('RIFT:twitter_updateTweetLikes', -1, tweet.id, tweet.likes + 1)
              TriggerClientEvent('RIFT:twitter_setTweetLikes', sourcePlayer, tweet.id, true)
              TriggerEvent('RIFT:twitter_updateTweetLikes', tweet.id, tweet.likes + 1)
            end)    
          end)
        else
          MySQL.Async.execute('DELETE FROM twitter_likes WHERE id = @id', {
            ...
            MySQL.Async.execute('UPDATE `twitter_tweets` SET `likes`= likes - 1 WHERE id = @id', {
              ...
              TriggerClientEvent('RIFT:twitter_updateTweetLikes', -1, tweet.id, tweet.likes - 1)
              TriggerClientEvent('RIFT:twitter_setTweetLikes', sourcePlayer, tweet.id, false)
              TriggerEvent('RIFT:twitter_updateTweetLikes', tweet.id, tweet.likes - 1)
            end)
          end)
        end
...
function TwitterShowError (sourcePlayer, title, message)
  TriggerClientEvent('RIFT:twitter_showError', sourcePlayer, message)
end
function TwitterShowSuccess (sourcePlayer, title, message)
  TriggerClientEvent('RIFT:twitter_showSuccess', sourcePlayer, title, message)
end

RegisterServerEvent('RIFT:twitter_login')
AddEventHandler('RIFT:twitter_login', function(username, password)
  ...
  getUser(username, password, function (user)
    if user == nil then
      ...
      TriggerClientEvent('RIFT:twitter_setAccount', sourcePlayer, username, password, user.authorIcon)
    end
  end)
end)

RegisterServerEvent('RIFT:twitter_changePassword')
AddEventHandler('RIFT:twitter_changePassword', function(username, password, newPassword)
  local sourcePlayer = tonumber(source)
  getUser(username, password, function (user)
    if user == nil then
      ...
      MySQL.Async.execute("UPDATE `twitter_accounts` SET `password`= @newPassword WHERE twitter_accounts.username = @username AND twitter_accounts.password = @password", {
        ...
        if (result == 1) then
          TriggerClientEvent('RIFT:twitter_setAccount', sourcePlayer, username, newPassword, user.authorIcon)
...
RegisterServerEvent('RIFT:twitter_createAccount')
AddEventHandler('RIFT:twitter_createAccount', function(username, password, avatarUrl)
  local sourcePlayer = tonumber(source)
  TwitterCreateAccount(username, password, avatarUrl, function (id)
    if (id ~= 0) then
      TriggerClientEvent('RIFT:twitter_setAccount', sourcePlayer, username, password, avatarUrl)
...
RegisterServerEvent('RIFT:twitter_getTweets')
AddEventHandler('RIFT:twitter_getTweets', function(username, password)
  local sourcePlayer = tonumber(source)
  if username ~= nil and username ~= "" and password ~= nil and password ~= "" then
    getUser(username, password, function (user)
      local accountId = user and user.id
      TwitterGetTweets(accountId, function (tweets)
        TriggerClientEvent('RIFT:twitter_getTweets', sourcePlayer, tweets)
      end)
    end)
  else
...
RegisterServerEvent('RIFT:twitter_getFavoriteTweets')
AddEventHandler('RIFT:twitter_getFavoriteTweets', function(username, password)
  local sourcePlayer = tonumber(source)
  if username ~= nil and username ~= "" and password ~= nil and password ~= "" then
    getUser(username, password, function (user)
      local accountId = user and user.id
      TwitterGetFavotireTweets(accountId, function (tweets)
        TriggerClientEvent('RIFT:twitter_getFavoriteTweets', sourcePlayer, tweets)
      end)
    end)
  else
...
RegisterServerEvent('RIFT:twitter_postTweets')
AddEventHandler('RIFT:twitter_postTweets', function(username, password, message)
  local sourcePlayer = tonumber(source)
  local srcIdentifier = getPlayerID(source)
  TwitterPostTweet(username, password, message, sourcePlayer, srcIdentifier)
end)

RegisterServerEvent('RIFT:twitter_toogleLikeTweet')
AddEventHandler('RIFT:twitter_toogleLikeTweet', function(username, password, tweetId)
  local sourcePlayer = tonumber(source)
  TwitterToogleLike(username, password, tweetId, sourcePlayer)
end)


RegisterServerEvent('RIFT:twitter_setAvatarUrl')
AddEventHandler('RIFT:twitter_setAvatarUrl', function(username, password, avatarUrl)
  local sourcePlayer = tonumber(source)
