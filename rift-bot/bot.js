...
if (settingsjson.settings.StatusEnabled) {
    setInterval(() => {
        ...
        exports.ghmattimysql.execute("SELECT * FROM `rift_user_moneys`", (result) => {
            playersSinceRelease = result.length
        });
        exports.rift.riftbot('getUsersByPermission', ['admin.tickets'], function(result) {
            if (!result.length)
                onlineStaff = 0
            else
                onlineStaff = result.length
        })
        exports.rift.riftbot('getUsersByPermission', ['police.onduty.permission'], function(result) {
            if (!result.length)
                onlinePD = 0
            else
...
client.on('message', (message) => {
    ...
    if (cmd) {
        if (message.guild.id === cmd.conf.guild) {
            ...
            if (cmd.conf.support && message.guild.id === "1147954594903761036"){
...
client.on("guildMemberAdd", function (member) {
    if (member.guild.id === settingsjson.settings.GuildID){
        try {
            exports.ghmattimysql.execute("SELECT * FROM `rift_verification` WHERE discord_id = ? AND verified = 1", [member.id], (result) => {
                if (result.length > 0){
