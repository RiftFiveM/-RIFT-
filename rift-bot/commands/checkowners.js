...
exports.runcmd = (fivemexports, client, message, params) => {
    ...
    let count = 0
    fivemexports.ghmattimysql.execute("SELECT * FROM `rift_user_vehicles` WHERE vehicle = ?", [params[0]], (result) => {
        var owners = []
        for (i = 0; i < result.length; i++) { 
            ...
            fivemexports.ghmattimysql.execute("SELECT * FROM `rift_users` WHERE id = ?", [user_id], (result) => {
