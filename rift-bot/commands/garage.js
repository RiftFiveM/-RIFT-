...
exports.runcmd = (fivemexports, client, message, params) => {
    ...
    let count = 0
    fivemexports.ghmattimysql.execute("SELECT * FROM `rift_user_vehicles` WHERE user_id = ?", [params[0]], (result) => {
        var vehicles = []
        for (i = 0; i < result.length; i++) {
            let results =  result.length
            let vehicle = result[i]; // get the current vehicle
            fivemexports.ghmattimysql.execute("SELECT * FROM rift_user_vehicles WHERE vehicle = ?", [vehicle.vehicle.toLowerCase()], (res) => {
