...
cocaine_sniff["Take"] = {function(player,choice)
  ...
  if user_id ~= nil then
    if Polar.tryGetInventoryItem(user_id,"Cocaine",1) then
      Polarclient.notify(player,{"~g~Taking Cocaine."})
      TriggerEvent('Polar:RefreshInventory', player)
      TriggerClientEvent('Polar:cocaineEffect', player)
    end
  end
end}
...
heroin_take["Take"] = {function(player,choice)
  local user_id = Polar.getUserId(player)
  if user_id ~= nil then
    if Polar.tryGetInventoryItem(user_id,"Heroin",1) then
      Polarclient.notify(player,{"~g~Taking Heroin."})
      TriggerEvent('Polar:RefreshInventory', player)
      TriggerClientEvent('Polar:heroinEffect', player)
    end
  end
end}
...
lsd_take["Take"] = {function(player,choice)
  local user_id = Polar.getUserId(player)
  if user_id ~= nil then
    if Polar.tryGetInventoryItem(user_id,"LSD",1) then
      Polarclient.notify(player,{"~g~Taking LSD."})
      TriggerEvent('Polar:RefreshInventory', player)
      TriggerClientEvent('Polar:doAcid', player)
    end
  end
end}
...
morphine_choices["Take"] = {function(player,choice)
  local user_id = Polar.getUserId(player)
  if user_id ~= nil then
    if Polar.tryGetInventoryItem(user_id,"Morphine",1) then
      TriggerEvent('Polar:RefreshInventory', player)
      TriggerClientEvent('Polar:applyMorphine', player)
    end
  end
end}
...
taco_choices["Take"] = {function(player,choice)
  local user_id = Polar.getUserId(player)
  if user_id ~= nil then
    if Polar.tryGetInventoryItem(user_id,"Taco",1) then
      TriggerEvent('Polar:RefreshInventory', player)
      TriggerClientEvent('Polar:eatTaco', player)
    end
  end
end}
