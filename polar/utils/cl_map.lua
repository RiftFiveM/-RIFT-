globalBlips = {}
function tPolar.addBlip(a, b, c, d, e, f, g, h)
...
function tPolar.removeBlip(j)
    RemoveBlip(j)
end
function tPolar.removeBlipAtCoords(x,y,z) -- used to remove green house blips for everyone when purchased
...
local k = {}
function tPolar.setNamedBlip(l, a, b, c, d, e, f, g)
    tPolar.removeNamedBlip(l)
    k[l] = tPolar.addBlip(a, b, c, d, e, f, g)
    return k[l]
end
function tPolar.removeNamedBlip(l)
    if k[l] ~= nil then
        tPolar.removeBlip(k[l])
        k[l] = nil
    end
end
function tPolar.setGPS(a, b)
    SetNewWaypoint(a + 0.0001, b + 0.0001)
end
function tPolar.setBlipRoute(j)
    SetBlipRoute(j, true)
end
local m = {}
...
local p = {}
local q = {}
function tPolar.addMarker(a, b, c, r, s, t, u, v, w, x, y, z, A, B, C, D, E, F, G, H)
    local I = {
...
function tPolar.removeMarker(j)
    if m[j] ~= nil then
        m[j] = nil
        n:free(j)
    end
...
function tPolar.setNamedMarker(l, a, b, c, r, s, t, u, v, w, x, y, z, A, B, C, D, E, F, G, H)
    tPolar.removeNamedMarker(l)
    o[l] = tPolar.addMarker(a, b, c, r, s, t, u, v, w, x, y, z, A, B, C, D, E, F, G, H)
    return o[l]
end
function tPolar.removeNamedMarker(l)
    if o[l] ~= nil then
        tPolar.removeMarker(o[l])
        o[l] = nil
    end
end
local J = {}
...
Citizen.CreateThread(
    function()
        while true do
            local M = tPolar.getPlayerCoords()
            J = {}
...
Citizen.CreateThread(
    function()
        while true do
            q = tPolar.getNearbyMarkers()
            Citizen.Wait(10000)
        end
    end
)
function tPolar.getNearbyMarkers()
    local N = {}
    local O = tPolar.getPlayerCoords()
    local P = 0
...
local Q = {}
local R = {}
function tPolar.getNearbyAreas()
    local S = {}
    local M = tPolar.getPlayerCoords()
    local P = 0
    for K, L in pairs(Q) do
...
function tPolar.setArea(l, a, b, c, T, U)
...
RegisterCommand("nextblip",function()
    a5 = a5 + 1
    if a4 then
        tPolar.removeBlip(a4)
    end
    a4 = tPolar.addBlip(1103.9739990234, 211.95138549805, -49.440101623535, a5, 0, "Chips Cashier", 0.8, true)
end)
