...
local h
local i = false
RMenu.Add("scubadiving","main",RageUI.CreateMenu("", "", tPolar.getRageUIMenuWidth(), tPolar.getRageUIMenuHeight(), "banners", "scubajob"))
RMenu:Get("scubadiving", "main"):SetSubtitle("Polar | ~b~Scuba Diving")
local function j()
...
AddEventHandler("Polar:onClientSpawn",function(l, m)
    if m then
        local n = tPolar.createDynamicPed(a.jobPedModel,a.jobPedPosition,a.jobPedHeading,true,"mini@strip_club@idles@bouncer@base","base",100,false,function(o)
        ...
        tPolar.addBlip(a.jobPosition.x, a.jobPosition.y, a.jobPosition.z, 308, 0, "Scuba Diving")
        tPolar.addMarker(a.jobPosition.x,a.jobPosition.y,a.jobPosition.z - 0.2,0.5,0.5,0.5,0,50,255,170,50,20,false,false,true)
        local q = function(r)
            j()
        end
        local s = function(r)
        ...
        tPolar.createArea("scubajob", a.jobPosition, 1.5, 6, q, s, t, {})
    end
end)
        ...
        RageUI.DrawContent({header=true,glare=false,instructionalButton=true},function()
            if not e then
                RageUI.Button("Request Job",nil,{RightLabel = "→→→"},true,function(u, v, w)
                    if w then
                        TriggerServerEvent("Polar:requestScubaJob")
                    end
                end)
            end
            if d ~= 0 then
                RageUI.Button("Claim Reward",nil,{RightLabel = "→→→"},true,function(u, v, w)
                    if w then
                        TriggerServerEvent("Polar:claimScubaReward", c)
                        d = 0
                        c = nil
                        e = false
                        i = false
                        ...
                        tPolar.setCustomization(h)
                        local x = PlayerPedId()
                        SetEnableScuba(x, false)
                        SetPedMaxTimeUnderwater(x, 10.0)
                        tPolar.setMaxUnderWaterUITimenewTime(10.0)
                    end
                end)
            end
        end)
...
local function y()
    for z, A in pairs(b) do
        tPolar.removeMarker(A.objectMarker)
        tPolar.removeArea(A.objectArea)
...
function startScubaDiving()
    tPolar.loadAnimDict("missheistchem2")
    local x = PlayerPedId()
    ...
    tPolar.drawNativeNotification("Press ~INPUT_PICKUP~ to start Scuba Diving.")
    while not F do
    ...
    TriggerServerEvent("Polar:finishScubaJob", c, d)
end
RegisterNetEvent("Polar:createScubaJob",function(J, K)
    c = K
    e = true
    tPolar.disableCustomizationSave(e)
    DoScreenFadeOut(500)
    Wait(600)
    ...
    SetBlipAlpha(L, 180)
    local q = function(r)
        tPolar.drawNativeNotification("Press ~INPUT_DETONATE~ to begin diving!")
    end
    local s = function(r)
    end
    local t = function(r)
        if IsControlJustReleased(1, 47) then
            local x = tPolar.getPlayerPed()
            local M = tPolar.getPlayerVehicle()
            if M ~= 0 then
                if GetEntityModel(M) ~= GetHashKey("dinghy") then
                    tPolar.notify("~r~You must be in a dinghy to start scuba diving!")
                else
                    tPolar.removeArea("scubajob_" .. J.name)
                    f = "~y~Search for treasures and return to your boat when finished"
                    createRewardObjects(J)
                    CreateThread(startScubaDiving)
                end
            else
                tPolar.notify("~r~You must be in a boat to start scuba diving!")
            end
        end
    end
    SetWaypointOff()
    local N = tPolar.addBlip(J.position.x, J.position.y, J.position.z, J.blipId, J.blipColour, "Scuba Diving")
    SetNewWaypoint(J.position.x, J.position.y)
    tPolar.createArea("scubajob_" .. J.name, J.position, 250, 15, q, s, t, {})
    h = tPolar.getCustomization()
    local p = math.random(1, 15)
    local O = PlayerPedId()
    ...
    g = tPolar.spawnVehicle("dinghy",J.dinghySpawnPositions[P].x,J.dinghySpawnPositions[P].y,J.dinghySpawnPositions[P].z,J.dinghySpawnHeading,true,true,true)
    ...
    TriggerServerEvent("Polar:scubaSetVehicle", c, R)
    DoScreenFadeIn(2000)
    SetPedScubaGearVariation(PlayerPedId())
    Citizen.CreateThread(function()
        while e do
            if i then
                ...
                SetWaypointOff()
                TriggerServerEvent("Polar:finishScubaJob", c, 0)
                TriggerServerEvent("Polar:claimScubaReward", c)
                d = 0
                c = nil
                e = false
...
RegisterNetEvent("Polar:ScubaBoatDestroyed",function(S)
    if e and S == c then
        i = true
    end
end)
function createRewardObjects(J)
    for z, A in pairs(J.rewardObjects) do
        local T = tPolar.loadModel(A.objectModel)
        ...
        local V =
            tPolar.addMarker(A.objectPosition.x,A.objectPosition.y,A.objectPosition.z + 0.6,0.5,0.5,0.5,255,223,0,150,50.0,0,false,true,true)
        local q = function(r)
            tPolar.drawNativeNotification("Press ~INPUT_PICKUP~ to collect!")
        end
        local s = function(r)
        end
        local t = function(r)
            if IsControlJustReleased(1, 38) and e then
                tPolar.notify("~g~Item collected.")
                d = d + 1
                tPolar.removeMarker(b[r.objectId].objectMarker)
                tPolar.removeArea(b[r.objectId].objectArea)
        ...
        tPolar.createArea("scubajob_" .. J.name .. "_obj_" .. z, A.objectPosition, 3.0, 3.0, q, s, t, {objectId = z})
        b[z] = {}
        b[z].objectId = U
        b[z].objectMarker = V
