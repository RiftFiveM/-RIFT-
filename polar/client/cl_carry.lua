...
RegisterCommand("carry",function(f, g)
    if IsPedInAnyVehicle(PlayerPedId(), true) then
        ...
        if not globalPlayerInPrisonZone or tPolar.isStaffedOn() then
            ...
            tPolar.notify("~r~You cannot carry in the prison.")
        end
    end
end,false)
RegisterNetEvent("CarryPeople:carryRequest",function()
    tPolar.notify("Someone is trying to carry you, press (~y~Y~w~) to accept (~r~L~w~) to refuse")
    e = true
    Citizen.CreateThread(function()
        while e do
            if IsControlJustPressed(0, 246) then
                tPolar.notify("~g~Request Accepted")
                ExecuteCommand("carryaccept")
                e = false
            elseif IsControlJustPressed(0, 182) then
                tPolar.notify("~r~Request Refused")
...
Citizen.CreateThread(function()
    while true do
        ...
        if carryingBackInProgress then
            ...
            local m, n = tPolar.getPlayerVehicle()
            if m ~= 0 and n then
                TriggerServerEvent("CarryPeople:stop", target)
                carryingBackInProgress = false
                ClearPedTasks(l)
            ...
            if not tPolar.isStaffedOn() and IsPedFalling(l) then
                TriggerServerEvent("CarryPeople:stop", target)
                carryingBackInProgress = false
                ClearPedTasks(l)
                StopAnimTask(l, "missfinale_c2mcs_1", "fin_c2_mcs_1_camman", 0)
...
RegisterNetEvent("CarryPeople:syncTarget",function(target, o, p, q, r, s, t, u, v)
    ...
    AttachEntityToEntity(tPolar.getPlayerPed(), w, 0, r, q, s, 0.5, 0.5, u, false, false, false, false, 2, false)
    if v == nil then
        v = 0
    end
...
function func_carryAnimation()
    if carryingBackInProgress then
        DisablePlayerFiring(tPolar.getPlayerId(), true)
        if not IsEntityPlayingAnim(tPolar.getPlayerPed(), b, a, 3) then
            TaskPlayAnim(tPolar.getPlayerPed(), b, a, 8.0, -8.0, 100000, c, 0, false, false, false)
        end
    end
end
tPolar.createThreadOnTick(func_carryAnimation)
