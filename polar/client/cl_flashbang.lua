...
RegisterNetEvent("Polar:flashbangExplode",function(coords)
    if #(coords - tPolar.getPlayerCoords()) <= b then
...
function explodeFlashbang(coords)
    ...
    TaskPlayAnim(tPolar.getPlayerPed(),"anim@heists@ornate_bank@thermal_charge","cover_eyes_intro",-8.0,-8.0,1000,50,8.0,0,0,0)
...
function func_checkForFlashbang()
    if not a then
        if GetSelectedPedWeapon(tPolar.getPlayerPed()) == GetHashKey("WEAPON_FLASHBANG") then
            a = true
        else
            a = false
        end
    else
        if IsPedShooting(tPolar.getPlayerPed()) and not c then
            c = true
            Citizen.CreateThread(function()
                ...
                if e ~= 0 then
                    ...
                    AddExplosion(coords, 25, 0.0, 1.0, nil, true, true)
                    TriggerServerEvent("Polar:flashbangThrown", f)
                end
            end)
...
tPolar.createThreadOnTick(func_checkForFlashbang)
