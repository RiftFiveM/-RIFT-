...
RMenu.Add('PolarStores', 'main', RageUI.CreateMenu("", "~b~Shop", tPolar.getRageUIMenuWidth(), tPolar.getRageUIMenuHeight(), 'banners', 'shop'))
RMenu.Add("PolarStores", "confirm", RageUI.CreateSubMenu(RMenu:Get('PolarStores', 'main',  tPolar.getRageUIMenuWidth(), tPolar.getRageUIMenuHeight())))

RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get("PolarStores", "main")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            for k, v in pairs(cfg.shopItems) do
                ...
                end, RMenu:Get("PolarStores", "confirm"))
            end
        end)
    end
end)
...
local Index = 1
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get("PolarStores", "confirm")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            ...
            RageUI.Button("Confirm Purchase" , nil, {RightLabel = "→"}, true, function(Hovered, Active, Selected)
                if Selected then
                    TriggerServerEvent("Polar:BuyStoreItem", cHash, tonumber(Index))
                end
            end, RMenu:Get("PolarStores", "main")) 
        end) 
    end
end)
        ...
        for k, v in pairs(cfg.shops) do
            ...
            if isInArea(v, 1.0) and inMenu == false then
                ...
                if IsControlJustPressed(0, 51) then 
                    ...
                    RageUI.Visible(RMenu:Get("PolarStores", "main"), true)
                    cLoaction = v
                end
            end
