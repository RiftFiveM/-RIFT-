...
local c
local d
AddEventHandler("Polar:onClientSpawn",function(e, f)
    if f then
        local g = function()
            if not c then
        ...
        local i = function(j)
            if not c and IsControlJustPressed(1, 51) then
                TriggerServerEvent("Polar:requestStartCooking", j.recipe)
        ...
        for k, l in pairs(a) do
            for m, n in pairs(l) do
                tPolar.createArea("cooking_" .. k .. "_" .. m,n.cookingPosition,1.5,6,g,h,i,{recipe = k, recipeTable = a[k], index = m})
                tPolar.addMarker(n.cookingPosition.x,n.cookingPosition.y,n.cookingPosition.z - 0.2,0.5,0.5,0.5,0,50,255,170,50,20,false,false,true)
            end
        end
    end
end)
...
local function t()
    for p, q in pairs(b) do
        for r, s in pairs(q) do
            local u = tPolar.loadModel(s.model)
...
AddEventHandler("Polar:onClientSpawn",function(e, f)
    if f then
        ...
        for w, x in pairs(b) do
            tPolar.createArea("cooking_staticobjects_" .. w, x[1].position, 100, 5, g, h, i, {})
        end
    end
end)
...
local function B()
    CreateThread(function()
        tPolar.startCircularProgressBar("",3000,nil,function()end)
    end)
    local C = PlayerPedId()
    tPolar.loadAnimDict("amb@prop_human_bum_bin@enter")
    TaskPlayAnim(C, "amb@prop_human_bum_bin@enter", "enter", 8.0, 8.0, -1, 1, 0.2)
    RemoveAnimDict("amb@prop_human_bum_bin@enter")
    Wait(1500)
    tPolar.loadAnimDict("amb@prop_human_bum_bin@exit")
...
local function D(E)
    local F = true
    local G
    tPolar.minigameCircularProgressBar({Difficulty = E, Timeout = 100000, onComplete = function(H)
        if H then
            G = true
        else
            G = false
...
function cookBBQ()
    ...
    tPolar.startCircularProgressBar("",10000,nil,function()end)
    local L = 0
    ...
    while L < 3 do
        ...
        if H then
            ...
            tPolar.announceMpSmallMsg("Disaster", "You dropped the burger patty, start again!", 6, 3000)
        end
        Wait(2500)
        tPolar.startCircularProgressBar("",10000,nil,function()end)
        Wait(2500)
    ...
    tPolar.announceMpSmallMsg("Success", "You made a burger!", 18, 3000)
    ClearPedTasks(C)
    StopParticleFxLooped(K, true)
end
local function M()
    local C = tPolar.getPlayerPed()
    local N = tPolar.getPlayerCoords()
    local u = tPolar.loadModel("prop_food_bs_tray_02")
    ...
    SetModelAsNoLongerNeeded(u)
    tPolar.loadAnimDict("anim@heists@box_carry@")
    TaskPlayAnim(C, "anim@heists@box_carry@", "idle", 2.0, 2.0, -1, 51, 0, false, false, false)
    Citizen.CreateThread(function()
        while tPolar.doesAreaExist("temp_foodorder_complete") do
...
local function T(U)
    ...
    local W = tPolar.addMarker(V.x, V.y, V.z + 0.3, 0.15, 0.15, 0.15, 255, 215, 0, 170, 50, 0, false, true, true)
    local X = tPolar.addBlip(V.x, V.y, V.z, 1, 2, "Delivery Point", 0.6)
    SetBlipFlashes(X, true)
    tPolar.createArea("temp_foodorder_complete",V,1.5,6.0,function()
        tPolar.removeArea("temp_foodorder_complete")
        tPolar.removeMarker(W)
        tPolar.removeBlip(X)
...
RegisterNetEvent("Polar:cookingInstruction",function(Z)
    for r, _ in pairs(R) do
        tPolar.removeArea("temp_cooking_instruction")
        tPolar.removeMarker(_)
    end
    for r, Y in pairs(S) do
        tPolar.removeBlip(Y)
    end
    ...
    local W = tPolar.addMarker(V.x, V.y, V.z + 0.3, 0.15, 0.15, 0.15, 255, 215, 0, 170, 50, 0, false, true, true)
    table.insert(R, W)
    local X = tPolar.addBlip(V.x, V.y, V.z, 1, 2, "Ingredient", 0.6)
    SetBlipFlashes(X, true)
    table.insert(S, X)
    local g = function()
    end
    ...
    local i = function()
        if IsControlJustPressed(1, 51) then
            ...
            if Z == "bbq" then
                ...
                tPolar.removeMarker(W)
                T(function()
                    TriggerEvent("Polar:playItemBoughtSound")
                    TriggerServerEvent("Polar:pickupCookingIngredient", c, Z)
                    tPolar.setCanAnim(true)
                    tPolar.announceMpSmallMsg("Success", "You earned £4,000, now get back to work!", 18, 3000)
                end)
            else
                tPolar.removeMarker(W)
                tPolar.removeBlip(X)
    ...
    tPolar.createArea("temp_cooking_instruction", V, 1.0, 6, g, h, i)
end)
RegisterNetEvent("Polar:beginCooking",function(a0)
    TriggerEvent("Polar:smallAnnouncement", "Cooking", "Make a " .. a0 .. ".", 15, 5000)
end)
RegisterNetEvent("Polar:finishedCooking",function()
    c = nil
end)
