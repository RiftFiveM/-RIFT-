...
Citizen.CreateThread(function()
	while true do
		...
		if GetVehiclePedIsTryingToEnter(m) ~= 0 or d then
			tPolar.setWeapon(m, "WEAPON_UNARMED", true)
		end
		if b then
						...
						if table.has(i, o) then
							...
							if p ~= GetHashKey("WEAPON_UNARMED") and not table.has(i, p) then
								...
								if o == "WEAPON_PDGLOCK" and GetPedDrawableVariation(n, 7) == 187 then
									tPolar.loadCustomisationPreset("TaserInHolster")
								end
								if not d then
									a = false
									b = false
						...
						if table.has(i, f) then
							...
							if p ~= GetHashKey("WEAPON_UNARMED") and not table.has(i, p) then
								...
								if h and GetPedDrawableVariation(n, 7) == 161 then
									tPolar.loadCustomisationPreset("TaserOutHolster")
								end
							end
						else
							loadAnimDict("reaction@intimidation@1h")
							...
							RemoveAnimDict("reaction@intimidation@1h")
							b = true
							tPolar.setWeapon(n, GetHashKey(f), true)
							Citizen.Wait(1300)
							tPolar.setWeapon(n, "WEAPON_UNARMED", true)
							Citizen.Wait(700)
							ClearPedTasks(n)
							a = true
							b = false
...
function tPolar.setWeapon(m, s, t)
