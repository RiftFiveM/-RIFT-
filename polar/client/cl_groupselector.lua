...
local d = false
local e = ""
RMenu.Add("main","groupselector",RageUI.CreateMenu("", "", tPolar.getRageUIMenuWidth(), tPolar.getRageUIMenuHeight()))
RMenu:Get("main", "groupselector"):SetSubtitle("~b~Select a job.")
AddEventHandler("Polar:onClientSpawn",function(D, E)
    if E and not tPolar.isPurge() then
		TriggerServerEvent("Polar:getJobSelectors")
        TriggerServerEvent('Polar:getFactionWhitelistedGroups')
	end
end)
RegisterNetEvent("Polar:gotJobSelectors",function(h)
    c = h
    local i = function(j)
        e = j.selectorId
    end
    ...
    for q, r in pairs(c) do
        tPolar.createArea("selector_" .. q, r.position, 1.5, 6, i, k, l, {selectorId = q})
        tPolar.addMarker(r.position.x, r.position.y, r.position.z - 1, 1.0, 1.0, 1.0, 255, 0, 0, 170, 50, 27)
        tPolar.addBlip(r.position.x,r.position.y,r.position.z,r._config.blipid,r._config.blipcolor,r._config.name)
    end
end)
                ...
                if q == e then
                    for s, t in pairs(r.jobs) do
                        RageUI.ButtonWithStyle(t[1],r._config.name,{RightLabel = "→→→"},true,function(u, v, w)
                            if w then
                                TriggerServerEvent("Polar:jobSelector", q, t[1])
                    ...
                    RageUI.ButtonWithStyle("Unemployed","",{RightLabel = "→→→"},true,function(u, v, w)
                        if w then
                            TriggerServerEvent("Polar:jobSelector", q, "Unemployed")
                            SetTimeout(1000,function()
                                TriggerServerEvent("Polar:refreshGaragePermissions")
