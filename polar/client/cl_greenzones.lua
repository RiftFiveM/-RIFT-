local a=module("Polar-vehicles","cfg_vehiclemaxspeeds")
isInGreenzone = false
local cityCap = true
local b = false
local c = false
...
local l = {
    ...
    },
    {
        colour = 2,
        id = 1,
        pos = vector3(92.538055419922,6359.3696289062,31.375844955444),
        dist = 20,
        nonRP = false,
        setBit = false,
        maxHeight = 60.0
    ...
    },
    {
        colour = 2,
        id = 1,
        pos = vector3(3068.6647949219,-4761.33203125,15.256357192993),
        dist = 150,
        nonRP = false,
        setBit = false,
        maxHeight = 210.0
...
local m = {
    ...
    {vector3(92.538055419922,6359.3696289062,31.375844955444), 20.0, 2, 20},
    {vector3(-734.51495361328,5816.4633789062,17.380125045776), 20.0, 2, 180},
    ...
    {vector3(1182.0738525391,-3267.5512695312,5.5286040306091), 100.0, 1, 180},
    {vector3(696.26892089844,-1011.2525634766,22.793746948242), 100.0, 1, 180},
    ...
    {vector3(3068.6647949219,-4761.33203125,15.256357192993), 150.0, 2, 180}
}
...
function tPolar.areGreenzonesDisabled()
    return i
end
function tPolar.setGreenzonesDisabled(p)
    i = p
end
n(
    function()
        for q, r in pairs(m) do
            if not r[5] and not tPolar.isPurge() or r[5] and tPolar.isPurge() then
...
n(
    function()
        local purgeActive = tPolar.isPurge()
        while true do
            local t = tPolar.getPlayerPed()
            local u = tPolar.getPlayerCoords()
            for v, w in pairs(l) do
                if not w.isPurge and not purgeActive or w.isPurge and purgeActive then
                    ...
                    SetPlayerInvincible(tPolar.getPlayerId(), false)
                    ...
                    SetNetworkVehicleAsGhost(tPolar.getPlayerVehicle(), false)
                end
            end
            o(250)
        end
...
AddEventHandler("Polar:onClientSpawn",function(y, z)
    if z then
        local A = function(B)
            inCityZone = true
        end
        ...
        tPolar.createArea("cityzone",vector3(-225.30703735352, -916.74755859375, 31.216938018799),750.0,100,A,C,D,{})
    end
end)
local function E()
    local F = tPolar.getPlayerCoords()
    local G = nil
    for H = 1, 25 do
    ...
    if G then
        local K, L = tPolar.getPlayerVehicle()
        if K ~= 0 then
    ...
    while true do
        ...
        if not tPolar.areGreenzonesDisabled() then
            ...
            if b or c then
                ...
                ClearPedBloodDamage(t)
                if usingDelgun then
                    tPolar.setWeapon(t, "WEAPON_STAFFGUN", true)
                else
                    tPolar.setWeapon(t, "WEAPON_UNARMED", true)
                end
                ResetPedVisibleDamage(t)
                ClearPedLastWeaponDamage(t)
            ...
            if b then
                ...
                local U, V = tPolar.getPlayerCombatTimer()
                if U > 0 and V then
                    E()
                end
            end
...
RegisterCommand('togglecitycap', function()
    if tPolar.getUserId() == 1 then
        cityCap = not cityCap
        tPolar.notify('City Cap: '..(cityCap and "~g~Enabled" or "~r~Disabled"))
    end
end)