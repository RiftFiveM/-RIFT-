local a = module("Polar-weapons", "cfg/cfg_weaponsonback")
...
local function g()
    ...
    local i = tPolar.getWeapons()
    local j = false
    local k = globalOnPoliceDuty and tPolar.getPlayerVehicle() ~= 0
    for l in pairs(i) do
        local m = GetHashKey(l)
        local n = a.weapons[m]
        if n then
            local o = (not k or n.type ~= "Heavy") and (tPolar.isEmergencyService() or not isInGreenzone) and not tPolar.isPlayerInAnimalForm()
