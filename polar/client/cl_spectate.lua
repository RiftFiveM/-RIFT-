...
RegisterNetEvent("Polar:spectatePlayer",function(k, l)
    local m = PlayerPedId()
...
RegisterNetEvent("Polar:stopSpectatePlayer",function()
    a = false
    ClearFocus()
    b = nil
...
local function q(r, s, t, u, v, w, x, y, z, A)
    tPolar.DrawText(r - t / 2, s - u / 2 + 0.005, w, v, 0, 1, {x, y, z, A})
end
    ...
    while true do
        ...
        if a then
            ...
            if B ~= -1 then
                ...
                tPolar.notify("~r~Couldn't spectate, person not in your zone")
            end
        end
    end
end)
...
tPolar.createThreadOnTick(X)
function tPolar.isInSpectate()
    return a
end