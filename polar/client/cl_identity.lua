    ...
    if true then 
        ...
        local i=function()
            if IsControlJustPressed(1,51)then 
                TriggerServerEvent('Polar:getIdentity')
                RageUI.ActuallyCloseAll()
                RageUI.Visible(RMenu:Get("identity","main"),not RageUI.Visible(RMenu:Get("identity","main")))
            end 
        end
        for j,k in pairs(b)do 
            tPolar.createArea("identity_"..j,k,1.5,6,g,h,i)
            tPolar.addMarker(k.x,k.y,k.z-0.2,0.5,0.5,0.5,0,50,255,170,50,20,false,false,true)
        end 
    end 
end)

RegisterNetEvent("Polar:gotCurrentIdentity")
AddEventHandler("Polar:gotCurrentIdentity", function(firstname,lastname,age)
    j=firstname
    k=lastname
    i=age
end)

RegisterNetEvent("Polar:gotNewIdentity")
AddEventHandler("Polar:gotNewIdentity", function(firstname,lastname,age)
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if j ~= "N/A" then
                ...
                RageUI.ButtonWithStyle("Change your Identity","£5000",{RightLabel="→→→"},true,function(l,m,n)
                    if n then 
                        RageUI.ActuallyCloseAll()
                        TriggerServerEvent('Polar:getNewIdentity')
                    end 
                end)
            end
        end)
    ...
    if RageUI.Visible(RMenu:Get('identity', 'confirm')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.ButtonWithStyle("Yes","",{RightLabel="→→→"},true,function(j,k,l)
                if l then 
                    TriggerServerEvent("Polar:ChangeIdentity", newfirstname, newlastname, tonumber(newage))
                    RageUI.ActuallyCloseAll()
                end 
...
RegisterNetEvent("Polar:showIdentity",function(v, w, x, y, z, A, B, C, D)
    o = true
    ...
    local H = tPolar.getFontId("Akrobat-ExtraLight")
    x = string.upper(x)
    y = string.upper(y)
    while o do
...
RegisterNetEvent("Polar:hideIdentity",function()
    o = false
end)
