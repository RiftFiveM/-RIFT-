...
local function f(g, h)
    local i = tPolar.getPlayerCoords()
    local j = 0
    local k = g
...
local function q()
    if IsControlJustReleased(0, 299) then
        if NetworkHasControlOfEntity(c) then
            TriggerServerEvent("Polar:toggleAmbulanceDoors", tPolar.getNetId(c, "Stretchers"))
        else
...
local function C(D, E)
    ...
    if not y(D) or b then
        ...
        tPolar.loadAnimDict(a.positions[e].animDict)
        DetachEntity(x, false, false)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if d ~= nil and d ~= 0 then
            ...
            if not b and not IsEntityPlayingAnim(x, "anim@heists@box_carry@", "idle", 3) and tPolar.getPlayerVehicle() == 0 then
            ...
            if IsEntityPlayingAnim(x, "anim@heists@box_carry@", "idle", 3) and c == 0 then
                ...
                if H ~= -1 and globalNHSOnDuty then
                    ...
                    if IsControlJustPressed(0, 183) then
                        ...
                        if L > 0 and M ~= 0 then
                            TriggerServerEvent("Polar:forcePlayerOnToStretcher", L, M)
                        end
                    end
                end
                r(K)
            ...
            if IsControlJustReleased(0, 51) and not IsEntityPlayingAnim(x, "anim@heists@box_carry@", "idle", 3) and not b then
                if DecorGetInt(d, "inside_ambulance_netid") ~= 0 then
                    local O = tPolar.getObjectId(DecorGetInt(d, "inside_ambulance_netid"), "Stretcher Thread")
...
function pushStretcher(x, d)
    Citizen.CreateThread(function()
        ...
        TriggerServerEvent("Polar:stretcherAttachPlayer", P)
        ...
        while IsEntityAttachedToEntity(d, x) do
            ...
            if DecorGetInt(d, "inside_ambulance_netid") ~= 0 then
                local O = tPolar.getObjectId(DecorGetInt(d, "inside_ambulance_netid"), "pushStretcher(ped, nearbyStretcher)")
                if DecorGetBool(O, "has_stretcher_inside") ~= false then
                    if not NetworkHasControlOfEntity(O) then
                        if GetGameTimer() - Q > 200 then
                            TriggerServerEvent("Polar:updateHasStretcherInsideDecor",tPolar.getNetId(O, "Stretchers"),false)
                            Q = GetGameTimer()
                        end
                    else
            ...
            if c ~= nil and c ~= 0 then
                ...
                if IsControlJustReleased(0, 300) and (GetVehicleDoorAngleRatio(c, 4) ~= 0 or IsThisModelAHeli(GetEntityModel(c))) and not R then
                    ...
                    if not NetworkHasControlOfEntity(c) then
                        TriggerServerEvent("Polar:updateHasStretcherInsideDecor", tPolar.getNetId(c, "Stretcher"), true)
                    else
                        DecorSetBool(c, "has_stretcher_inside", true)
                    end
                    if not NetworkHasControlOfEntity(d) then
                        TriggerServerEvent("Polar:updateStretcherLocation",tPolar.getNetId(d, "Stretchers"),tPolar.getNetId(c, "Stretchers"))
                    else
                        DecorSetInt(d, "inside_ambulance_netid", tPolar.getNetId(c, "Stretchers"))
                    end
...
RegisterNetEvent("Polar:stretcherAttachPlayer",function(P, S)
...
RegisterCommand("stretcher",function()
    if globalNHSOnDuty then
        ...
        local g = tPolar.loadModel("trump-stretcher")
        local D = tPolar.spawnVehicle(g, v.x, v.y, v.z, GetEntityHeading(x), false, true, true)
        ...
        tPolar.notify("Stretcher created.")
    end
end,false)
RegisterCommand("removestretcher",function()
    if globalNHSOnDuty then
        ...
        if V ~= nil then
            TriggerServerEvent("Polar:removeStretcher", tPolar.getNetId(V, "Stretchers"))
        end
    end
end,false)
RegisterNetEvent("Polar:setStretcherInside",function(P, W)
    local D = tPolar.getObjectId(P, "Polar:setStretcherInside")
    DecorSetInt(D, "inside_ambulance_netid", W)
end)
RegisterNetEvent("Polar:setHasStretcherInsideDecor",function(X, Y)
    local c = tPolar.getObjectId(X, "Polar:setHasStretcherInsideDecor")
    DecorSetBool(c, "has_stretcher_inside", Y)
end)
RegisterNetEvent("Polar:toggleAmbulanceDoorStatus",function(X)
    local c = tPolar.getObjectId(X, "Polar:toggleAmbulanceDoorStatus")
...
function tPolar.isUsingStretcher()
    return b
end
RegisterNetEvent("Polar:forcePlayerOnToStretcher",function(P)
    if not NetworkDoesNetworkIdExist(P) then
        return
    end
    local D = NetworkGetEntityFromNetworkId(P)
