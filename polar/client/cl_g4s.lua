...
local function D()
    ...
    local F = 500 * math.ceil(GetDistanceBetweenCoords(tPolar.getPlayerCoords(), GetEntityCoords(b), true) / 10)
    if j then
...
RMenu.Add("g4s","shiftHandler",RageUI.CreateMenu("","Security Company",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners", "g4s"))
RMenu:Get("g4s", "shiftHandler"):SetSubtitle("Security Company")
RMenu:Get("g4s", "shiftHandler").Closable = true
RMenu.Add("g4s","main",RageUI.CreateMenu("","Security Company",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners", "g4s"))
...
tPolar.addMarker(d.x, d.y, d.z, 1.2, 1.2, 1.2, 0, 255, 125, 125, 50, 0, true, true)
local H = tPolar.addBlip(d.x, d.y, d.z, 457, 18, "G4S Depot", 1.0, false)
local I = function()
...
tPolar.createArea("g4sDepot_", d, 1.5, 6, I, J, K)
Citizen.CreateThread(function()
    while true do
        local E = tPolar.getPlayerPed()
        local L = tPolar.getPlayerCoords()
        if a then
            if not u then
                displayJob()
            end
...
Citizen.CreateThread(function()
    while true do
        if not A.hackingEngaged and not a and not A.awaitingResponse then
            if table.count(s) > 0 and tPolar.getPlayerVehicle() == 0 then
                for M, N in pairs(s) do
                    ...
                    if table.has(p, Q) or not DoesEntityExist(P) then
                        ...
                        if IsVehicleStopped(P) then
                            local R = #(tPolar.getPlayerCoords() - GetEntityCoords(P))
                            ...
                            if R <= 11.0 then
                                ...
                                if IsControlJustPressed(1, 18) then
                                    ...
                                    A.entity = P
                                    A.src = Q
                                    TriggerServerEvent("Polar:requestHackingChanceServer", O, Q)
...
function toggleCashbox()
    if C.active then
        ...
        local o = tPolar.loadModel("cashbox")
        local E = tPolar.getPlayerPed()
...
local W = "HONGKONG"
local X = 8
RegisterNetEvent("Polar:hackingChanceReturn",function(b, Y, Z, _)
    if _ then
        ...
        local o = tPolar.loadModel("prop_cs_tablet")
        x = CreateObject(o, 0, 0, 0, true, true, true)
        AttachEntityToEntity(x,tPolar.getPlayerPed(),GetPedBoneIndex(tPolar.getPlayerPed(), 57005),0.17,0.10,-0.13,24.0,180.0,180.0,true,true,false,true,1,true)
        SetModelAsNoLongerNeeded(o)
    else
        A.awaitingResponse = false
    end
        ...
        if HasScaleformMovieLoaded(T) then
            ...
            if IsScaleformMovieMethodReturnValueReady(U) then
                ...
                elseif ProgramID == 87 then
                    ...
                    TriggerServerEvent("Polar:updateHackingOutcome", A.netId, A.src, W, false)
                    DeleteObject(x)
                    break
                ...
                elseif ProgramID == 86 then
                    ...
                    TriggerServerEvent("Polar:updateHackingOutcome", A.netId, A.src, W, true)
                    DeleteObject(x)
                    break
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('g4s', 'main')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if not j then
                if tPolar.getPlayerVehicle() == 0 and y < 4 then
                ...
                RageUI.ButtonWithStyle("End Shift","End your shift",{RightLabel = "→→→"},true,function(a0, a1, a2)
                    if a2 then
                        TriggerServerEvent("Polar:toggleShiftG4S", false)
                        a = false
                    end
                end)
                ...
                if not h then
                    RageUI.ButtonWithStyle("Activate Panic","Activate your panic alarm",{RightLabel = "→→→"},true,function(a0, a1, a2)
                        if a2 then
                            TriggerServerEvent("Polar:activatePanicServer", c)
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if not a then
                RageUI.ButtonWithStyle("Start Shift","Start your shift working for G4S.",{RightLabel = "G4S"},true,function(a0, a1, a2)
                    if a2 then
                        RageUI.Visible(RMenu:Get("g4s", "shiftHandler"), false)
                        TriggerServerEvent("Polar:toggleShiftG4S", true)
                    end
                end)
            else
...
function newG4sVehicle()
    ...
    if DoesEntityExist(b) and not IsVehicleDriveable(b, true) then
        ...
        b = tPolar.spawnVehicle(n, a3.x, a3.y, a3.z, GetEntityHeading(E), true, true)
        c = VehToNet(b)
        TriggerServerEvent("Polar:updateCurrentVehicle", c)
        y = y + 1
    end
end
function securityCheck(E)
    local a4 = tPolar.getPlayerVehicle()
    if a4 ~= 0 and a4 ~= nil and b ~= nil and b ~= 0 then
        if a4 ~= b then
            ...
            SetTimeout(15000,function()
                a4 = GetVehiclePedIsIn(tPolar.getPlayerPed(), false)
                if a4 ~= 0 then
                    if a4 ~= b then
                        TriggerServerEvent("Polar:toggleShiftG4S", false)
                        tPolar.removeBlip(k)
...
RegisterNetEvent("Polar:resetPanicStatus",function()
    h = false
end)
RegisterNetEvent("Polar:updateJobInformation",function(a5)
    if l ~= 0 then
        tPolar.removeBlip(l)
    end
    if a5.jobActive then
        if not a5.collected then
            ...
            k = tPolar.addBlip(a5.collectionCoords.x,a5.collectionCoords.y,a5.collectionCoords.z,500,18,"G4S Collection",1.0,false)
            SetBlipRoute(k, true)
            SetBlipRouteColour(k, 18)
        elseif not a5.deposited then
            tPolar.removeBlip(k)
            l = tPolar.addBlip(a5.depositCoords.x,a5.depositCoords.y,a5.depositCoords.z,500,47,"G4S Deposit",1.0,false)
            SetBlipRoute(l, true)
        ...
        Wait(3000)
        TriggerServerEvent("Polar:acceptJob")
    end
end)
...
RegisterNetEvent("Polar:startShiftG4S",function(a6)
    local E = tPolar.getPlayerPed()
    local L = tPolar.getPlayerCoords()
    a = true
    tPolar.disableCustomizationSave(a)
    b = tPolar.spawnVehicle(n, B.x, B.y, B.z, B.heading, true, true)
    c = VehToNet(b)
    ...
    z = json.encode(tPolar.getCustomization())
    if aa then
        tPolar.loadCustomisationPreset("G4SMale")
    else
        tPolar.loadCustomisationPreset("G4SFemale")
    end
    Wait(2000)
    TaskVehicleDriveToCoord(tPolar.getPlayerPed(),b,-672.356,282.3297,81.43005,convert(250.0),1,GetHashKey(GetEntityModel(b)),60,5,true)
    Wait(5000)
    TaskVehicleTempAction(tPolar.getPlayerPed(), b, 27, 10.0)
    Wait(2000)
    imageNotification("CHAR_MP_MEX_DOCKS", "CHAR_MP_MEX_DOCKS", "Use /g4s to access the G4S menu.", a9, "G4S")
end)
RegisterNetEvent("Polar:endShiftG4S",function()
    ...
    local E = tPolar.getPlayerPed()
    tPolar.setCustomization(json.decode(z))
    if C.active then
        toggleCashbox()
    end
    if k ~= 0 then
...
RegisterNetEvent("Polar:requestMoneyInVehicle",function()
    ...
    while not f do
        ...
        local R = #(tPolar.getPlayerCoords() - ag)
        if R <= 1.0 then
            af = true
    ...
    TriggerServerEvent("Polar:updateMoneyInVehicle")
    t = false
end)

RegisterNetEvent("Polar:requestMoneyOutVehicle",function()
    local ae = tPolar.getPlayerPed()
    while IsPedInAnyVehicle(ae, true) do
        Wait(400)
    end
    ...
    y = 1
    TriggerServerEvent("Polar:updateMoneyOutVehicle")
end)
...
RegisterNetEvent("Polar:openG4SMenu",function()
    RageUI.Visible(RMenu:Get("g4s", "main"), true)
end)
RegisterNetEvent("Polar:imageNotification",function(al, am, an, ao, ap)
...
RegisterNetEvent("Polar:bottomMessage",function(ak, av)
    bottomMessage(ak, av)
end)
...
RegisterNetEvent("Polar:activatePanicAll",function(az, aA)
...
RegisterNetEvent("Polar:activeVansUpdate",function(az, Q, aE)
    if not a then
        local b = tPolar.getObjectId(az, "Polar:activeVansUpdate 1")
        if b ~= 0 and b ~= nil and DoesEntityExist(b) then
            if aE then
                s[az] = nil
            else
                if not table.has(s, az) then
                    local b = tPolar.getObjectId(az, "Polar:activeVansUpdate")
                    s[az] = {Q, b, az}
                end
            end
        end
...
RegisterNetEvent("Polar:terminateHacking",function()
