...
function tPolar.getDrawables(d)
    local e, f = b(d)
    if e then
...
function tPolar.getDrawableTextures(d, g)
    local e, f = b(d)
    if e then
...
function tPolar.getCustomization()
    local h = PlayerPedId()
    local i = {}
...
local l = 0
function tPolar.setCustomization(i, k, l)
    if i then
      changingPed = true
        local h = PlayerPedId()
        local m = nil
        ...
        local n = tPolar.loadModel(m)
        local o = GetEntityModel(h)
        if n then
            if o ~= n or k then
                local p = tPolar.getWeapons()
                local q = GetEntityHealth(h)
                SetPlayerModel(PlayerId(), m)
                Wait(0)
                tPolar.giveWeapons(p, true)
                if l == nil or l == false then
                    print("[Polar] Customisation, setting health to ", q)
                    tPolar.setHealth(q)
                end
                ...
                --print("[Polar] Same model detected, not changing model.")
            end
            ...
            print("[Polar] Failed to load model", m)
        end
        changingPed = false
    end
end
function tPolar.isPedScriptGuidChanging()
    return k or GetGameTimer() - l < 3000
end
function tPolar.loadCustomisationPreset(t)
  local u = a.presets[t]
  assert(u, string.format("Preset %s does not exist.", t))
  if u.model then
      tPolar.setCustomization({modelhash = u.model})
      Citizen.Wait(100)
  end
  local v = PlayerPedId()
