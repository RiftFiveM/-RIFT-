...
local b = 10
RegisterNetEvent("Polar:beginBusJob",function()
    a.tempVehicle = tPolar.spawnVehicle("bus",a.spawnVehicleVector.x,a.spawnVehicleVector.y,a.spawnVehicleVector.z,343,true,true,true)
    a.onJob = true
    tPolar.notify("~g~Bus Job started, exit the bus station and head to the first bus stop.")
    while a.onJob do
...
RegisterNetEvent("Polar:endBusJob",function()
...
RegisterNetEvent("Polar:nextStopReachedBusJob",function(d)
    ...
    local B=tPolar.loadModel(passengerPeds[a.stopNumber].pedModel)
    local coords = tPolar.getPosition()
    local radius = 10.0
...
RegisterNetEvent("Polar:setNextBusJobBlip",function(f)
    a.tempBlip = AddBlipForCoord(f.x, f.y, f.z)
    SetBlipSprite(a.tempBlip, 1)
    SetBlipRoute(a.tempBlip, true)
    a.tempMarker = tPolar.addMarker(f.x, f.y, f.z, 1.2, 1.2, 1.2, 255, 0, 0, 125, 50, 0, true, true)
end)
RegisterNetEvent('Polar:onClientSpawn')
AddEventHandler("Polar:onClientSpawn",function(g,h)
    if h then
        local i = function(j)
        ...
        local l = function(j)
            if IsControlJustReleased(1, 38) and not a.onJob then
                TriggerServerEvent("Polar:attemptBeginBusJob")
            end
        end
        tPolar.addBlip(a.startVector.x, a.startVector.y, a.startVector.z, 106, 1, "Bus Driver Job")
        tPolar.addMarker(a.startVector.x,a.startVector.y,a.startVector.z - 1,1.0,1.0,1.0,255,0,0,70,50,39,false,false,true)
        tPolar.createArea("bus", a.startVector, 1.5, 6, i, k, l, {})
    end
end)
function alert(msg) 
