...
Citizen.CreateThread(function()
    tPolar.createDynamicPed(a.ped.modelHash,a.ped.position,215.0,true,a.ped.animDict,a.ped.animName,30,false,false)
    tPolar.addBlip(a.ped.position.x, a.ped.position.y, a.ped.position.z, 357, 81, "Vehicle Impound", 0.8, false)
    Wait(2000)
    local d = function()
        TriggerServerEvent('Polar:getImpoundedVehicles')
        RageUI.Visible(RMenu:Get("Polarimpound", "main"), true)
    end
    local e = function()
    ...
    tPolar.createArea("vehicle_impound", a.ped.position, 3.0, 6, d, e, f, {})
end)
RMenu.Add("Polarimpound","reasons",RageUI.CreateMenu("","~b~Impounding Vehicle...",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners", "impound"))
RMenu.Add("Polarimpound","main",RageUI.CreateMenu("","~b~Your Impounded Vehicles",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners", "impound"))
RMenu.Add("Polarimpound", "vehicle_information", RageUI.CreateSubMenu(RMenu:Get("Polarimpound", "main")))
RMenu.Add("Polarimpound", "pay_fine_confirmation", RageUI.CreateSubMenu(RMenu:Get("Polarimpound", "vehicle_information")))
local g = 0
...
RageUI.CreateWhile(1.0, true, function()
    ...
    if RageUI.Visible(RMenu:Get('Polarimpound', 'reasons')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.ButtonWithStyle("~g~Confirm Selection","",{RightLabel = "→→→"},true,function(i, k, j)
                if j then
                    ...
                    TriggerServerEvent("Polar:impoundVehicle",b.owner_id,b.owner_name,b.vehicle,b.vehicle_name,m,b.vehicle_net_id)
                    RageUI.ActuallyCloseAll()
                end
            end,function()end)
    ...
    if RageUI.Visible(RMenu:Get('Polarimpound', 'main')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.Separator("View your impounded vehicles here.")
                ...
                if c ~= nil and c ~= {} then
                    for o, p in pairs(c) do
                        ...
                        RMenu:Get("Polarimpound", "vehicle_information"))
                    end
                else
    ...
    if RageUI.Visible(RMenu:Get('Polarimpound', 'vehicle_information')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if g ~= 0 then
                ...
                end,RMenu:Get("Polarimpound", "pay_fine_confirmation"))
            end
        end)
    end
    if RageUI.Visible(RMenu:Get('Polarimpound', 'pay_fine_confirmation')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if g ~= 0 then
                ...
                RageUI.ButtonWithStyle("~g~Pay","Pay the fine",{RightLabel = "→→→"},true,function(i, k, j)
                    if j then
                        TriggerServerEvent("Polar:releaseImpoundedVehicle", g.vehicle)
                    end
                end,RMenu:Get("Polarimpound", "main"))
...
RegisterNetEvent("Polar:receiveInfoForVehicleToImpound",function(r, s, t, u, v)
    b = {owner_id = tonumber(r), owner_name = s, vehicle = t, vehicle_name = u, vehicle_net_id = v}
end)

RegisterNetEvent("Polar:receiveImpoundedVehicles",function(w)
    c = w
end)

RegisterNetEvent("Polar:impoundSuccess",function(x, t, y, z, A, B, C, D)
    ...
    if O ~= vector3(0, 0, 0) and N ~= vector3(0, 0, 0) then
        tPolar.loadModel("flatbed")
        tPolar.loadModel("a_m_m_prolhost_01")
        P = tPolar.spawnVehicle("flatbed", O.x, O.y, O.z, M, false, true, true)
        ...
        TriggerServerEvent("Polar:deleteImpoundEntities",x,NetworkGetNetworkIdFromEntity(G),NetworkGetNetworkIdFromEntity(F))
        u = tPolar.spawnVehicle(z, C.x, C.y, C.z, D, false, true, false)
        ...
        TriggerServerEvent("Polar:awaitTowTruckArrival", x, R, T)
...
RegisterNetEvent("Polar:attachVehToTowCl",function(x, W)
    local u = NetworkGetEntityFromNetworkId(x)
...
function tPolar.impoundVehicleOptions(Z, _, x, a0)
    local a1 = g_vehicleHashToVehicleName[_]
    if X(a0) then
        tPolar.notifyPicture("polnotification","notification","The vehicle cannot be impounded with a person inside.","Metropolitan Police","Impound",nil,nil)
        return
    end
    local a2 = GetVehicleClass(a0)
    if a2 == 18 then
        tPolar.notifyPicture("polnotification","notification","Emergency vehicles cannot be impounded.","Metropolitan Police","Impound",nil,nil)
    elseif a.disallowedVehicleClasses[a2] then
        tPolar.notifyPicture("polnotification","notification","That vehicle is too large to be impounded.","Metropolitan Police","Impound",nil,nil)
    else
        TriggerServerEvent("Polar:fetchInfoForVehicleToImpound", Z, a1, x)
        RageUI.Visible(RMenu:Get("Polarimpound", "reasons"), true)
    end
end

function tPolar.isVehicleImpounded(u)
    return c[u] ~= nil
end
...
exports("impound", tPolar.impoundVehicleOptions)
exports("isImpounded", tPolar.isVehicleImpounded)
