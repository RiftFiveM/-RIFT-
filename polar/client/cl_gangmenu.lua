...
local n = m["Red"]
local o = GetResourceKvpString("Polar_gang_colour") or "Red"
local function p()
    return PolarGangMembers
end



RegisterNetEvent("Polar:GotGangData")
AddEventHandler(
    "Polar:GotGangData",
    function(q, r, k, s, t, u, v)
        if q == nil then
            ...
            PlayerIsInGang = true
            TriggerServerEvent("Polar:setPersonalGangBlipColour", o)
            e = {}
            gangLogs = {}
            h = 1
            i = 1
...
RegisterNetEvent("Polar:disbandedGang")
AddEventHandler(
    "Polar:disbandedGang",
    function()
        d = "none"
...
RegisterNetEvent("Polar:ForceRefreshData")
AddEventHandler(
    "Polar:ForceRefreshData",
    function()
        TriggerServerEvent("Polar:GetGangData")
    end
)
RegisterNetEvent("Polar:InviteReceived")
AddEventHandler(
    "Polar:InviteReceived",
    function(y, z)
        PolarGangInvites[PolarGangInviteIndex] = z
        PolarGangInviteIndex = PolarGangInviteIndex + 1
        tPolar.notify(y)
    end
)
RegisterNetEvent("Polar:gangNameNotTaken")
AddEventHandler(
    "Polar:gangNameNotTaken",
    function()
        d = "main"
        PlayerIsInGang = true
    end
...
function func_drawGangUI()
    if d == "none" then
        ...
        DrawAdvancedText(0.558, 0.303, 0.005, 0.0028, 0.539, "Polar Gangs", 255, 255, 255, 255, 7, 0)
        ...
        for A, B in pairs(PolarGangInvites) do
        ...
        if CursorInArea(0.35, 0.415, 0.415, 0.46) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if createGangName ~= nil and createGangName ~= "null" and createGangName ~= "" then
                    TriggerServerEvent("Polar:BJVJFVRJFNVKFVMKJKVJFJVNFKVCCCKLVJFKVNF", createGangName) 
                    Citizen.Wait(10000)
                else
                    tPolar.notify("~r~No gang name entered!")
                end
            end
        else
            CreateGangSelectionRed = 0
        ...
        if CursorInArea(0.435, 0.51, 0.415, 0.46) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if selectedGangInvite ~= nil then
                    selectedGangInvite = PolarGangInvites[selectedGangInvite]
                    TriggerServerEvent("Polar:addUserToGang", selectedGangInvite)
                    PolarGangInvites = {}
                    d = "main"
                    PolarGangInviteIndex = 0
                    PlayerIsInGang = true
                else
                    tPolar.notify("~r~No gang invite selected")
                end
            end
        else
            JoinGangSelectionRed = 0
    ...
    if d == "funds" then
        ...
        DrawAdvancedText(0.591, 0.378, 0.005, 0.0028, 0.48, gangID .." - Funds", 255, 255, 255, 255, 7, 0)
        ...
        if CursorInArea(0.3083, 0.3718, 0.5490, 0.5999) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if amount ~= nil then
                    TriggerServerEvent("Polar:depositGangBalance", gangID, amount)
                else
                    tPolar.notify("~r~No amount entered!")
                end
            end
        else
        ...
        if CursorInArea(0.4083, 0.4718, 0.5490, 0.5999) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                TriggerServerEvent("Polar:depositAllGangBalance", gangID)
            end
        else
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if amount ~= nil then
                    if gangPermission >= 3 then
                        TriggerServerEvent("Polar:withdrawGangBalance", gangID, amount)
                    else
                        tPolar.notify("~r~You don't have a high enough rank to withdraw")
                    end
                else
                    tPolar.notify("~r~No amount entered!")
                end
        ...
        if CursorInArea(0.6088, 0.6739, 0.5481, 0.6018) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if gangPermission >= 3 then
                    TriggerServerEvent("Polar:withdrawAllGangBalance", gangID)
                else
                    tPolar.notify("~r~You don't have a high enough rank to withdraw")
                end
            end
    ...
    if d == "contributions" then
        ...
        DrawAdvancedText(0.591, 0.312, 0.005, 0.0028, 0.48, gangID .." - Contributions", 255, 255, 255, 255, 7, 0)
        ...
        for A, C in pairs(e[f]) do
            ...
            DrawAdvancedText(0.63,0.361 + 0.0287 * A,0.005,0.0028,0.4, C.contributions.date, 255,255,255,255,6,0)
            DrawAdvancedText(0.730,0.361 + 0.0287 * A,0.005,0.0028,0.4, C.contributions.amount, 255,255,255,255,6,0)
        end
        if CursorInArea(0.419271, 0.482813, 0.667593, 0.699074) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if f <= 1 then
                    tPolar.notify("~r~Lowest page reached")
                else
                    f = f - 1
                end
            end
        ...
        if CursorInArea(0.512500, 0.575521, 0.667593, 0.699074) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if f >= h then
                    tPolar.notify("~r~Max page reached")
                else
                    f = f + 1
                end
            end
    ...
    if d == "members" then
        DrawRect(0.501, 0.525, 0.421, 0.387, 0, 0, 0, 150)
        DrawRect(0.501, 0.308, 0.421, 0.047, k.theme.r, k.theme.g, k.theme.b, 248)
        DrawAdvancedText(0.591, 0.312, 0.005, 0.0028, 0.48, gangID .." - Members", 255, 255, 255, 255, 7, 0)
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if selectedMember ~= nil and PlayerIsInGang and gangID ~= nil then
                    if gangPermission >= 4 then
                        TriggerServerEvent("Polar:PromoteUser", gangID, tonumber(selectedMember))
                    else
                        tPolar.notify("~r~You don't have permission to promote!")
                    end
                else
                    tPolar.notify("~r~No gang member selected")
                end
            end
        else
        ...
        if CursorInArea(0.6182, 0.6822, 0.435, 0.491) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if selectedMember ~= nil and PlayerIsInGang and gangID ~= nil then
                    if gangPermission >= 4 then
                        if selectedMember == tPolar.getUserId() then
                            tPolar.notify("~r~You can't demote yourself.")
                        else
                            TriggerServerEvent("Polar:DemoteUser", gangID, selectedMember)
                        end
                    else
                        tPolar.notify("~r~You don't have permission to demote!")
                    end
                else
                    tPolar.notify("~r~No gang member selected")
                end
                ...
                if selectedMember ~= nil then
                    if gangPermission >= 3 then
                        if YesNoConfirm() then
                            TriggerServerEvent("Polar:kickMemberFromGang", gangID, selectedMember)
                        end
                    else
                        tPolar.notify("~r~You don't have permission to kick!")
                    end
                else
                    tPolar.notify("~r~No gang member selected")
                end
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if D ~= nil then
                    if gangPermission >= 2 then
                        TriggerServerEvent("Polar:InviteUserToGang", gangID, D)
                    else
                        tPolar.notify("~r~You don't have permission to invite players")
                    end
                else
                    tPolar.notify("No player name entered")
                end
            end
        else
    ...
    if d == "logs" then
        DrawRect(0.501, 0.525, 0.421, 0.387, 0, 0, 0, 150)
        DrawRect(0.501, 0.308, 0.421, 0.047, k.theme.r, k.theme.g, k.theme.b, 248)
        DrawAdvancedText(0.591, 0.312, 0.005, 0.0028, 0.48, gangID .." - Logs", 255, 255, 255, 255, 7, 0)
        DrawRect(0.502, 0.518, 0.387, 0.283, 0, 0, 0, 150)
        DrawAdvancedText(0.449, 0.365, 0.005, 0.0028, 0.4, "Name", 255, 255, 255, 255, 4, 0)
        ...
        if CursorInArea(0.33, 0.395, 0.667593, 0.699074) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                TriggerServerEvent("Polar:SetGangWebhook", gangID)
            end
        else
    ...
    if d == "settings" then
        DrawRect(0.501, 0.525, 0.421, 0.387, 0, 0, 0, 150)
        DrawRect(0.501, 0.308, 0.421, 0.047, k.theme.r, k.theme.g, k.theme.b, 248)
        DrawAdvancedText(0.591, 0.312, 0.005, 0.0028, 0.48, gangID .." - Settings", 255, 255, 255, 255, 7, 0)
        ...
        if CursorInArea(0.3187, 0.3937, 0.3712, 0.4462) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                k.blips = not k.blips
                SetResourceKvp("Polar_gang_blips", tostring(k.blips))
            end
        else
        ...
        if CursorInArea(0.4197, 0.4932, 0.3712, 0.4462) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                k.pings = not k.pings
                SetResourceKvp("Polar_gang_pings", tostring(k.pings))
            end
        else
        ...
        if CursorInArea(0.3187, 0.3937, 0.4712, 0.5462) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                k.names = not k.names
                SetResourceKvp("Polar_gang_names", tostring(k.names))
                print("Names", k.names)
            end
        else
        ...
        if CursorInArea(0.4197, 0.4932, 0.4712, 0.5462) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if newGangName ~= nil and newGangName ~= "null" and newGangName ~= "" and gangID ~= nil then
                    TriggerServerEvent("Polar:RenameGang", gangID, newGangName)
                else
                    tPolar.notify("~r~No gang name entered!")
                end
            end
        ...
        if CursorInArea(0.3187, 0.3937, 0.5712, 0.6462) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if YesNoConfirm() then
                    TriggerServerEvent("Polar:memberLeaveGang", gangID)
                    setCursor(0)
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if gangPermission >= 4 then
                    if YesNoConfirm() == true and gangID ~= nil then
                        TriggerServerEvent("Polar:DeleteGang", gangID)
                    end
                else
                    tPolar.notify("~r~You don't have permission to disband!")
                end
            end
        else
        ...
        if CursorInArea(0.31, 0.39, 0.6712, 0.7064) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                SetResourceKvp("Polar_gang_healthui", tostring(k.healthui))
            end
        else
        ...
        if CursorInArea(0.41, 0.49, 0.6712, 0.7064) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if gangPermission >= 4 then
                    TriggerServerEvent("Polar:setGangFit", gangID)
                else
                    tPolar.notify("~r~You don't have permission to set gang fit!")
                end
            end
        else
        ...
        if CursorInArea(0.595, 0.645, 0.6155, 0.6405) then
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                SetResourceKvp("Polar_gang_colour", o)
    ...
    if d == "turfs" then
        ...
        DrawRect(0.501, 0.308, 0.421, 0.047, k.theme.r, k.theme.g, k.theme.b, 248)
        DrawAdvancedText(0.591, 0.312, 0.005, 0.0028, 0.48, gangID .." - Turfs", 255, 255, 255, 255, 7, 0)
        DrawAdvancedText(
            0.399,
            0.38,
            0.005,
    ...
    if d == "security" then
        DrawRect(0.501, 0.525, 0.421, 0.387, 0, 0, 0, 150)
        DrawRect(0.501, 0.308, 0.421, 0.047, k.theme.r, k.theme.g, k.theme.b, 248)
        DrawAdvancedText(0.591, 0.312, 0.005, 0.0028, 0.48, "Polar gang - security", 255, 255, 255, 255, 7, 0)
        DrawAdvancedText(
            0.4,
            0.375,
            0.005,
        ...
        if CursorInArea(0.31, 0.65, 0.56, 0.61) then
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if gangPermission >= 4 then
                    TriggerServerEvent("Polar:LockGangFunds", gangID)
                else
                    notify("~r~You must be a leader to change security.")
                end
            end
    ...
    if d == "theme" then
        DrawRect(0.501, 0.525, 0.421, 0.387, 0, 0, 0, 150)
        DrawRect(0.501, 0.308, 0.421, 0.047, k.theme.r, k.theme.g, k.theme.b, 248)
        DrawAdvancedText(0.591, 0.312, 0.005, 0.0028, 0.48, "Polar Gang - Theme", 255, 255, 255, 255, 7, 0)
        DrawAdvancedText(
            0.7,
            0.360,
            0.005,
        ...
        if CursorInArea(0.5187, 0.5828, 0.4138, 0.4694) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                tPolar.CopyToClipboard(k.theme.r .. "," .. k.theme.g .. "," .. k.theme.b)
                tPolar.notify("~g~Theme copied to clipboard.")
            end
        else
        ...
        if CursorInArea(0.5187, 0.5828, 0.5407, 0.5962) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                tPolar.gangMenuTheme(math.random(1, 255), math.random(1, 255), math.random(1, 255))
            end
        else
        ...
        if CursorInArea(0.6182, 0.6822, 0.4138, 0.4694) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                tPolar.gangMenuTheme(18, 82, 228)
            end
        else
                    ...
                    function(O)
                        if O ~= "" then
                            ...
                            if P[1] ~= nil and P[2] ~= nil and P[3] ~= nil then
                                tPolar.gangMenuTheme(tonumber(P[1]), tonumber(P[2]), tonumber(P[3]))
                            else
        ...
        if CursorInArea(0.3633, 0.4106, 0.6712, 0.7064) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                tPolar.gangMenuTheme(255, 0, 255)
            end
        else
        ...
        if CursorInArea(0.5433, 0.5906, 0.6712, 0.7064) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                tPolar.gangMenuTheme(0, 100, 100)
            end
        else
        ...
        if CursorInArea(0.6033, 0.6506, 0.6712, 0.7064) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                tPolar.gangMenuTheme(255, 165, 0)
            end
        else
    ...
    if d == "main" then
        ...
        DrawAdvancedText(0.591, 0.399, 0.005, 0.0028, 0.51, "Polar Gangs", 255, 255, 255, 255, 7, 0)
...
tPolar.createThreadOnTick(b5)
Citizen.CreateThread(
    function()
        k = {
            blips = GetResourceKvpString("Polar_gang_blips") == "true",
            pings = GetResourceKvpString("Polar_gang_pings") == "true",
            names = GetResourceKvpString("Polar_gang_names") == "true",
            healthui = GetResourceKvpString("Polar_gang_healthui") == "true"
        }
        k.theme = json.decode(GetResourceKvpString("Polar_gang_theme")) or {r = 18, g = 82, b = 228}
        while true do
...
RegisterNetEvent(
    "Polar:setGangMemberColour",
    function(Q, M)
        for k, n in pairs(PolarGangMembers) do
            if n[2] == Q then
                PolarGangMembers[k].colour = M
            end
        end
    end
)
...
function tPolar.gangMenuTheme(V, W, X)
    if V ~= nil and W ~= nil and X ~= nil then
        k.theme = {r = V, g = W, b = X}
        SetResourceKvp("Polar_gang_theme", json.encode(k.theme))
    else
        tPolar.notify("~r~Invalid value")
    end
end
function tPolar.hasGangNamesEnabled()
    local P = p()
    return P and k.names
end


function tPolar.isPlayerInSelectedGang(Y)
    local P = p()
    if P then
        local Q = tPolar.clientGetUserIdFromSource(Y)
        if Q and tPolar.getJobType(Q) == "" then
            for a, b in pairs(PolarGangMembers) do
                if Q == b[2] then
...
function tPolar.hasGangBlipsEnabled()
...
RegisterCommand(
    "drawmarker",
    function()
       if k.pings and not tPolar.isEmergencyService() and not globalHideUi and not tPolar.inEvent() then
            local _ = GetGameplayCamCoord()
            local a0 = tPolar.rotationToDirection(GetGameplayCamRot(2))
            ...
            if c ~= vector3(0.0, 0.0, 0.0) then
                TriggerServerEvent("Polar:sendGangMarker", gangID, c)
            end
        end
    end
)
...
local function a2(a3)
    if tPolar.getGangPingMarkerIndex() == 2 and not tPolar.isEmergencyService() and not tPolar.inEvent() then
...
local function a2(_)
    if tPolar.displayPingsOnMinimap() then
        tmpBlip = AddBlipForCoord(_.x, _.y, _.z)
...
RegisterNetEvent("Polar:drawGangMarker")
AddEventHandler(
    "Polar:drawGangMarker",
    function(a8, a9)
        if k.pings and not tPolar.isEmergencyService() and not globalHideUi and not tPolar.inEvent() then
            ...
            if aa < 1000.0 then
                ...
                if ab and ab.checkpoint then
                    DeleteCheckpoint(ab.mapBlip)
                    ab.checkpoint = nil
                end
                if ab and ab.mapBlip then
                    RemoveBlip(ab.mapBlip)
                    ab.mapBlip = nil
                end
                if a9 then
                    Z[a8] = {coords = a9, time = GetGameTimer(), dist = aa, creator = a8, checkpoint = a2(a9)}
                    if tPolar.getGangPingSoundEnabled() then
    ...
    function()
        while true do
            if k ~= nil and k.pings and PlayerIsInGang then
                for a, b in pairs(Z) do
                    if Z[a] ~= nil then
                        if tPolar.getGangPingMarkerIndex() == 3 then
                            tPolar.DrawSprite3d(
                                {
                                    pos = b.coords + vector3(0.0, 0.0, b.dist / 100),
                                    textureDict = "Polar_gang",
                                    textureName = "ping",
                                    width = 0.06,
                                    height = 0.1,
                                    r = k.theme.r,
                        ...
                        tPolar.DrawText3D(b.coords, b.creator .. "\n" .. tostring(math.floor(b.dist)) .. "m", 0.2)
                    end
                end
            end
            Citizen.Wait(0)
...
local ac = {}
RegisterNetEvent("Polar:sendGangHPStats",
    function(ad)
        ac = ad
...
local function an()
    ...
    if k ~= nil and k.healthui and PlayerIsInGang then
       if P and not tPolar.isEmergencyService() and not globalHideUi and not tPolar.inEvent() then
            local ap = 0
            local aq = tPolar.getShowHealthPercentageFlag()
            if PolarGangMembers ~= nil then
                for ar, as in pairs(PolarGangMembers) do
                ...
                --   if lastseen == "~g~Online" and tPolar.getJobType(id) == "" then
                        local at = true
                        local au = nil
                        local av = nil
                        local ad = ac[id]
                        ...
                        local aw = tPolar.getTempFromPerm(id)
                        if aw then
                        ...
                        if au and av then
                            local az = ap * 0.05 + af
                            tPolar.DrawText(ae, az, name, 0.3)
...
--end
    ...
    if l and l == P then
        if ao <= 0 then
            TriggerServerEvent("Polar:getGangHealthTable", nil)
            l = nil
        end
    else
        if ao > 0 then
            TriggerServerEvent("Polar:getGangHealthTable", gangID)
            l = P
        end
    end
end
tPolar.createThreadOnTick(an)
function tPolar.setGangUIXPos(az)
    local aA = tonumber(string.match(az, "%d+")) / 100
    ab = 0.008 * aA
end
function tPolar.setGangUIYPos(az)
    local aA = tonumber(string.match(az, "%d+")) / 100
    ac = 0.35 * aA
end
