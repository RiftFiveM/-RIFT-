...
RegisterCommand("panic",function()
    if globalOnPoliceDuty or globalNHSOnDuty or globalOnPrisonDuty or PlayerIsInGang then
        if b.Cooling == 0 then
            local f = {}
            f.Player = tPolar.getPlayerId()
            f.Ped = tPolar.getPlayerPed()
            ...
            if globalOnPoliceDuty then
                TriggerServerEvent("Polar:newPanic", f, 3)
            elseif globalNHSOnDuty then
                TriggerServerEvent("Polar:newPanic", f, 2)
            elseif globalOnPrisonDuty then
                TriggerServerEvent("Polar:newPanic", f, 4)
            elseif PlayerIsInGang then
                TriggerServerEvent("Polar:newPanic", f, 5)
            end
...
RegisterNetEvent("Polar:returnPanic",function(g, f, h)
    if globalOnPoliceDuty or globalNHSOnDuty or globalOnPrisonDuty or PlayerIsInGang and h == 5 then
        SendNUIMessage({PayloadType = "Panic", Payload = g})
        f.Headshot, handle = getHeadshot(GetPlayerPed(GetPlayerFromServerId(f.Source)))
        if h == 2 then
            tPolar.notifyPicture2(f.Headshot,2,string.format("NHS - %s", f.Name),false,"~r~NHS Paramedic in distress! - " .. f.Location)
        elseif h == 3 then
            tPolar.notifyPicture2(f.Headshot,2,string.format("MET Police - %s", f.Name),false,"~r~Officer in distress! - " .. f.Location)
        elseif h == 4 then
            tPolar.notifyPicture2(f.Headshot,2,string.format("HM Prison - %s", f.Name),false,"~r~Prison Guard in distress! - " .. f.Location)
        elseif h == 5 then
            tPolar.notifyPicture2(f.Headshot,2,string.format("Gang - %s", f.Name),false,"~r~Gang Panic Alarm Activated! - " .. f.Location)
        end
        if handle ~= nil then
            UnregisterPedheadshot(handle)
        end
