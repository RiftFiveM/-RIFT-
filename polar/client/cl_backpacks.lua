...
function previewBackpack(c)
    ...
    SetPedComponentVariation(tPolar.getPlayerPed(), prop0, prop1, prop2, 2)
end

RegisterNetEvent("Polar:boughtBackpack",function(prop0, prop1, prop2, size, backpackname)
...
RegisterNetEvent("Polar:removeBackpack",function()
    if currentBackpack ~= nil then
        ped = tPolar.getPlayerPed()
        SetPedComponentVariation(tPolar.getPlayerPed(), 5, 0, 0, 2)
        currentBackpack = nil
        b = nil
        c = nil
    end
...
AddEventHandler("Polar:onClientSpawn",function(D, E)
    if E then
        ...
        local i = function(j)
            ...
            DrawAdvancedText(0.595, 0.213, 0.005, 0.0028, 1.0, "Polar Backpacks", 255, 255, 255, 255, 1, 0)
                ...
                if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                    ...
                    if currentBackpack ~= backpackname then
                        if c ~= nil then
                            ...
                            TriggerServerEvent("Polar:BuyBackpack",prop0,prop1,prop2,backpackname,price,size,backpackstorename)
                        else
                            tPolar.notify("No backpack selected!")
                        end
                    else
                        tPolar.notify("~r~You already have this backpack!")
                    end
                    Wait(250)
                    EnableAllControlActions(1)
                end
        ...
        for q, r in pairs(a.stores) do
            for s, t in pairs(r) do
                if s == "_config" then
                    ...
                    if B then
                        tPolar.addBlip(u, v, w, x, y, z)
                    end
                    tPolar.addMarker(u, v, w, 1.0, 1.0, 1.0, 255, 0, 0, 170, 50, 27)
                    tPolar.createArea("backpack_" .. q, vector3(u, v, w), 1.5, 6, g, h, i, {storeName = q})
                end
            end
        end
    end
