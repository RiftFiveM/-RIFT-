...
local b = 2
RMenu.Add("Polarpdelevators","main",RageUI.CreateMenu("Polar Elevators", "~b~Vespucci PD", tPolar.getRageUIMenuWidth(), tPolar.getRageUIMenuHeight()))
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('Polarpdelevators', 'main')) then
...
AddEventHandler("Polar:onClientSpawn",function(i, j)
    if j then
        local k = function()
            RageUI.Visible(RMenu:Get("Polarpdelevators", "main"), true)
        end
        local l = function()
            RageUI.Visible(RMenu:Get("Polarpdelevators", "main"), false)
        end
        local m = function()
            RMenu:Get("Polarpdelevators", "main"):SetSubtitle(string.format("~w~You are on ~b~%s", a[b].name))
        end
        for n, o in pairs(a) do
            tPolar.createArea("pdelevator_" .. n, o.coords, 1.5, 6, k, l, m)
            tPolar.addMarker(o.coords.x,o.coords.y,o.coords.z - 0.98,1.0001,1.0001,0.5001,255,255,255,200,20.0,27,true,false,false,nil,nil,0.0,0.0,0.0)
        end
    end
end)
