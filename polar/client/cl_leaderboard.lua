local a = {
    ...
    rows = {
        {"Spexxster", "~b~BLUE TEAM", "", false, 5, 2, 0, "1.4"},
        {"Tylon", "~b~BLUE TEAM", "", false, 7, 5, 0, "0.5"},
        {"Foid", "~r~RED TEAM", "", false, 5, 0, 2, "0.5"},
        {"Spexxster", "~b~BLUE TEAM", "", false, 5, 0, 2, "1.4"},
        {"Tylon", "~b~BLUE TEAM", "", false, 7, 5, 0, "0.5"},
        {"Foid", "~r~RED TEAM", "", false, 5, 2, 0, "0.5"},
        {"Spexxster", "~b~BLUE TEAM", "", false, 5, 2, 0, "1.4"},
        {"Tylon", "~b~BLUE TEAM", "", false, 7, 5, 0, "0.5"},
        {"Foid", "~r~RED TEAM", "", false, 5, 2, 0, "0.5"}
    }
...
RegisterCommand("leaderboardtest",function()
    if tPolar.isDev() then
        local b = tPolar.createLeaderboard(a)
        tPolar.notify("~y~Leaderboard will close in 5 secconds")
        Wait(5000)
        tPolar.closeLeaderboard(b)
    end
end,false)
local b = 1
local c = {}
...
function tPolar.closeLeaderboard(b)
    c[b] = false
end
function tPolar.createLeaderboard(m)
    local n = Scaleform("SC_LEADERBOARD")
    n.RunFunction("SET_DISPLAY_TYPE", {1})
    n.RunFunction("SET_MULTIPLAYER_TITLE", {"Polar RP Event"})
...
RegisterNetEvent(
    "Polar:initScoreboard",
    function(t)
        d = t
    end
)
function tPolar.updateScoreboard(u, v, w)
    local x
    ...
    if u == -1 then
        ...
        if x then
            ...
            print("error: Polar:updateScoreboard couldn't find row with values", u, v, w)
        end
    end
end
RegisterNetEvent(
    "Polar:updateScoreboard",
    function(u, v, w)
        tPolar.updateScoreboard(u, v, w)
    end
)
function tPolar.setPlayerCanOpenLeaderboard(F)
    e = F
end
function tPolar.clearLeaderboardData()
    d = {columns = {}, rows = {}}
end
function tPolar.isLeaderboardSetup()
    return table.count(d.rows) > 0
end
function tPolar.removePlayerFromLeaderboard(u)
