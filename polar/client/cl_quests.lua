...
local function e()
    tPolar.loadAnimDict("anim@mp_player_intcelebrationfemale@air_guitar")
...
local function i()
    ...
    if j==0 then 
        a=true
        SetResourceKvpInt("Polar_quests_completedall",1)
        TriggerServerEvent("Polar:claimQuestReward") -- this needs to be done for adding plat days
    end
    return a 
end
local function l()
    hasCompletedAllQuestsInt=GetResourceKvpInt("Polar_quests_completedall")
    if not hasCompletedAllQuestsInt then 
        SetResourceKvpInt("Polar_quests_completedall",0)
    end
    if hasCompletedAllQuestsInt==0 then 
        for m=1,50 do 
            local n=GetResourceKvpInt("Polar_quests_q"..m.."_status")
            if not n then 
                SetResourceKvpInt("Polar_quests_q"..m.."_status",0)
                n=0 
            end
            if n==0 then 
                c[m]=true 
...
local function o(p)
    c[p]=nil
    SetResourceKvpInt("Polar_quests_q"..p.."_status",1)
    i()
    TriggerServerEvent("Polar:setQuestCompleted")
end
local function q()
    local j=50
    for k in pairs(c)do 
...
Citizen.CreateThread(function()
    ...
    for w,k in pairs(c)do 
        ...
        tPolar.createArea("quests_prop_id_"..w,undiscoveredQ.pos,100.0,6,x,z,A,{undiscoveredQID=w,undiscoveredQ=undiscoveredQ})
        local B=function()
        end
        local C=function()
        end
        local A=function(y)
            ...
            if IsControlJustReleased(0,38)then 
                ...
                if q()~=50 then 
                    ...
                    DoScreenFadeIn(2000)
                    tPolar.removeArea("quests_prop_id_"..y.undiscoveredQID)
                    tPolar.removeArea("quests_text_id_"..y.undiscoveredQID)
                else 
                    r("~g~+7 Days Polar Club Platinum","You collected them all!",600)
                    PlaySoundFrontend(-1,"Hack_Success","DLC_HEIST_BIOLAB_PREP_HACKING_SOUNDS",1)
                    ShakeGameplayCam("FAMILY5_DRUG_TRIP_SHAKE",1.0)
        ...
        tPolar.createArea("quests_text_id_"..w,undiscoveredQ.pos,2.0,6,B,C,A,{undiscoveredQ=undiscoveredQ,undiscoveredQID=w})
    end 
end)
Citizen.CreateThread(function()
    Wait(5000)
