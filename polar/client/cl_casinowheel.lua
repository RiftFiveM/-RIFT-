...
RegisterNetEvent("Polar:frameworkCleanup")
AddEventHandler("Polar:frameworkCleanup",function()
    DeleteEntity(b)
end)
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('luckywheel', 'casino')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle("Spin the wheel of Polar (50,000 Chips)","",{RightLabel = "→→→"},true,function(e, f, g)
                if g then
                    TriggerServerEvent("Polar:requestSpinLuckyWheel")
                end
            end)
        end)
    end
...
Citizen.CreateThread(function()
    ...
    local i = tPolar.loadModel("vw_prop_vw_luckywheel_02a")
    local j = tPolar.loadModel(c)
...
function goToWheel()
    if not d then
        ...
        local D = false
        while not D do
            local E = GetEntityCoords(tPolar.getPlayerPed())
        ...
        SetEntityHeading(tPolar.getPlayerPed(), 10.9754)
        Wait(2000)
...
AddEventHandler("Polar:onClientSpawn",function(D, E)
    if E then
        ...
        for M, N in pairs(a) do
            tPolar.addBlip(N.entryPosition.x, N.entryPosition.y, N.entryPosition.z, 681, 0, "Polar Wheel", 0.7, true)
            tPolar.createArea("Polarwheel_" .. M, N.entryPosition, 1.5, 6, H, J, K, {})
        end
	end
end)

RegisterNetEvent("Polar:syncLuckyWheel")
AddEventHandler("Polar:syncLuckyWheel",function(O)
    print(O)
    Wait(900)
    actuallyRollWheel(O)
end)

RegisterNetEvent("Polar:spinLuckyWheel")
AddEventHandler("Polar:spinLuckyWheel",function()
    goToWheel()
end)

RegisterNetEvent("Polar:spinLuckyWheelReaction")
AddEventHandler("Polar:spinLuckyWheelReaction",function(P)
    local y = tPolar.getPlayerPed()
