...
local function f()
    ...
    g.position=tPolar.getPlayerCoords()
    g.radius=g.radius+0.0
    g.vehicles.speed=g.vehicles.speed+0.0
    TriggerServerEvent("Polar:createRPZone",g)
end
...
local function h()
    ...
    if i then 
        TriggerServerEvent("Polar:removeRPZone",i)
    end 
end
local inRPZone = false
RMenu.Add("rpzones","mainmenu",RageUI.CreateMenu("","~b~Main Menu",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners","admin"))
RMenu.Add("rpzones","weapons",RageUI.CreateSubMenu(RMenu:Get("rpzones","mainmenu"),"","~b~Weapons",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners","admin"))
RMenu.Add("rpzones","vehicles",RageUI.CreateSubMenu(RMenu:Get("rpzones","mainmenu"),"","~b~Vehicles",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners","admin"))
...
RegisterNetEvent("Polar:createRPZone",function(g)
...
RegisterNetEvent("Polar:removeRPZone",function(r)
...
local function G(g)
    local l,u=GetCurrentPedWeapon(tPolar.getPlayerPed())
    if u~=0 then 
        t(g,u)
    end
    local y,H=tPolar.getPlayerVehicle()
    if y~=0 and H then 
        A(g,y)
    end
...
tPolar.createThreadOnTick(I)

function tPolar.GetRPZoneInfo()
    local K=tPolar.getPlayerCoords()
    for l,g in ipairs(d)do 
        if#(K-g.position)<g.radius then
            return g
