...
RMenu.Add('LicenseCentre', 'main', RageUI.CreateMenu("", "~b~" .. "Job Centre", tPolar.getRageUIMenuWidth(), tPolar.getRageUIMenuHeight(), "banners", "groups"))
RMenu.Add("LicenseCentre", "confirm", RageUI.CreateSubMenu(RMenu:Get('LicenseCentre', 'main',  tPolar.getRageUIMenuWidth(), tPolar.getRageUIMenuHeight())))
    ...
    if q then
        ...
        local u = function(s)
            if IsControlJustPressed(1, 38) then
                TriggerServerEvent('Polar:getOwnedLicenses')
                Wait(500)
            ...
            tPolar.addMarker(licensecentre.location.x, licensecentre.location.y,licensecentre.location.z-0.98,0.7,0.7,0.5,255, 255,255,250,50,27,false,false)
        end
        tPolar.createArea("licensecentre", licensecentre.location, 1.5, 6, r, t, u, {})
        tPolar.addBlip(licensecentre.location.x, licensecentre.location.y, licensecentre.location.z, 457, 2, "Job Centre", 0.6, true)
    end
end)
...
function func_licenseMenuControl()
    if IsControlJustPressed(0, 167) then
        if a then
            ...
            TriggerServerEvent("Polar:GetLicenses")
        end
    end
    if a then
        if tPolar.isNewPlayer() then
        ...
        DrawAdvancedText(0.595, 0.213, 0.005, 0.0028, 1.0, "Polar Licenses", 255, 255, 255, 255, 1, 0)
...
if not tPolar.isPurge() then
    tPolar.createThreadOnTick(func_licenseMenuControl)
end
RegisterNetEvent("Polar:RecievedLicenses",function(i)
    b = i
    a = true
end)
RegisterNetEvent("Polar:gotOwnedLicenses",function(i)
    for k,v in pairs(licensecentre.licenses) do
        v.notOwned = true
