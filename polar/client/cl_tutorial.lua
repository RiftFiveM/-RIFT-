...
RegisterNetEvent("Polar:initMoney",function(n, o)
    m = o
end)
local function p(q)
...
local function r(s, t)
    ...
    while #(tPolar.getPlayerCoords() - s) > 4.0 do
        Citizen.Wait(0)
    end
    RemoveBlip(v)
    DeleteCheckpoint(u)
...
local function w(x, y, z)
    tPolar.loadModel("metrotrain")
    tPolar.loadModel("s_m_m_lsmetro_01")
    local A = 0
    while true do
...
local function N()
    TriggerEvent("Polar:Announce", "FOLLOW THE YELLOW MARKERS!")
...
local function O()
    local P = tPolar.addMarker(b.x, b.y, b.z, 1.0, 1.0, 1.0, 50, 205, 50, 150, 50, 32, false, false, true)
    ...
    ScaleformMovieMethodAddParamTextureNameString("Polar TUTORIAL")
    ScaleformMovieMethodAddParamTextureNameString("~g~Welcome to Polar")
    ...
    local E = PlayerPedId()
    while true do
        local R = #(tPolar.getPlayerCoords() - b)
        if R < 3.0 then
    ...
    tPolar.removeMarker(P)
    SetScaleformMovieAsNoLongerNeeded(Q)
    tPolar.loadAnimDict("cellphone@")
    tPolar.loadModel("prop_amb_phone")
    p("ring")
    Citizen.Wait(2000)
    local H = tPolar.getPlayerCoords()
...
local function V()
    ...
    while tPolar.getPlayerVehicle() ~= l do
        Citizen.Wait(0)
    end
    h = nil
    DeleteCheckpoint(u)
...
local function X(Y, Z, _)
    ...
    while #(tPolar.getPlayerCoords() - Y) > a0 do
        if _ then
            _()
        end
        Citizen.Wait(0)
...
local function a1()
    ...
    local C = tPolar.getPlayerVehicle()
    while tPolar.getPlayerVehicle() ~= 0 do
        Citizen.Wait(0)
    end
    ...
    while table.count(VehiclesFetchedTable) == 0 do
        ...
        elseif DoesEntityExist(testDriveCar) then
            ...
            local a2 = #(vector3(-34.113563537598, -1101.7242431641, 25.422456741333) - tPolar.getPlayerCoords()) <= 2.5
            if a2 then
...
local function a3()
    ...
    while table.count(globalVehicleModelHashMapping) == 0 or tPolar.getPlayerVehicle() == 0 do
        if RageUI.Visible(RMenu:Get("PolarGarages", "main")) then
            ...
            h = "Select Garages"
        elseif RageUI.Visible(RMenu:Get("PolarGarages", "owned_vehicles")) then
            i = "This lists all your garage types. For now you will only have the Standard Garage."
            h = "Select Standard Garage"
        elseif RageUI.Visible(RMenu:Get("PolarGarages", "owned_vehicles_submenu")) then
            i = "This lists all the vehicles you have bought for this garage type."
            h = "Select your newly purchased vehicle"
        elseif RageUI.Visible(RMenu:Get("PolarGarages", "owned_vehicles_submenu_manage")) then
            i = "You can spawn your vehicle, or choose to sell and rent it to another player here."
            h = "Press Spawn Vehicle"
        elseif RageUI.Visible(RMenu:Get("PolarGarages", "settings")) then
            i = nil
            h = "Go back to the main menu"
        elseif
            RageUI.Visible(RMenu:Get("PolarGarages", "rented_vehicles")) or RageUI.Visible(RMenu:Get("PolarGarages", "customfolders"))
         then
            RageUI.Visible(RMenu:Get("PolarGarages", "main"), true)
            notify("~r~This section is not avaliable during the tutorial")
        else
            local a2 = #(vector3(-51.893981933594, -1112.7712402344, 25.438014984131) - tPolar.getPlayerCoords()) < 2.5
            if a2 then
    ...
    Citizen.Wait(1000)
    SetEntityInvincible(tPolar.getPlayerVehicle(), true)
    i = nil
    h = nil
end
local function a4()
...
local function a6()
    ...
    local a7 = tPolar.getCustomization()
    local a8 = false
    while true do
        if not RageUI.IsAnyMenuOfTypeVisible("Polarclothing") then
            if a8 then
                break
            end
            local a2 = #(vector3(123.60697174072, -219.52917480469, 53.557830810547) - tPolar.getPlayerCoords()) < 2.5
            if a2 then
            ...
            if RageUI.Visible(RMenu:Get("Polarclothing", "mainMenu")) then
                ...
                h = "Select Clothes"
            elseif RageUI.Visible(RMenu:Get("Polarclothing", "changeClothing")) then
                i = "You can pick from an arrangement of stock and custom clothing. Scroll through to see each item."
                h = "Change your outfit"
            elseif RageUI.Visible(RMenu:Get("Polarclothing", "changePed")) then
            ...
            elseif RageUI.Visible(RMenu:Get("Polarclothing", "clearProps")) then
                i = "This can be used to remove accessories. You can also toggle them through the Y menu."
                h = "Go back to pick an outfit"
            end
            if not table.contentEquals(tPolar.getCustomization(), a7) then
                a8 = true
            end
        end
        Citizen.Wait(0)
...
local function ac()
    ...
    EndScaleformMovieMethod()
    p("questcomplete")
    TriggerServerEvent("Polar:setCompletedTutorial")
    local U = GetGameTimer()
...
local function ad()
    ...
    D()
    V()
    if table.count(VehiclesFetchedTable) == 0 or tPolar.isDevMode() then
        if m > 20000 then
            a1()
        end
    else
...
RegisterCommand("tutorial",function()
    if tPolar.isDevMode() then
        ad()
    end
end,false)
RegisterNetEvent("Polar:playTutorial", ad)
AddEventHandler("Polar:onClientSpawn",function(ae, af)
    if af and not tPolar.isPurge() then
        Citizen.Wait(10000)
        TriggerServerEvent("Polar:checkTutorial")
    end
end)
local function ag()
    if g then
        ...
        if not tPolar.isPurge() then
            ...
            tPolar.setWeapon(E, "WEAPON_UNARMED", true)
        end
    end
end
tPolar.createThreadOnTick(ag)
RegisterNetEvent("Polar:setIsNewPlayer",function()
    f = true
end)
function tPolar.isNewPlayer()
    return f
end
function tPolar.isInTutorial()
    return g
end
