...
local m = module("Polar-vehicles", "cfg_garages")
m=m.garages
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.ButtonWithStyle("Check Plate Availability", "", {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                if Selected then
                    ...
                    if u ~= "" then
                        TriggerServerEvent("Polar:checkPlateAvailability", u)
                    end
                end
            end)
        end)
    ...
    if RageUI.Visible(RMenu:Get('plateshop', 'changeplate')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.Button("Change Number Plate", "~g~Changing plate of "..selectedCarName, {RightLabel = "~g~£50,000"}, true, function(Hovered, Active, Selected)
                if Selected then
                    TriggerServerEvent("Polar:ChangeNumberPlate", selectedCar)
                end
            end)
        end)
    end
    ...
    if i then
        ...
        local m = function(k)
            if IsControlJustPressed(1, 38) then
                TriggerServerEvent('Polar:getCars')
                RageUI.ActuallyCloseAll()
                RageUI.Visible(RMenu:Get("plateshop", "main"), not RageUI.Visible(RMenu:Get("plateshop", "main")))
            end
            tPolar.DrawText3D(location, "Press [E] to open License Plate Management", 0.2)
        end
        tPolar.createArea("licenseplate", location, 1.5, 6, j, l, m, {})
        tPolar.addMarker(location.x, location.y, location.z - 1, 1.0, 1.0, 1.0, 255, 0, 0, 170, 50, 27)
        tPolar.addBlip(location.x, location.y, location.z, 606, 2, "Licence Plate Manager")
    end
end)


RegisterNetEvent("Polar:RecieveNumberPlate")
AddEventHandler("Polar:RecieveNumberPlate", function(numplate)
...
RegisterNetEvent("Polar:carsTable")
AddEventHandler("Polar:carsTable",function(cars)
    carstable = cars
end)
