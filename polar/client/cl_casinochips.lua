...
RMenu.Add('PolarChips','cashier',RageUI.CreateMenu("","",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"shopui_title_casino","shopui_title_casino"))
RMenu:Get('PolarChips','cashier'):SetSubtitle("~b~CHIPS")
RMenu.Add('casino_enter','casino',RageUI.CreateMenu("","",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"shopui_title_casino","shopui_title_casino"))
RMenu:Get('casino_enter','casino'):SetSubtitle("~b~ENTER")
RMenu.Add('casino_exit','casino',RageUI.CreateMenu("","",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"shopui_title_casino","shopui_title_casino"))
RMenu:Get('casino_exit','casino'):SetSubtitle("~b~EXIT")
RMenu.Add('casino_rooftop_exit','casino',RageUI.CreateMenu("","",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"shopui_title_casino","shopui_title_casino"))
...
Citizen.CreateThread(function()
    for i, j in pairs(h) do
        tPolar.createDynamicPed(j.model,j.pedPosition,100.0,true,"mini@strip_club@idles@bouncer@base","base",75.0,nil,function(k)
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('PolarChips', 'cashier')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle("Buy chips","",{RightLabel="→→→"},true,function(n,o,p)
                if p then 
                    ...
                    if tonumber(q)then 
                        q=tonumber(q)
                        if q>0 then 
                            TriggerServerEvent("Polar:buyChips",q)
                        else 
                            tPolar.notify("~r~Negative amount.")
                        end 
                    else 
            ...
            RageUI.ButtonWithStyle("Buy all chips","",{RightLabel="→→→"},true,function(n,o,p)
                if p then 
                    TriggerServerEvent("Polar:buyChips")
                end 
            end)
            RageUI.ButtonWithStyle("Sell chips","",{RightLabel="→→→"},true,function(n,o,p)
                if p then 
                    ...
                    if tonumber(q)then 
                        q=tonumber(q)
                        if q>0 then 
                            TriggerServerEvent("Polar:sellChips",q)
                        else 
                            tPolar.notify("~r~Negative amount.")
                        end 
                    else 
            ...
            RageUI.ButtonWithStyle("Sell all chips","",{RightLabel="→→→"},true,function(n,o,p)
                if p then 
                    TriggerServerEvent("Polar:sellChips")
                end 
            end)
        end)
    end
    if RageUI.Visible(RMenu:Get('casino_exit', 'casino')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle("Exit Diamond Casino","",{RightLabel="→→→"},true,function(n,o,p)
                if p then 
                    TriggerServerEvent("Polar:exitDiamondCasino")
                    tPolar.teleport(e.x,e.y,e.z)
                    if cardObjects then 
    ...
    if RageUI.Visible(RMenu:Get('casino_enter', 'casino')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle("Enter Diamond Casino","",{RightLabel="→→→"},true,function(n,o,p)
                if p and tPolar.getPlayerCombatTimer() == 0 then 
                    TriggerServerEvent("Polar:enterDiamondCasino")
                    tPolar.teleport(f.x,f.y,f.z)
                end 
            end)
            RageUI.ButtonWithStyle("Diamond Casino Rooftop","",{RightLabel="→→→"},true,function(n,o,p)
                if p then 
                    tPolar.teleport(g.x,g.y,g.z)
                end 
            end)
        end)
    end
...
function showCasinoChipsCashier(u)
    ...
    RageUI.Visible(RMenu:Get('PolarChips','cashier'),u)
end
...
Citizen.CreateThread(function()
    while true do 
        local v=GetEntityCoords(tPolar.getPlayerPed())
        a=#(v-e)
        b=#(v-f)
        c=#(v-g)
        Wait(100)
...
RegisterNetEvent("Polar:setDisplayChips")
AddEventHandler("Polar:setDisplayChips",function(x)
    d=getMoneyStringFormatted(tostring(x))
end)

RegisterNetEvent("Polar:chipsUpdated",function()
...
AddEventHandler("Polar:onClientSpawn",function(D, E)
    if E then
        ...
        for H,k in pairs(h)do 
            tPolar.addBlip(k.entryPosition.x,k.entryPosition.y,k.entryPosition.z,683,0,"Chips Cashier",0.7,true)
            tPolar.addMarker(k.entryPosition.x,k.entryPosition.y,k.entryPosition.z,1.0,1.0,1.0,138,43,226,70,50,27)
            tPolar.createArea("chipscashier_"..H,k.entryPosition,1.5,6,C,E,F,{})
        end
        for I,J in pairs(z)do 
            tPolar.createArea("casinos_"..I,J.position,J.radius,6,function()
            end,function()
            end,G,{})
        end 
    end
