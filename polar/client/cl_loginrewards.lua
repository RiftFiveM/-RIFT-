...
RMenu.Add("loginrewards","mainmenu",RageUI.CreateMenu("","Main Menu",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners", "rewards"))
local b = 0
local c = 0
local function d(e)
    ...
    if RageUI.Visible(RMenu:Get('loginrewards', 'mainmenu')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.Button(string.format("Day %d", t),g(s),{RightLabel = n(t, not s)},s,function(p, p, u)
                if u then
                    TriggerServerEvent("Polar:claimNextLoginReward")
                    b = b + 1
...
RegisterNetEvent("Polar:setDailyRewardInfo",function(w, x, y)
    b = w
    c = GetGameTimer() + (86400 - (y - x)) * 1000
    if w == 0 then
        Citizen.Wait(20000)
        tPolar.notify("~y~You have not started your daily reward. Do /dailyrewards to start now!")
    end
end)
...
Citizen.CreateThread(function()
    ...
    while true do
        if r() and b > 0 then
            tPolar.notify("~g~You can claim your next daily reward at /dailyrewards!")
            break
        end
        Citizen.Wait(6000)
    end
