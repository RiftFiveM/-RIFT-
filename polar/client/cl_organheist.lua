...
AddEventHandler("Polar:onClientSpawn",function(p, q)
    if q then
        tPolar.addMarker(b.x, b.y, b.z, 0.7, 0.7, 0.5, 0, 125, 255, 125, 150, 20, false, false, true)
        tPolar.addBlip(b.x, b.y, b, 378, 3, "Organ Heist Police")
        tPolar.addMarker(c.x, c.y, c.z, 0.7, 0.7, 0.5, 255, 0, 0, 125, 150, 20, false, false, true)
        tPolar.addBlip(c.x, c.y, c.z, 378, 1, "Organ Heist Criminals")
    end
end)
...
Citizen.CreateThread(function()
    while true do
        if f then
            ...
            if IsControlJustPressed(0, 38) then
                if not globalOnPoliceDuty then
                    if not g then
                        TriggerServerEvent("Polar:joinOrganHeist")
                        Wait(100)
                    else
                        tPolar.notify("~r~Already joined the Organ Heist!")
                    end
                else
                    tPolar.notify("~r~You are police, please use the other entrance!")
                end
            end
        elseif e then
            ...
            if IsControlJustPressed(0, 38) then
                if globalOnPoliceDuty then
                    ...
                    tPolar.notify("~r~You are a civillian, please use the other entrance!")
                end
            end
        end
        ...
        if inGamePhase or inWaitingStage then
            ...
            RemoveWeaponFromPed(tPolar.getPlayerPed(), "WEAPON_MOLOTOV")
            RemoveWeaponFromPed(tPolar.getPlayerPed(), "WEAPON_FLASHBANG")
            RemoveWeaponFromPed(tPolar.getPlayerPed(), "WEAPON_SMOKEGRENADE")
            RemoveWeaponFromPed(tPolar.getPlayerPed(), "WEAPON_BZGAS")
            RemoveWeaponFromPed(tPolar.getPlayerPed(), "WEAPON_STUNGUN")
            local r = PlayerPedId()
...
RegisterNetEvent("Polar:teleportToOrganHeist",function(v, w, x, y)
    local r = PlayerPedId()
    d = w
    canPlayAnim = false
    h = x
    ...
    if x == "civ" then
        ...
        tPolar.createAtm("Organ Heist", k.atmLocation)
    elseif x == "police" then
        SetPedRelationshipGroupHash(PlayerPedId(), m)
    end
    for l, z in pairs(k.gunStores[h]) do
        tPolar.createGunStore(z[1], z[2], z[3])
    end
    tPolar.setFriendlyFire(false)
    DoScreenFadeOut(1000)
...
RegisterNetEvent("Polar:addOrganHeistPlayer",function(A, x)
    if x == "civ" then
        j[A] = true
    elseif x == "police" then
        i[A] = true
...
RegisterNetEvent("Polar:organHeistKillConfirmed",function(B)
    PlaySoundFrontend(-1, "Weapon_Upgrade", "DLC_GR_Weapon_Upgrade_Soundset", true)
    tPolar.playScreenEffect("MP_Celeb_Win", 0.25)
    tPolar.notify("~g~Killed " .. B .. " received £25,000")
end)
    ...
    while true do
        if inWaitingStage then
            ...
            if #(s - k.pastGate) < 3.0 then
                ...
                NetworkFadeInEntity(PlayerPedId(), 0)
                DoScreenFadeIn(1000)
                tPolar.notify("~r~You got too far from the organ heist and have been teleported back.")
            end
        end
        if inGamePhase then
            local s = GetEntityCoords(PlayerPedId())
...
RegisterNetEvent("Polar:startOrganHeist",function()
...
RegisterNetEvent("Polar:removeFromOrganHeist",function(F)
    if i[F] then
        i[F] = nil
    end
    if j[F] then
...
RegisterNetEvent("Polar:endOrganHeist",function()
    ...
    inGamePhase = false
    d = 600
    tPolar.deleteAtm("Organ Heist")
    for l, z in pairs(k.gunStores[h]) do
        tPolar.deleteGunStore(z[1])
    end
    h = ""
    k = nil
...
RegisterNetEvent("Polar:endOrganHeistWinner",function(x)
    Wait(10000)
    winOrganHeist(x)
    local r = PlayerPedId()
    tPolar.setFriendlyFire(true)
...
function tPolar.setDeathInOrganHeist()
    inOrganHeist = false
    inWaitingStage = false
    inGamePhase = false
end
