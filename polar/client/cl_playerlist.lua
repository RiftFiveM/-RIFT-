...
function func_playerlistControl()
    if IsUsingKeyboard(2) then
        if IsControlJustPressed(0, 212) then
            a = not a
            TriggerServerEvent("Polar:getPlayerListData")
            Wait(100)
...
tPolar.createThreadOnTick(func_playerlistControl)
RegisterNUICallback("closePolarPlayerList",function(j, k)
    SetNuiFocus(false, false)
    TriggerScreenblurFadeOut(100.0)
end)
AddEventHandler("Polar:onClientSpawn",function(h, l)
    if l then
        TriggerServerEvent("Polar:getPlayerListData")
    end
end)

RegisterNetEvent("Polar:gotFullPlayerListData",function(staff, police, nhs, lfb, hmp, civillians)
...
local u,v,x
RegisterNetEvent("Polar:playerListMetaUpdate",function(t)
...
function sendFullPlayerListData()
    ...
    SendNUIMessage({setServerMetaData = '<img src="playerlist_images/Polar.png" align="top" width="20px",height="20px"><span class="staff">' ..tostring(H) .. "</span>"})
...
Citizen.CreateThread(function()
    while true do
        ...
        if u ~= nil and v ~= nil and x ~= nil then
            local A = tPolar.getUserId()
            if tPolar.isDev() then
                A = 0.5
            end
            SetDiscordAppId(1163584587562242178)
            SetDiscordRichPresenceAsset("Polar")
            SetDiscordRichPresenceAssetText("discord.gg/f3BWQpG3bR")
            SetDiscordRichPresenceAssetSmall("Polar")
            SetDiscordRichPresenceAssetSmallText("Polar British Roleplay")
            SetRichPresence("[ID:" .. tostring(A) .."] | " .. tostring(v) .. "/" .. tostring(x) .. "\n" .. tPolar.getPlayerName(PlayerId()))
        end
        Wait(15000)
    end
end)