...
local aZ = false
function tPolar.giveWeapons(d,e)
    local f=PlayerPedId()
    if e then 
...
function tPolar.hasWeapon(hash)
...
function tPolar.getWeapons()
    ...
    for b,c in pairs(a.weapons)do 
        if HasPedGotWeapon(f,c.hash)then
            ...
            --h.attachments=tPolar.getAllWeaponAttachments(b)
            d[b]=h 
        end 
    end
    return d 
end
function tPolar.removeAllWeapons()
    RemoveAllPedWeapons(tPolar.getPlayerPed())
end
...
RegisterCommand("storecurrentweapon",function()
    ...
    if s+3000<GetGameTimer()then 
        ...
        if HasPedGotWeapon(PlayerPedId(),'WEAPON_PISTOL50')or HasPedGotWeapon(PlayerPedId(),'WEAPON_MACHINEPISTOL')then 
            ...
            TriggerServerEvent("Polar:forceStoreSingleWeapon",g)
        end 
    else 
        tPolar.notify("~r~Store weapons cooldown, please wait.")
    end 
end)
            ...
            if not table.has(X, k) then
                for k,v in pairs(a.weapons) do
                    if i == GetHashKey(k) then
                        TriggerServerEvent('Polar:acType17', v.name)
                        Wait(60000)
                    end
                end
            end
