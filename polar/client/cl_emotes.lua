...
RMenu.Add("emotesmenu","mainmenu",RageUI.CreateMenu("","Main Menu",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners", "emotes"))
RMenu.Add("emotesmenu","emotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "mainmenu"),"","Emotes",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners", "emotes"))
RMenu.Add("emotesmenu","searchemotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "mainmenu"),"","Search Emotes",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners", "emotes"))
RMenu.Add("emotesmenu","danceemotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "emotes"),"","Dance Emotes",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners", "emotes"))
RMenu.Add("emotesmenu","customemotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "emotes"),"","Custom Emotes",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners", "emotes"))
RMenu.Add("emotesmenu","gunemotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "emotes"),"","Gun Emotes",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners", "emotes"))
RMenu.Add("emotesmenu","propemotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "emotes"),"","Prop Emotes",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners", "emotes"))
RMenu.Add("emotesmenu","sharedemotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "emotes"),"","Shared Emotes",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners", "emotes"))
RMenu.Add("emotesmenu","walkingstyles",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "mainmenu"),"","Walking Styles",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners", "emotes"))
RMenu.Add("emotesmenu","moods",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "mainmenu"),"","Moods",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners", "emotes"))
local b = false
local c = ""
local d = {}
local e = 0
...
local function p(q)
    ...
    if not q and GetGameTimer() - e < 1500 then
        tPolar.notify("~r~Emotes are being rate limited.")
        return
    end
    b = false
    f = nil
...
local function F(E)
    if tPolar.getPlayerVehicle() ~= 0 then
        tPolar.notify("~r~Can not use scenarios whilst in a vehicle.")
        return
    end
    local A = PlayerPedId()
    ...
    elseif E[1] == "MaleScenario" then
        if tPolar.getModelGender() == "male" then
            ...
            tPolar.notify("~r~This scenario is male only.")
        end
...
local function a0(E)
    ...
    if a5 and a5.ptfxAsset then
        ...
        tPolar.notify(h.info)
    end
    K(E)
end
local a7 = {2, 160, 161, 163, 167}
local function a6(E)
    if not tPolar.canAnim() or tPolar.isHandcuffed() then
        return true
    end
    local A = PlayerPedId()
    ...
    local a9 = tPolar.getPlayerVehicle()
...
local function a7(E)
    if a6(E) then
        tPolar.notify("~r~Can not this emote at this time.")
        return
    end
...
local function a8()
    ...
    local aa = -1
    local ab = 2.0
    local ac = tPolar.getPlayerCoords()
...
local function ag(ah)
    if a6() then
        tPolar.notify("~r~Can not use shared emotes at this time.")
        return
    end
    local ai = a8()
    if ai ~= 0 then
        i = ah
        j = ai
        TriggerServerEvent("Polar:sendSharedEmoteRequest", ai, ah)
    else
        tPolar.notify("~r~No player is near by.")
    end
end
local function aj(E)
    Citizen.CreateThreadNow(function()
        local ak = E[1]
        tPolar.loadAnimDict(ak)
    ...
    if RageUI.Visible(RMenu:Get('emotesmenu', 'searchemotes')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if foundMatch == false then
                emoteString = tPolar.KeyboardInput("Enter Emote Name", "", 10)
...
local function aF(aC, aD, aE)
    if #aD < 1 then
        tPolar.notify("~r~No emote name was specified.")
        return
    end
    ...
    elseif a.guns[ah] then
        ...
        tPolar.notify("~r~Invalid emote name was specified.")
    end
end
...
RegisterCommand("walk",function(aC, aD, aE)
    if #aD < 1 then
        tPolar.notify("~r~No walk name was specified.")
        return
    end
    local ah = aD[1]
    if not ah then
    ...
    if a.walks[ah] then
        ...
        tPolar.notify("~r~Invalid walk name was specified.")
    end
end)
RegisterNetEvent("Polar:sendSharedEmoteRequest",function(aG, ah)
    if a.shared[ah] and not a6() then
        k = ah
        l = aG
        m = GetGameTimer()
        tPolar.notify("~y~Y~w~ to accept, ~r~L~w~ to refuse (~g~" .. a.shared[ah][3] .. "~w~)")
    end
end)
RegisterNetEvent("Polar:receiveSharedEmoteRequest",function(ah)
    p(false)
    Citizen.Wait(300)
    a7(a.shared[ah])
end)
RegisterNetEvent("Polar:receiveSharedEmoteRequestSource",function()
...
local function aT()
    if k then
        ...
        elseif IsControlJustPressed(1, 246) then
            if a6() then
                tPolar.notify("~r~You can not use emotes at this time.")
            else
                TriggerServerEvent("Polar:receiveSharedEmoteRequest", l, a.shared[k][4])
            end
            aK()
        end
    end
    if b and f and g then
        ...
        if not IsEntityPlayingAnim(PlayerPedId(), f, g, 3) or tPolar.getPlayerCombatTimer() > 0 then
            p(true)
        end
    end
end
function tPolar.playEmote(ah)
    if a.emotes[ah] then
        a7(a.emotes[ah])
    elseif a.dances[ah] then
        a7(a.dances[ah])
