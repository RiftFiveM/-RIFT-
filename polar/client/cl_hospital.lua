...
Citizen.CreateThread(function()
    if true then 
        local e=function(f)
            tPolar.drawNativeNotification("Press ~INPUT_PICKUP~ to recieve medical attention.")
        end
        local g=function(f)
        end
        local h=function(f)
            if IsControlJustPressed(1,51)then 
                ...
                if not tPolar.isInComa()then 
                    if tPolar.getPlayerVehicle()==0 then 
                        if GetGameTimer()>a+30000 then 
                            SetEntityHealth(i,200)
                            tPolar.notify("~g~Healed, free of charge by the NHS.")
                            a=GetGameTimer()
                        else 
                            tPolar.notify("~r~Healing cooldown, come back later.")
                        end 
                    else 
                        tPolar.notify("~r~You can not heal whilst in a vehicle.")
                    end
                else tPolar.notify("~r~You are bleeding out, call a doctor ASAP!")
                end 
            end 
        end
        for j,k in pairs(b)do 
            tPolar.addMarker(k.x,k.y,k.z,1.0,1.0,1.0,0,0,255,100,50,27,false,false,true)
            tPolar.createArea(j.."_hospital",k,1.5,6,e,g,h,{})
        end 
    end 
end)