...
local b = false
RMenu.Add("Polaratm","mainmenu",RageUI.CreateMenu("", "", tPolar.getRageUIMenuWidth(), tPolar.getRageUIMenuHeight(), "banners", "atm"))
RMenu:Get("Polaratm", "mainmenu"):SetSubtitle("~b~ATM")
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('Polaratm', 'mainmenu')) then
                ...
                if Selected then
                    ...
                    if tonumber(e) then
                        if GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
                            if a then
                                tPolar.playAnim(false, {{"amb@prop_human_atm@male@exit", "exit"}}, false)
                                TriggerServerEvent('Polar:Deposit', tonumber(e))
                            else
                                tPolar.notify("~r~Not near ATM.")
                            end
                        else
                            tPolar.notify("~r~Get out your vehicle to use the ATM")
                        end
                    else
                ...
                if Selected then
                    ...
                    if tonumber(e) then
                        if GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
                            if a then
                                tPolar.playAnim(false, {{"amb@prop_human_atm@male@exit", "exit"}}, false)
                                TriggerServerEvent('Polar:Withdraw', tonumber(e))
                            else
                                tPolar.notify("~r~Not near ATM.")
                            end
                        else
            ...
            RageUI.Button("Deposit All", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected) 
                if Selected then
                    if GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
                        if a then
                            tPolar.playAnim(false, {{"amb@prop_human_atm@male@exit", "exit"}}, false)
                            TriggerServerEvent('Polar:DepositAll')
                        else
                            tPolar.notify("~r~Not near ATM.")
                        end
                    else
            ...
            RageUI.Button("Withdraw All", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected) 
                if Selected then
                    if GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
                        if a then
                            tPolar.playAnim(false, {{"amb@prop_human_atm@male@exit", "exit"}}, false)
                            TriggerServerEvent('Polar:WithdrawAll')
                        else
                            tPolar.notify("~r~Not near ATM.")
                        end
                    else
...
local function g()
    RageUI.CloseAll()
    RageUI.Visible(RMenu:Get("Polaratm", "mainmenu"), true)
end
local function h()
    RageUI.CloseAll()
    RageUI.Visible(RMenu:Get("Polaratm", "mainmenu"), false)
end
Citizen.CreateThread(function()
    ...
    for m, n in pairs(a.atms) do
        tPolar.createArea("atm_" .. m, n, 1.5, 6, i, k, l, {atmId = m})
        local o = tPolar.addBlip(n.x, n.y, n.z, 108, 4, "ATM", 0.8, true)
        tPolar.addMarker(n.x, n.y, n.z, 0.7, 0.7, 0.5, 0, 255, 125, 125, 50, 29, false, false, true)
...
function tPolar.createAtm(t, u)
    ...
    tPolar.createArea(v,u,1.5,6,i,k,function()end)
    local w = tPolar.addMarker(u.x, u.y, u.z, 0.7, 0.7, 0.5, 0, 255, 125, 125, 50, 29, false, false, true)
    s[t] = {area = v, marker = w}
end
function tPolar.deleteAtm(t)
    local x = s[t]
    if x then
        tPolar.removeMarker(x.marker)
        tPolar.removeArea(x.area)
        s[t] = nil
    end
end
local y = false
local function z(A)
    ...
    for p = 1, math.random(3, 4) do
        ...
        local E = true
        tPolar.minigameCircularProgressBar({Difficulty = D, Timeout = 25000, onComplete = function(F)
            B = F
            E = false
        ...
        if not B then
            ...
            tPolar.startCircularProgressBar("",2000,nil,function()end)
            TriggerServerEvent("Polar:atmWireCutSparks", A, false)
            Citizen.Wait(2000)
        end
    end
    C = true
...
RegisterNetEvent("Polar:atmWireCuttingSuccess",function()
...
RegisterNetEvent("Polar:atmWireCutSparks",function(A)
    local L = a.robberyAtms[A]
    local M = G(L)
    tPolar.loadPtfx("core")
...
RegisterNetEvent("Polar:startAtmWireCutting",function(A)
    y = true
    local V = PlayerPedId()
    local L = a.robberyAtms[A]
    tPolar.setCanAnim(false)
    tPolar.setPlayerCombatTimer(30, false)
    tPolar.setWeapon(V, "WEAPON_UNARMED", true)
    ...
    tPolar.loadClipSet("move_ped_crouched")
...
RegisterNetEvent("Polar:atmInkArea",function(A)
    local L = a.robberyAtms[A]
    tPolar.loadPtfx("veh_xs_vehicle_mods")
    for p = 1, 10 do
...
RegisterNetEvent("Polar:atmGenericAlarm",function(A)
    local L = a.robberyAtms[A]
...
local function a1(j)
    Y = math.random(6, 12)
    a0 = 0
    TriggerServerEvent("Polar:getAtmHasBeenRobbed", j.robberyId)
end
local function a2(j)
    if y then
        TriggerServerEvent("Polar:atmStopWireCutting", j.robberyId)
    end
    Z = 0
end
local function a3()
    ...
    Citizen.Wait(500)
    local a4 = tPolar.getPlayerCoords()
...
local function a7()
    ...
    tPolar.loadPtfx(a8[1])
    Citizen.Wait(500)
    UseParticleFxAsset(a8[1])
    local a4 = tPolar.getPlayerCoords() + GetEntityForwardVector(PlayerPedId()) * 1.0
    ...
    if select(2, GetCurrentPedWeapon(V)) == "WEAPON_CROWBAR" then
        ...
        if IsDisabledControlJustPressed(0, 24) and not _ then
            Citizen.CreateThreadNow(function()
                ...
                tPolar.loadAnimDict("melee@small_wpn@streamed_core")
                V = PlayerPedId()
                ...
                Z = Z + 1
                if Z >= Y then
                    TriggerServerEvent("Polar:startAtmWireCutting", j.robberyId)
                end
...
Citizen.CreateThread(function()
    for A, L in pairs(a.robberyAtms) do
        tPolar.createArea("atmrobbery_" .. A, L.xyz, 1.5, 6, a1, a2, a9, {robberyId = A})
    end
end)
RegisterNetEvent("Polar:setAtmHasBeenRobbed",function(ac)
    a0 = ac
end)
