...
RMenu.Add('Polarhighrollers','casino',RageUI.CreateMenu("","",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"shopui_title_casino","shopui_title_casino"))
RMenu:Get('Polarhighrollers','casino'):SetSubtitle("~b~MEMBERSHIP")
RMenu.Add('Polarhighrollers','confirmadd',RageUI.CreateSubMenu(RMenu:Get('Polarhighrollers','casino'),"","~b~Are you sure?",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight()))
RMenu.Add('Polarhighrollers','confirmremove',RageUI.CreateSubMenu(RMenu:Get('Polarhighrollers','casino'),"","~b~Are you sure?",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight()))

RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('Polarhighrollers', 'casino')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            end,RMenu:Get('Polarhighrollers','confirmadd'))
            ...
            end,RMenu:Get('Polarhighrollers','confirmremove'))
        end)
    end
    if RageUI.Visible(RMenu:Get('Polarhighrollers', 'confirmadd')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle("No","",{RightLabel="→→→"},true,function(b,c,d)
                if d then 
                    tPolar.notify("~y~Cancelled!")
                end 
            end,RMenu:Get('Polarhighrollers','casino'))
            RageUI.ButtonWithStyle("Yes","",{RightLabel="→→→"},true,function(b,c,d)
                if d then 
                    TriggerServerEvent("Polar:purchaseHighRollersMembership")
                end 
            end,RMenu:Get('Polarhighrollers','casino'))
        end)
    end
    if RageUI.Visible(RMenu:Get('Polarhighrollers', 'confirmremove')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle("No","",{RightLabel="→→→"},true,function(b,c,d)
                if d then 
                    tPolar.notify("~y~Cancelled!")
            ...
            RageUI.ButtonWithStyle("Yes","",{RightLabel="→→→"},true,function(b,c,d)
                if d then 
                    TriggerServerEvent("Polar:removeHighRollersMembership")
                end 
            end,RMenu:Get('Polarhighrollers','casino'))
        end)
    end
...
function showCasinoMembership(e)
    RageUI.Visible(RMenu:Get('Polarhighrollers','casino'),e)
end
Citizen.CreateThread(function()
    ...
    for g,h in pairs(a)do 
        local i=tPolar.loadModel('u_f_m_casinocash_01')
...
AddEventHandler("Polar:onClientSpawn",function(D, E)
    if E then
        ...
        for q,h in pairs(a)do 
            tPolar.addBlip(h.entryPosition.x,h.entryPosition.y,h.entryPosition.z,682,0,"Casino Memberships",0.7,true)
            tPolar.addMarker(h.entryPosition.x,h.entryPosition.y,h.entryPosition.z,1.0,1.0,1.0,138,43,226,70,50,27)
            tPolar.createArea("casinomembership_"..q,h.entryPosition,1.5,6,m,o,p,{})
        end 
	end
end)