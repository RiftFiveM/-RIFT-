local function a()
    local b = GetResourceKvpString("Polar_scope_horizontal")
    if b == nil or b == "" then
        return 50
    else
        return tonumber(b)
...
local function f()
    local b = GetResourceKvpString("Polar_scope_vertical")
    if b == nil or b == "" then
        return 30
    else
        return tonumber(b)
...
local i = GetResourceKvpString("Polar_scope_enabled") == "true"
local j = false

RMenu.Add("scope","main",RageUI.CreateMenu("", "Scope Settings", tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners","scope"))
...
function tPolar.doesCurrentWeaponHaveScope()
    local p = PlayerPedId()
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        if i and (j or tPolar.doesCurrentWeaponHaveScope()) then
            if j then
