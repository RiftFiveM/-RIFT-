...
local b = module("Polar-weapons", "cfg/weapons")
    ...
    if not a.InProgress and u then
        ...
        if i then
            ...
            if w ~= -1 then
                ...
                if x then
                    tPolar.setWeapon(PlayerPedId(), foundWeapon, true)
                    a.InProgress = true
                    a.startTime = GetGameTimer()
                    a.targetSrc = w
                    TriggerServerEvent("Polar:takeHostageSync", w)
                    a.type = "agressor"
                else
...
RegisterNetEvent("Polar:takeHostageSyncTarget",function(y)
...
RegisterNetEvent("Polar:takeHostageReleaseHostage",function()
    a.InProgress = false
    a.type = ""
    DetachEntity(PlayerPedId(), true, false)
    q("reaction@shove")
...
RegisterNetEvent("Polar:takeHostageKillHostage",function()
    a.InProgress = false
    a.type = ""
...
RegisterNetEvent("Polar:takeHostageCl_stop",function()
    a.InProgress = false
    a.type = ""
    ClearPedSecondaryTask(PlayerPedId())
    DetachEntity(PlayerPedId(), true, false)
    ...
    while true do
        if a.type == "agressor" then
            ...
            if IsEntityDead(PlayerPedId()) then
                ...
                RemoveAnimDict("reaction@shove")
                TriggerServerEvent("Polar:takeHostageReleaseHostage", a.targetSrc)
            end
            ...
            elseif IsDisabledControlJustPressed(0, 74) and GetGameTimer() - a.startTime > 4000 then
                ...
                RemoveAnimDict("anim@gangops@hostage@")
                TriggerServerEvent("Polar:takeHostageKillHostage", a.targetSrc)
                TriggerServerEvent("Polar:takeHostageStop", a.targetSrc)
                Wait(100)
...
function tPolar.takeHostageInProgress()
    return a.InProgress
end
