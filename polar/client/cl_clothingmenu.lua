RMenu.Add('Polarclothing','mainMenu',RageUI.CreateMenu("","Clothing Store",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners","cstore"))
RMenu.Add('Polarclothing','changePed',RageUI.CreateSubMenu(RMenu:Get('Polarclothing','mainMenu'),"","Change Gender"))
RMenu.Add('Polarclothing','changeClothing',RageUI.CreateSubMenu(RMenu:Get('Polarclothing','mainMenu'),"","Change your Clothes"))
RMenu.Add('Polarclothing','clearProps',RageUI.CreateSubMenu(RMenu:Get('Polarclothing','mainMenu'),"","Clear Props"))
...
local r = {
    ...
    vector3(-442.43130493164,6013.0131835938,36.995670318604),
    ...
    while true do 
        ...
        for H=1,#r do 
            if#(G-r[H])<=2.5 and not t then 
                ...
                h=true
                RageUI.Visible(RMenu:Get('Polarclothing','mainMenu'),true)
            elseif s==H then 
                ...
                if not RageUI.Visible(RMenu:Get('Polarclothing','changeClothing'))then 
                    F()
                end 
            end 
        end
...
RageUI.CreateWhile(1.0, true, function()
    if not n then
        if RageUI.Visible(RMenu:Get('Polarclothing', 'mainMenu')) then
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                ...
                if PlayerIsInGang then
                    RageUI.Button('Apply Gang Outfit', 'Applys the outfit set by the Gang Leader.', {RightLabel="→→→"}, true, function(L,M,N)
                        if N then
                                TriggerServerEvent("Polar:applyGangFit", gangID)
                            end
                        end)
                    end
                RageUI.Button('Apply Outfit Code','Applys an outfit code given by a player with ~y~Polar Plus~w~ or ~y~Polar Platinum~w~.',{RightLabel=">>>"},true,function(L,M,N)
                    if N then 
                        tPolar.clientPrompt("Outfit Code:","",function(Q)
                            local R = string.gsub(Q, "%s+", "")
                            if #R == 5 then
                                TriggerServerEvent("Polar:applyOutfitCode", string.upper(R))
                            else
                ...
                end,RMenu:Get('Polarclothing','clearProps'))
            end)
        end
        if RageUI.Visible(RMenu:Get('Polarclothing', 'changePed')) then
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                ...
                RageUI.Button('MP Female','WARNING: Resets all your Body Options',{},true,function(L,M,N)
                    if N then 
                        if GetEntityHealth(PlayerPedId()) > 102 then
                            tPolar.loadCustomisationPreset("DefaultFemale")
                        else
                            tPolar.notify("~r~Cannot change gender, you're dead.")
                        end
                    end 
        ...
        if RageUI.Visible(RMenu:Get('Polarclothing', 'changeClothing')) then
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                m=true
                if h then 
                    for H=1,#a,1 do 
                        ...
                        RageUI.List(P.name,get_drawables(P.type,P.index),a[H].currentListIndex,"Current Texture Index: "..k.textureN.."/"..j-1,{},true,function(L,M,N,Q)
                            ...
                            if N then 
                                ...
                                if k.index==2 then 
                                    ...
                                    TriggerServerEvent("Polar:saveClothingHairData",GetPedDrawableVariation(tPolar.getPlayerPed(),2),GetPedHairColor(tPolar.getPlayerPed()))
...
function current_clothing(R,i,S)
    if R=="prop"then 
        if S then 
            if GetPedPropTextureIndex(tPolar.getPlayerPed(),i)~=nil then 
                return GetPedPropTextureIndex(tPolar.getPlayerPed(),i)
            else 
                return 0 
            end 
        else 
            if GetPedPropIndex(tPolar.getPlayerPed(),i)~=nil then 
                if GetPedPropIndex(tPolar.getPlayerPed(),i)==-1 then 
                    return GetPedPropIndex(tPolar.getPlayerPed(),i)+2 
                else 
                    return GetPedPropIndex(tPolar.getPlayerPed(),i)+1 
                end 
            else 
                return 0 
            end 
    ...
    elseif R=="drawable"then 
        ...
        elseif S then 
            if GetPedTextureVariation(tPolar.getPlayerPed(),i)~=nil then 
                return GetPedTextureVariation(tPolar.getPlayerPed(),i)
            else 
                return 0 
            end 
        else 
            if GetPedDrawableVariation(tPolar.getPlayerPed(),i)~=nil then 
                return GetPedDrawableVariation(tPolar.getPlayerPed(),i)+1 
            else 
                return 0 
            end 
        end 
...
function get_textures(T,R,i)
    if T=="drawable"then 
        ...
        j=GetNumberOfPedTextureVariations(tPolar.getPlayerPed(),R,i-1)
        return j 
    else 
        j=GetNumberOfPedPropTextureVariations(tPolar.getPlayerPed(),R,i-1)
        return j 
    end 
end
local function Y()
...
Citizen.CreateThread(function()
    while true do 
        if k.index~=nil and RageUI.Visible(RMenu:Get('Polarclothing','changeClothing'))then 
...
AddEventHandler("Polar:onClientSpawn",function(D, E)
    if E then
		for a7,a8 in pairs(r)do 
            tPolar.addBlip(a8.x,a8.y,a8.z,73,3,"Clothing Store",0.7)
            tPolar.addMarker(a8.x,a8.y,a8.z,0.6,0.6,0.6,10,255,81,170,50,9,false,false,true,"dp_clothing","top",90.0,90.0,0.0)
        end 
	end
end)
