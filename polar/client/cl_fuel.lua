...
AddEventHandler("Polar:onClientSpawn",function(D, E)
    if E then
		for b, c in pairs(a.stations) do
			tPolar.addBlip(c.x, c.y, c.z, 361, 4, "Petrol Station", 0.6)
		end
	end
end)
local d = false
...
AddEventHandler("fuel:startFuelUpTick",function(y, s, j)
	...
	if y then
		TriggerServerEvent("Polar:takeAmount", math.floor(g))
	end
	local G = GetEntityModel(j)
	local H = tPolar.getVehicleIdFromHash(G)
	if H then
		local I = GetFuel(j)
		TriggerServerEvent("Polar:updateFuel", H, math.floor(I * 1000) / 1000)
	end
	g = 0.0
end)
AddEventHandler("fuel:refuelFromPump",function(y, s, j)
	...
	tPolar.loadAnimDict("timetable@gardener@filling_can")
				...
				elseif d then
					...
					if h >= a.jerryCanCost then
						if not HasPedGotWeapon(s, 883325847) then
							...
							if IsControlJustReleased(0, 38) then
								tPolar.allowWeapon("883325847")
								GiveWeaponToPed(s, 883325847, 4500, false, true)
								TriggerServerEvent("Polar:takeAmount", a.jerryCanCost)
								h = 50000
							end
						else
							...
							if W > 0 then
								...
								if IsControlJustReleased(0, 38) then
									TriggerServerEvent("Polar:takeAmount", math.floor(W))
