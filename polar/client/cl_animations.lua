...
function tPolar.GetClosestPlayer(ao)
    local ap=-1
    local aq=-1
...
RegisterCommand("facepalm", function(c, d)
	...
	if DoesEntityExist(f) and (not IsEntityDead(f)) then
		...
		if IsEntityPlayingAnim(f, e, "idle_a", 3) then
			...
			Wait(100)
			ClearPedSecondaryTask(tPolar.getPlayerPed())
		else
...
RegisterCommand("umbrella",function(f,g)
    ...
    if DoesEntityExist(k)and canAnim()and not IsEntityDead(k)and d()then 
        ...
        if IsEntityPlayingAnim(k,i,"base",3)then 
			...
			DecorSetInt(prop, "PolarACVeh", 955)
...
function tPolar.globalSurrenderring()
...
RegisterNetEvent("Polar:startRobbingPlayer",function()
    D=GetGameTimer()
end)
RegisterNetEvent("Polar:endRobbingPlayer",function()
    D=nil 
end)
...
Citizen.CreateThread(function()
    ...
    while true do
        if (IsDisabledControlPressed(1,19) or IsControlPressed(1, 19)) and (IsDisabledControlPressed(1,63) or IsControlPressed(1, 63)) and d()and not sittingAtBlackjackTable then
            if not a2 and tPolar.globalNHSOnDuty() then 
                a2=true
                ...
                detachFlag=1
                boneIndex=11816
                local aa=tPolar.GetClosestPlayer(3)
                if aa then 
                    local ab=GetPlayerServerId(aa)
                    if ab~=-1 then 
                        TriggerServerEvent("Polar:animationSync",aa,a3,a4,a5,a6,a7,y,ab,a9,a8,controlFlagMe,controlFlagTarget,animFlagTarget,detachFlag,boneIndex)
                    end 
                end 
            else 
                a2=false
                ...
                if ac~=0 then 
                    TriggerServerEvent("Polar:animationSyncStop",ac)
                end 
            end
            Wait(250)
        end
...
RegisterNetEvent("Polar:animationSyncClStop")
AddEventHandler("Polar:animationSyncClStop",function()
    ClearPedSecondaryTask(PlayerPedId())
    DetachEntity(PlayerPedId())
end)
RegisterCommand("baseball",function(f,g)
    ...
    if aa~=nil and not c()and GetEntityHealth(GetPlayerPed(aa))>102 and d()and canAnim()then 
        TriggerServerEvent("Polar:animationSync",aa,a3,a4,a5,a6,a7,y,ac,a9)
...
RegisterCommand("baseball9", function(f, g)
	...
	local ac = GetPlayerServerId(aa)
	if aa ~= nil and (not c()) and GetEntityHealth(GetPlayerPed(aa)) > 102 and d() and canAnim() then
		TriggerServerEvent("Polar:animationSync", aa, a3, a4, a5, a6, a7, y, ac, a9, a8, controlFlagMe, controlFlagTarget, animFlagTarget, detachFlag)
	end
end, false)
...
RegisterNetEvent("Polar:animationSyncTarget")
AddEventHandler("Polar:animationSyncTarget",function(ac,ad,ae,a6,a7,y,a9,a8,af,animFlagTarget,detachFlag,boneIndex)
    local e=PlayerPedId()
...
RegisterNetEvent("Polar:animationSyncMe")
AddEventHandler("Polar:animationSyncMe",function(ad,ah,a9,af,ai)
    local e=PlayerPedId()
    RequestAnimDict(ad)
    while not HasAnimDictLoaded(ad)do 
        Citizen.Wait(10)
...
exports("canAnim",function()
    return tPolar.canAnim()
end)


local ah = false
RegisterNetEvent("Polar:startShavingPlayer",function(ai)
	...
	tPolar.loadAnimDict("misshair_shop@hair_dressers")
	...
	tPolar.loadModel("prop_clippers_01")
...
RegisterNetEvent("Polar:startBeingShaved",function(aq)
...
RegisterNetEvent("Polar:playDelayedShave",function(au)
	...
	local aw = tPolar.getPlayerCoords()
...
RegisterNetEvent("Polar:setAsShaved",function(ay)
	ah = true
	Citizen.Wait(ay)
	ah = false
end)
