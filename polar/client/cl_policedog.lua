RMenu.Add("policedog","main",RageUI.CreateMenu("Dog Support Unit", "~b~Dog Management", tPolar.getRageUIMenuWidth(), tPolar.getRageUIMenuHeight()))
AddEventHandler("Polar:setPoliceOnDuty",function(a)
    if a then
        RegisterKeyMapping("k9attack", "Police Dog Attack", "keyboard", "INSERT")
    end
end)
AddEventHandler("Polar:setPrisonGuardOnDuty",function(b)
    if b then
        RegisterKeyMapping("k9attack", "Police Dog Attack", "keyboard", "INSERT")
    end
end)
...
local i = "K9"
local j = "K9TARGET"
RegisterNetEvent("Polar:policeDogMenu",function()
    ...
    if globalOnPrisonDuty and tPolar.hasHmpCallsign() then
        RMenu:Get("Polarscenemenu", "main"):SetSubtitle("~b~HMP~w~: " .. tPolar.getHmpCallsign() .. " - " .. tPolar.getHmpRank() .. " - " .. tPolar.getHmpName())
    end
    RageUI.Visible(RMenu:Get("policedog", "main"), true)
end)
RegisterNetEvent("Polar:sendClientRagdoll",function()
    SetPedToRagdoll(tPolar.getPlayerPed(), 12000, 12000, 0, 0, 0, 0)
    showNotification("~y~~h~Alert~h~~s~: " .. "You have been bitten by a police dog.")
end)
RegisterNetEvent("Polar:policeDogAttack",function()
...
local m = {breed = 1}
local function n()
    local o = GetResourceKvpString("Polar_policedog_customisations")
...
local function r(s)
    ...
    local q = k[m.breed]
    p[q] = s
    SetResourceKvp("Polar_policedog_customisations", json.encode(p))
end
...
function tpDog()
    local z = GetEntityCoords(tPolar.getPlayerPed(), true)
                                    ...
                                    if M < 2.0 then
                                        ...
                                        for O, P in pairs(N) do
                                            if I == GetPlayerPed(P) then
                                                ...
                                                TriggerServerEvent("Polar:performDogLog","Instructed dog to attack ID: " .. Q)
                                                TriggerServerEvent("Polar:serverDogAttack", Q)
                                                H = true
                                                Wait(1000)
    ...
    while f.currentAction == d.SearchNearby and DoesEntityExist(f.handle) do
        ...
        for W, X in pairs(GetActivePlayers()) do
            ...
            if GetVehiclePedIsUsing(Y) == 0 and IsEntityVisible(Y) and #(GetEntityCoords(Y, true) - L) < 5.0 then
                ...
                local _ = tPolar.clientGetUserIdFromSource(Z)
                if tPolar.getJobType(_) == "" then
        ...
        if #V > 0 then
            ...
            TriggerServerEvent("Polar:policeDogSniffPlayer", a0.source)
        end
        Citizen.Wait(5000)
    end
end
function putInVehicle()
    local a2 = tPolar.getNearestVehicle(7.0)
    Citizen.Trace(a2)
...
function removeFromVehicle()
    if IsPedInAnyVehicle(tPolar.getPlayerPed(), true) then
...
function newDog(q, a4)
    local a5 = tPolar.getPlayerPed()
    if not IsPedInAnyPoliceVehicle(a5) then
        local a6 = tPolar.loadModel(q)
        ...
        if DoesEntityExist(f.handle) then
            ...
            TriggerServerEvent("Polar:performDogLog", "Created a new police dog")
        end
    end
end
function deleteDog()
    ...
    f.active = false
    f.handle = 0
    TriggerServerEvent("Polar:performDogLog", "Deleted dog")
end
function dogFollow()
    if not f.active then
        return
    ...
    if f.currentAction ~= d.Follow then
        ClearPedTasks(f.handle)
        TaskFollowToOffsetOfEntity(f.handle, tPolar.getPlayerPed(), 0.0, 0.0, 0.0, 7.0, -1, 10.0, true)
        f.currentAction = d.Follow
    else
        ClearPedTasks(f.handle)
        f.currentAction = d.Follow
...
RegisterNetEvent("Polar:policeDogIndicate",function(ad)
    ...
    while f.currentAction == d.Indicate and DoesEntityExist(af) do
        ...
        elseif GetGameTimer() - ag > 3500 then
            if not IsEntityPlayingAnim(f.handle, g.Sit.dict, g.Sit.anims.base, 3) then
                tPolar.loadAnimDict(g.Sit.dict)
