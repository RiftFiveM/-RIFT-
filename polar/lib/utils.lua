...
function module(rsc, path) -- load a LUA resource file as module
    if path == nil then -- shortcut for Polar, can omit the resource parameter
        path = rsc
        rsc = "Polar"
    end
    local key = rsc .. path
    ...
    if modules[key] then -- cached module
        ...
        if f then
            ...
            if ar[1] then
                ...
                modules[key] = nil
                print("[Polar] error loading module " .. rsc .. "/" .. path .. ":" .. ar[2])
            end
        else
            print("[Polar] error parsing module " .. rsc .. "/" .. path .. ":" .. err)
        end
    end
end
