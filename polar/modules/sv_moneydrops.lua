...
function tPolar.MoneyDrop()
    ...
    if money > 0 then
        ...
        if Polar.tryPayment(user_id,money) then
            MoneydropEntities[moneydropnetid].Money = money
        end
    end
end

RegisterNetEvent('Polar:Moneydrop')
AddEventHandler('Polar:Moneydrop', function(netid)
    local source = source
    if MoneydropEntities[netid] and not MoneydropEntities[netid][3] and #(GetEntityCoords(MoneydropEntities[netid][1]) - GetEntityCoords(GetPlayerPed(source))) < 10.0 then
        ...
        local user_id = Polar.getUserId(source)
        if user_id ~= nil then
            TriggerClientEvent("Polar:MoneyNotInBag",source)
            if MoneydropEntities[netid].Money ~= 0 then
                Polar.giveMoney(user_id,MoneydropEntities[netid].Money)
                Polarclient.notify(source,{"~g~You have taken £"..tonumber(MoneydropEntities[netid].Money)})
                MoneydropEntities[netid].Money = 0
            end
        else
            Polarclient.notify(source,{"~r~The money drop is already being taken"})
