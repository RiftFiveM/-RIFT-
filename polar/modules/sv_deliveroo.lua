...
RegisterNetEvent("Polar:attemptBeginDeliverooJob")
AddEventHandler("Polar:attemptBeginDeliverooJob",function()
    local source=source
    local user_id=Polar.getUserId(source)
    local deliverooJobTable={}
    if Polar.hasGroup(user_id,"Deliveroo")then
        if not usersInDeliverooJob[user_id] then
            ...
            TriggerClientEvent("Polar:beginDeliverooJob",source)
            Wait(100)
            TriggerClientEvent("Polar:deliverooJobSetNextBlip",source,deliverooStops[deliverooJobTable.stopNumber].stopPosition)
        else
            Polarclient.notify(source,{"~r~You are already a Deliveroo Driver."})
        end
    else
        Polarclient.notify(source,{"~r~You are not clocked on as a Deliveroo Driver."})
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(deliverooStops)do
                    if Polar.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(Polar.getUserSource(k)))-deliverooStops[v.currentJob.stopNumber].stopPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            Polar.giveMoney(userid,amount)(k,pay)
                            TriggerClientEvent("Polar:deliverooJobReachedNextStop",Polar.getUserSource(k),pay)
                            if v.currentJob.stopNumber==#deliverooStops then
                                TriggerClientEvent("Polar:deliverooJobEnd", Polar.getUserSource(k), true)
                                v.currentJob.jobActive=false
                                usersInDeliverooJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("Polar:deliverooJobSetNextBlip",Polar.getUserSource(k),deliverooStops[v.currentJob.stopNumber].stopPosition)
                            end
                        end
                    end
                end
