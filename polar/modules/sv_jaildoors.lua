...
AddEventHandler("Polar:playerSpawn", function(user_id, source, first_spawn)
    if first_spawn then
        TriggerClientEvent('Polar:policeSyncAllDoors', source, doors, doorStates)
    end
end)

RegisterNetEvent("Polar:policeSetDoorState")
AddEventHandler("Polar:policeSetDoorState", function(doorHash, state)
    local source = source
    local user_id = Polar.getUserId(source)
    if Polar.hasPermission(user_id, 'police.onduty.permission') then
        doorStates[doorHash] = state
        TriggerClientEvent('Polar:policeSyncDoor', -1, doorHash, state)
    end
end)

RegisterNetEvent("Polar:enterPoliceSyncDoors")
AddEventHandler("Polar:enterPoliceSyncDoors", function()
    local source = source
    TriggerClientEvent('Polar:gotAllDoorStates', source, doorStates)
end)