...
RegisterNetEvent("Polar:attemptBeginBusJob")
AddEventHandler("Polar:attemptBeginBusJob",function()
    local source=source
    local user_id=Polar.getUserId(source)
    local BusJobTable={}
    if Polar.hasGroup(user_id,"Bus Driver")then
        if not usersInBusJob[user_id] then
            ...
            TriggerClientEvent("Polar:beginBusJob",source)
            Wait(100)
            TriggerClientEvent("Polar:setNextBusJobBlip",source,busJobStops[BusJobTable.stopNumber].stopPosition)
        else
            Polarclient.notify(source,{"~r~You are already a Bus Driver."})
        end
    else
        Polarclient.notify(source,{"~r~You are not clocked on as a Bus Driver."})
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(busJobStops)do
                    if Polar.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(Polar.getUserSource(k)))-busJobStops[v.currentJob.stopNumber].stopPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            Polar.giveBankMoney(k,pay)
                            TriggerClientEvent("Polar:nextStopReachedBusJob",Polar.getUserSource(k),pay)
                            if v.currentJob.stopNumber==#busJobStops then
                                TriggerClientEvent("Polar:endBusJob", Polar.getUserSource(k))
                                v.currentJob.jobActive=false
                                usersInBusJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("Polar:setNextBusJobBlip",Polar.getUserSource(k),busJobStops[v.currentJob.stopNumber].stopPosition)
                            end
                        end
                    end
                end
