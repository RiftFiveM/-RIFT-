...
Tunnel.bindInterface("Polar", {}) -- tylon place your tunnel in here
...
RegisterServerEvent("Polar:EnableGangBlips")
AddEventHandler("Polar:EnableGangBlips", function()
    local source = source
    ...
    if playerGang then
        ...
        for _, memberID in ipairs(gangMembers) do
            if memberID ~= user_id and Polar.isPlayerOnline(memberID) then
                table.insert(filteredMembers, memberID)
            end
        end

        local blipsData = GetBlipsData(filteredMembers)
        TriggerClientEvent("Polar:ReceiveGangBlipsData", source, blipsData)
    end
end)
