...
RegisterNetEvent('Polar:getStoreRobBlips')
AddEventHandler('Polar:getStoreRobBlips', function()
    local source = source
    TriggerClientEvent('Polar:updateStoreRobBlips', -1, stores)
end)


RegisterNetEvent('Polar:initiateStoreRobbery')
AddEventHandler('Polar:initiateStoreRobbery', function(store)
    local source = source
    local user_id = Polar.getUserId(source)
    if Polar.hasPermission(user_id, "police.onduty.permission") then
        TriggerClientEvent('Polar:resetStorePed', source, store)
    else
        if #Polar.getUsersByPermission('police.onduty.permission') > 2 then
            for k,v in pairs(stores) do
                if k == store then
                    if v.cooldown == 0 then
                        ...
                        v.cooldown = 300
                        TriggerClientEvent('Polar:updateStoreRobBlips', -1, stores)
                        TriggerClientEvent('Polar:beginStoreRobbingAnimations', -1, store)
                        TriggerClientEvent('Polar:storeRobberyInProgress', source, true, store)
                        ...
                        TriggerEvent('Polar:PDRobberyCall', source, v.storename, v.position)
                    else
            ...
            Polarclient.notify(source, {'~r~There are not enough police on duty to rob a store.'})
            TriggerClientEvent('Polar:resetStorePed', source, store)
        end
    end
end)

RegisterNetEvent('Polar:completeSafeCracking')
AddEventHandler('Polar:completeSafeCracking', function(store)
    local source = source
    local user_id = Polar.getUserId(source)
    Polar.giveMoney(user_id, math.random(85000, 150000))
    TriggerClientEvent('Polar:syncCloseSafeDoor', -1, store)
    TriggerClientEvent('Polar:resetStorePed', -1, store)
    for k,v in pairs(stores) do
        if k == store then
            v.beingrobbed = false
        end
    ...
    TriggerClientEvent('Polar:updateStoreRobBlips', -1, stores)
    TriggerClientEvent('Polar:storeRobberyInProgress', source, false, store)
end)

RegisterNetEvent('Polar:forceEndRobbery')
AddEventHandler('Polar:forceEndRobbery', function(store)
    local source = source
    local user_id = Polar.getUserId(source)
    TriggerClientEvent('Polar:resetStorePed', -1, store)
    for k,v in pairs(stores) do
...
RegisterNetEvent('Polar:syncOpenSafeDoor')
AddEventHandler('Polar:syncOpenSafeDoor', function(store)
    local source = source
    TriggerClientEvent('Polar:syncOpenSafeDoor', -1, store)
end)
