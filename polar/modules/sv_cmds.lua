	...
	if not chatCooldown[source] then 
		for word in pairs(blockedWords) do
			if(string.gsub(string.gsub(string.gsub(string.gsub(string.gsub(string.gsub(message:lower(), "-", ""), ",", ""), "%.", ""), " ", ""), "*", ""), "+", ""):find(blockedWords[word])) then
				TriggerClientEvent('Polar:chatFilterScaleform', source, 10, 'That word is not allowed.')
				CancelEvent()
				return
			end
		end
		tPolar.sendWebhook('anon', "Polar Chat Logs", "```"..message.."```".."\n> Player Name: **"..GetPlayerName(source).."**\n> Player PermID: **"..user_id.."**\n> Player TempID: **"..source.."**")
		TriggerClientEvent('chatMessage', -1, "^4Twitter @^1Anonymous: ", { 128, 128, 128 }, message, "ooc")
		chatCooldown[source] = true
	end
end)

function tPolar.ooc(source, args, raw)
	if #args <= 0 then 
		return 
	end
	local source = source
	...
	if not chatCooldown[source] then 
		...
		tPolar.sendWebhook('ooc', "Polar Chat Logs", "```"..message.."```".."\n> Player Name: **"..GetPlayerName(source).."**\n> Player PermID: **"..user_id.."**\n> Player TempID: **"..source.."**")
	else
		TriggerClientEvent('chatMessage', source, "^1[Polar]", { 128, 128, 128 }, " Chat Spam | Retry in 3 Seconds", "alert")
		chatCooldown[source] = true
	end
end

RegisterCommand("ooc", function(source, args, raw)
    tPolar.ooc(source, args, raw)
end)
...
function Polar.ooc(source, args) -- ooc from chat with //
	args[1] = args[1]:sub(3)
	tPolar.ooc(source, args)
end


RegisterCommand('cc', function(source, args, rawCommand)
    local user_id = Polar.getUserId(source)
    if Polar.hasPermission(user_id, 'admin.ban') then
