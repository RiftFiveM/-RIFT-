window.addEventListener("load", function() {
    ...
    wprompt.onClose = function() {
        $.post("https://Polar/prompt", JSON.stringify({ act: "close", result: wprompt.result.substring(0, 1000) }));
    };

    requestmgr.onResponse = function(id, ok) {
        $.post("https://Polar/request", JSON.stringify({ act: "response", id: id, ok: ok }));
    };
