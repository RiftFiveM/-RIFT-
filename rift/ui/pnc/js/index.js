...
async function RegisterVueComponent(component, path) {
    const response = await fetch("nui://rift/ui/pnc/components/"+path);
        ...
        methods: {
            addMarker() {
                if(!this.doesPlayerHaveWarningMarkerOfType(this.selectedWarningMarkerType)) {
                    $.post('https://rift/addmarker', JSON.stringify({
            ...
            wipeAllMarkers() {
                $.post('https://rift/wipeallmarkers', JSON.stringify({
            ...
            addPoint() {
                $.post('https://rift/addpoint', JSON.stringify({
            ...
            switchPage(page) {
                if(this.personPages.includes(page)){
                    if (this.selectedPerson.id){
                        ...
                        console.log("[RIFT PNC] Person needs to be selected to enter this page")
                    }
                }else {
                    this.page = page;
                }
            ...
            closePNC() {
                $.post('https://rift/exit', JSON.stringify({}));
            },
            searchPerson(){
                let first = this.personFirstName;
                let last = this.personLastName;
                if (first == "" || last == "") {
                    console.log("[RIFT PNC] Full name needs to be provided")
                }else{
                    $.post('https://rift/personsearch', JSON.stringify({
                        firstname: first,
                        lastname: last
                    }));
                }
            ...
            searchPlate(){
                ...
                $.post("https://rift/platesearch", JSON.stringify({
                    plate: this.plate
                }))
            },
            selectPerson(person){
            ...
            submitFine() {
                ...
                $.post('https://rift/submitfine', JSON.stringify({
            ...
            addAD(){
                ...
                if (this.ADInput.length === 0) {
                    ...
                    $.post('https://rift/addattentiondrawn', JSON.stringify({
                        ad: this.ADInput
                    }));
                }
            ...
            removeAD(index, ad) {
                if(this.askedIfSure) {
                    ...
                    $.post('https://rift/removeattentiondrawn', JSON.stringify({
            ...
            saveNotes() {
                $.post('https://rift/savenotes', JSON.stringify({
            ...
            savePersonNotes() {
                $.post("https://rift/savepersonnotes", JSON.stringify({
                    user_id: this.selectedPerson.id,
                    notes: this.selectedPerson.notes
                }))
            },
            GetWarrant() {
                $.post("https://rift/generatewarrant", JSON.stringify({}))
            }
            },
