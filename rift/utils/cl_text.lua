function tRIFT.add3DTextForCoord(a, b, c, d, e)
    local function f(g)
        tRIFT.DrawText3D(g.coords, g.text, 0.2)
    end
    local h = tRIFT.generateUUID("3dtext", 8, "alphanumeric")
    tRIFT.createArea("3dtext_" .. h,vector3(b, c, d),e,6.0,function()end,function()end,f,{coords = vector3(b, c, d), text = a})
end
function tRIFT.drawFloatingHelpText(a, i)
...
function tRIFT.DrawText3D(i, a, j, k, l, m)
...
function tRIFT.DrawText(b, c, a, j, k, p, m, q)
    SetTextScale(j, j)
    SetTextFont(k or 0)
    if p then
