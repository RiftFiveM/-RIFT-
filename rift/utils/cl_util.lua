...
function tPolar.getStreetNameAtCoord(v, w, H)
...
function tPolar.randomNum(d, e)
    math.randomseed(GetGameTimer() * math.random() * 2)
    return math.random(d, e)
end
soundEventCode = 0
TriggerServerEvent("Polar:soundCodeServer")
RegisterNetEvent("Polar:soundCode",function(aH)
    soundEventCode = aH
end)
RegisterNetEvent("Polar:playClientNuiSound",function(aI, aJ, ap) -- use for cuffing and other sounds in areas
    local ax = tPolar.getPlayerCoords()
    if #(ax - aI) <= ap then
        SendNUIMessage({transactionType = aJ})
    end
end)
function tPolar.getNetId(aW, aX)
    if aX == nil then
        aX = ""
    end
    local aY = 0
    ...
    if not aZ then
        tPolar.debugLog(string.format("no such entity %s", aX))
    else
        aY = NetworkGetNetworkIdFromEntity(aW)
        if aY == aW then
            tPolar.debugLog(string.format("no such networked entity %s", aX))
        end
    end
    return aY
end
function tPolar.getObjectId(a_, aX)
    ...
    if not b0 then
        tPolar.debugLog(string.format("no object by ID %s\n%s", a_, aX))
    else
  ...
  local aU = {}
  Citizen.CreateThread(function()
    local a = module("Polar-vehicles", "cfg_garages")
...
function tPolar.getVehicleNameFromId(aZ)
    if aT[string.lower(aZ)] then
        return aT[string.lower(aZ)].name
    end
    return ""
end
function tPolar.getGarageNameFromId(aZ)
    return aT[string.lower(aZ)].garageType
end
function tPolar.getVehicleIdFromModel(s)
    return aU[s]
end
local a_ = math.rad
local b0 = math.cos
local b1 = math.sin
local b2 = math.abs
function tPolar.rotationToDirection(b3)
    local B = a_(b3.x)
    local D = a_(b3.z)
