...
function tRIFT.getStreetNameAtCoord(v, w, H)
...
function tRIFT.randomNum(d, e)
    math.randomseed(GetGameTimer() * math.random() * 2)
    return math.random(d, e)
end
soundEventCode = 0
TriggerServerEvent("RIFT:soundCodeServer")
RegisterNetEvent("RIFT:soundCode",function(aH)
    soundEventCode = aH
end)
RegisterNetEvent("RIFT:playClientNuiSound",function(aI, aJ, ap) -- use for cuffing and other sounds in areas
    local ax = tRIFT.getPlayerCoords()
    if #(ax - aI) <= ap then
        SendNUIMessage({transactionType = aJ})
    end
end)
function tRIFT.getNetId(aW, aX)
    if aX == nil then
        aX = ""
    end
    local aY = 0
    ...
    if not aZ then
        tRIFT.debugLog(string.format("no such entity %s", aX))
    else
        aY = NetworkGetNetworkIdFromEntity(aW)
        if aY == aW then
            tRIFT.debugLog(string.format("no such networked entity %s", aX))
        end
    end
    return aY
end
function tRIFT.getObjectId(a_, aX)
    ...
    if not b0 then
        tRIFT.debugLog(string.format("no object by ID %s\n%s", a_, aX))
    else
  ...
  local aU = {}
  Citizen.CreateThread(function()
    local a = module("rift-vehicles", "cfg_garages")
...
function tRIFT.getVehicleNameFromId(aZ)
    if aT[string.lower(aZ)] then
        return aT[string.lower(aZ)].name
    end
    return ""
end
function tRIFT.getGarageNameFromId(aZ)
    return aT[string.lower(aZ)].garageType
end
function tRIFT.getVehicleIdFromModel(s)
    return aU[s]
end
local a_ = math.rad
local b0 = math.cos
local b1 = math.sin
local b2 = math.abs
function tRIFT.rotationToDirection(b3)
    local B = a_(b3.x)
    local D = a_(b3.z)
