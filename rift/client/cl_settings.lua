...
Citizen.CreateThread(
    function()
        local o = GetResourceKvpString("Polar_diagonalweapons") or "false"
        if o == "false" then
            b = false
            TriggerEvent("Polar:setVerticalWeapons")
        else
            b = true
            TriggerEvent("Polar:setDiagonalWeapons")
        end
        local p = GetResourceKvpString("Polar_frontars") or "false"
        if p == "false" then
            c = false
            TriggerEvent("Polar:setBackAR")
        else
            c = true
            TriggerEvent("Polar:setFrontAR")
        end
        local q = GetResourceKvpString("Polar_hitmarkersounds") or "false"
        if q == "false" then
            d = false
            TriggerEvent("Polar:hsSoundsOff")
        else
            d = true
            TriggerEvent("Polar:hsSoundsOn")
        end
        local r = GetResourceKvpString("Polar_reducedchatopacity") or "false"
        if r == "false" then
            f = false
            TriggerEvent("Polar:chatReduceOpacity", false)
        else
            f = true
            TriggerEvent("Polar:chatReduceOpacity", true)
        end
        local s = GetResourceKvpString("Polar_hideeventannouncement") or "false"
        if s == "false" then
            g = false
        else
            g = true
        end
        local t = GetResourceKvpString("Polar_healthpercentage") or "false"
        if t == "false" then
            h = false
        else
            h = true
        end
        local u = GetResourceKvpString("Polar_flashlightnotaiming") or "false"
        if u == "false" then
            i = false
        else
            i = true
        ...
        local v = GetResourceKvpInt("Polar_gang_name_distance")
        if v > 0 then
            j = v
            if k[j] then
                TriggerEvent("Polar:setGangNameDistance", k[j])
            end
        end
        local G = GetResourceKvpInt("Polar_gang_ping_sound")
        if G > 0 then
            m = G
        end
        local H = GetResourceKvpInt("Polar_gang_ping_volume")
        if H > 0 then
            o = H
        end
        local J = GetResourceKvpInt("Polar_gang_position_x")
        if J > 0 then
            r = J
            tPolar.setGangUIXPos(s[r])
        end
        local K = GetResourceKvpInt("Polar_gang_position_y")
        if K > 0 then
            t = K
            tPolar.setGangUIYPos(u[t])
        end
        local w = GetResourceKvpString("Polar_gang_ping_sound") or "false"
        if w == "false" then
            m = false
        else
            m = true
        end
        local x = GetResourceKvpInt("Polar_gang_ping_marker")
        if x > 0 then
            f = x
        end
    end
)
function tPolar.setDiagonalWeaponSetting(i)
    SetResourceKvp("Polar_diagonalweapons", tostring(i))
end
function tPolar.setFrontARSetting(i)
    SetResourceKvp("Polar_frontars", tostring(i))
end
function tPolar.setHitMarkerSetting(i)
    SetResourceKvp("Polar_hitmarkersounds", tostring(i))
end
function tPolar.setCODHitMarkerSetting(i)
    SetResourceKvp("Polar_codhitmarkersounds", tostring(i))
end
function tPolar.setKillListSetting(u)
    SetResourceKvp("Polar_killlistsetting", tostring(u))
end
function tPolar.setOldKillfeed(u)
    SetResourceKvp("Polar_oldkillfeed", tostring(u))
end
function tPolar.setDamageIndicator(G)
    SetResourceKvp("Polar_damageindicator", tostring(G))
end
function tPolar.setDamageIndicatorColour(G)
    SetResourceKvp("Polar_damageindicatorcolour", tostring(G))
end
function tPolar.setReducedChatOpacity(q)
    SetResourceKvp("Polar_reducedchatopacity", tostring(q))
end
function tPolar.setHideEventAnnouncementFlag(q)
    SetResourceKvp("Polar_hideeventannouncement", tostring(q))
end
function tPolar.getHideEventAnnouncementFlag()
    return g
end
function tPolar.setShowHealthPercentageFlag(q)
    SetResourceKvp("Polar_healthpercentage", tostring(q))
end
function tPolar.setFlashlightNotAimingFlag(x)
    SetFlashLightKeepOnWhileMoving(x)
    i = x
    SetResourceKvp("Polar_flashlightnotaiming", tostring(x))
end
function tPolar.getShowHealthPercentageFlag()
    return h
end
function tPolar.setGangPingSoundEnabled(q)
    SetResourceKvp("Polar_gang_ping_sound", tostring(q))
end
function tPolar.getGangPingSoundEnabled()
    return m
end
function tPolar.getGangPingMarkerIndex()
    return f
end
local function y(j)
...
local function K(C, D, L)
    ...
    SetResourceKvp("Polar_graphic_presets", M)
end
local N = {
    "0%",
    "5%",
...
local function Y()
    ...
    SetResourceKvp("Polar_kill_effects", Z)
end
local _ = {head = 1, body = 1, arms = 1, legs = 1}
local function a0()
    local a1 = json.encode(_)
    SetResourceKvp("Polar_blood_effects", a1)
end
Citizen.CreateThread(
    function()
        Citizen.Wait(1000)
        local M = GetResourceKvpString("Polar_graphic_presets")
        if M and M ~= "" then
            w = json.decode(M) or {}
        end
        ...
        local Z = GetResourceKvpString("Polar_kill_effects")
        if Z and Z ~= "" then
        ...
        local a1 = GetResourceKvpString("Polar_blood_effects")
        if a1 and a1 ~= "" then
    ...
    function()
            ...
            RageUI.DrawContent(
                ...
                function()
                    RageUI.Checkbox(
                        "Streetnames",
                        "",
                        tPolar.isStreetnamesEnabled(),
                        {RightBadge = RageUI.CheckboxStyle.Car},
                        function(a4, a6, a5, a8)
                        end,
                        function()
                            tPolar.setStreetnamesEnabled(true)
                        end,
                        function()
                            tPolar.setStreetnamesEnabled(false)
                        end
                    )
                    RageUI.Checkbox(
                        "Compass",
                        "",
                        tPolar.isCompassEnabled(),
                        {RightBadge = RageUI.CheckboxStyle.Car},
                        function(a4, a6, a5, a8)
                        end,
                        function()
                            tPolar.setCompassEnabled(true)
                        end,
                        function()
                            tPolar.setCompassEnabled(false)
                        end
                    )
                    local function a9()
                        tPolar.hideUI()
                    ...
                    RageUI.Checkbox(
                        ...
                        function()
                            f = true
                            tPolar.setReducedChatOpacity(true)
                            TriggerEvent("Polar:chatReduceOpacity", true)
                        end,
                        function()
                            f = false
                            tPolar.setReducedChatOpacity(false)
                            TriggerEvent("Polar:chatReduceOpacity", false)
                        end
                    )
                    RageUI.Checkbox(
                        ...
                        function()
                            h = true
                            tPolar.setShowHealthPercentageFlag(true)
                        end,
                        function()
                            h = false
                            tPolar.setShowHealthPercentageFlag(false)
                        end
                    )
                    RageUI.ButtonWithStyle(
                        "Crosshair",
            ...
            RageUI.DrawContent(
                ...
                function()
                    local function a9()
                        TriggerEvent("Polar:setDiagonalWeapons")
                        b = true
                        tPolar.setDiagonalWeaponSetting(b)
                    end
                    local function aa()
                        TriggerEvent("Polar:setVerticalWeapons")
                        b = false
                    ...
                    local function a9()
                        TriggerEvent("Polar:hsSoundsOn")
                        d = true
                        tPolar.setHitMarkerSetting(d)
                        tPolar.notify("~y~Experimental Headshot sounds now set to " .. tostring(d))
                    end
                    local function aa()
                        TriggerEvent("Polar:hsSoundsOff")
                        d = false
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(a4, a5, a6)
                            if a6 then
                                ...
                                r = nil
                                t = nil
                                TriggerServerEvent("Polar:getCustomWeaponsOwned")
                            end
                        end,
                        RMenu:Get("SettingsMenu", "weaponswhitelist")
                    )
                ...
                function()
                    RageUI.List(
                        ...
                        function(ab, ac, ad, ae)
                            if ae ~= f then
                                f = ae
                                SetResourceKvpInt("Polar_gang_ping_marker", ae)
                            end
                        end
                    )
                    RageUI.List(
                        ...
                        function(ak, al, am, an)
                            if an ~= o then
                                o = an
                                SetResourceKvpInt("Polar_gang_ping_volume", an)
                            end
                        end
                    )
                    RageUI.Checkbox(
                        ...
                        function()
                            m = true
                            tPolar.setGangPingSoundEnabled(true)
                        end,
                        function()
                            m = false
                            tPolar.setGangPingSoundEnabled(false)
                        end
                    )
                    -- RageUI.List(
                    --     "Health UI X",
                    ...
                    --         if an ~= r then
                    --             r = an
                    --             tPolar.setGangUIXPos(s[an])
                    --             SetResourceKvpInt("Polar_gang_position_x", r)
                    --         end
                    --     end
                    -- )
                    -- RageUI.List(
                    ...
                    --         if an ~= t then
                    --             t = an
                    --             tPolar.setGangUIYPos(u[an])
                    --             SetResourceKvpInt("Polar_gang_position_y", t)
                    --         end
                    --     end
                    -- )
                    RageUI.List(
                        ...
                        function(ab, ac, ad, ae)
                            if ae ~= j then
                                j = ae
                                SetResourceKvpInt("Polar_gang_name_distance", ae)
                                TriggerEvent("Polar:setGangNameDistance", k[ae])
                            end
                        end
                    )
                end
            ...
            RageUI.DrawContent(
                ...
                function()
                    RageUI.Checkbox(
                        ...
                        function()
                            tPolar.setFlashlightNotAimingFlag(true)
                        end,
                        function()
                            tPolar.setFlashlightNotAimingFlag(false)
                        end
                    )
                    RageUI.ButtonWithStyle(
                        ...
                        function(a4, a5, a6)
                            if a6 then
                                TriggerServerEvent("Polar:changeLinkedDiscord")
                            end
                        end
                    )
                end
        ...
        if RageUI.Visible(RMenu:Get("SettingsMenu", "inventorysettings")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = false},
                function()
                    RageUI.Checkbox("Use In-Game name","Swaps between players name and custom text for inventory title",i,{},function()
                    end,function()
                        i = true
                        tPolar.SetInventoryTextName(i)
                    end,function()
                        i = false
                        tPolar.ResetInventoryText(i)
                    end)
                    RageUI.ButtonWithStyle("Change Inventory Text","Set inventory text with a custom value.",{RightLabel = "→→→"},true,function(ad, ae, af)
                        if af then
                            tPolar.setInventoryText()
                        end
                    end)
                    ...
                    RageUI.ButtonWithStyle("Randomize All Colours","Set all with a randomized Colour with custom RGB values.",{RightLabel = "→→→"},true,function(ad, ae, af)
                        if af then
                            tPolar.setRandomInventoryColour()
                        end
                    end)
                    ...
                    RageUI.ButtonWithStyle("Reset All Colours","Reset Background, small bar & header to defualt RGB values.",{RightLabel = "→→→"},true,function(ad, ae, af)
                        if af then
                            tPolar.ResetHeaderColour()
                            tPolar.ResetBackgroundColour()
                            tPolar.ResetSmallBarColour()
                            tPolar.notify("~g~Succesfully reset all colours to defualt.")
                        end
                    end)
                    RageUI.ButtonWithStyle("Reset Inventory Text","Reset inventory text to defualt values.",{RightLabel = "→→→"},true,function(ad, ae, af)
                        if af then
                            tPolar.ResetInventoryText()
                            tPolar.notify("~g~Inventory text reset.")
                        end
                    end)
                    RageUI.ButtonWithStyle("Reset Small Bar Colour","Reset Small Bar Colour to defualt RGB values.",{RightLabel = "→→→"},true,function(ad, ae, af)
                        if af then
                        tPolar.ResetSmallBarColour()
                        tPolar.notify("~g~Inventory Small Bar colour reset.")
                        end
                    end)
                    ...
                    RageUI.ButtonWithStyle("Reset Background Colour","Reset Background to defualt RGB values.",{RightLabel = "→→→"},true,function(ad, ae, af)
                        if af then
                            tPolar.ResetBackgroundColour()
                            tPolar.notify("~g~Inventory background colour reset.")
                        end
                    end)
                end
            )
            ...
            RageUI.DrawContent(
                ...
                function()
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(a4, a5, a6)
                            if a6 then
                                TriggerServerEvent("Polar:enterDiscordCode")
                            end
                        end
                    )
                end
            ...
            RageUI.DrawContent(
                ...
                function()
                            ...
                            if a6 then
                                ...
                                if tonumber(ah) then
                                    ...
                                    if ah > 0 then
                                        ...
                                        TriggerServerEvent("Polar:generateWeaponAccessCode", r, ah)
                                    end
                                end
                            end
                        end
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(a4, a5, a6)
                            if a6 then
                                TriggerServerEvent("Polar:requestWhitelistedUsers", r)
                            end
                        end,
            ...
            RageUI.DrawContent(
                ...
                function()
                    ...
                    RageUI.List(
                        ...
                        function(a4, a5, a6, a7)
                            if a5 and W.particle ~= a7 then
                                if not tPolar.isPlusClub() and not tPolar.isPlatClub() then
                                    notify(
                                        "~y~You need to be a subscriber of Polar Plus or Polar Platinum to use this feature."
                                    )
                                    notify("~y~Available @ store.Polarstudios.uk")
                                end
                                W.particle = a7
                                Y()
                            end
                    ...
                    if GetGameTimer() - X > al + 1000 then
                        tPolar.addKillEffect(PlayerPedId(), true)
                        X = GetGameTimer()
                    end
                ...
                function()
                    RageUI.List(
                        ...
                        function(a4, a5, a6, a7)
                            ...
                            if a6 then
                                tPolar.addBloodEffect("head", 0x796E, PlayerPedId())
                            end
                        end
                    )
                    RageUI.List(
                        ...
                        function(a4, a5, a6, a7)
                            ...
                            if a6 then
                                tPolar.addBloodEffect("body", 0x0, PlayerPedId())
                            end
                        end
                    )
                    RageUI.List(
                        ...
                        function(a4, a5, a6, a7)
                            ...
                            if a6 then
                                tPolar.addBloodEffect("arms", 0xBB0, PlayerPedId())
                                tPolar.addBloodEffect("arms", 0x58B7, PlayerPedId())
                            end
                        end
                    )
                    RageUI.List(
                        ...
                        function(a4, a5, a6, a7)
                            ...
                            if a6 then
                                tPolar.addBloodEffect("legs", 0x3FCF, PlayerPedId())
                                tPolar.addBloodEffect("legs", 0xB3FE, PlayerPedId())
                            end
                        end
                    )
                end
...
RegisterNetEvent("Polar:OpenSettingsMenu")
AddEventHandler(
    "Polar:OpenSettingsMenu",
    function(am)
        if not am then
...
RegisterCommand(
    "opensettingsmenu",
    function()
        TriggerServerEvent("Polar:OpenSettings")
    end
)
...
Citizen.CreateThread(
    function()
        while true do
            ...
            if not (tPolar.getStaffLevel() > 0) then
...
function tPolar.addKillEffect(az, aA)
    if W.lightning then
        ForceLightningFlash()
    end
    if W.pedFlash then
    ...
    if W.particle ~= 1 and (tPolar.isPlatClub() or aA) then
        Citizen.CreateThreadNow(
            function()
                aw(az)
            end
...
function tPolar.addBloodEffect(aB, aC, ad)
    local aD = _[aB]
    if aD > 1 then
        local ax = U[aD]
        tPolar.loadPtfx(ax[1])
...
AddEventHandler(
    ...
    function(aE)
        ...
        if aG then
            ...
            tPolar.addBloodEffect(aJ, aC, aE)
        end
    end
)
RegisterNetEvent("Polar:gotDiscord")
AddEventHandler(
    "Polar:gotDiscord",
    function()
