...
function tRIFT.isPlayerRappeling()
    return k
end
function tRIFT.isPlayerInPoliceHeli()
    return l
end
local r = (a + b) * 0.5
local s = 0
...
local function X(Y)
    ...
    tRIFT.DrawText(0.45, 0.9, "Model: " .. _ .. "\nPlate: " .. a0, 0.55)
end
local function a1()
    while true do
        if l then
            local a2 = tRIFT.getPlayerPed()
            local heli = tRIFT.getPlayerVehicle()
            ...
            while m and not IsEntityDead(a2) and GetVehiclePedIsIn(a2) == heli and IsHeliHighEnough(heli) do
                ...
                if o then
                    ...
                    if ai - p >= 500 then
                        ...
                        TriggerServerEvent("RIFT:updateSpotlight", ak)
                        p = ai
                    end
...
local av = {}
RegisterNetEvent("RIFT:updateSpotlight",function(aw, ax)
    local ay = GetPlayerFromServerId(aw)
    if ay == -1 then
        return
    end
    local az = GetPlayerPed(ay)
    if az == 0 or az == tRIFT.getPlayerPed() then
        return
    end
