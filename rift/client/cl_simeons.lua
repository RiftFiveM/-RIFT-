...
RegisterNetEvent("Polar:gotCarDealerInstances",function(m)
    a = m
end)
RegisterNetEvent("Polar:gotCarDealerCategories",function(n)
    b = n
end)
RegisterNetEvent("Polar:carDealerPurchased",function()
    DeleteVehicle(f)
    f = 0
    g = 0
    h = false
    local o = a[d].posVector
    SetEntityCoords(tPolar.getPlayerPed(), o.x, o.y, o.z)
end)
local function p(q)
    if q ~= nil then
        d = q
...
local function s(t, u, v, w)
    ...
    if HasModelLoaded(x) then
        local z = CreateVehicle(x, u, v, w, GetEntityHeading(tPolar.getPlayerPed()), false, false)
        DecorSetInt(z, "PolarACVeh", 955)
        ...
        TaskWarpPedIntoVehicle(tPolar.getPlayerPed(), z, -1)
        ...
        tPolar.notify("~r~Could not load vehicle")
        return -1
    end
end
CreateThread(function()
    for A, B in pairs(l.simeonsInstances) do
        ...
        tPolar.addBlip(C.x, C.y, C.z, D, E)
        tPolar.addMarker(C.x, C.y, C.z, 0.7, 0.7, 0.5, G[1], G[2], G[3], 125, 50, F, true, true)
    end
end)
...
function isNearPreviewVector(J)
    if #(GetEntityCoords(tPolar.getPlayerPed()) - J) < 5.0 then
        return true
    end
    return false
end
...
Citizen.CreateThread(function()
    while true do
        if a ~= nil then
            for A, B in pairs(a) do
                local C, D, E, F, G, H, I =B.posVector,B.blipId,B.blipColour,B.markerId,B.markerColourTable,B.permissionsTable,B.categorys
                c[A] = #(GetEntityCoords(tPolar.getPlayerPed()) - C)
            end
        end
        Wait(250)
    end
end)
RMenu.Add("cardealer","mainmenu",RageUI.CreateMenu("", "", tPolar.getRageUIMenuWidth(), tPolar.getRageUIMenuHeight(), "banners",  "dealership"))
RMenu:Get("cardealer", "mainmenu"):SetSubtitle("~b~Categories")
RMenu.Add("cardealer","categories",RageUI.CreateSubMenu(RMenu:Get("cardealer", "mainmenu"),"","~b~Vehicles",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight()))
RMenu.Add("cardealer","vehicle",RageUI.CreateSubMenu(RMenu:Get("cardealer", "categories"),"","~b~Options",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight()))
RMenu.Add("cardealer","confirm",RageUI.CreateSubMenu(RMenu:Get("cardealer", "vehicle"),"","~b~Are you sure?",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight()))
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if d ~= nil then
                ...
                RageUI.ButtonWithStyle("Test Drive " .. i[1],"",{RightLabel = "→→→"},true,function(N, O, P)
                    if P then
                        DeleteVehicle(GetVehiclePedIsIn(tPolar.getPlayerPed()))
                        if not k then
                            RageUI.ActuallyCloseAll()
                            hash = GetHashKey(j)
                            ...
                            if HasModelLoaded(hash) then
                                ...
                                testDriveSeconds = 60
                                local S = GetEntityCoords(tPolar.getPlayerPed())
                                DecorSetInt(testDriveCar, "PolarACVeh", 955)
                                SetModelAsNoLongerNeeded(hash)
                                TaskWarpPedIntoVehicle(tPolar.getPlayerPed(), testDriveCar, -1)
                                setVehicleFuel(testDriveCar, 100)
                                tPolar.notify("~g~You have 1 minute to test drive this vehicle!")
                                for T = 0, 24 do
                                ...
                                SetTimeout(60000,function()
                                    if k then
                                        ...
                                        SetEntityCoords(tPolar.getPlayerPed(), S.x, S.y, S.z)
                                        tPolar.notify("~b~Test drive over!")
                                        p()
                                    end
                                end)
                                Citizen.CreateThread(function()
                                ...
                                Citizen.CreateThread(function()
                                    while k do
                                        local U = GetVehiclePedIsIn(tPolar.getPlayerPed(), false)
                                        if U ~= nil then
    ...
    if RageUI.Visible(RMenu:Get('cardealer', 'confirm')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle("Yes","",{RightLabel = "→→→"},true,function(N, O, P)
                if P then
                    TriggerServerEvent("Polar:purchaseCarDealerVehicle", e, j)
                    TriggerServerEvent('Polar:FetchCars', 'Standard Garage')
                end
            end,RMenu:Get("cardealer", "mainmenu"))
            RageUI.ButtonWithStyle("No","",{RightLabel = "→→→"},true,function(N, O, P)
                if P then
                    tPolar.notify("~y~Cancelled!")
                end
            end,
...
tPolar.createThreadOnTick(func_previewSimeonsVehicle)
