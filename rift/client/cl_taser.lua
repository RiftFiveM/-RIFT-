...
RegisterNetEvent("RIFT:receiveBarbs",function(e)
    c = true
    d = e
end)
RegisterNetEvent("RIFT:receiveActivation",function()
    if not tRIFT.isStaffedOn() then
        displayTaserNotification("You are currently being ~b~activated~w~.")
        SetPedToRagdoll(tRIFT.getPlayerPed(), 5000, 5000, 0, 0, 0, 0)
    end
end)
RegisterNetEvent("RIFT:arcSuccessful",function()
    ...
    TriggerServerEvent("RIFT:playTaserSound", GetEntityCoords(tRIFT.getPlayerPed()), "arcsound")
    b = false
...
RegisterNetEvent("playTaserSoundClient",function(f, g)
    local h = GetEntityCoords(tRIFT.getPlayerPed(), true)
    local i = #(h - f)
    if i <= 15 then
        SendNUIMessage({transactionType = g})
    end
end)
RegisterNetEvent("RIFT:barbsNoLonger",function()
    c = false
    d = 0
    displayTaserNotification("Your taser ~b~barbs ~w~have been ~b~disconnected~w~.")
end)
RegisterNetEvent("RIFT:barbsRippedOut",function(e)
...
RegisterNetEvent("RIFT:reloadTaser",function(j)
    if GetSelectedPedWeapon(tRIFT.getPlayerPed()) == GetHashKey(a.hash) then
        MakePedReload(tRIFT.getPlayerPed())
        if a.cartridges == 0 then
            a.cartridges = 2
            if a.reactivate1.active then
                TriggerServerEvent("RIFT:barbsNoLongerServer", a.reactivate1.serverId)
            end
            if a.reactivate2.active then
                TriggerServerEvent("RIFT:barbsNoLongerServer", a.reactivate2.serverId)
            end
            a.reactivate1 = {active = false, ped = 0, serverId = 0}
            a.reactivate2 = {active = false, ped = 0, serverId = 0}
        ...
        if k.weapon == GetHashKey(a.hash) then
            if IsControlJustPressed(1, 308) then
                if a.reactivate1.active == true then
                    ...
                    TriggerServerEvent("RIFT:playTaserSound", k.playerCoords, "reactivate")
                    TriggerServerEvent("RIFT:reactivatePed", a.reactivate1.serverId)
                end
            elseif IsControlJustPressed(1, 307) then
                if a.reactivate2.active == true then
                    ...
                    TriggerServerEvent("RIFT:playTaserSound", k.playerCoords, "reactivate")
                    TriggerServerEvent("RIFT:reactivatePed", a.reactivate2.serverId)
                end
            elseif IsControlJustPressed(1, 121) and b then
                TriggerServerEvent("RIFT:arcTaser")
            end
            if a.cartridges < 1 then
                ...
                if IsPedShooting(k.playerPed) then
                    TriggerServerEvent("RIFT:playTaserSound", k.playerCoords, "taser")
                    a.cartridges = a.cartridges - 1
                    local l, m = GetEntityPlayerIsFreeAimingAt(k.playerId)
                    if l and IsEntityAPed(k.playerPed) then
                        local n = tRIFT.getPedServerId(m)
                        TriggerServerEvent("RIFT:receiveBarbsServer", n)
                        if n ~= nil then
                            if a.cartridges == 1 then
...
tRIFT.createThreadOnTick(func_taserHandler)
...
Citizen.CreateThread(function()
    SetPedMinGroundTimeForStungun(tRIFT.getPlayerPed(), 15000)
    while true do
        if IsPedBeingStunned(tRIFT.getPlayerPed()) then
            stunGun()
        end
        Wait(100)
    end
        ...
        if c then
            while c do
                ...
                if IsControlJustPressed(1, 38) then
                    c = false
                    if d and d > 0 then
                        TriggerServerEvent("RIFT:barbsRippedOutServer", d)
                    end
                    d = 0
...
function tRIFT.setStunExitsEarly()
    p = true
end
