...
local function f()
    ...
    g.position=tRIFT.getPlayerCoords()
    g.radius=g.radius+0.0
    g.vehicles.speed=g.vehicles.speed+0.0
    TriggerServerEvent("RIFT:createRPZone",g)
end
...
local function h()
    ...
    if i then 
        TriggerServerEvent("RIFT:removeRPZone",i)
    end 
end
local inRPZone = false
RMenu.Add("rpzones","mainmenu",RageUI.CreateMenu("","~b~Main Menu",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"banners","admin"))
RMenu.Add("rpzones","weapons",RageUI.CreateSubMenu(RMenu:Get("rpzones","mainmenu"),"","~b~Weapons",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"banners","admin"))
RMenu.Add("rpzones","vehicles",RageUI.CreateSubMenu(RMenu:Get("rpzones","mainmenu"),"","~b~Vehicles",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"banners","admin"))
...
RegisterNetEvent("RIFT:createRPZone",function(g)
...
RegisterNetEvent("RIFT:removeRPZone",function(r)
...
local function G(g)
    local l,u=GetCurrentPedWeapon(tRIFT.getPlayerPed())
    if u~=0 then 
        t(g,u)
    end
    local y,H=tRIFT.getPlayerVehicle()
    if y~=0 and H then 
        A(g,y)
    end
...
tRIFT.createThreadOnTick(I)

function tRIFT.GetRPZoneInfo()
    local K=tRIFT.getPlayerCoords()
    for l,g in ipairs(d)do 
        if#(K-g.position)<g.radius then
            return g
