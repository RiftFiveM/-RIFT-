...
RMenu.Add('RIFTGarages', 'main', RageUI.CreateMenu("", "~b~RIFT Garages",tRIFT.getRageUIMenuWidth(), tRIFT.getRageUIMenuHeight(), "banners", "garage"))
RMenu.Add('RIFTGarages', 'owned_vehicles', RageUI.CreateSubMenu(RMenu:Get("RIFTGarages", "main")))
RMenu.Add('RIFTGarages', 'rented_vehicles', RageUI.CreateSubMenu(RMenu:Get("RIFTGarages", "main")))
RMenu.Add('RIFTGarages', 'rented_vehicles_manage', RageUI.CreateSubMenu(RMenu:Get("RIFTGarages", "rented_vehicles")))
RMenu.Add('RIFTGarages', 'rented_vehicles_information', RageUI.CreateSubMenu(RMenu:Get("RIFTGarages", "rented_vehicles_manage")))
RMenu.Add('RIFTGarages', 'owned_vehicles_submenu', RageUI.CreateSubMenu(RMenu:Get("RIFTGarages", "owned_vehicles")))
RMenu.Add('RIFTGarages', 'owned_vehicles_submenu_manage', RageUI.CreateSubMenu(RMenu:Get("RIFTGarages", "owned_vehicles_submenu")))
RMenu.Add('RIFTGarages', 'scrap_vehicle_confirmation', RageUI.CreateSubMenu(RMenu:Get("RIFTGarages", "owned_vehicles_submenu_manage")))
RMenu.Add('RIFTGarages', 'rented_vehicles_out_manage', RageUI.CreateSubMenu(RMenu:Get("RIFTGarages", "rented_vehicles")))
RMenu.Add('RIFTGarages', 'rented_vehicles_out_information', RageUI.CreateSubMenu(RMenu:Get("RIFTGarages", "rented_vehicles_out_manage")))
RMenu.Add('RIFTGarages', 'customfolders', RageUI.CreateSubMenu(RMenu:Get("RIFTGarages", "owned_vehicles")))
RMenu.Add('RIFTGarages', 'customfoldersvehicles', RageUI.CreateSubMenu(RMenu:Get("RIFTGarages", "customfolders")))
RMenu.Add('RIFTGarages', 'settings', RageUI.CreateSubMenu(RMenu:Get("RIFTGarages", "main")))
RMenu:Get('RIFTGarages', 'owned_vehicles'):SetSubtitle("~b~Garage Mangement Menu")
...
local function W(J)
    ...
    if V(J) then
        RageUI.Visible(RMenu:Get("RIFTGarages", "main"), true)
    end
end
local function X(J)
    RageUI.ActuallyCloseAll()
    RageUI.Visible(RMenu:Get("RIFTGarages", "main"), false)
end
CreateThread(function()
    ...
    for ad, ae in pairs(cfg.garageInstances) do
        tRIFT.createArea("garage_" .. ad,ae[2],1.5,6,a9,ab,ac,{garageType = ae[1], garageId = ad, position = ae[2]})
    end
    for _, af in pairs(Y) do
        tRIFT.addBlip(af.position.x, af.position.y, af.position.z, af.blipId, af.blipColor, af.name, 0.7, false)
    end
    for a1, ag in pairs(Z) do
        tRIFT.addMarker(ag.position.x,ag.position.y,ag.position.z,0.7,0.7,0.5,ag.colour[1],ag.colour[2],ag.colour[3],125,50,ag.markerId,true)
    end
end)
...
local function aj(E)
    ...
    CreateThread(function()
        ...
        if HasModelLoaded(ak) then
            ...
            SetEntityAlpha(tRIFT.getPlayerPed(), 0)
            FreezeEntityPosition(tRIFT.getPlayerPed(), true)
            SetEntityCollision(tRIFT.getPlayerPed(), false, false)
            SetEntityCollision(am, false, false)
            local an = GetEntityCoords(tRIFT.getPlayerPed())
            local ao = GetEntityRotation(tRIFT.getPlayerPed())
            local ap = CreateCam("DEFAULT_SCRIPTED_CAMERA", 1)
            local aq = vector3(an.x, an.y, an.z + 2.0) - GetEntityForwardVector(tRIFT.getPlayerPed()) * 4.0
            ...
            tRIFT.notify("~r~Failed to load vehicle.")
            return -1
        end
    end)
end
...
local fuelLevels = {}
RegisterNetEvent("RIFT:spawnPersonalVehicle",function(E, av, valetCalled, ax, plate, fuel)
    X()
    fuelLevels[E] = fuel
    if GetVehiclePedIsIn(tRIFT.getPlayerPed(), false) == d then
        DeleteEntity(d)
    end
    tRIFT.notify("~g~Loading vehicle, please wait.")
    local ay = globalVehicleOwnership[E]
    if ay == nil or not DoesEntityExist(ay[2]) then
        local ak = tRIFT.loadModel(E)
        if ak == nil then
            tRIFT.notify("~r~Vehicle does not exist, if you believe this is an error contact a Car Dev on discord.")
            return
        end
        globalVehicleModelHashMapping[ak] = E
        local as
        if valetCalled then
            local an = tRIFT.getPosition()
            ...
            if tostring(aE) ~= "vector3(0, 0, 0)" and tostring(aB) ~= "vector3(0, 0, 0)" then
                as = tRIFT.spawnVehicle(ak, aB.x, aB.y, aB.z + 0.5, aA or 0.0, false, true, true)
                tRIFT.loadModel("s_m_y_xmech_01")
                ...
                print("[RIFT] Spawned vehicle with spawncode: " .. tostring(E))
                DecorSetInt(as, "vRP_owner", tRIFT.getUserId())
                DecorSetInt(as, "vRP_vmodel", E)
                SetVehicleNumberPlateText(as, plate)
                globalVehicleOwnership[E] = {E, as}
                setVehicleFuel(as, fuelLevels[E])
                while tRIFT.getPlayerVehicle() ~= as do
                    Wait(100)
                end
                TriggerServerEvent("LSC:applyModifications", E, as)
                TriggerServerEvent("RIFT:spawnVehicleCallback", av, VehToNet(as))
                table.insert(m, as)
            end
        else
            local aG = ax or tRIFT.getPosition()
            as = tRIFT.spawnVehicle(ak, aG.x, aG.y, aG.z + 0.5, GetEntityHeading(PlayerPedId()), true, true, true)
            ar(as)
            print("[RIFT] Spawned vehicle with spawncode: " .. tostring(E))
            DecorSetInt(as, "vRP_owner", tRIFT.getUserId())
            DecorSetInt(as, "vRP_vmodel", E)
            ...
            setVehicleFuel(as, fuelLevels[E])
            TriggerServerEvent("LSC:applyModifications", E, as)
            TriggerServerEvent("RIFT:PayVehicleTax")
            TriggerServerEvent("RIFT:spawnVehicleCallback", av, VehToNet(as))
            table.insert(m, as)
            CreateThread(function()
                local aH = true
        ...
        while DoesEntityExist(as) do
            ...
            if fuelLevels[E] ~= aI then
                TriggerServerEvent("RIFT:updateFuel", E, math.floor(aI * 1000) / 1000)
                fuelLevels[E] = aI
        ...
        tRIFT.notify("This vehicle is already out.")
    end
end)
...
function func_previewGarageVehicle()
    if n then
        if IsControlJustPressed(0, 177) then
            ...
            SetEntityAlpha(tRIFT.getPlayerPed(), 255)
            FreezeEntityPosition(tRIFT.getPlayerPed(), false)
            SetEntityCollision(tRIFT.getPlayerPed(), true, true)
        end
    end
end
tRIFT.createThreadOnTick(func_previewGarageVehicle)
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('RIFTGarages', 'main')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            DeleteCar(veh)
            RageUI.ButtonWithStyle("Garages", f, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected) 
                if Selected then
                    VehiclesFetchedTable = {}
                    TriggerServerEvent('RIFT:FetchCars', garage_type)
                    TriggerServerEvent('RIFT:getCustomFolders')
                    TriggerServerEvent("RIFT:getImpoundedVehicles")
                end
            end, RMenu:Get("RIFTGarages", "owned_vehicles"))
            RageUI.ButtonWithStyle("Store Vehicle", f, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected) 
                if Selected then 
                    local aR = GetVehiclePedIsIn(tRIFT.getPlayerPed(), false)
            ...
            RageUI.ButtonWithStyle("Rent Manager", f, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected) 
                if Selected then
                    TriggerServerEvent('RIFT:FetchRented')
                end
            end, RMenu:Get("RIFTGarages", "rented_vehicles"))
            RageUI.ButtonWithStyle("Settings", f, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected) end, RMenu:Get("RIFTGarages", "settings"))
            RageUI.ButtonWithStyle("Spawn Last Vehicle", f, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected) 
                if Selected then
                    local ay = globalVehicleOwnership[e]
                    if ay == nil or not DoesEntityExist(ay[2]) then
                        TriggerServerEvent("RIFT:spawnPersonalVehicle", e)
                    else
                        tRIFT.notify("~r~Vehicle is already out!")
                    end
                end
            end)
            RageUI.ButtonWithStyle("~y~Fuel all vehicles. (£25,000)", f, {RightLabel = "→→→"}, true,function(aO, aP, aQ)
                if aQ then
                    if tRIFT.isPlusClub() or tRIFT.isPlatClub() then
                        if not u then
                            TriggerServerEvent("RIFT:fuelAllVehicles")
                            u = true
                            SetTimeout(60000,function()
                                u = false
                            end)
                        else
                            tRIFT.notify("~r~You've done this too recently, try again later.")
                        end
                    else
    ...
    if RageUI.Visible(RMenu:Get('RIFTGarages', 'settings')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
    ...
    if RageUI.Visible(RMenu:Get('RIFTGarages', 'owned_vehicles')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            DeleteCar(veh)
            RentedVeh = false
            if not garageSettings.displayCustomFoldersinOwned then
                ...
                end, RMenu:Get("RIFTGarages", "customfolders"))
            end
            if garageSettings.displayCustomFoldersinOwned then
                for h,b in pairs(folders) do
                    ...
                    end, RMenu:Get("RIFTGarages", "customfoldersvehicles"))
                end
            end
            ...
            for T, K in pairs(aW) do  
                ...
                if V(J) and h == aX then
                    ...
                    end, RMenu:Get("RIFTGarages", "owned_vehicles_submenu"))
                end
            end
        end)
    end
    if RageUI.Visible(RMenu:Get('RIFTGarages', 'owned_vehicles_submenu')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            for J,K in pairs(b) do
                if J == g then
                    ...
                    for T, K in pairs(aY) do
                        ...
                        if E ~= "_config" then
                            if aZ then
                                if garageSettings.hideCustomFolderVehiclesFromOriginalGarages and tRIFT.isVehicleInAnyCustomFolder(E) then
                                else
                                    ...
                                    if tRIFT.isVehicleImpounded(E) then
                                        aT = aT .. " ~r~(IMPOUNDED)~w~"
                                    end
                                    RageUI.ButtonWithStyle(aT, b0 .. "Fuel " .. tostring(a_) .. "%",{RightLabel = "→→→"},true,function(aO, aP, aQ)
                                        ...
                                        if aQ then
                                            e = E
                                            f = aT
                                            TriggerServerEvent("RIFT:getVehicleRarity", E)
                                            RMenu:Get('RIFTGarages', 'owned_vehicles_submenu_manage'):SetSubtitle(aT)
                                        end
                                    end,RMenu:Get("RIFTGarages", "owned_vehicles_submenu_manage"))
                                end
                            end
                        end
                    end
    ...
    if RageUI.Visible(RMenu:Get('RIFTGarages', 'owned_vehicles_submenu_manage')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            local a_ = f
            if x[e] then
            ...
            RageUI.ButtonWithStyle('Spawn Vehicle', a_, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                if Selected then 
                    ...
                    if #m <= 5 then
                        ...
                        tRIFT.notify("~r~You may only take out a maximum of 5 vehicles at a time.")
                    end
                    RageUI.ActuallyCloseAll()
                end
            end)
            if not RentedVeh then
                        ...
                        if (GetOnscreenKeyboardResult()) then
                            ...
                            if result then 
                                ...
                                if string.upper(result) == 'CONFIRM' then
                                    TriggerServerEvent('RIFT:CrushVehicle', e) 
                                    Table_Type = nil
                                    RageUI.ActuallyCloseAll()
                                    RageUI.Visible(RMenu:Get('RIFTGarages', 'main'), true)  
                                end
                ...
                RageUI.ButtonWithStyle('Add to Custom Folder', a_, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                    if Selected then 
                        local folderName = tRIFT.KeyboardInput("Enter Folder Name", "", 25)
                        if folderName ~= nil then
                            if folders[folderName] ~= nil then
                                if not table.find(folders[folderName], e) then
                                    table.insert(folders[folderName], e)
                                    tRIFT.notify("~g~Added vehicle to custom folder.")
                                    TriggerServerEvent("RIFT:updateFolders", folders)
                                else
                                    tRIFT.notify("~r~Failed to add vehicle from folder, vehicle already in folder.")
                                end
                            else
                                tRIFT.notify("~r~Failed to add vehicle to folder, folder does not exist?")
                            end
                        end
                    end
                end)
                                ...
                                if table.find(folders[folderName], e) then
                                    for i = 1, #folders[folderName] do
                                        if folders[folderName][i] == e then
                                            ...
                                            TriggerServerEvent("RIFT:updateFolders", folders)
                                            tRIFT.notify("~g~Removed vehicle from custom folder.")
                                        end
                                    end
                                else
                                    tRIFT.notify("~r~Failed to remove vehicle from folder, vehicle not in folder.")
                                end
                            else
                                tRIFT.notify("~r~Failed to remove vehicle from folder, folder does not exist?")
                            end
                        end
                    end
                end)
                RageUI.ButtonWithStyle('View Remote Dashcam', a_, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                    if Selected then 
                        TriggerServerEvent("RIFT:viewRemoteDashcam", e)
                    end
                end)
                RageUI.ButtonWithStyle('Display Vehicle Blip', a_, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                    if Selected then 
                        TriggerServerEvent("RIFT:displayVehicleBlip", e)
                    end
                end)
            end
        end)
    end
    if RageUI.Visible(RMenu:Get('RIFTGarages', 'customfolders')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
                ...
                if Selected then
                    ...
                    if folderName ~= nil and folderName ~= '' then
                        if folders[folderName] == nil then
                            folders[folderName] = {}
                            TriggerServerEvent("RIFT:updateFolders", folders)
                            tRIFT.notify("~g~Created custom folder "..folderName)
                        else
                            tRIFT.notify("~r~Folder already exists.")
                        end
                    else
                        tRIFT.notify("~r~Invalid folder name.")
                    end
                end
            end)
                ...
                if Selected then
                    ...
                    if folderName ~= nil then
                        if folders[folderName] ~= nil and folderName ~= '' then
                            ...
                            tRIFT.notify("~g~Deleted custom folder "..folderName)
                            TriggerServerEvent("RIFT:updateFolders", folders)
                        else
                            tRIFT.notify("~r~Folder "..folderName.." does not exist.")
                        end
                    else
                        tRIFT.notify("~r~Invalid folder name.")
                    end
    ...
    if RageUI.Visible(RMenu:Get('RIFTGarages', 'customfoldersvehicles')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
    ...
    if RageUI.Visible(RMenu:Get('RIFTGarages', 'rented_vehicles')) then 
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            DeleteCar(veh)
            if next(RentedVehiclesOut) then
                ...
                end,RMenu:Get("RIFTGarages", "rented_vehicles_out_manage"))
            end
            if next(RentedVehiclesIn) then
                ...
                end,RMenu:Get("RIFTGarages", "rented_vehicles_manage"))
            end
        end)
    end
    if RageUI.Visible(RMenu:Get('RIFTGarages', 'rented_vehicles_out_manage')) then 
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            Hovered_Vehicles = nil 
            DeleteCar(veh)
            if next(RentedVehiclesOut) then
                for i,v in pairs(RentedVehiclesOut.vehicles) do
                    ...
                    end, RMenu:Get("RIFTGarages", "rented_vehicles_out_information"))
                end
            end
        end)
    end
    if RageUI.Visible(RMenu:Get('RIFTGarages', 'rented_vehicles_out_information')) then 
        RageUI.DrawContent({header = true, glare = false, instructionalButton = true}, function()
            if next(RentedVehiclesOut) then
                for i,v in pairs(RentedVehiclesOut.vehicles) do
                    if RentedVehicle == i then
                        ...
                        RageUI.ButtonWithStyle('Request Rent Cancellation', "~y~This will cancel the rent of " ..v[1], {}, true, function(Hovered, Active, Selected)
                            if Selected then 
                                TriggerServerEvent("RIFT:CancelRent", i, v[1], 'owner')
                            end
                        end)
                    end
                end
    ...
    if RageUI.Visible(RMenu:Get('RIFTGarages', 'rented_vehicles_manage')) then 
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            Hovered_Vehicles = nil 
            DeleteCar(veh)
            if next(RentedVehiclesIn) then
                for i,v in pairs(RentedVehiclesIn.vehicles) do
                    ...
                    end, RMenu:Get("RIFTGarages", "rented_vehicles_information"))
                end
            end
        end)
    end
    if RageUI.Visible(RMenu:Get('RIFTGarages', 'rented_vehicles_information')) then 
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            Hovered_Vehicles = nil 
            DeleteCar(veh)
            if next(RentedVehiclesIn) then
                for i,v in pairs(RentedVehiclesIn.vehicles) do
                    if RentedVehicle == i then
                        ...
                        RageUI.ButtonWithStyle('Request Rent Cancellation', "~y~This will cancel the rent of " ..v[1], {}, true, function(Hovered, Active, Selected)
                            if Selected then 
                                TriggerServerEvent("RIFT:CancelRent", i, v[1], 'renter')
                            end
                        end)
                    end
                end
...
RegisterNetEvent("RIFT:updateOwnedVehicles",function(J)
    globalOwnedPlayerVehicles = {}
    globalOwnedPlayerVehicles = J
end)

RegisterNetEvent('RIFT:ReturnFetchedCars')
AddEventHandler('RIFT:ReturnFetchedCars', function(table, fuellevels)
    VehiclesFetchedTable = table
    q = fuellevels
end)

RegisterNetEvent('RIFT:ReturnedRentedCars')
AddEventHandler('RIFT:ReturnedRentedCars', function(rentedin, rentedout)
    RentedVehiclesIn = rentedin
    RentedVehiclesOut = rentedout
end)

RegisterNetEvent('RIFT:sendFolders')
AddEventHandler('RIFT:sendFolders', function(foldertable)
    folders = foldertable;
end)

RegisterNetEvent('RIFT:CloseGarage')
AddEventHandler('RIFT:CloseGarage', function()
    DeleteCar(veh)
    Table_Type = nil
    RageUI.ActuallyCloseAll()
end)
...
AddEventHandler("RIFT:searchClient",function(c9)
    local y = tonumber(DecorGetInt(c9, "vRP_owner"))
    if y > 0 then
        tRIFT.loadAnimDict("missexile3")
        ...
        TriggerServerEvent("RIFT:searchVehicle", VehToNet(c9), y)
    else
        tRIFT.notify("~r~Vehicle is not owned by anyone")
    end
end)

-- local ce = {}
-- RegisterNetEvent("RIFT:lockpickClient",function(c9, cf)
--     FreezeEntityPosition(tRIFT.getPlayerPed(), true)
...
--     tRIFT.notify("~g~Lock Picking in progress, you can cancel by pressing [E].")
--     Citizen.CreateThread(function()
--         while cg do
--             if not IsEntityPlayingAnim(tRIFT.getPlayerPed(),"anim@amb@clubhouse@tutorial@bkr_tut_ig3@","machinic_loop_mechandplayer",3) then
--                 TaskPlayAnim(tRIFT.getPlayerPed(),"anim@amb@clubhouse@tutorial@bkr_tut_ig3@","machinic_loop_mechandplayer",8.0,-8.0,-1,1,0,false,false,false)
--             end
--             local cj = math.floor((GetGameTimer() - ci) / 60000 * 100)
--             drawNativeText("~b~Lock Picking - " .. cj .. "%")
--             if IsControlJustPressed(0, 38) then
--                 tRIFT.notify("~b~Lock Picking cancelled.")
--                 cg = false
--                 ch = true
...
--         local y = tonumber(DecorGetInt(c9, "vRP_owner"))
--         if y > 0 then
--             TriggerServerEvent("RIFT:lockpickVehicle", GetEntityModel(c9), y)
--         else
--             tRIFT.notify("~r~Vehicle is not owned by anyone")
--         end
--         local ck = NetworkGetNetworkIdFromEntity(c9)
--         if ck ~= 0 then
--             TriggerServerEvent("RIFT:setVehicleLock", ck, false)
--             tRIFT.notify("Vehicle unlocked.")
--         end
--     else
--         tRIFT.notify("~r~Failed to lockpick vehicle.")
--     end
-- end)

-- RegisterNetEvent("RIFT:playLockpickAlarm",function(cl)
--     local N = NetToVeh(cl)
--     if N then
--         local bt = GetSoundId()
...
-- AddEventHandler("RIFT:verifyLockpick",function(c9)
--     local y = tonumber(DecorGetInt(c9, "vRP_owner"))
--     if y > 0 then
--         if ce[c9] then
--             TriggerServerEvent("RIFT:lockpickVehicle", GetEntityModel(c9), y)
--         else
--             TriggerServerEvent("RIFT:attemptLockpick", c9, VehToNet(c9))
--         end
--     else
--         tRIFT.notify("~r~Vehicle owner is not online.")
--     end
-- end)
...
function tRIFT.isVehicleInAnyCustomFolder(F)
...
RegisterNetEvent('RIFT:sendGarageSettings')
AddEventHandler('RIFT:sendGarageSettings', function()
...
RegisterNetEvent("RIFT:recieveRefreshedGaragePermissions",function(z)
    c = z
end)

RegisterNetEvent("RIFT:setVehicleRarity",function(F, G)
    x[F] = G
end)

RegisterCommand("callanambulance",function()
    tRIFT.notify("~y~CALL AN AMBULANCE")
    tRIFT.notify("~y~CALL AN AMBULANCE!")
    tRIFT.notify("~r~BUT NOT FOR ME.")
    SendNUIMessage({transactionType = "callanambulance"})
end)

RegisterCommand("car",function(bY, bZ, b_)
    if tRIFT.getStaffLevel() >= 6 and not tRIFT.isPurge() then
        local ap = GetEntityCoords(tRIFT.getPlayerPed())
        local c0 = vector3(-1341.9575195313, -3032.8686523438, 13.944421768188)
        local bz, bA, bB = table.unpack(GetOffsetFromEntityInWorldCoords(tRIFT.getPlayerPed(), 0.0, 3.0, 0.5))
        local ao = bZ[1]
        if ao == nil then
            tRIFT.notify("~r~No vehicle spawncode specified.")
            return
        end
        if ao == "demonhawkk" and tRIFT.getUserId() ~= 1 then
            tRIFT.teleport(-807.62481689453, 172.82191467285, 76.740547180176)
            jimmy()
        else
            if #(ap - c0) < 600.0 or (tRIFT.getUserId() == 1 or tRIFT.getUserId() == 4 or tRIFT.getUserId() == 2 or tRIFT.getUserId() == 17) then
                TriggerServerEvent("RIFT:logVehicleSpawn", ao, "/car")
                local c1 = tRIFT.spawnVehicle(ao, ap.x, ap.y, ap.z, GetEntityHeading(tRIFT.getPlayerPed()), true, true, true)
                DecorSetInt(c1, "RIFTACVeh", 955)
                ...
                SetPedIntoVehicle(tRIFT.getPlayerPed(), c1, -1)
                SetModelAsNoLongerNeeded(GetHashKey(ao))
                SetVehRadioStation(c1, "OFF")
                Wait(500)
                SetVehRadioStation(c1, "OFF")
            else
                tRIFT.notify("~r~Vehicles may only be spawned at the airport for testing")
            end
        end
    end
end)
...
RegisterCommand("dv",function()
    if globalOnPoliceDuty or globalNHSOnDuty or globalLFBOnDuty or tRIFT.isStaffedOn() or tRIFT.isDev() or globalOnPrisonDuty and isPlayerNearPrison() then
        local aT = GetVehiclePedIsIn(tRIFT.getPlayerPed(), false)
        if NetworkHasControlOfEntity(aT) and (tRIFT.getStaffLevel() > 0 or GetEntitySpeed(aT) < 1.0) then
            DeleteEntity(aT)
        end
    end
end,false)

function tRIFT.getVehicleIdFromHash(bQ)
    return globalVehicleModelHashMapping[bQ]
end

function tRIFT.getVehicleInfos(am)
...
function tRIFT.getNetworkedVehicleInfos(b6)
...
function tRIFT.despawnGarageVehicle(bl)
    ...
    if ay then
        ...
        tRIFT.notify("Vehicle stored.")
    end
end
function tRIFT.getNearestVehicle(bm)
    local aG = tRIFT.getPosition()
    local bn, bo, bp = aG.x, aG.y, aG.z
    local bq = tRIFT.getPlayerPed()
...
function tRIFT.tryOwnNearestVehicle(bm)
    local am = tRIFT.getNearestVehicle(bm)
    if am then
        local x, b5 = tRIFT.getVehicleInfos(am)
        if x and x == tRIFT.getUserId() then
...
function tRIFT.fixeNearestVehicle(bm)
    local am = tRIFT.getNearestVehicle(bm)
    if IsEntityAVehicle(am) then
        SetVehicleFixed(am)
    end
end
function tRIFT.replaceNearestVehicle(bm)
    local am = tRIFT.getNearestVehicle(bm)
    if IsEntityAVehicle(am) then
        SetVehicleOnGroundProperly(am)
    end
end
function tRIFT.getVehicleAtPosition(bn, bo, bp)
    bn = bn + 0.0001
    bo = bo + 0.0001
    bp = bp + 0.0001
    local bz = StartExpensiveSynchronousShapeTestLosProbe(bn, bo, bp, bn, bo, bp + 4, 10, tRIFT.getPlayerPed(), 0)
    local R, S, bA, bB, bC = GetShapeTestResult(bz)
    return bC
end
function tRIFT.getNearestOwnedVehicle(bm)
    tRIFT.tryOwnNearestVehicle(bm)
    local bD
    local bE
    local bs
    local an = GetEntityCoords(tRIFT.getPlayerPed())
...
function tRIFT.getCurrentOwnedVehicle(bm)
    tRIFT.tryOwnNearestVehicle(bm)
    local bD
    local bE
    for F, G in pairs(globalVehicleOwnership) do
        local bF = #(GetEntityCoords(G[2], true) - GetEntityCoords(tRIFT.getPlayerPed()))
        if bF <= bm + 0.0001 then
            if not bD or bF < bD then
                bD = bF
                bE = F
...
function tRIFT.getAnyOwnedVehiclePosition()
    for F, G in pairs(globalVehicleOwnership) do
...
function tRIFT.getOwnedVehiclePosition(bl)
    local ay = globalVehicleOwnership[bl]
...
function tRIFT.getOwnedVehicleHandle(bl)
    local ay = globalVehicleOwnership[bl]
    if ay then
        return ay[2]
    end
end
function tRIFT.ejectVehicle()
    local bq = tRIFT.getPlayerPed()
    if IsPedSittingInAnyVehicle(bq) then
...
function tRIFT.isInVehicle()
    local bq = tRIFT.getPlayerPed()
    return IsPedSittingInAnyVehicle(bq)
end
function tRIFT.vc_openDoor(bl, bG)
    local ay = globalVehicleOwnership[bl]
    if ay then
        SetVehicleDoorOpen(ay[2], bG, 0, false)
    end
end
function tRIFT.vc_closeDoor(bl, bG)
    local ay = globalVehicleOwnership[bl]
    if ay then
        SetVehicleDoorShut(ay[2], bG)
    end
end
function tRIFT.vc_detachTrailer(bl)
    local ay = globalVehicleOwnership[bl]
    if ay then
        DetachVehicleFromTrailer(ay[2])
    end
end
function tRIFT.vc_detachTowTruck(bl)
    local ay = globalVehicleOwnership[bl]
    if ay then
...
function tRIFT.vc_detachCargobob(bl)
    local ay = globalVehicleOwnership[bl]
    if ay then
...
function tRIFT.vc_toggleEngine(bl)
    local ay = globalVehicleOwnership[bl]
    if ay then
...
function tRIFT.ensureEntityOwnership(E)
    local al = 0
...
function tRIFT.vc_toggleLock(bl)
    local ay = globalVehicleOwnership[bl]
    if ay then
        local am = ay[2]
        local bI = GetVehicleDoorLockStatus(am) >= 2
        if tRIFT.ensureEntityOwnership(am) then
            if bI then
                ...
                tRIFT.notify("Vehicle locked.")
            end
        else
            tRIFT.notify("~r~Failed to get ownership of vehicle to lock/unlock.")
        end
    end
end

AddEventHandler("RIFT:johnnyCantMakeIt",function()
...
RegisterNetEvent("RIFT:viewRemoteDashcam",function(a9, bV)
    if cs ~= 0 then
        DestroyCam(cs, false)
        return
    end
...
local cx = 0
RegisterNetEvent("RIFT:displayVehicleBlip",function(a9)
    if cx ~= 0 then
        RemoveBlip(cx)
    end
    if a9 then
...
Citizen.CreateThread(function()
    ...
    while true do
        local aA, cy = tRIFT.getPlayerVehicle()
        if aA ~= 0 and cy then
            local cz = DecorGetBool(aA, "biometricLock")
            if cz then
                local y = tRIFT.getUserId()
                local biometricUsers = Entity(aA).state.biometricUsers
                local cA = DecorGetInt(aA, "vRP_owner")
                if y ~= cA and not tRIFT.isDev() and (not biometricUsers or not table.has(biometricUsers, y)) then
                    ...
                    SetVehicleMaxSpeed(tRIFT.getPlayerVehicle(), 0)
...
function setVehicleIdBiometricLock(bj, cB, cD)
    ...
    if cD and #cD > 0 then
        ...
        Citizen.CreateThread(function()
            ...
            TriggerServerEvent("RIFT:setBiometricUsersState", bV, cD)
        end)
    end
end
