...
local c
local d
AddEventHandler("RIFT:onClientSpawn",function(e, f)
    if f then
        local g = function()
            if not c then
        ...
        local i = function(j)
            if not c and IsControlJustPressed(1, 51) then
                TriggerServerEvent("RIFT:requestStartCooking", j.recipe)
        ...
        for k, l in pairs(a) do
            for m, n in pairs(l) do
                tRIFT.createArea("cooking_" .. k .. "_" .. m,n.cookingPosition,1.5,6,g,h,i,{recipe = k, recipeTable = a[k], index = m})
                tRIFT.addMarker(n.cookingPosition.x,n.cookingPosition.y,n.cookingPosition.z - 0.2,0.5,0.5,0.5,0,50,255,170,50,20,false,false,true)
            end
        end
    end
end)
...
local function t()
    for p, q in pairs(b) do
        for r, s in pairs(q) do
            local u = tRIFT.loadModel(s.model)
...
AddEventHandler("RIFT:onClientSpawn",function(e, f)
    if f then
        ...
        for w, x in pairs(b) do
            tRIFT.createArea("cooking_staticobjects_" .. w, x[1].position, 100, 5, g, h, i, {})
        end
    end
end)
...
local function B()
    CreateThread(function()
        tRIFT.startCircularProgressBar("",3000,nil,function()end)
    end)
    local C = PlayerPedId()
    tRIFT.loadAnimDict("amb@prop_human_bum_bin@enter")
    TaskPlayAnim(C, "amb@prop_human_bum_bin@enter", "enter", 8.0, 8.0, -1, 1, 0.2)
    RemoveAnimDict("amb@prop_human_bum_bin@enter")
    Wait(1500)
    tRIFT.loadAnimDict("amb@prop_human_bum_bin@exit")
...
local function D(E)
    local F = true
    local G
    tRIFT.minigameCircularProgressBar({Difficulty = E, Timeout = 100000, onComplete = function(H)
        if H then
            G = true
        else
            G = false
...
function cookBBQ()
    ...
    tRIFT.startCircularProgressBar("",10000,nil,function()end)
    local L = 0
    ...
    while L < 3 do
        ...
        if H then
            ...
            tRIFT.announceMpSmallMsg("Disaster", "You dropped the burger patty, start again!", 6, 3000)
        end
        Wait(2500)
        tRIFT.startCircularProgressBar("",10000,nil,function()end)
        Wait(2500)
    ...
    tRIFT.announceMpSmallMsg("Success", "You made a burger!", 18, 3000)
    ClearPedTasks(C)
    StopParticleFxLooped(K, true)
end
local function M()
    local C = tRIFT.getPlayerPed()
    local N = tRIFT.getPlayerCoords()
    local u = tRIFT.loadModel("prop_food_bs_tray_02")
    ...
    SetModelAsNoLongerNeeded(u)
    tRIFT.loadAnimDict("anim@heists@box_carry@")
    TaskPlayAnim(C, "anim@heists@box_carry@", "idle", 2.0, 2.0, -1, 51, 0, false, false, false)
    Citizen.CreateThread(function()
        while tRIFT.doesAreaExist("temp_foodorder_complete") do
...
local function T(U)
    ...
    local W = tRIFT.addMarker(V.x, V.y, V.z + 0.3, 0.15, 0.15, 0.15, 255, 215, 0, 170, 50, 0, false, true, true)
    local X = tRIFT.addBlip(V.x, V.y, V.z, 1, 2, "Delivery Point", 0.6)
    SetBlipFlashes(X, true)
    tRIFT.createArea("temp_foodorder_complete",V,1.5,6.0,function()
        tRIFT.removeArea("temp_foodorder_complete")
        tRIFT.removeMarker(W)
        tRIFT.removeBlip(X)
...
RegisterNetEvent("RIFT:cookingInstruction",function(Z)
    for r, _ in pairs(R) do
        tRIFT.removeArea("temp_cooking_instruction")
        tRIFT.removeMarker(_)
    end
    for r, Y in pairs(S) do
        tRIFT.removeBlip(Y)
    end
    ...
    local W = tRIFT.addMarker(V.x, V.y, V.z + 0.3, 0.15, 0.15, 0.15, 255, 215, 0, 170, 50, 0, false, true, true)
    table.insert(R, W)
    local X = tRIFT.addBlip(V.x, V.y, V.z, 1, 2, "Ingredient", 0.6)
    SetBlipFlashes(X, true)
    table.insert(S, X)
    local g = function()
    end
    ...
    local i = function()
        if IsControlJustPressed(1, 51) then
            ...
            if Z == "bbq" then
                ...
                tRIFT.removeMarker(W)
                T(function()
                    TriggerEvent("RIFT:playItemBoughtSound")
                    TriggerServerEvent("RIFT:pickupCookingIngredient", c, Z)
                    tRIFT.setCanAnim(true)
                    tRIFT.announceMpSmallMsg("Success", "You earned £4,000, now get back to work!", 18, 3000)
                end)
            else
                tRIFT.removeMarker(W)
                tRIFT.removeBlip(X)
    ...
    tRIFT.createArea("temp_cooking_instruction", V, 1.0, 6, g, h, i)
end)
RegisterNetEvent("RIFT:beginCooking",function(a0)
    TriggerEvent("RIFT:smallAnnouncement", "Cooking", "Make a " .. a0 .. ".", 15, 5000)
end)
RegisterNetEvent("RIFT:finishedCooking",function()
    c = nil
end)
