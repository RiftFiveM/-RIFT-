...
RMenu.Add("emotesmenu","mainmenu",RageUI.CreateMenu("","Main Menu",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"banners", "emotes"))
RMenu.Add("emotesmenu","emotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "mainmenu"),"","Emotes",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"banners", "emotes"))
RMenu.Add("emotesmenu","searchemotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "mainmenu"),"","Search Emotes",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"banners", "emotes"))
RMenu.Add("emotesmenu","danceemotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "emotes"),"","Dance Emotes",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"banners", "emotes"))
RMenu.Add("emotesmenu","customemotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "emotes"),"","Custom Emotes",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"banners", "emotes"))
RMenu.Add("emotesmenu","gunemotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "emotes"),"","Gun Emotes",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"banners", "emotes"))
RMenu.Add("emotesmenu","propemotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "emotes"),"","Prop Emotes",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"banners", "emotes"))
RMenu.Add("emotesmenu","sharedemotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "emotes"),"","Shared Emotes",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"banners", "emotes"))
RMenu.Add("emotesmenu","walkingstyles",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "mainmenu"),"","Walking Styles",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"banners", "emotes"))
RMenu.Add("emotesmenu","moods",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "mainmenu"),"","Moods",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"banners", "emotes"))
local b = false
local c = ""
local d = {}
local e = 0
...
local function p(q)
    ...
    if not q and GetGameTimer() - e < 1500 then
        tRIFT.notify("~r~Emotes are being rate limited.")
        return
    end
    b = false
    f = nil
...
local function F(E)
    if tRIFT.getPlayerVehicle() ~= 0 then
        tRIFT.notify("~r~Can not use scenarios whilst in a vehicle.")
        return
    end
    local A = PlayerPedId()
    ...
    elseif E[1] == "MaleScenario" then
        if tRIFT.getModelGender() == "male" then
            ...
            tRIFT.notify("~r~This scenario is male only.")
        end
...
local function a6(E)
    if not tRIFT.canAnim() or tRIFT.isHandcuffed() then
        return true
    end
    local A = PlayerPedId()
    ...
    local a9 = tRIFT.getPlayerVehicle()
...
local function a7(E)
    if a6(E) then
        tRIFT.notify("~r~Can not this emote at this time.")
        return
    end
...
local function a8()
    ...
    local aa = -1
    local ab = 2.0
    local ac = tRIFT.getPlayerCoords()
...
local function ag(ah)
    if a6() then
        tRIFT.notify("~r~Can not use shared emotes at this time.")
        return
    end
    local ai = a8()
    if ai ~= 0 then
        i = ah
        j = ai
        TriggerServerEvent("RIFT:sendSharedEmoteRequest", ai, ah)
    else
        tRIFT.notify("~r~No player is near by.")
    end
end
local function aj(E)
    Citizen.CreateThreadNow(function()
        local ak = E[1]
        tRIFT.loadAnimDict(ak)
    ...
    if RageUI.Visible(RMenu:Get('emotesmenu', 'searchemotes')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if foundMatch == false then
                emoteString = tRIFT.KeyboardInput("Enter Emote Name", "", 10)
...
local function aF(aC, aD, aE)
    if #aD < 1 then
        tRIFT.notify("~r~No emote name was specified.")
        return
    end
    ...
    elseif a.guns[ah] then
        ...
        tRIFT.notify("~r~Invalid emote name was specified.")
    end
end
...
RegisterCommand("walk",function(aC, aD, aE)
    if #aD < 1 then
        tRIFT.notify("~r~No walk name was specified.")
        return
    end
    local ah = aD[1]
    if not ah then
    ...
    if a.walks[ah] then
        ...
        tRIFT.notify("~r~Invalid walk name was specified.")
    end
end)
RegisterNetEvent("RIFT:sendSharedEmoteRequest",function(aG, ah)
    if a.shared[ah] and not a6() then
        k = ah
        l = aG
        m = GetGameTimer()
        tRIFT.notify("~y~Y~w~ to accept, ~r~L~w~ to refuse (~g~" .. a.shared[ah][3] .. "~w~)")
    end
end)
RegisterNetEvent("RIFT:receiveSharedEmoteRequest",function(ah)
    p(false)
    Citizen.Wait(300)
    a7(a.shared[ah])
end)
RegisterNetEvent("RIFT:receiveSharedEmoteRequestSource",function()
...
local function aT()
    if k then
        ...
        elseif IsControlJustPressed(1, 246) then
            if a6() then
                tRIFT.notify("~r~You can not use emotes at this time.")
            else
                TriggerServerEvent("RIFT:receiveSharedEmoteRequest", l, a.shared[k][4])
            end
            aK()
        end
    end
    if b and f and g then
        ...
        if not IsEntityPlayingAnim(PlayerPedId(), f, g, 3) or tRIFT.getPlayerCombatTimer() > 0 then
            p(true)
        end
    end
end
function tRIFT.playEmote(ah)
    if a.emotes[ah] then
        a7(a.emotes[ah])
    elseif a.dances[ah] then
        a7(a.dances[ah])
