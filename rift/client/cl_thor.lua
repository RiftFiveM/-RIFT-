...
RegisterNetEvent("Polar:useTheForceTarget")
AddEventHandler("Polar:useTheForceTarget",function()
    FreezeEntityPosition(PlayerPedId(),true)
    Wait(2500)
    tPolar.loadAnimDict("ragdoll@human")
    TaskPlayAnim(tPolar.getPlayerPed(),"ragdoll@human","electrocute",3.0,1.0,-1,01,0,0,0,0)
    Wait(5000)
    SetEntityHealth(tPolar.getPlayerPed(),0)
    FreezeEntityPosition(PlayerPedId(),false)
end)

RegisterNetEvent("Polar:useTheForceSync")
AddEventHandler("Polar:useTheForceSync",function(q,r,s)
    if#(q-GetEntityCoords(PlayerPedId()))<25.0 then 
        TriggerEvent("Polar:PlaySound", "unlimitedpower")
        local s={}
        local a=0
    ...
    if tPolar.isDev() then
        ...
        if j then
            ...
            if A ~= -1 then
                ...
                local t = {}
                local s = r - q
                TriggerServerEvent("Polar:theForceSync", A, q, r, s)
                for u = 1, 10, 1 do
                    table.insert(t, q + vector3(s.x / 10 * u, s.y / 10 * u, s.z / 10 * u))
                end
                TriggerEvent("Polar:PlaySound", "unlimitedpower")
                FreezeEntityPosition(PlayerPedId(), true)
                tPolar.loadAnimDict("anim@amb@clubhouse@bar@drink@idle_a")
                TaskPlayAnim(tPolar.getPlayerPed(),"anim@amb@clubhouse@bar@drink@idle_a","idle_a_bartender",3.0,1.0,-1,01,0,0,0,0)
