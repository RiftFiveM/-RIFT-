...
Citizen.CreateThread(function()
    tRIFT.createDynamicPed(a.ped.modelHash,a.ped.position,215.0,true,a.ped.animDict,a.ped.animName,30,false,false)
    tRIFT.addBlip(a.ped.position.x, a.ped.position.y, a.ped.position.z, 357, 81, "Vehicle Impound", 0.8, false)
    Wait(2000)
    local d = function()
        TriggerServerEvent('RIFT:getImpoundedVehicles')
        RageUI.Visible(RMenu:Get("riftimpound", "main"), true)
    end
    local e = function()
    ...
    tRIFT.createArea("vehicle_impound", a.ped.position, 3.0, 6, d, e, f, {})
end)
RMenu.Add("riftimpound","reasons",RageUI.CreateMenu("","~b~Impounding Vehicle...",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"banners", "impound"))
RMenu.Add("riftimpound","main",RageUI.CreateMenu("","~b~Your Impounded Vehicles",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"banners", "impound"))
RMenu.Add("riftimpound", "vehicle_information", RageUI.CreateSubMenu(RMenu:Get("riftimpound", "main")))
RMenu.Add("riftimpound", "pay_fine_confirmation", RageUI.CreateSubMenu(RMenu:Get("riftimpound", "vehicle_information")))
local g = 0
...
RageUI.CreateWhile(1.0, true, function()
    ...
    if RageUI.Visible(RMenu:Get('riftimpound', 'reasons')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.ButtonWithStyle("~g~Confirm Selection","",{RightLabel = "→→→"},true,function(i, k, j)
                if j then
                    ...
                    TriggerServerEvent("RIFT:impoundVehicle",b.owner_id,b.owner_name,b.vehicle,b.vehicle_name,m,b.vehicle_net_id)
                    RageUI.ActuallyCloseAll()
                end
            end,function()end)
    ...
    if RageUI.Visible(RMenu:Get('riftimpound', 'main')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.Separator("View your impounded vehicles here.")
                ...
                if c ~= nil and c ~= {} then
                    for o, p in pairs(c) do
                        ...
                        RMenu:Get("riftimpound", "vehicle_information"))
                    end
                else
    ...
    if RageUI.Visible(RMenu:Get('riftimpound', 'vehicle_information')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if g ~= 0 then
                ...
                end,RMenu:Get("riftimpound", "pay_fine_confirmation"))
            end
        end)
    end
    if RageUI.Visible(RMenu:Get('riftimpound', 'pay_fine_confirmation')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if g ~= 0 then
                ...
                RageUI.ButtonWithStyle("~g~Pay","Pay the fine",{RightLabel = "→→→"},true,function(i, k, j)
                    if j then
                        TriggerServerEvent("RIFT:releaseImpoundedVehicle", g.vehicle)
                    end
                end,RMenu:Get("riftimpound", "main"))
...
RegisterNetEvent("RIFT:receiveInfoForVehicleToImpound",function(r, s, t, u, v)
    b = {owner_id = tonumber(r), owner_name = s, vehicle = t, vehicle_name = u, vehicle_net_id = v}
end)

RegisterNetEvent("RIFT:receiveImpoundedVehicles",function(w)
    c = w
end)

RegisterNetEvent("RIFT:impoundSuccess",function(x, t, y, z, A, B, C, D)
    ...
    if O ~= vector3(0, 0, 0) and N ~= vector3(0, 0, 0) then
        tRIFT.loadModel("flatbed")
        tRIFT.loadModel("a_m_m_prolhost_01")
        P = tRIFT.spawnVehicle("flatbed", O.x, O.y, O.z, M, false, true, true)
        ...
        TriggerServerEvent("RIFT:deleteImpoundEntities",x,NetworkGetNetworkIdFromEntity(G),NetworkGetNetworkIdFromEntity(F))
        u = tRIFT.spawnVehicle(z, C.x, C.y, C.z, D, false, true, false)
        ...
        TriggerServerEvent("RIFT:awaitTowTruckArrival", x, R, T)
...
RegisterNetEvent("RIFT:attachVehToTowCl",function(x, W)
    local u = NetworkGetEntityFromNetworkId(x)
...
function tRIFT.impoundVehicleOptions(Z, _, x, a0)
    local a1 = g_vehicleHashToVehicleName[_]
    if X(a0) then
        tRIFT.notifyPicture("polnotification","notification","The vehicle cannot be impounded with a person inside.","Metropolitan Police","Impound",nil,nil)
        return
    end
    local a2 = GetVehicleClass(a0)
    if a2 == 18 then
        tRIFT.notifyPicture("polnotification","notification","Emergency vehicles cannot be impounded.","Metropolitan Police","Impound",nil,nil)
    elseif a.disallowedVehicleClasses[a2] then
        tRIFT.notifyPicture("polnotification","notification","That vehicle is too large to be impounded.","Metropolitan Police","Impound",nil,nil)
    else
        TriggerServerEvent("RIFT:fetchInfoForVehicleToImpound", Z, a1, x)
        RageUI.Visible(RMenu:Get("riftimpound", "reasons"), true)
    end
end

function tRIFT.isVehicleImpounded(u)
    return c[u] ~= nil
end
...
exports("impound", tRIFT.impoundVehicleOptions)
exports("isImpounded", tRIFT.isVehicleImpounded)
