    ...
    while true do
        if a.active and a.id ~= 0 then
            ...
            if h ~= GetHashKey("WEAPON_UNARMED") and (not b[a.id].allowPistol or not table.has(c, h)) then
                tRIFT.setWeapon(g, "WEAPON_UNARMED", true)
            end
...
RMenu.Add("policeshields","main",RageUI.CreateMenu("Police Shields", "~b~Shield Management", tRIFT.getRageUIMenuWidth(), tRIFT.getRageUIMenuHeight()))
...
RegisterNetEvent("RIFT:toggleShieldMenu",function(l)
...
function removeShield(m)
    if DoesEntityExist(a.prop) then
        ...
        if m then
            tRIFT.notify("~g~Success~w~: Shield removed")
        end
    end
end
function createShield(n)
    ...
    local o = false
    local g = tRIFT.getPlayerPed()
    local p = tRIFT.getPlayerCoords()
    if a.active then
        removeShield(false)
    end
    a.id = n
    local q = tRIFT.loadModel(b[n].model)
    ...
    tRIFT.loadAnimDict(b[n].animDict)
