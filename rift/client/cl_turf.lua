...
Citizen.CreateThread(function()
	local j = tRIFT.addBlip(-552.90362548828, 5326.1733398438, 72.599586486816, 403, 3, "Meth Trader")
	local k = tRIFT.addBlip(133.73462677002, -1305.0340576172, 29.268741607666, 403, 4, "Cocaine Trader")
	local l = tRIFT.addBlip(103.47916412354, -1937.1091308594, 19.803705215454, 140, 2, "Weed Trader")
	local m = tRIFT.addBlip(3569.8334960938, 3662.5893554688, 33.920593261718, 586, 1, "Heroin Trader")
	local n = tRIFT.addBlip(-1118.4926757813, 4926.1889648438, 218.35691833496, 150, 1, "Large Arms Trader")
	local o = tRIFT.addBlip(5065.6201171875, -4591.3857421875, 2.8652405738831, 150, 1, "Large Arms Trader")
	local p = tRIFT.addBlip(1317.0300292969, 4309.8359375, 38.005485534668, 604, 8, "LSD North Trader", 1.0)
	local q = tRIFT.addBlip(2485.7429199219, -430.13989257813, 92.992835998535, 604, 8, "LSD South Trader", 1.0)
	local r = {
		{
			title = "Weed Trader",
			type = "radius",
...
AddEventHandler("RIFT:onClientSpawn",function(v, w)
	if w then
		TriggerServerEvent("RIFT:refreshTurfOwnershipData")
		TriggerServerEvent("RIFT:requestDrugPriceUpdate")
	end
end)
RegisterNetEvent("RIFT:attackGangCapture",function(x, y)
	f = true
	a = y
	otherMembersStartGangCapture(x, y, h[x].position)
end)
RegisterNetEvent("RIFT:updateTurfOwner",function(x, z)
	turfData[x].gangOwner = z
end)
RegisterNetEvent("RIFT:defendGangCapture",function(x, y)
	a = y
	e = true
...
RegisterNetEvent("RIFT:turfDefenseInterrupt",function()
	e = false
end)
RegisterNetEvent("RIFT:initiateGangCaptureCheck",function(y, C)
	b[y] = C
end)
RegisterNetEvent("RIFT:gotTurfOwnershipData",function(D)
...
function tRIFT.isPlayerInTurf()
    for E, F in pairs(h) do
        if #(tRIFT.getPlayerCoords() - F.position) < F.distance then
            return true
        end
    end
    return false
...
Citizen.CreateThread(function()
	for B, G in pairs(h) do
		tRIFT.addMarker(G.position.x,G.position.y,G.position.z,0.7,0.7,0.5,200,50,0,200,50,24,false,false,true)
	end
	for H, I in pairs(i) do
		tRIFT.addMarker(I.position.x,I.position.y,I.position.z,0.7,0.7,0.5,0,50,200,200,50,30,false,false,true)
	end
end)
local J = -1
	...
	if w then
		...
		local O = function(M)
			if IsControlJustPressed(1, 51) then
				if not PlayerIsInGang then
					tRIFT.notify("~r~You need to be in a gang to capture a turf!")
				elseif not f then
					TriggerServerEvent("RIFT:refreshTurfOwnershipData")
					TriggerServerEvent("RIFT:checkTurfCapture", M.turfId)
				end
				Wait(10000)
			end
		end
		for B, P in pairs(h) do
			tRIFT.createArea("turf_" .. B, P.position, 1.5, 6, L, N, O, {turfId = B})
		end
		local Q = function(M)
		...
		local S = function(M)
			if M.draw then
				...
				DrawAdvancedText(0.578, 0.349, 0.005, 0.0028, 0.437, "RIFT Trader Manager", 255, 255, 255, 255, 4, 0)
						...
						if T ~= nil and tonumber(T) then
							...
							if T <= 25 and T >= 0 then
								...
								if GetGameTimer() > K + 30000 then
									...
									if B == 1 then
										TriggerServerEvent("RIFT:setNewWeedPrice", T)
									elseif B == 2 then
										TriggerServerEvent("RIFT:setNewCocainePrice", T)
									elseif B == 3 then
										TriggerServerEvent("RIFT:setNewMethPrice", T)
									elseif B == 4 then
										TriggerServerEvent("RIFT:setNewHeroinPrice", T)
									elseif B == 5 then
										TriggerServerEvent("RIFT:setNewLargeArmsCommission", T)
									elseif B == 6 then
										TriggerServerEvent("RIFT:setNewLSDNorthPrice", T)
									elseif B == 7 then
										TriggerServerEvent("RIFT:setNewLSDSouthPrice", T)
									end
								else
									tRIFT.notify("~r~Rate limit, please wait 30s to change the commission.")
								end
							end
						end
					end
		...
		for B, G in pairs(i) do
			tRIFT.createArea("turfmanager_" .. B, G.position, 1.5, 6, Q, R, S, {turfId = B, draw = false})
		end
	end
end)
RegisterNetEvent("RIFT:captureOwnershipReturned",function(B, U, V)
	if not U then
		startGangCapture(B, h[B].position, V)
	else
		tRIFT.notify("~r~Turf already owned!")
	end
end)
function otherMembersStartGangCapture(x, y, W)
    if b[y] then
        ...
        Citizen.CreateThread(function()
			local tmpMarker = tRIFT.addMarker(W.x, W.y, W.z - 10, 75, 75, 75, 0, 200, 0, 50, 1000)
			while f do
				local X = GetEntityCoords(tRIFT.getPlayerPed(), false)
				local Y = #(X - W)
				if Y >= 37.5 then
					tRIFT.notify("~r~Turf capture cancelled, you got too far!")
					f = false
					TriggerServerEvent("RIFT:gangDefenseLocationUpdate", y, "Attackers", true)
				elseif GetEntityHealth(tRIFT.getPlayerPed()) <= 102 then
					tRIFT.notify("~r~Turf capture failed, you died")
					f = false
					TriggerServerEvent("RIFT:gangDefenseLocationUpdate", y, "Attackers", true)
				end
				Wait(1000)
			end
			tRIFT.removeMarker(tmpMarker)
		end)
        Citizen.CreateThread(function()
			while f do
				if not d[y] then
...
function startGangCapture(x, W, y)
    if turfData[x].owned then
        tRIFT.notify("~r~You already own this turf!")
        TriggerServerEvent("RIFT:failCaptureTurfOwned", x)
    else
        TriggerServerEvent("RIFT:initiateGangCapture", x, y)
        local timer = g
        while b[y] == nil do
            Wait(0)
        end
        if b[y] then
            ...
            Citizen.CreateThread(function()
				tmpMarker = tRIFT.addMarker(W.x, W.y, W.z - 10, 75, 75, 75, 0, 200, 0, 50, 1000)
				while f do
					local X = GetEntityCoords(tRIFT.getPlayerPed(), false)
					local Y = #(X - W)
					if Y >= 37.5 then
...
function defendGangCapture(x, a, Z, _, a0)
    ...
    Citizen.CreateThread(function()
		local tmpMarker = tRIFT.addMarker(Z, _, a0 - 10, 75, 75, 75, 0, 200, 0, 50, 1000)
		while e do
			local X = GetEntityCoords(tRIFT.getPlayerPed(), false)
			local Y = #(X - vector3(h[x].x, h[x].y, h[x].z))
			if Y < 37.5 and GetEntityHealth(tRIFT.getPlayerPed()) > 102 then
				TriggerServerEvent("RIFT:gangDefenseLocationUpdate", a, "Defenders", true)
			elseif GetEntityHealth(tRIFT.getPlayerPed()) > 102 then
				TriggerServerEvent("RIFT:gangDefenseLocationUpdate", a, "Defenders", false)
			elseif GetEntityHealth(tRIFT.getPlayerPed()) <= 102 then
				tRIFT.notify("~r~Turf defense failed, you died")
				e = false
				TriggerServerEvent("RIFT:gangDefenseLocationUpdate", a, "Defenders", false)
			else
				TriggerServerEvent("RIFT:gangDefenseLocationUpdate", a, "Defenders", false)
...
RegisterNetEvent("RIFT:setBlockedStatus",function(y, a1)
	d[y] = a1
end)
RegisterNetEvent("RIFT:gangDefenseSuccess",function()
	e = false
end)
function gangCaptureSuccess(x, a)
    turfData[x].owned = true
    TriggerServerEvent("RIFT:gangCaptureSuccess", a)
end
function gangDefenseSuccess()
    TriggerServerEvent("RIFT:gangDefenseSuccess", a)
end
