...
RMenu.Add('RIFTStores', 'main', RageUI.CreateMenu("", "~b~Shop", tRIFT.getRageUIMenuWidth(), tRIFT.getRageUIMenuHeight(), 'banners', 'shop'))
RMenu.Add("RIFTStores", "confirm", RageUI.CreateSubMenu(RMenu:Get('RIFTStores', 'main',  tRIFT.getRageUIMenuWidth(), tRIFT.getRageUIMenuHeight())))

RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get("RIFTStores", "main")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            for k, v in pairs(cfg.shopItems) do
                ...
                end, RMenu:Get("RIFTStores", "confirm"))
            end
        end)
    end
end)
...
local Index = 1
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get("RIFTStores", "confirm")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            ...
            RageUI.Button("Confirm Purchase" , nil, {RightLabel = "→"}, true, function(Hovered, Active, Selected)
                if Selected then
                    TriggerServerEvent("RIFT:BuyStoreItem", cHash, tonumber(Index))
                end
            end, RMenu:Get("RIFTStores", "main")) 
        end) 
    end
end)
        ...
        for k, v in pairs(cfg.shops) do
            ...
            if isInArea(v, 1.0) and inMenu == false then
                ...
                if IsControlJustPressed(0, 51) then 
                    ...
                    RageUI.Visible(RMenu:Get("RIFTStores", "main"), true)
                    cLoaction = v
                end
            end
