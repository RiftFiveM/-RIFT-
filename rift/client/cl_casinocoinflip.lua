...
RegisterNetEvent("RIFT:sendCoinflipTableData")
AddEventHandler("RIFT:sendCoinflipTableData",function(C)
    f = C
end)

Citizen.CreateThread(function()
    Wait(1000)
    TriggerServerEvent("RIFT:requestCoinflipTableData")
    for q = 0, #c.coinflipTables do
        local D = c.coinflipTables[q]
        local DD = tRIFT.loadModel(D.tableModel)
...
Citizen.CreateThread(function()
    while true do
        if d then
            ...
            if not l and IsControlJustPressed(0, 202) and not g and not RageUI.Visible(RMenu:Get("riftcoinflip", "confirm")) then
                local K = tRIFT.loadAnimDict("anim_casino_b@amb@casino@games@shared@player@")
                NetworkStopSynchronisedScene(Local_198f_255)
                TaskPlayAnim(tRIFT.getPlayerPed(), K, "sit_exit_left", 1.0, 1.0, 2500, 0)
                d = false
                e = true
                showHowToCoinflip(false)
                TriggerServerEvent("RIFT:cancelCoinflip", true)
                TriggerServerEvent("RIFT:leaveCoinflipTable")
                h = {}
                m = false
        ...
        if not d then
            if a ~= -1 and b < 2 then
                if IsControlJustPressed(0, 38) then
                    if f[a] == false then
                        TriggerServerEvent("RIFT:requestSitAtCoinflipTable", a)
                    end
                end
            end
        end
...
RegisterNetEvent("RIFT:sitAtCoinflipTable")
AddEventHandler("RIFT:sitAtCoinflipTable",function(o, L)
    if L then
        h[L.betId] = L
    end
    goToCoinflipSeat(o)
...
function goToCoinflipSeat(o)
    ...
    TaskGoStraightToCoord(tRIFT.getPlayerPed(), R.x, R.y, R.z, 1.0, 5000, S.z, 0.01)
    Wait(500)
    local T = v(o)
    local U = A(o)
    local V = NetworkCreateSynchronisedScene(T.x, T.y, T.z, U.x, U.y, U.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(tRIFT.getPlayerPed(),V,"anim_casino_b@amb@casino@games@shared@player@",M(x),2.0,-2.0,13,16,2.0,0)
    ...
    NetworkAddPedToSynchronisedScene(tRIFT.getPlayerPed(),Locali98f_55,"anim_casino_b@amb@casino@games@shared@player@","idle_cardgames",2.0,-2.0,13,16,1148846080,0)
...
function showHowToCoinflip(W)
    ...
    RageUI.Visible(RMenu:Get("riftcoinflip", "instructions"), W)
end
function showCoinflipMenu(W)
    RageUI.Visible(RMenu:Get("riftcoinflip", "mainmenu"), W)
end
RMenu.Add("riftcoinflip", "instructions", RageUI.CreateMenu("", "~b~COINFLIP", 0, 100, "banners", "coinflip"))
RMenu.Add("riftcoinflip", "mainmenu", RageUI.CreateMenu("", "~b~COINFLIP", 0, 100, "banners", "coinflip"))
RMenu.Add("riftcoinflip", "confirm", RageUI.CreateSubMenu(RMenu:Get("riftcoinflip", "mainmenu"), "", "~b~COINFLIP", 0, 100, "banners", "coinflip"))


RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('riftcoinflip', 'instructions')) then
    ...
    if RageUI.Visible(RMenu:Get('riftcoinflip', 'mainmenu')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.Button("Propose Bet","",{RightLabel = "→→→"},true,function(X, Y, Z)
                if Z then
                    ...
                    if tonumber(_) then
                        _ = tonumber(_)
                        if _ > 0 then
                            TriggerServerEvent("RIFT:proposeCoinflip", _)
                        end
                    end
                end
            end)
                ...
                for a0, L in pairs(h) do
                    RageUI.Button("~y~£" .. getMoneyStringFormatted(L.betAmount),"",{RightBadge=RageUI.BadgeStyle.GoldMedal},true,function(X, Y, Z)
                        if Z then
                            ...
                            if L.user_id == tRIFT.getUserId() then
                                k = true
                            else
                                k = false
                            end
                    ...
                    RMenu:Get("riftcoinflip", "confirm"))
                end
            else
    ...
    if RageUI.Visible(RMenu:Get('riftcoinflip', 'confirm')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if not h[i] then
                RageUI.Visible(RMenu:Get("riftcoinflip", "confirm"), false)
                RageUI.Visible(RMenu:Get("riftcoinflip", "mainmenu"), true)
            end
...
RegisterNetEvent("RIFT:addCoinflipProposal",function(a0, L)
    h[a0] = L
end)

RegisterNetEvent("RIFT:cancelCoinflipBet",function(a0)
    h[a0] = nil
end)

RegisterNetEvent("RIFT:clearCoinflipBets",function()
    h = {}
end)

RegisterNetEvent("RIFT:coinflipOutcome",function(a3, a4)
    l = true
    ...
    SetTimeout(10000,function()
        a6 = true
        if a3 then
            tRIFT.notify("~g~ You won £" .. getMoneyStringFormatted(a4.amount) .. "!")
            SendNUIMessage({transactionType = "playCasinoWin"})
        else
            tRIFT.notify("~r~ You lost £" .. getMoneyStringFormatted(a4.amount) .. "!")
