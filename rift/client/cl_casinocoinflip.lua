...
RegisterNetEvent("Polar:sendCoinflipTableData")
AddEventHandler("Polar:sendCoinflipTableData",function(C)
    f = C
end)

Citizen.CreateThread(function()
    Wait(1000)
    TriggerServerEvent("Polar:requestCoinflipTableData")
    for q = 0, #c.coinflipTables do
        local D = c.coinflipTables[q]
        local DD = tPolar.loadModel(D.tableModel)
...
Citizen.CreateThread(function()
    while true do
        if d then
            ...
            if not l and IsControlJustPressed(0, 202) and not g and not RageUI.Visible(RMenu:Get("Polarcoinflip", "confirm")) then
                local K = tPolar.loadAnimDict("anim_casino_b@amb@casino@games@shared@player@")
                NetworkStopSynchronisedScene(Local_198f_255)
                TaskPlayAnim(tPolar.getPlayerPed(), K, "sit_exit_left", 1.0, 1.0, 2500, 0)
                d = false
                e = true
                showHowToCoinflip(false)
                TriggerServerEvent("Polar:cancelCoinflip", true)
                TriggerServerEvent("Polar:leaveCoinflipTable")
                h = {}
                m = false
        ...
        if not d then
            if a ~= -1 and b < 2 then
                if IsControlJustPressed(0, 38) then
                    if f[a] == false then
                        TriggerServerEvent("Polar:requestSitAtCoinflipTable", a)
                    end
                end
            end
        end
...
RegisterNetEvent("Polar:sitAtCoinflipTable")
AddEventHandler("Polar:sitAtCoinflipTable",function(o, L)
    if L then
        h[L.betId] = L
    end
    goToCoinflipSeat(o)
...
function goToCoinflipSeat(o)
    ...
    TaskGoStraightToCoord(tPolar.getPlayerPed(), R.x, R.y, R.z, 1.0, 5000, S.z, 0.01)
    Wait(500)
    local T = v(o)
    local U = A(o)
    local V = NetworkCreateSynchronisedScene(T.x, T.y, T.z, U.x, U.y, U.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(tPolar.getPlayerPed(),V,"anim_casino_b@amb@casino@games@shared@player@",M(x),2.0,-2.0,13,16,2.0,0)
    ...
    NetworkAddPedToSynchronisedScene(tPolar.getPlayerPed(),Locali98f_55,"anim_casino_b@amb@casino@games@shared@player@","idle_cardgames",2.0,-2.0,13,16,1148846080,0)
...
function showHowToCoinflip(W)
    ...
    RageUI.Visible(RMenu:Get("Polarcoinflip", "instructions"), W)
end
function showCoinflipMenu(W)
    RageUI.Visible(RMenu:Get("Polarcoinflip", "mainmenu"), W)
end
RMenu.Add("Polarcoinflip", "instructions", RageUI.CreateMenu("", "~b~COINFLIP", 0, 100, "banners", "coinflip"))
RMenu.Add("Polarcoinflip", "mainmenu", RageUI.CreateMenu("", "~b~COINFLIP", 0, 100, "banners", "coinflip"))
RMenu.Add("Polarcoinflip", "confirm", RageUI.CreateSubMenu(RMenu:Get("Polarcoinflip", "mainmenu"), "", "~b~COINFLIP", 0, 100, "banners", "coinflip"))


RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('Polarcoinflip', 'instructions')) then
    ...
    if RageUI.Visible(RMenu:Get('Polarcoinflip', 'mainmenu')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.Button("Propose Bet","",{RightLabel = "→→→"},true,function(X, Y, Z)
                if Z then
                    ...
                    if tonumber(_) then
                        _ = tonumber(_)
                        if _ > 0 then
                            TriggerServerEvent("Polar:proposeCoinflip", _)
                        end
                    end
                end
            end)
                ...
                for a0, L in pairs(h) do
                    RageUI.Button("~y~£" .. getMoneyStringFormatted(L.betAmount),"",{RightBadge=RageUI.BadgeStyle.GoldMedal},true,function(X, Y, Z)
                        if Z then
                            ...
                            if L.user_id == tPolar.getUserId() then
                                k = true
                            else
                                k = false
                            end
                    ...
                    RMenu:Get("Polarcoinflip", "confirm"))
                end
            else
    ...
    if RageUI.Visible(RMenu:Get('Polarcoinflip', 'confirm')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if not h[i] then
                RageUI.Visible(RMenu:Get("Polarcoinflip", "confirm"), false)
                RageUI.Visible(RMenu:Get("Polarcoinflip", "mainmenu"), true)
            end
...
RegisterNetEvent("Polar:addCoinflipProposal",function(a0, L)
    h[a0] = L
end)

RegisterNetEvent("Polar:cancelCoinflipBet",function(a0)
    h[a0] = nil
end)

RegisterNetEvent("Polar:clearCoinflipBets",function()
    h = {}
end)

RegisterNetEvent("Polar:coinflipOutcome",function(a3, a4)
    l = true
    ...
    SetTimeout(10000,function()
        a6 = true
        if a3 then
            tPolar.notify("~g~ You won £" .. getMoneyStringFormatted(a4.amount) .. "!")
            SendNUIMessage({transactionType = "playCasinoWin"})
        else
            tPolar.notify("~r~ You lost £" .. getMoneyStringFormatted(a4.amount) .. "!")
