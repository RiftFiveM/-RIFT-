            ...
            if h == prevPos and i == previousHeading and j == prevCameraHeading then
                if d > 0 then
                    if k and d < 1500 and not e then
                        e = true
                        TriggerServerEvent("RIFT:factionAfkAlert", "5 minutes")
                    end
                    if d == 1202 then
                        e = false
                        if not e and k then
                            TriggerServerEvent("RIFT:factionAfkAlert", "10 minutes")
                            e = true
                        end
                    end
                    if b and d == math.ceil(a / 2) then
                        tRIFT.notify(string.format("~r~You will be set to AFK in: %s seconds", d))
                    end
                    ...
                    if not f then
                        if k then
                            TriggerServerEvent("RIFT:factionAfkAlert", "30 minutes - Player set to AFK")
                        end
                ...
                if e and k then
                    TriggerServerEvent("RIFT:factionAfkAlert", "No longer AFK - player moved again")
                    e = false
                end
                if f then
                    TriggerServerEvent("RIFT:setNoLongerAFK")
                end
                c = false
                d = a
                f = false
