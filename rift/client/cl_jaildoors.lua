...
RegisterNetEvent("RIFT:policeSyncAllDoors",function(k, l)
    b = k
    a = l
end)
    ...
    if n then
        ...
        ontick_openPoliceDoor = function(o)
            ...
            if IsControlJustPressed(0, 38) and globalOnPoliceDuty then
                tRIFT.loadAnimDict("anim@heists@keycard@")
                ...
                if a[o.doorHash] == 4 then
                    TriggerServerEvent("RIFT:policeSetDoorState", o.doorHash, 5)
                else
                    TriggerServerEvent("RIFT:policeSetDoorState", o.doorHash, 4)
                end
            end
        end
        for q = 1, #b, 1 do
            tRIFT.createArea("openPoliceDoor_" .. b[q].doorHash,b[q].position,1.5,5,enter_openPoliceDoor,exit_openPoliceDoor,ontick_openPoliceDoor,{doorHash = b[q].doorHash, position = b[q].position})
        end
        for q = 1, #b do
        ...
        local r = function()
            TriggerServerEvent("RIFT:enterPoliceSyncDoors")
        end
        for q = 1, #c do
            tRIFT.createArea("policeSyncDoorsOnAreaEnter",c[q],250,250,r,function()end,function()end,{})
        end
    end
end)
RegisterNetEvent("RIFT:policeSyncDoor",function(u, v)
...
RegisterNetEvent("RIFT:gotAllDoorStates",function(doorStates)
    a = doorStates
