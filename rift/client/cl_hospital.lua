...
Citizen.CreateThread(function()
    if true then 
        local e=function(f)
            tRIFT.drawNativeNotification("Press ~INPUT_PICKUP~ to recieve medical attention.")
        end
        local g=function(f)
        end
        local h=function(f)
            if IsControlJustPressed(1,51)then 
                ...
                if not tRIFT.isInComa()then 
                    if tRIFT.getPlayerVehicle()==0 then 
                        if GetGameTimer()>a+30000 then 
                            SetEntityHealth(i,200)
                            tRIFT.notify("~g~Healed, free of charge by the NHS.")
                            a=GetGameTimer()
                        else 
                            tRIFT.notify("~r~Healing cooldown, come back later.")
                        end 
                    else 
                        tRIFT.notify("~r~You can not heal whilst in a vehicle.")
                    end
                else tRIFT.notify("~r~You are bleeding out, call a doctor ASAP!")
                end 
            end 
        end
        for j,k in pairs(b)do 
            tRIFT.addMarker(k.x,k.y,k.z,1.0,1.0,1.0,0,0,255,100,50,27,false,false,true)
            tRIFT.createArea(j.."_hospital",k,1.5,6,e,g,h,{})
        end 
    end 
end)