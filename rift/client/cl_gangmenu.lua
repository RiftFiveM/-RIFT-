...
local n = m["Red"]
local o = GetResourceKvpString("rift_gang_colour") or "Red"
local function p()
    return RIFTGangMembers
end



RegisterNetEvent("RIFT:GotGangData")
AddEventHandler(
    "RIFT:GotGangData",
    function(q, r, k, s, t, u, v)
        if q == nil then
            ...
            PlayerIsInGang = true
            TriggerServerEvent("RIFT:setPersonalGangBlipColour", o)
            e = {}
            gangLogs = {}
            h = 1
            i = 1
...
RegisterNetEvent("RIFT:disbandedGang")
AddEventHandler(
    "RIFT:disbandedGang",
    function()
        d = "none"
...
RegisterNetEvent("RIFT:ForceRefreshData")
AddEventHandler(
    "RIFT:ForceRefreshData",
    function()
        TriggerServerEvent("RIFT:GetGangData")
    end
)
RegisterNetEvent("RIFT:InviteReceived")
AddEventHandler(
    "RIFT:InviteReceived",
    function(y, z)
        RIFTGangInvites[RIFTGangInviteIndex] = z
        RIFTGangInviteIndex = RIFTGangInviteIndex + 1
        tRIFT.notify(y)
    end
)
RegisterNetEvent("RIFT:gangNameNotTaken")
AddEventHandler(
    "RIFT:gangNameNotTaken",
    function()
        d = "main"
        PlayerIsInGang = true
    end
...
function func_drawGangUI()
    if d == "none" then
        ...
        DrawAdvancedText(0.558, 0.303, 0.005, 0.0028, 0.539, "RIFT Gangs", 255, 255, 255, 255, 7, 0)
        ...
        for A, B in pairs(RIFTGangInvites) do
        ...
        if CursorInArea(0.35, 0.415, 0.415, 0.46) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if createGangName ~= nil and createGangName ~= "null" and createGangName ~= "" then
                    TriggerServerEvent("RIFT:BJVJFVRJFNVKFVMKJKVJFJVNFKVCCCKLVJFKVNF", createGangName) 
                    Citizen.Wait(10000)
                else
        ...
        if CursorInArea(0.435, 0.51, 0.415, 0.46) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if selectedGangInvite ~= nil then
                    selectedGangInvite = RIFTGangInvites[selectedGangInvite]
                    TriggerServerEvent("RIFT:addUserToGang", selectedGangInvite)
                    RIFTGangInvites = {}
                    d = "main"
                    RIFTGangInviteIndex = 0
                    PlayerIsInGang = true
                else
                    tRIFT.notify("~r~No gang invite selected")
                end
            end
        else
            JoinGangSelectionRed = 0
    ...
    if d == "funds" then
        ...
        if CursorInArea(0.3083, 0.3718, 0.5490, 0.5999) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if amount ~= nil then
                    TriggerServerEvent("RIFT:depositGangBalance", gangID, amount)
                else
                    tRIFT.notify("~r~No amount entered!")
                end
            end
        else
        ...
        if CursorInArea(0.4083, 0.4718, 0.5490, 0.5999) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                TriggerServerEvent("RIFT:depositAllGangBalance", gangID)
            end
        else
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if amount ~= nil then
                    if gangPermission >= 3 then
                        TriggerServerEvent("RIFT:withdrawGangBalance", gangID, amount)
                    else
                        tRIFT.notify("~r~You don't have a high enough rank to withdraw")
                    end
                else
                    tRIFT.notify("~r~No amount entered!")
                end
        ...
        if CursorInArea(0.6088, 0.6739, 0.5481, 0.6018) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if gangPermission >= 3 then
                    TriggerServerEvent("RIFT:withdrawAllGangBalance", gangID)
                else
                    tRIFT.notify("~r~You don't have a high enough rank to withdraw")
                end
            end
    ...
    if d == "contributions" then
        ...
        if CursorInArea(0.419271, 0.482813, 0.667593, 0.699074) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if f <= 1 then
                    tRIFT.notify("~r~Lowest page reached")
                else
                    f = f - 1
                end
            end
        ...
        if CursorInArea(0.512500, 0.575521, 0.667593, 0.699074) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if f >= h then
                    tRIFT.notify("~r~Max page reached")
                else
                    f = f + 1
                end
            end
    ...
    if d == "members" then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if selectedMember ~= nil and PlayerIsInGang and gangID ~= nil then
                    if gangPermission >= 4 then
                        TriggerServerEvent("RIFT:PromoteUser", gangID, tonumber(selectedMember))
                    else
                        tRIFT.notify("~r~You don't have permission to promote!")
                    end
                else
                    tRIFT.notify("~r~No gang member selected")
                end
            end
        else
        ...
        if CursorInArea(0.6182, 0.6822, 0.435, 0.491) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if selectedMember ~= nil and PlayerIsInGang and gangID ~= nil then
                    if gangPermission >= 4 then
                        if selectedMember == tRIFT.getUserId() then
                            tRIFT.notify("~r~You can't demote yourself.")
                        else
                            TriggerServerEvent("RIFT:DemoteUser", gangID, selectedMember)
                        end
                    else
                        tRIFT.notify("~r~You don't have permission to demote!")
                    end
                else
                    tRIFT.notify("~r~No gang member selected")
                end
                ...
                if selectedMember ~= nil then
                    if gangPermission >= 3 then
                        if YesNoConfirm() then
                            TriggerServerEvent("RIFT:kickMemberFromGang", gangID, selectedMember)
                        end
                    else
                        tRIFT.notify("~r~You don't have permission to kick!")
                    end
                else
                    tRIFT.notify("~r~No gang member selected")
                end
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if D ~= nil then
                    if gangPermission >= 2 then
                        TriggerServerEvent("RIFT:InviteUserToGang", gangID, D)
                    else
                        tRIFT.notify("~r~You don't have permission to invite players")
                    end
                else
                    tRIFT.notify("No player name entered")
                end
            end
        else
    ...
    if d == "logs" then
        ...
        if CursorInArea(0.33, 0.395, 0.667593, 0.699074) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                TriggerServerEvent("RIFT:SetGangWebhook", gangID)
            end
        else
    ...
    if d == "settings" then
        ...
        if CursorInArea(0.3187, 0.3937, 0.3712, 0.4462) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                k.blips = not k.blips
                SetResourceKvp("rift_gang_blips", tostring(k.blips))
            end
        else
        ...
        if CursorInArea(0.4197, 0.4932, 0.3712, 0.4462) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                k.pings = not k.pings
                SetResourceKvp("rift_gang_pings", tostring(k.pings))
            end
        else
        ...
        if CursorInArea(0.3187, 0.3937, 0.4712, 0.5462) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                k.names = not k.names
                SetResourceKvp("rift_gang_names", tostring(k.names))
                print("Names", k.names)
            end
        else
        ...
        if CursorInArea(0.4197, 0.4932, 0.4712, 0.5462) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if newGangName ~= nil and newGangName ~= "null" and newGangName ~= "" and gangID ~= nil then
                    TriggerServerEvent("RIFT:RenameGang", gangID, newGangName)
                else
                    tRIFT.notify("~r~No gang name entered!")
                end
            end
        ...
        if CursorInArea(0.3187, 0.3937, 0.5712, 0.6462) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if YesNoConfirm() then
                    TriggerServerEvent("RIFT:memberLeaveGang", gangID)
                    setCursor(0)
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if gangPermission >= 4 then
                    if YesNoConfirm() == true and gangID ~= nil then
                        TriggerServerEvent("RIFT:DeleteGang", gangID)
                    end
                else
                    tRIFT.notify("~r~You don't have permission to disband!")
                end
            end
        else
        ...
        if CursorInArea(0.31, 0.39, 0.6712, 0.7064) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                SetResourceKvp("rift_gang_healthui", tostring(k.healthui))
            end
        else
        ...
        if CursorInArea(0.41, 0.49, 0.6712, 0.7064) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if gangPermission >= 4 then
                    TriggerServerEvent("RIFT:setGangFit", gangID)
                else
                    tRIFT.notify("~r~You don't have permission to set gang fit!")
                end
            end
        else
        ...
        if CursorInArea(0.595, 0.645, 0.6155, 0.6405) then
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                SetResourceKvp("rift_gang_colour", o)
    ...
    if d == "security" then
        DrawRect(0.501, 0.525, 0.421, 0.387, 0, 0, 0, 150)
        DrawRect(0.501, 0.308, 0.421, 0.047, k.theme.r, k.theme.g, k.theme.b, 248)
        DrawAdvancedText(0.591, 0.312, 0.005, 0.0028, 0.48, "RIFT gang - security", 255, 255, 255, 255, 7, 0)
        DrawAdvancedText(
            0.4,
            0.375,
            0.005,
        ...
        if CursorInArea(0.31, 0.65, 0.56, 0.61) then
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if gangPermission >= 4 then
                    TriggerServerEvent("RIFT:LockGangFunds", gangID)
                else
                    notify("~r~You must be a leader to change security.")
                end
            end
    ...
    if d == "theme" then
        DrawRect(0.501, 0.525, 0.421, 0.387, 0, 0, 0, 150)
        DrawRect(0.501, 0.308, 0.421, 0.047, k.theme.r, k.theme.g, k.theme.b, 248)
        DrawAdvancedText(0.591, 0.312, 0.005, 0.0028, 0.48, "RIFT Gang - Theme", 255, 255, 255, 255, 7, 0)
        DrawAdvancedText(
            0.7,
            0.360,
            0.005,
        ...
        if CursorInArea(0.5187, 0.5828, 0.4138, 0.4694) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                tRIFT.CopyToClipboard(k.theme.r .. "," .. k.theme.g .. "," .. k.theme.b)
                tRIFT.notify("~g~Theme copied to clipboard.")
            end
        else
        ...
        if CursorInArea(0.5187, 0.5828, 0.5407, 0.5962) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                tRIFT.gangMenuTheme(math.random(1, 255), math.random(1, 255), math.random(1, 255))
            end
        else
                    ...
                    function(O)
                        if O ~= "" then
                            ...
                            if P[1] ~= nil and P[2] ~= nil and P[3] ~= nil then
                                tRIFT.gangMenuTheme(tonumber(P[1]), tonumber(P[2]), tonumber(P[3]))
                            else
    ...
    if d == "main" then
        ...
        DrawAdvancedText(0.591, 0.399, 0.005, 0.0028, 0.51, "RIFT Gangs", 255, 255, 255, 255, 7, 0)
...
tRIFT.createThreadOnTick(b5)
Citizen.CreateThread(
    function()
        k = {
            blips = GetResourceKvpString("rift_gang_blips") == "true",
            pings = GetResourceKvpString("rift_gang_pings") == "true",
            names = GetResourceKvpString("rift_gang_names") == "true",
            healthui = GetResourceKvpString("rift_gang_healthui") == "true"
        }
        k.theme = json.decode(GetResourceKvpString("rift_gang_theme")) or {r = 18, g = 82, b = 228}
        while true do
...
RegisterNetEvent(
    "RIFT:setGangMemberColour",
    function(Q, M)
        for k, n in pairs(RIFTGangMembers) do
            if n[2] == Q then
                RIFTGangMembers[k].colour = M
            end
        end
    end
)
...
function tRIFT.gangMenuTheme(V, W, X)
    if V ~= nil and W ~= nil and X ~= nil then
        k.theme = {r = V, g = W, b = X}
        SetResourceKvp("rift_gang_theme", json.encode(k.theme))
    else
        tRIFT.notify("~r~Invalid value")
    end
end
function tRIFT.hasGangNamesEnabled()
    local P = p()
    return P and k.names
end


function tRIFT.isPlayerInSelectedGang(Y)
    local P = p()
    if P then
        local Q = tRIFT.clientGetUserIdFromSource(Y)
        if Q and tRIFT.getJobType(Q) == "" then
            for a, b in pairs(RIFTGangMembers) do
                if Q == b[2] then
...
function tRIFT.hasGangBlipsEnabled()
...
RegisterCommand(
    "drawmarker",
    function()
       if k.pings and not tRIFT.isEmergencyService() and not globalHideUi and not tRIFT.inEvent() then
            local _ = GetGameplayCamCoord()
            local a0 = tRIFT.rotationToDirection(GetGameplayCamRot(2))
            ...
            if c ~= vector3(0.0, 0.0, 0.0) then
                TriggerServerEvent("RIFT:sendGangMarker", gangID, c)
            end
        end
    end
)
...
local function a2(a3)
    if tRIFT.getGangPingMarkerIndex() == 2 and not tRIFT.isEmergencyService() and not tRIFT.inEvent() then
...
local function a2(_)
    if tRIFT.displayPingsOnMinimap() then
...
RegisterNetEvent("RIFT:drawGangMarker")
AddEventHandler(
    "RIFT:drawGangMarker",
    function(a8, a9)
        if k.pings and not tRIFT.isEmergencyService() and not globalHideUi and not tRIFT.inEvent() then
            ...
            if aa < 1000.0 then
                ...
                if a9 then
                    ...
                    if tRIFT.getGangPingSoundEnabled() then
    ...
    function()
        while true do
            if k ~= nil and k.pings and PlayerIsInGang then
                for a, b in pairs(Z) do
                    if Z[a] ~= nil then
                        if tRIFT.getGangPingMarkerIndex() == 3 then
                            tRIFT.DrawSprite3d(
                                {
                                    pos = b.coords + vector3(0.0, 0.0, b.dist / 100),
                                    textureDict = "rift_gang",
                                    textureName = "ping",
                                    width = 0.06,
                                    height = 0.1,
                                    r = k.theme.r,
                        ...
                        tRIFT.DrawText3D(b.coords, b.creator .. "\n" .. tostring(math.floor(b.dist)) .. "m", 0.2)
                    end
                end
            end
            Citizen.Wait(0)
...
local ac = {}
RegisterNetEvent("RIFT:sendGangHPStats",
    function(ad)
        ac = ad
...
local function an()
    ...
    if k ~= nil and k.healthui and PlayerIsInGang then
       if P and not tRIFT.isEmergencyService() and not globalHideUi and not tRIFT.inEvent() then
            local ap = 0
            local aq = tRIFT.getShowHealthPercentageFlag()
            if RIFTGangMembers ~= nil then
                for ar, as in pairs(RIFTGangMembers) do
                ...
                --   if lastseen == "~g~Online" and tRIFT.getJobType(id) == "" then
                        local at = true
                        local au = nil
                        local av = nil
                        local ad = ac[id]
                        ...
                        local aw = tRIFT.getTempFromPerm(id)
                        if aw then
...
--end
    ...
    if l and l == P then
        if ao <= 0 then
            TriggerServerEvent("RIFT:getGangHealthTable", nil)
            l = nil
        end
    else
        if ao > 0 then
            TriggerServerEvent("RIFT:getGangHealthTable", gangID)
            l = P
        end
    end
end
tRIFT.createThreadOnTick(an)
function tRIFT.setGangUIXPos(az)
    local aA = tonumber(string.match(az, "%d+")) / 100
    ab = 0.008 * aA
end
function tRIFT.setGangUIYPos(az)
    local aA = tonumber(string.match(az, "%d+")) / 100
    ac = 0.35 * aA
end
