...
function tRIFT.isPlayerInDrone()
    return c.active
end
local function d()
    ...
    if c.cameraEnabled then
        ...
        local f = 500 * math.ceil(#(tRIFT.getPlayerCoords() - GetEntityCoords(c.vehicleHandle)) / 10)
        ...
        local g = tRIFT.loadModel("prop_cs_tablet")
        c.tablet = CreateObject(g, tRIFT.getPlayerCoords(), true)
        ...
        tRIFT.loadAnimDict("amb@code_human_in_bus_passenger_idles@female@tablet@idle_a")
...
RegisterNetEvent("toggleDrone",function()
    if not c.active then
        if tRIFT.getPlayerVehicle() == 0 and not tRIFT.isInComa() then
            TriggerServerEvent("RIFT:performDroneLog", "Powered on their DJI Inspire Drone")
            newDrone()
        else
            tRIFT.notify("You are not able to use a drone right now.")
        end
    else
        TriggerServerEvent("RIFT:performDroneLog", "Powered off their DJI Inspire Drone")
        deleteDrone()
    end
end)
Citizen.CreateThread(function()
    ...
    while true do
        if c.active then
            ...
            if not NetworkHasControlOfEntity(c.driverHandle) or not NetworkHasControlOfEntity(c.vehicleHandle) then
                tRIFT.notify("You have ~b~lost control ~w~of the drone, after going out of range")
                deleteDrone()
            end
...
Citizen.CreateThread(function()
    while true do
        local p = tRIFT.getPlayerVehicle()
        if p ~= 0 then
            local q = GetEntityModel(p)
            if q == GetHashKey(a) then
                o = o + 1
                if o > 3 then
                    tRIFT.notify("~r~Access Denied~w~: Engaging defences in 3, 2, 1...")
                    Wait(500)
                    local r = tRIFT.getPlayerCoords()
                    AddOwnedExplosion(tRIFT.getPlayerPed(), r.x, r.y, r.z, 4, 17.0, true, false, 10.0)
                    ...
                    if tRIFT.getPlayerVehicle() ~= nil and DoesEntityExist(tRIFT.getPlayerVehicle()) then
                        DeleteEntity(tRIFT.getPlayerVehicle())
                    end
                end
            end
        end
...
function newDrone()
    local g = tRIFT.loadModel(a)
    local v = tRIFT.loadModel("s_m_m_bouncer_01")
    local e = tRIFT.getPlayerPed()
    local r = GetOffsetFromEntityInWorldCoords(e, 0.0, 4.0, 0.0)
    local w = GetEntityHeading(e)
    c.vehicleHandle = tRIFT.spawnVehicle(a, r, w, true)
    SetModelAsNoLongerNeeded(g)
    SetEntityNoCollisionEntity(tRIFT.getPlayerPed(), c.vehicleHandle, 1)
    Wait(1000)
    ...
    c.active = true
    tRIFT.notify("Drone ~b~created~w~.")
    Wait(1000)
    if globalNHSOnDuty then
        tRIFT.notify("~y~Reminder~w~: These drones are reserved for usage by HEMS, Specialised Off Road Unit")
        tRIFT.notify("and Senior Pilots only. If you are not in these divisions, you ~r~should not ~r~be using this.")
    end
    tRIFT.notify("To remove your drone, use ~b~/drone")
end
function deleteDrone()
    ...
    tRIFT.notify("Drone ~b~removed~w~.")
end
...
local function B()
    ...
    if p == 0 then
        if z then
            if #(z - tRIFT.getPlayerCoords()) > 20.0 then
                local C = PlayerPedId()
    ...
    if #(z - D) > 20.0 then
        ...
        if IsDisabledControlJustPressed(0, 23) or IsDisabledControlJustPressed(0, 75) then
            tRIFT.notify("You must be near the ~y~start position~w~ to exit.")
        end
    end
end
tRIFT.createThreadOnTick(B)
