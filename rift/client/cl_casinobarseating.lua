...
function sitDownAtBar(l)
    ...
    tRIFT.loadAnimDict(a)
    followMachineVector = m
    followMachineRotation = n
    TaskGoStraightToCoord(tRIFT.getPlayerPed(),followMachineVector.x,followMachineVector.y,followMachineVector.z,1.0,50000,312.2,10.0)
    local h = GetEntityCoords(tRIFT.getPlayerPed())
...
function createSitAtBarScene(o, p)
    ...
    NetworkAddPedToSynchronisedScene(tRIFT.getPlayerPed(), q, a, animName, 2.0, -1000, 9, 0, 1148846080, 0)
...
Citizen.CreateThread(function()
    while true do
        if insideDiamondCasino then
            if e then
                SetPedCapsule(tRIFT.getPlayerPed(), 0.13)
                if IsControlJustPressed(0, 202) then
                    tRIFT.loadAnimDict("anim@amb@office@seating@male@var_a@base@")
                    TaskPlayAnim(tRIFT.getPlayerPed(),"anim@amb@office@seating@male@var_a@base@","exit",1.0,1.0,-1,0)
