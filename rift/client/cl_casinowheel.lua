...
RegisterNetEvent("RIFT:frameworkCleanup")
AddEventHandler("RIFT:frameworkCleanup",function()
    DeleteEntity(b)
end)
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('luckywheel', 'casino')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle("Spin the wheel of RIFT (50,000 Chips)","",{RightLabel = "→→→"},true,function(e, f, g)
                if g then
                    TriggerServerEvent("RIFT:requestSpinLuckyWheel")
                end
            end)
        end)
    end
...
Citizen.CreateThread(function()
    ...
    local i = tRIFT.loadModel("vw_prop_vw_luckywheel_02a")
    local j = tRIFT.loadModel(c)
...
function goToWheel()
    if not d then
        ...
        local D = false
        while not D do
            local E = GetEntityCoords(tRIFT.getPlayerPed())
        ...
        SetEntityHeading(tRIFT.getPlayerPed(), 10.9754)
        Wait(2000)
...
AddEventHandler("RIFT:onClientSpawn",function(D, E)
    if E then
        ...
        for M, N in pairs(a) do
            tRIFT.addBlip(N.entryPosition.x, N.entryPosition.y, N.entryPosition.z, 681, 0, "RIFT Wheel", 0.7, true)
            tRIFT.createArea("riftwheel_" .. M, N.entryPosition, 1.5, 6, H, J, K, {})
        end
	end
end)

RegisterNetEvent("RIFT:syncLuckyWheel")
AddEventHandler("RIFT:syncLuckyWheel",function(O)
    print(O)
    Wait(900)
    actuallyRollWheel(O)
end)

RegisterNetEvent("RIFT:spinLuckyWheel")
AddEventHandler("RIFT:spinLuckyWheel",function()
    goToWheel()
end)

RegisterNetEvent("RIFT:spinLuckyWheelReaction")
AddEventHandler("RIFT:spinLuckyWheelReaction",function(P)
    local y = tRIFT.getPlayerPed()
