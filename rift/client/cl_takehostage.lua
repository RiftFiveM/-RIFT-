...
local b = module("rift-weapons", "cfg/weapons")
    ...
    if not a.InProgress and u then
        ...
        if i then
            ...
            if w ~= -1 then
                ...
                if x then
                    tRIFT.setWeapon(PlayerPedId(), foundWeapon, true)
                    a.InProgress = true
                    a.startTime = GetGameTimer()
                    a.targetSrc = w
                    TriggerServerEvent("RIFT:takeHostageSync", w)
                    a.type = "agressor"
                else
...
RegisterNetEvent("RIFT:takeHostageSyncTarget",function(y)
...
RegisterNetEvent("RIFT:takeHostageReleaseHostage",function()
    a.InProgress = false
    a.type = ""
    DetachEntity(PlayerPedId(), true, false)
    q("reaction@shove")
...
RegisterNetEvent("RIFT:takeHostageKillHostage",function()
    a.InProgress = false
    a.type = ""
...
RegisterNetEvent("RIFT:takeHostageCl_stop",function()
    a.InProgress = false
    a.type = ""
    ClearPedSecondaryTask(PlayerPedId())
    DetachEntity(PlayerPedId(), true, false)
    ...
    while true do
        if a.type == "agressor" then
            ...
            if IsEntityDead(PlayerPedId()) then
                ...
                RemoveAnimDict("reaction@shove")
                TriggerServerEvent("RIFT:takeHostageReleaseHostage", a.targetSrc)
            end
            ...
            elseif IsDisabledControlJustPressed(0, 74) and GetGameTimer() - a.startTime > 4000 then
                ...
                RemoveAnimDict("anim@gangops@hostage@")
                TriggerServerEvent("RIFT:takeHostageKillHostage", a.targetSrc)
                TriggerServerEvent("RIFT:takeHostageStop", a.targetSrc)
                Wait(100)
...
function tRIFT.takeHostageInProgress()
    return a.InProgress
end
