...
local l = 10.0
local m = {0, 0, 0, 255}
function tRIFT.setMaxUnderWaterUITimenewTime(n)
    l = n
end
local function o(p, q, r, s, t, u, v, w)
    DrawRect(p + r / 2, q + s / 2, r, s, t, u, v, w)
end
function tRIFT.getCachedMinimapAnchor()
    return k
end
function tRIFT.getCachedResolution()
    ...
    while true do
        if not globalHideUi then
            if not globalHideEmergencyCallUI then
                ...
                local E = tRIFT.getShowHealthPercentageFlag()
                if C > 1.0 then
                    C = 1.0
                end
                ...
                if E then
                    tRIFT.DrawText(A.leftX + A.Width / 2.0,A.bottomY - 0.0075,tostring(math.floor(B * 100)) .. "%",0.175,nil,nil,m)
                end
                ...
                elseif C > 0.0 then
                    ...
                    if E then
                        tRIFT.DrawText(A.leftX + A.Width / 2.0,A.bottomY + 0.006,tostring(math.floor(C * 100)) .. "%",0.175,nil,nil,m)
                    end
                end
            end
        end
