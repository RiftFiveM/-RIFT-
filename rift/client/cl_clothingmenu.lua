RMenu.Add('RIFTclothing','mainMenu',RageUI.CreateMenu("","Clothing Store",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"banners","cstore"))
RMenu.Add('RIFTclothing','changePed',RageUI.CreateSubMenu(RMenu:Get('RIFTclothing','mainMenu'),"","Change Gender"))
RMenu.Add('RIFTclothing','changeClothing',RageUI.CreateSubMenu(RMenu:Get('RIFTclothing','mainMenu'),"","Change your Clothes"))
RMenu.Add('RIFTclothing','clearProps',RageUI.CreateSubMenu(RMenu:Get('RIFTclothing','mainMenu'),"","Clear Props"))
...
Citizen.CreateThread(function()
    while true do 
        local G=GetEntityCoords(tRIFT.getPlayerPed())
        for H=1,#r do 
            if#(G-r[H])<=2.5 and not t then 
                ...
                h=true
                RageUI.Visible(RMenu:Get('RIFTclothing','mainMenu'),true)
            elseif s==H then 
                ...
                if not RageUI.Visible(RMenu:Get('RIFTclothing','changeClothing'))then 
                    F()
                end 
            end 
        end
...
RageUI.CreateWhile(1.0, true, function()
    if not n then
            ...
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                ...
                if PlayerIsInGang then
                    RageUI.Button('Apply Gang Outfit', 'Applys the outfit set by the Gang Leader.', {RightLabel="→→→"}, true, function(L,M,N)
                        if N then
                                TriggerServerEvent("RIFT:applyGangFit", gangID)
                            end
                        end)
                    end
                RageUI.Button('Apply Outfit Code','Applys an outfit code given by a player with ~y~RIFT Plus~w~ or ~y~RIFT Platinum~w~.',{RightLabel=">>>"},true,function(L,M,N)
                    if N then 
                        tRIFT.clientPrompt("Outfit Code:","",function(Q)
                            local R = string.gsub(Q, "%s+", "")
                            if #R == 5 then
                                TriggerServerEvent("RIFT:applyOutfitCode", string.upper(R))
                            else
                ...
                end,RMenu:Get('RIFTclothing','clearProps'))
            end)
        end
        if RageUI.Visible(RMenu:Get('RIFTclothing', 'changePed')) then
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                ...
                RageUI.Button('MP Female','WARNING: Resets all your Body Options',{},true,function(L,M,N)
                    if N then 
                        if GetEntityHealth(PlayerPedId()) > 102 then
                            tRIFT.loadCustomisationPreset("DefaultFemale")
                        else
                            tRIFT.notify("~r~Cannot change gender, you're dead.")
                        end
                    end 
        ...
        if RageUI.Visible(RMenu:Get('RIFTclothing', 'changeClothing')) then
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                m=true
                if h then 
                    for H=1,#a,1 do 
                        ...
                        RageUI.List(P.name,get_drawables(P.type,P.index),a[H].currentListIndex,"Current Texture Index: "..k.textureN.."/"..j-1,{},true,function(L,M,N,Q)
                            ...
                            if N then 
                                ...
                                if k.index==2 then 
                                    ...
                                    TriggerServerEvent("RIFT:saveClothingHairData",GetPedDrawableVariation(tRIFT.getPlayerPed(),2),GetPedHairColor(tRIFT.getPlayerPed()))
...
function current_clothing(R,i,S)
    if R=="prop"then 
        if S then 
            if GetPedPropTextureIndex(tRIFT.getPlayerPed(),i)~=nil then 
                return GetPedPropTextureIndex(tRIFT.getPlayerPed(),i)
            else 
                return 0 
            end 
        else 
            if GetPedPropIndex(tRIFT.getPlayerPed(),i)~=nil then 
                if GetPedPropIndex(tRIFT.getPlayerPed(),i)==-1 then 
                    return GetPedPropIndex(tRIFT.getPlayerPed(),i)+2 
                else 
                    return GetPedPropIndex(tRIFT.getPlayerPed(),i)+1 
                end 
            else 
                return 0 
            end 
    ...
    elseif R=="drawable"then 
        ...
        elseif S then 
            if GetPedTextureVariation(tRIFT.getPlayerPed(),i)~=nil then 
                return GetPedTextureVariation(tRIFT.getPlayerPed(),i)
            else 
                return 0 
            end 
        else 
            if GetPedDrawableVariation(tRIFT.getPlayerPed(),i)~=nil then 
                return GetPedDrawableVariation(tRIFT.getPlayerPed(),i)+1 
            else 
                return 0 
            end 
        end 
...
function get_textures(T,R,i)
    if T=="drawable"then 
        ...
        j=GetNumberOfPedTextureVariations(tRIFT.getPlayerPed(),R,i-1)
        return j 
    else 
        j=GetNumberOfPedPropTextureVariations(tRIFT.getPlayerPed(),R,i-1)
        return j 
    end 
end
local function Y()
...
Citizen.CreateThread(function()
    while true do 
        if k.index~=nil and RageUI.Visible(RMenu:Get('RIFTclothing','changeClothing'))then 
...
AddEventHandler("RIFT:onClientSpawn",function(D, E)
    if E then
		for a7,a8 in pairs(r)do 
            tRIFT.addBlip(a8.x,a8.y,a8.z,73,3,"Clothing Store",0.7)
            tRIFT.addMarker(a8.x,a8.y,a8.z,0.6,0.6,0.6,10,255,81,170,50,9,false,false,true,"dp_clothing","top",90.0,90.0,0.0)
        end 
	end
end)
