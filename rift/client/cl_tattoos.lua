RMenu.Add("RIFTtattoos","mainMenu",RageUI.CreateMenu("","Tattoo Store",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"shopui_title_tattoos2","shopui_title_tattoos2"))
...
AddEventHandler("RIFT:onClientSpawn",function(D, E)
    if E then
        local F = function()
            ...
            c = true
            i = deepcopy(h)
            j = tRIFT.getCustomization()
            RageUI.Visible(RMenu:Get("RIFTtattoos", "mainMenu"), true)
        end
        local G = function()
            ...
            RageUI.Visible(RMenu:Get("RIFTtattoos", "mainMenu"), false)
            tRIFT.setCustomization(j)
            j = nil
            ClearPedDecorations(tRIFT.getPlayerPed())
            for H = 1, #i, 1 do
                AddPedDecorationFromHashes(tRIFT.getPlayerPed(),GetHashKey(i[H].collection),GetHashKey(e[i[H].collection][i[H].current].nameHash))
            end
            c = false
            h = {}
        end
        local I = function()
            ...
            if RageUI.Visible(RMenu:Get("RIFTtattoos", "mainMenu")) then
        ...
        for B, J in pairs(f) do
            ...
            Wait(100)
            tRIFT.createArea("tattoParlour_" .. B, J, 1.5, 6, F, G, I)
            tRIFT.addBlip(J.x, J.y, J.z, 75, 0, "Tattoo Parlour")
            tRIFT.addMarker(J.x,J.y,J.z,0.6,0.6,0.6,10,255,81,170,50,9,false,false,true,"mptattoos1","tattoo_defend_safehouse",90.0,90.0,0.0)
        end
    end
end)
function deepcopy(K)
...
Citizen.CreateThread(function()
    ...
    while true do
        Wait(500)
        if tRIFT.isInComa() and not P then
            P = true
        end
        if not tRIFT.isInComa() and P then
            Wait(5000)
            TriggerServerEvent("RIFT:getPlayerTattoos")
            P = false
        end
...
RegisterNetEvent("RIFT:setTattoos",function(Q)
    Citizen.Wait(500)
    ...
    for H = 1, #R, 1 do
        ...
        AddPedDecorationFromHashes(tRIFT.getPlayerPed(),GetHashKey(R[H].collection),GetHashKey(e[R[H].collection][R[H].current].nameHash))
        table.insert(h, R[H])
    end
end)
                ...
                RageUI.List(T.name,get_num_tattoos(T.value),d[H].currentListIndex,"Tattoo Add/Removal Price: £1,000",{},true,function(U, V, W, X)
                    if V then
                        ...
                        if c and not l and IsControlJustReleased(0, 178) then
                            ...
                            TriggerServerEvent("RIFT:saveTattoos", i, 1000)
                        end
                    end
                    if W then
...
function drawTattoo(_, a0, a1)
    SetEntityHeading(tRIFT.getPlayerPed(), 297.7296)
    local C = tRIFT.getPlayerPed()
    local a2 = false
    if _ ~= nil and a0 ~= nil and a1 ~= nil then
        if p then
            ...
            if GetEntityModel(tRIFT.getPlayerPed()) == -1667301416 then
                tRIFT.loadCustomisationPreset("TattooMale")
            else
                tRIFT.loadCustomisationPreset("TattooFemale")
            end
            ClearPedDecorations(tRIFT.getPlayerPed())
            for H = 1, #i, 1 do
                AddPedDecorationFromHashes(tRIFT.getPlayerPed(),GetHashKey(i[H].collection),GetHashKey(e[i[H].collection][i[H].current].nameHash))
            end
            AddPedDecorationFromHashes(tRIFT.getPlayerPed(), GetHashKey(a0), GetHashKey(e[a0][_].nameHash))
            if not DoesCamExist(b) then
                b = CreateCam("DEFAULT_SCRIPTED_CAMERA", true)
                SetCamCoord(b, GetEntityCoords(tRIFT.getPlayerPed()))
                SetCamRot(b, 0.0, 0.0, 0.0)
                SetCamActive(b, true)
                RenderScriptCams(true, false, 0, true, true)
                SetCamCoord(b, GetEntityCoords(tRIFT.getPlayerPed()))
            end
            local S, a3, a4 = table.unpack(GetEntityCoords(tRIFT.getPlayerPed()))
