...
RMenu.Add('LicenseCentre', 'main', RageUI.CreateMenu("", "~b~" .. "Job Centre", tRIFT.getRageUIMenuWidth(), tRIFT.getRageUIMenuHeight(), "banners", "groups"))
RMenu.Add("LicenseCentre", "confirm", RageUI.CreateSubMenu(RMenu:Get('LicenseCentre', 'main',  tRIFT.getRageUIMenuWidth(), tRIFT.getRageUIMenuHeight())))
    ...
    if q then
        ...
        local u = function(s)
            if IsControlJustPressed(1, 38) then
                TriggerServerEvent('RIFT:getOwnedLicenses')
                Wait(500)
            ...
            tRIFT.addMarker(licensecentre.location.x, licensecentre.location.y,licensecentre.location.z-0.98,0.7,0.7,0.5,255, 255,255,250,50,27,false,false)
        end
        tRIFT.createArea("licensecentre", licensecentre.location, 1.5, 6, r, t, u, {})
        tRIFT.addBlip(licensecentre.location.x, licensecentre.location.y, licensecentre.location.z, 457, 2, "Job Centre", 0.6, true)
    end
end)
...
function func_licenseMenuControl()
    if IsControlJustPressed(0, 167) then
        if a then
            ...
            TriggerServerEvent("RIFT:GetLicenses")
        end
    end
    if a then
        if tRIFT.isNewPlayer() then
        ...
        DrawAdvancedText(0.595, 0.213, 0.005, 0.0028, 1.0, "RIFT Licenses", 255, 255, 255, 255, 1, 0)
...
if not tRIFT.isPurge() then
    tRIFT.createThreadOnTick(func_licenseMenuControl)
end
RegisterNetEvent("RIFT:RecievedLicenses",function(i)
    b = i
    a = true
end)
RegisterNetEvent("RIFT:gotOwnedLicenses",function(i)
    for k,v in pairs(licensecentre.licenses) do
        v.notOwned = true
