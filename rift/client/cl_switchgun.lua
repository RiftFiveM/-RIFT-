...
Citizen.CreateThread(function()
	while true do
		...
		if GetVehiclePedIsTryingToEnter(m) ~= 0 or d then
			tRIFT.setWeapon(m, "WEAPON_UNARMED", true)
		end
		if b then
						...
						if table.has(i, o) then
							...
							if p ~= GetHashKey("WEAPON_UNARMED") and not table.has(i, p) then
								...
								if o == "WEAPON_PDGLOCK" and GetPedDrawableVariation(n, 7) == 187 then
									tRIFT.loadCustomisationPreset("TaserInHolster")
								end
								if not d then
									a = false
									b = false
						...
						if table.has(i, f) then
							...
							if p ~= GetHashKey("WEAPON_UNARMED") and not table.has(i, p) then
								...
								if h and GetPedDrawableVariation(n, 7) == 161 then
									tRIFT.loadCustomisationPreset("TaserOutHolster")
								end
							end
						else
							loadAnimDict("reaction@intimidation@1h")
							...
							RemoveAnimDict("reaction@intimidation@1h")
							b = true
							tRIFT.setWeapon(n, GetHashKey(f), true)
							Citizen.Wait(1300)
							tRIFT.setWeapon(n, "WEAPON_UNARMED", true)
							Citizen.Wait(700)
							ClearPedTasks(n)
							a = true
							b = false
...
function tRIFT.setWeapon(m, s, t)
