...
local function a2(K)
    l = true
    TriggerServerEvent("RIFT:setInVehicleTestingBucket", true)
    z = tRIFT.getPlayerCoords()
    w = 1
    ...
    m = tRIFT.spawnVehicle(K, a4.x, a4.y, a4.z, a5, true, false, false)
    n = GetGameTimer()
end
                ...
                function()
                    ...
                    if U.manuallyRedeemable then
                        RageUI.ButtonWithStyle(
                            ...
                            function(X, Y, Z)
                                if Z then
                                    TriggerServerEvent("RIFT:getStoreLockedVehicleCategories")
                                end
                            end,
                            RMenu:Get("store", "redeem")
                        )
                            ...
                            function(X, Y, Z)
                                if Z then
                                    if isInGreenzone then
                                        TriggerServerEvent("RIFT:startSellStoreItem", d)
                                    else
                ...
                function()
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(X, Y, Z)
                            if Z then
                                if T(U) then
                                    TriggerServerEvent("RIFT:redeemStoreItem", d, e)
                                else
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(X, Y, Z)
                            if Z then
                                ...
                                TriggerServerEvent("RIFT:setInVehicleTestingBucket", false)
...
local function az()
    if l then
        ...
        local vehicle, aA = tRIFT.getPlayerVehicle()
...
function tRIFT.isInStoreTesting()
    return l
end
RegisterNetEvent(
    "RIFT:storeDrawEffects",
    function()
        tRIFT.loadPtfx("scr_xs_celebration")
        tRIFT.loadPtfx("scr_rcpaparazzo1")
        for I = 1, 4 do
...
RegisterNetEvent(
    "RIFT:setStoreLockedVehicleCategories",
    function(aB)
        c = aB
    end
)
