...
local e = false
local f = false
function tRIFT.isKnockedOut()
    return a
end
function tRIFT.putInNearestVehicleAsPassenger(j)
    local k = tRIFT.getClosestVehicle(j)
    if IsEntityAVehicle(k) then
        for l = 1, math.max(GetVehicleMaxNumberOfPassengers(k), 3) do
            if IsVehicleSeatFree(k, l) then
                SetPedIntoVehicle(tRIFT.getPlayerPed(), k, l)
                return true
            end
        end
    end
...
function tRIFT.putInNetVehicleAsPassenger(m)
    local k = tRIFT.getObjectId(m)
    if IsEntityAVehicle(k) then
...
function tRIFT.putInVehiclePositionAsPassenger(n, o, p)
    local k = tRIFT.getVehicleAtPosition(n, o, p)
    if IsEntityAVehicle(k) then
        for l = 1, GetVehicleMaxNumberOfPassengers(k) do
            if IsVehicleSeatFree(k, l) then
                SetPedIntoVehicle(tRIFT.getPlayerPed(), k, l)
...
RegisterNetEvent("RIFT:knockOut",function()
    if not a and not tRIFT.isPurge() then
        tRIFT.setCanAnim(false)
        a = true
        b = r()
    end
end)
RegisterNetEvent("RIFT:knockOutDisable",function()
    if a then
        local s = PlayerPedId()
...
RegisterNetEvent("RIFT:drag")
AddEventHandler("RIFT:drag",function(t)
    d = t
    e = not e
end)
RegisterNetEvent("RIFT:undrag")
AddEventHandler("RIFT:undrag",function(t)
    e = false
end)
RegisterNetEvent("RIFT:clearPoliceStuff",function()
...
local u = {}
RegisterNetEvent("RIFT:addS60",function(v, w, x)
...
RegisterNetEvent("RIFT:removeS60",function(x)
    if u[t] == nil then
        return
    else
        local D = u[x]
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlPressed(0, 323) or IsControlPressed(0, 27) and not IsUsingKeyboard(2) then
            if not globalSurrenderring and not tRIFT.isInComa() and not tRIFT.isHandcuffed() and (tRIFT.canAnim() or tRIFT.isTazedByRevive()) then
                DisablePlayerFiring(tRIFT.getPlayerPed(), true)
                DisableControlAction(0, 22, true)
                DisableControlAction(0, 25, true)
                DisableControlAction(0, 154, true)
                if not IsEntityDead(tRIFT.getPlayerPed()) then
                    if not c and not GetIsTaskActive(tRIFT.getPlayerPed(), 298) then
                        c = true
                        tRIFT.loadAnimDict("missminuteman_1ig_2")    
                        TaskPlayAnim(tRIFT.getPlayerPed(),"missminuteman_1ig_2","handsup_enter",7.0,1.0,-1,50,0,false,false,false)
                        RemoveAnimDict("missminuteman_1ig_2")
                    end
                end
            end
        end
        if (IsControlJustReleased(1, 323) or IsControlJustReleased(1, 27)) and not globalSurrenderring and c and not tRIFT.isInComa() and not tRIFT.isHandcuffed() and tRIFT.canAnim() then
            c = false
        ...
        if a then
            if tRIFT.isStaffedOn() then
                TriggerEvent("RIFT:knockOutDisable")
            elseif not tRIFT.isInComa() then
                local s = PlayerPedId()
                if not IsEntityPlayingAnim(s, b[1], b[2], 3) then
                    tRIFT.loadAnimDict(b[1])
                ...
                tRIFT.notify("~r~You have been knocked out!")
            end
        end
        Wait(0)
    end
...
RMenu.Add("policehandbook","main",RageUI.CreateMenu("Police Handbook", "~b~Officer Handbook", tRIFT.getRageUIMenuWidth(), tRIFT.getRageUIMenuHeight()))
...
RegisterNetEvent("RIFT:breathTestResult",function(M,N)
    local O = N
    ...
    TaskPlayAnim(tRIFT.getPlayerPed(),"weapons@first_person@aim_rng@generic@projectile@shared@core","idlerng_med",1.0,-1,10000,50,0,false,false,false)
...
RegisterNetEvent("RIFT:beingBreathalysed",function()
    RageUI.Text({message = "~w~You are currently being ~b~breathalysed ~w~by a police officer."})
end)
RegisterNetEvent("RIFT:breathalyserCommand",function()
    local E = tRIFT.getPlayerPed()
        ...
        for Q, R in pairs(P) do
            if GetPlayerPed(R) ~= E then
                ...
                if T < 3.0 then
                    ...
                    TriggerServerEvent("RIFT:breathalyserRequest", U)
                    break
                end
            end
        end
...
RegisterNetEvent("RIFT:flashWarrantCard",function()
    local E = PlayerPedId()
    local V = tRIFT.loadModel("prop_fib_badge")
...
RegisterNetEvent("RIFT:startSearchingSuspect",function()
    tRIFT.setCanAnim(false)
    tRIFT.loadAnimDict("custom@police")
    ...
    while GetGameTimer() - a3 < 10000 do
        if IsDisabledControlJustPressed(0, 73) then
            TriggerServerEvent("RIFT:cancelPlayerSearch")
            return
        end
        Citizen.Wait(0)
    end
...
local a4 = false
RegisterNetEvent("RIFT:startBeingSearching",function(a5)
    ...
    a4 = true
    tRIFT.setCanAnim(false)
    tRIFT.loadAnimDict("custom@suspect")
    local s = tRIFT.getPlayerPed()
...
RegisterNetEvent("RIFT:cancelPlayerSearch",function()
    a4 = false
    tRIFT.setCanAnim(true)
    local s = tRIFT.getPlayerPed()
    DetachEntity(s)
...
local ae = ""
local af = false
RegisterNetEvent("RIFT:receivePoliceCallsign",function(ag, ah, ai)
    a8 = ah
    a9 = ag
    aa = ai
    ab = true
...
RegisterNetEvent("RIFT:receiveHmpCallsign",function(ag, ah, ai)
    ac = ah
    ad = ag
    ae = ai
    af = true
...
function tRIFT.getPoliceCallsign()
    return a9
end
function tRIFT.getPoliceRank()
    return a8
end
function tRIFT.getPoliceName()
    return aa
end
function tRIFT.hasPoliceCallsign()
    return ab
end
function tRIFT.getHmpCallsign()
    return ad
end
function tRIFT.getHmpRank()
    return ac
end
function tRIFT.getHmpName()
    return ae
end
function tRIFT.hasHmpCallsign()
    return af
end
...
tRIFT.createThreadOnTick(func_drawCallsign)
local aj = 0
local function ak()
    local s = tRIFT.getPlayerPed()
...
tRIFT.createThreadOnTick(ak)
function tRIFT.hasRecentlyShotGun()
    return aj ~= 0 and GetGameTimer() - aj < 600000
end

AddEventHandler("RIFT:onClientSpawn",function()
    aj = 0
end)

RMenu.Add("trainingWorlds","mainmenu",RageUI.CreateMenu("Training Worlds", "Main Menu", tRIFT.getRageUIMenuWidth(), tRIFT.getRageUIMenuHeight()))
local ap = {}
local aq = false
RegisterNetEvent("RIFT:trainingWorldOpen",function(ar)
    aq = ar
    ...
    if RageUI.Visible(RMenu:Get('trainingWorlds', 'mainmenu')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            for x, at in pairs(ap) do
                ...
                local av = at.bucket == tRIFT.getPlayerBucket()
                ...
                RageUI.ButtonWithStyle(at.name,au,aw,true,function(J, K, L)
                    if K and aq then
                        ...
                        if IsControlJustPressed(0, 214) then
                            TriggerServerEvent("RIFT:trainingWorldRemove", x)
                        end
                    end
                    if L then
                        TriggerServerEvent("RIFT:trainingWorldJoin", x)
                    end
                end)
                if av then
                    as = av
            ...
            if as then
                RageUI.ButtonWithStyle("~r~Leave Training World",nil,{},true,function(J, K, L)
                    if L then
                        TriggerServerEvent("RIFT:trainingWorldLeave")
                    end
                end)
            end
            if aq then
                RageUI.ButtonWithStyle("~b~Create Training World",nil,{},true,function(J, K, L)
                    if L then
                        TriggerServerEvent("RIFT:trainingWorldCreate")
                    end
                end)
            end
        end)
...
RegisterNetEvent("RIFT:trainingWorldSend",function(x, ax)
    ap[x] = ax
end)
RegisterNetEvent("RIFT:trainingWorldSendAll",function(ax)
    ap = ax
end)
RegisterNetEvent("RIFT:trainingWorldRemove",function(x)
    ap[x] = nil
end)
