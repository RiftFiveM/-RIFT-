...
local e = false
local f = false
function tPolar.isKnockedOut()
    return a
end
function tPolar.putInNearestVehicleAsPassenger(j)
    local k = tPolar.getClosestVehicle(j)
    if IsEntityAVehicle(k) then
        for l = 1, math.max(GetVehicleMaxNumberOfPassengers(k), 3) do
            if IsVehicleSeatFree(k, l) then
                SetPedIntoVehicle(tPolar.getPlayerPed(), k, l)
                return true
            end
        end
    end
...
function tPolar.putInNetVehicleAsPassenger(m)
    local k = tPolar.getObjectId(m)
    if IsEntityAVehicle(k) then
...
function tPolar.putInVehiclePositionAsPassenger(n, o, p)
    local k = tPolar.getVehicleAtPosition(n, o, p)
    if IsEntityAVehicle(k) then
        for l = 1, GetVehicleMaxNumberOfPassengers(k) do
            if IsVehicleSeatFree(k, l) then
                SetPedIntoVehicle(tPolar.getPlayerPed(), k, l)
...
RegisterNetEvent("Polar:knockOut",function()
    if not a and not tPolar.isPurge() then
        tPolar.setCanAnim(false)
        a = true
        b = r()
    end
end)
RegisterNetEvent("Polar:knockOutDisable",function()
    if a then
        local s = PlayerPedId()
...
RegisterNetEvent("Polar:drag")
AddEventHandler("Polar:drag",function(t)
    d = t
    e = not e
end)
RegisterNetEvent("Polar:undrag")
AddEventHandler("Polar:undrag",function(t)
    e = false
end)
RegisterNetEvent("Polar:clearPoliceStuff",function()
...
local u = {}
RegisterNetEvent("Polar:addS60",function(v, w, x)
    ...
    TriggerEvent("Polar:showNotification",
    {
...
RegisterNetEvent("Polar:removeS60",function(x)
    if u[t] == nil then
        return
    else
        local D = u[x]
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlPressed(0, 323) or IsControlPressed(0, 27) and not IsUsingKeyboard(2) then
            if not globalSurrenderring and not tPolar.isInComa() and not tPolar.isHandcuffed() and (tPolar.canAnim() or tPolar.isTazedByRevive()) then
                DisablePlayerFiring(tPolar.getPlayerPed(), true)
                DisableControlAction(0, 22, true)
                DisableControlAction(0, 25, true)
                DisableControlAction(0, 154, true)
                if not IsEntityDead(tPolar.getPlayerPed()) then
                    if not c and not GetIsTaskActive(tPolar.getPlayerPed(), 298) then
                        c = true
                        tPolar.loadAnimDict("missminuteman_1ig_2")    
                        TaskPlayAnim(tPolar.getPlayerPed(),"missminuteman_1ig_2","handsup_enter",7.0,1.0,-1,50,0,false,false,false)
                        RemoveAnimDict("missminuteman_1ig_2")
                    end
                end
            end
        end
        if (IsControlJustReleased(1, 323) or IsControlJustReleased(1, 27)) and not globalSurrenderring and c and not tPolar.isInComa() and not tPolar.isHandcuffed() and tPolar.canAnim() then
            c = false
        ...
        if a then
            if tPolar.isStaffedOn() then
                TriggerEvent("Polar:knockOutDisable")
            elseif not tPolar.isInComa() then
                local s = PlayerPedId()
                if not IsEntityPlayingAnim(s, b[1], b[2], 3) then
                    tPolar.loadAnimDict(b[1])
                ...
                tPolar.notify("~r~You have been knocked out!")
            end
        end
        Wait(0)
    end
...
RMenu.Add("policehandbook","main",RageUI.CreateMenu("Police Handbook", "~b~Officer Handbook", tPolar.getRageUIMenuWidth(), tPolar.getRageUIMenuHeight()))
...
RegisterNetEvent("Polar:breathTestResult",function(M,N)
    local O = N
    ...
    TaskPlayAnim(tPolar.getPlayerPed(),"weapons@first_person@aim_rng@generic@projectile@shared@core","idlerng_med",1.0,-1,10000,50,0,false,false,false)
...
RegisterNetEvent("Polar:beingBreathalysed",function()
    RageUI.Text({message = "~w~You are currently being ~b~breathalysed ~w~by a police officer."})
end)
RegisterNetEvent("Polar:breathalyserCommand",function()
    local E = tPolar.getPlayerPed()
        ...
        for Q, R in pairs(P) do
            if GetPlayerPed(R) ~= E then
                ...
                if T < 3.0 then
                    ...
                    TriggerServerEvent("Polar:breathalyserRequest", U)
                    break
                end
            end
        end
...
RegisterNetEvent("Polar:flashWarrantCard",function()
    local E = PlayerPedId()
    local V = tPolar.loadModel("prop_fib_badge")
...
RegisterNetEvent("Polar:startSearchingSuspect",function()
    tPolar.setCanAnim(false)
    tPolar.loadAnimDict("custom@police")
    ...
    while GetGameTimer() - a3 < 10000 do
        if IsDisabledControlJustPressed(0, 73) then
            TriggerServerEvent("Polar:cancelPlayerSearch")
            return
        end
        Citizen.Wait(0)
    end
...
local a4 = false
RegisterNetEvent("Polar:startBeingSearching",function(a5)
    ...
    a4 = true
    tPolar.setCanAnim(false)
    tPolar.loadAnimDict("custom@suspect")
    local s = tPolar.getPlayerPed()
...
RegisterNetEvent("Polar:cancelPlayerSearch",function()
    a4 = false
    tPolar.setCanAnim(true)
    local s = tPolar.getPlayerPed()
    DetachEntity(s)
...
local ae = ""
local af = false
RegisterNetEvent("Polar:receivePoliceCallsign",function(ag, ah, ai)
    a8 = ah
    a9 = ag
    aa = ai
    ab = true
...
RegisterNetEvent("Polar:receiveHmpCallsign",function(ag, ah, ai)
    ac = ah
    ad = ag
    ae = ai
    af = true
...
function tPolar.getPoliceCallsign()
    return a9
end
function tPolar.getPoliceRank()
    return a8
end
function tPolar.getPoliceName()
    return aa
end
function tPolar.hasPoliceCallsign()
    return ab
end
function tPolar.getHmpCallsign()
    return ad
end
function tPolar.getHmpRank()
    return ac
end
function tPolar.getHmpName()
    return ae
end
function tPolar.hasHmpCallsign()
    return af
end
...
tPolar.createThreadOnTick(func_drawCallsign)
local aj = 0
local function ak()
    local s = tPolar.getPlayerPed()
...
tPolar.createThreadOnTick(ak)
function tPolar.hasRecentlyShotGun()
    return aj ~= 0 and GetGameTimer() - aj < 600000
end

AddEventHandler("Polar:onClientSpawn",function()
    aj = 0
end)

RMenu.Add("trainingWorlds","mainmenu",RageUI.CreateMenu("Training Worlds", "Main Menu", tPolar.getRageUIMenuWidth(), tPolar.getRageUIMenuHeight()))
local ap = {}
local aq = false
RegisterNetEvent("Polar:trainingWorldOpen",function(ar)
    aq = ar
    ...
    if RageUI.Visible(RMenu:Get('trainingWorlds', 'mainmenu')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            for x, at in pairs(ap) do
                ...
                local av = at.bucket == tPolar.getPlayerBucket()
                ...
                RageUI.ButtonWithStyle(at.name,au,aw,true,function(J, K, L)
                    if K and aq then
                        ...
                        if IsControlJustPressed(0, 214) then
                            TriggerServerEvent("Polar:trainingWorldRemove", x)
                        end
                    end
                    if L then
                        TriggerServerEvent("Polar:trainingWorldJoin", x)
                    end
                end)
                if av then
                    as = av
            ...
            if as then
                RageUI.ButtonWithStyle("~r~Leave Training World",nil,{},true,function(J, K, L)
                    if L then
                        TriggerServerEvent("Polar:trainingWorldLeave")
                    end
                end)
            end
            if aq then
                RageUI.ButtonWithStyle("~b~Create Training World",nil,{},true,function(J, K, L)
                    if L then
                        TriggerServerEvent("Polar:trainingWorldCreate")
                    end
                end)
            end
        end)
...
RegisterNetEvent("Polar:trainingWorldSend",function(x, ax)
    ap[x] = ax
end)
RegisterNetEvent("Polar:trainingWorldSendAll",function(ax)
    ap = ax
end)
RegisterNetEvent("Polar:trainingWorldRemove",function(x)
    ap[x] = nil
end)
