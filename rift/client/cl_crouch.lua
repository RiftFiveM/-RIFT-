...
local function _()
    ...
    local o = PlayerPedId()
    if not n(o) or IsEntityInWater(o) or not tRIFT.canAnim() then
        return
    end
    i = true
    if Pointing then
    ...
    tRIFT.loadAnimDict("move_crawl")
    tRIFT.loadAnimDict("move_crawlprone2crawlfront")
    if M(o) then
