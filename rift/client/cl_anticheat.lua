...
Citizen.CreateThread(
    function()
        while true do
            ...
            a(
                function(d)
                    if DecorGetInt(d, "RIFTACVeh") ~= 955 then
...
Citizen.CreateThread(
    function()
        ...
        while true do
            ...
            local h = tRIFT.getPlayerId()
            local i = tRIFT.getPlayerVehicle()
            if i == 0 then
                ...
                if not f and GetUsingseethrough() and not tRIFT.isPlayerInPoliceHeli() and not tRIFT.isPlayerInDrone() then
                    TriggerServerEvent("RIFT:acType13")
                    f = true
                end
            end
            SetPedInfiniteAmmoClip(g, false)
            for c, k in pairs(tRIFT.getWeapons()) do
...
AddEventHandler(
    ...
    function(n)
        ...
        TriggerServerEvent("RIFT:acType4")
        l = true
        n(nil)
    end
)
...
Citizen.CreateThread(
    function()
        while true do
            ...
            if r > 1.0 then
                TriggerServerEvent("RIFT:acType8", "PlayerWeaponDamageModifier", r)
                return
            end
            if s > 1.0 then
                TriggerServerEvent("RIFT:acType8", "PlayerWeaponDefenseModifier", s)
                return
            end
            if t > 1.0 then
                TriggerServerEvent("RIFT:acType8", "PlayerWeaponDefenseModifier_2", t)
                return
            end
            if u > 1.0 then
                TriggerServerEvent("RIFT:acType8", "PlayerVehicleDamageModifier", u)
                return
            end
            if v > 1.0 then
                TriggerServerEvent("RIFT:acType8", "PlayerVehicleDefenseModifier", v)
                return
            end
            if y > 1.0 then
                TriggerServerEvent("RIFT:acType8", "GetWeaponDamageModifier", y)
                return
            end
            if z > 1.0 then
                TriggerServerEvent("RIFT:acType8", "GetPlayerMeleeWeaponDamageModifier", z)
                return
            end
...
function tRIFT.isPlayerAboveGround()
    local A = tRIFT.getPlayerCoords()
...
Citizen.CreateThread(
    function()
        ...
        while true do
            ...
            N = O
            if
                P > 0.4 and not IsPedFalling(g) and tRIFT.getStaffLevel() < 2 and not IsPedInParachuteFreeFall(g) and
                    not carryingBackInProgress and
                    not piggyBackInProgress and
                    not tRIFT.takeHostageInProgress() and
                    GetPedParachuteState(g) <= 0 and
                    not IsPedRagdoll(g) and
                    not IsPedRunning(g) and
                    not tRIFT.isPlayerRappeling() and
                    not tRIFT.isPlayerAboveGround() and
                    not tRIFT.isPlayerHidingInBoot() and
                    not tRIFT.isSpectatingEvent() and
                    not noclipActive
             then
                if not IsPedInAnyVehicle(g, 1) then
                    D = D + 1
                    if D > 100 then
                        TriggerServerEvent("RIFT:acType1", false)
                        D = 0
                    end
                end
            end
            local d, Q = tRIFT.getPlayerVehicle()
            if DoesEntityExist(d) and Q and P > 0.2 and tRIFT.getStaffLevel() < 2 then
                if F ~= d then
                    E = 0
                    F = d
                end
                ...
                if J < 5.0 and R < 2.5 then
                    ...
                    if E > 100 and GetGameTimer() - G > 4000 then
                        TriggerServerEvent("RIFT:acType1", true)
                        E = 0
                        G = GetGameTimer()
                    end
                end
            end
            if NetworkIsInSpectatorMode() and not tRIFT.inSpectate() then
                TriggerServerEvent("RIFT:acType21")
                return
            end
            if GetCamFov(GetRenderingCam()) == 50.0 then
                TriggerServerEvent("RIFT:acType24")
                Wait(60000)
            end
            Wait(0)
        end
    ...
    function()
        while true do
            if GetLabelText("notification_buffer") ~= "NULL" then
                TriggerServerEvent("RIFT:acType7")
            end
        ...
        while true do
            ...
            for S, T in ipairs(WeaponBL) do
                ...
                if HasPedGotWeapon(PlayerPedId(), GetHashKey(T), false) == 1 then
                    ...
                    TriggerServerEvent("RIFT:acType2", T)
                    return
                end
            end
            local S, r = StatGetInt(GetHashKey("mp0_shooting_ability"), true)
            if r > 100 then
                TriggerServerEvent("RIFT:acType18")
                return
            end
        end
    end
...
Citizen.CreateThread(
    function()
        ...
        local S = 0
        while true do
            if S >= 100 and not tRIFT.isInComa() then
                TriggerServerEvent("RIFT:acType6")
                return
            end
            if
                not tRIFT.isStaffedOn() and GetEntityHealth(PlayerPedId()) > 102 and not spawning and
                    not tRIFT.isPedScriptGuidChanging()
             then
                local h = PlayerId()
                local g = PlayerPedId()
                ...
                if g ~= 0 then
                    ...
                    tRIFT.setHealth(GetEntityHealth(g) + 2)
                end
            else
                Citizen.Wait(1000)
            end
        ...
        while true do
            ...
            if IsPedShooting(f) and not table.has(V, i) then
                ...
                if R == GetMaxAmmoInClip(f, i) then
                    TriggerServerEvent("RIFT:acType8")
                    Wait(60000)
                end
            end
            Wait(0)
...
local function at()
    ...
    local ax = #(aw - Y)
    if
        ax < 50.0 or av or carryingBackInProgress or piggyBackInProgress or tRIFT.isPlayerHidingInBoot() or
    ...
    if aq(aB, 100.0) then
        if au - a2 > 5000 then
            TriggerServerEvent("RIFT:sendVelocityLimit", aw, Y)
            a2 = au
        end
    end
end
RegisterNetEvent(
    "RIFT:settingPlayerIntoVehicle",
    function()
        a1 = GetGameTimer()
    end
)
...
local function a_()
    ...
    if aW(b0, aC) or aW(b1, aD) or aW(b2, aE) or aW(b3, aF) then
        ...
        if
         ...
         then
            local aQ = aP(d)
            local b4 = tRIFT.getVehicleIdFromHash(GetEntityModel(d)) or "N/A"
            TriggerServerEvent("RIFT:sendVehicleStats", b0, aC, b1, aD, b2, aE, b3, aF, aQ, b4)
            aI = au
        end
    end
    aC = b0
...
Citizen.CreateThread(
    function()
        ...
        while true do
            if
                GetPlayerInvincible(PlayerId()) and not isInGreenzone and not tRIFT.isInsideLsCustoms() and
                    not tRIFT.isStaffedOn() and
                    not noclipActive and
                    not tRIFT.isInComa() and
                    not tRIFT.isInSpectate() and
                    not tRIFT.getInRPZone() and
                    not tRIFT.isInTutorial()
             then
                TriggerServerEvent("RIFT:acType15")
                Citizen.Wait(60000)
            end
            if
                GetPlayerInvincible_2(PlayerId()) and not isInGreenzone and not tRIFT.isInsideLsCustoms() and
                    not tRIFT.isStaffedOn() and
                    not noclipActive and
                    not tRIFT.isInComa() and
                    not tRIFT.isInSpectate() and
            ...
            if
                not IsEntityVisible(PlayerPedId()) and not tRIFT.isInsideLsCustoms() and not noclipActive and
                    not tRIFT.isInSpectate() and
                    not tRIFT.isInTutorial() and
                    not tRIFT.inEvent() and
                    not tRIFT.isPlayerInDrone() and
                    not tRIFT.isSpectatingEvent() and
                    not spawning and
                    tRIFT.getStaffLevel() < 3
                then
                    TriggerServerEvent("RIFT:acType25")
                    Citizen.Wait(60000)
                end
            Citizen.Wait(1000)
        end
...
RegisterNUICallback(
    "NuiDevTool",
    function()
        TriggerServerEvent("RIFT:acType22")
    end
)
AddEventHandler(
    "onClientResourceStop",
    function(b8)
        TriggerServerEvent("RIFT:acType23")
    end
)
AddEventHandler(
    "onResourceStop",
    function(b8)
        TriggerServerEvent("RIFT:acType23", b8)
    end
)
print("[RIFT] - Anti-Cheat initialised")
