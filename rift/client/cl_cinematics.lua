RMenu.Add("mainmenu","cinematic",RageUI.CreateMenu("", "", tRIFT.getRageUIMenuWidth(), tRIFT.getRageUIMenuHeight(), "new_editor", "r_editor_header"))
RMenu:Get("mainmenu", "cinematic"):SetSubtitle("~b~RIFT Cinematic Editor")
RMenu.Add("load_scene","cinematic",RageUI.CreateSubMenu(RMenu:Get("mainmenu", "cinematic"),"","",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"new_editor","r_editor_header"))
RMenu:Get("load_scene", "cinematic"):SetSubtitle("~b~Load Scene")
RMenu.Add("scene_manager","cinematic",RageUI.CreateSubMenu(RMenu:Get("load_scene", "cinematic"),"","",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"new_editor","r_editor_header"))
RMenu:Get("scene_manager", "cinematic"):SetSubtitle("~b~Scene Manager")
RMenu.Add("camera_manager","cinematic",RageUI.CreateSubMenu(RMenu:Get("scene_manager", "cinematic"),"","",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"new_editor","r_editor_header"))
RMenu:Get("camera_manager", "cinematic"):SetSubtitle("~b~Camera Manager")
RMenu.Add("screeneffect_manager","cinematic",RageUI.CreateSubMenu(RMenu:Get("camera_manager", "cinematic"),"","",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"new_editor","r_editor_header"))
RMenu:Get("screeneffect_manager", "cinematic"):SetSubtitle("~b~Screen Effect Manager")
RMenu.Add("timecycle_manager","cinematic",RageUI.CreateSubMenu(RMenu:Get("camera_manager", "cinematic"),"","",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"new_editor","r_editor_header"))
RMenu:Get("timecycle_manager", "cinematic"):SetSubtitle("~b~Timecycle Effect Manager")
RMenu.Add("shake_manager","cinematic",RageUI.CreateSubMenu(RMenu:Get("camera_manager", "cinematic"),"","",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"new_editor","r_editor_header"))
RMenu:Get("shake_manager", "cinematic"):SetSubtitle("~b~Shake Effect Manager")
RMenu.Add("add_camera","cinematic",RageUI.CreateSubMenu(RMenu:Get("camera_manager", "cinematic"),"","",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"new_editor","r_editor_header"))
RMenu:Get("add_camera", "cinematic"):SetSubtitle("~b~Add Camera")
RMenu.Add("camera_focus_manager","cinematic",RageUI.CreateSubMenu(RMenu:Get("camera_manager", "cinematic"),"","",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"new_editor","r_editor_header"))
RMenu:Get("camera_focus_manager", "cinematic"):SetSubtitle("~b~Camera Focus")
RMenu.Add("weather_time_manager","cinematic",RageUI.CreateSubMenu(RMenu:Get("mainmenu", "cinematic"),"","",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"new_editor","r_editor_header"))
RMenu:Get("weather_time_manager", "cinematic"):SetSubtitle("~b~Time/Weather Manager")
RMenu.Add("dof_manager","cinematic",RageUI.CreateSubMenu(RMenu:Get("camera_manager", "cinematic"),"","",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"new_editor","r_editor_header"))
...
Citizen.CreateThread(function()
    m = json.decode(GetResourceKvpString("rift_scenedata") or "{}")
end)
RegisterNetEvent("RIFT:openCinematicMenu",function()
    a(true)
end)
function tRIFT.createCinematicScene(P)
    m[P] = {}
    SetResourceKvp("rift_scenedata", json.encode(m))
    tRIFT.notify("~g~Scene created & saved.")
end
function tRIFT.deleteCinematicScene(P)
    m[P] = nil
    SetResourceKvp("rift_scenedata", json.encode(m))
    tRIFT.notify("~g~Scene deleted & saved.")
end
function tRIFT.createCamera(P, Q)
    if #m[P] == 0 then
        Q.transition = 100
    end
    table.insert(m[P], Q)
    ...
    SetResourceKvp("rift_scenedata", json.encode(m))
    tRIFT.notify("~g~Camera created & saved.")
end
function tRIFT.modifyCamera(P, Q, R)
    m[P][R] = Q
    SetResourceKvp("rift_scenedata", json.encode(m))
    tRIFT.notify("~g~Camera modified & saved.")
end
function tRIFT.deleteCamera(P, S)
    table.remove(m[P], S)
    SetResourceKvp("rift_scenedata", json.encode(m))
    tRIFT.notify("~g~Camera deleted & saved.")
end
function tRIFT.modifyTransition(n, q, T)
    m[n][q].transition = tonumber(T)
    SetResourceKvp("rift_scenedata", json.encode(m))
    tRIFT.notify("~g~Transition change saved.")
end
function tRIFT.modifyCameraBlending(n, q, U)
    m[n][q].blending = l[U]
    SetResourceKvp("rift_scenedata", json.encode(m))
    tRIFT.notify("~g~Camera Blending change saved.")
end
function tRIFT.modifyScreeneffect(n, q, V)
    m[n][q].screeneffect = V
    SetResourceKvp("rift_scenedata", json.encode(m))
    tRIFT.notify("~g~Screeneffect change saved.")
end
function tRIFT.modifyTimecycleEffect(n, q, W)
    m[n][q].timecycleEffect = W
    SetResourceKvp("rift_scenedata", json.encode(m))
    tRIFT.notify("~g~Timecycle Effect change saved.")
end
function tRIFT.setTimecycleIntensity(n, q, f)
    m[n][q].timecycleIntensity = f
    SetResourceKvp("rift_scenedata", json.encode(m))
    tRIFT.notify("~g~Timecycle intensity change saved.")
end
function tRIFT.modifyShakeEffect(n, q, X)
    m[n][q].shake = X
    SetResourceKvp("rift_scenedata", json.encode(m))
    tRIFT.notify("~g~Shake change saved.")
end
function tRIFT.modifyCameraType(n, q, type)
    m[n][q].type = type
    SetResourceKvp("rift_scenedata", json.encode(m))
    tRIFT.notify("~g~Camera type change saved.")
end
function tRIFT.addCameraFocus(n, q, Y, Z)
    m[n][q].focusType = Y
    m[n][q].focusData = Z
    SetResourceKvp("rift_scenedata", json.encode(m))
    tRIFT.notify("~g~Follow player change saved.")
end
function tRIFT.setCameraFov(n, q)
    m[n][q].fov = i[A]
    SetResourceKvp("rift_scenedata", json.encode(m))
    tRIFT.notify("~g~FOV change saved.")
end
...
RageUI.CreateWhile(1.0, true, function()
            ...
            if q and m[n][q] then
                ...
                RageUI.ButtonWithStyle("~b~Transition: " .. m[n][q].transition .. "ms","",{RightLabel = "→→→"},true,function(a1, a2, a3)
                    if a3 then
                        ...
                        if tonumber(T) then
                            tRIFT.modifyTransition(n, q, T)
                        else
                            tRIFT.notify("~r~Failed to change transition time.")
                        end
                    end
                end,RMenu:Get("camera_manager", "cinematic"))
                ...
                RageUI.List("Camera Type:",j,w,"",{},true,function(a1, a2, a3, a8)
                    if a8 ~= w then
                        w = a8
                        tRIFT.modifyCameraType(n, q, j[w])
                    end
                end,function()end,nil)
                RageUI.List("Camera Blending:",k,x,"",{},true,function(a1, a2, a3, a8)
                    if a8 ~= x then
                        x = a8
                        tRIFT.modifyCameraBlending(n, q, k[x])
                    end
                end,function()end,nil)
                ...
                RageUI.List("Timecycle Intensity",f,e,"",{},true,function(a1, a2, a3, a8)
                    ...
                    if a8 ~= e then
                        ...
                        if m[n][q].timecycleEffect ~= "" then
                            ...
                            tRIFT.setTimecycleIntensity(n, q, f[e])
                        end
                    end
                end,function()end,nil)
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            for g = 1, #c do
                RageUI.ButtonWithStyle(c[g],"",{RightLabel = "→→→"},true,function(a1, a2, a3)
                    ...
                    if a3 then
                        tRIFT.modifyScreeneffect(n, q, c[g])
                    end
                end,RMenu:Get("camera_manager", "cinematic"))
            end
        end)
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            for g = 1, #d do
                RageUI.ButtonWithStyle(d[g],"",{RightLabel = "→→→"},true,function(a1, a2, a3)
                    ...
                    if a3 then
                        tRIFT.modifyTimecycleEffect(n, q, d[g])
                    end
                end,RMenu:Get("camera_manager", "cinematic"))
            end
        end)
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            for g = 1, #h do
                RageUI.ButtonWithStyle(h[g],"",{RightLabel = "→→→"},true,function(a1, a2, a3)
                    ...
                    if a3 then
                        tRIFT.modifyShakeEffect(n, q, h[g])
                    end
                end,RMenu:Get("camera_manager", "cinematic"))
            end
        end)
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.ButtonWithStyle("Player","",{RightLabel = "→→→"},true,function(a1, a2, a3)
                if a3 then
                    ...
                    if a9 ~= "" then
                        tRIFT.addCameraFocus(n, q, "player", a9)
                    end
                end
            end,RMenu:Get("camera_manager", "cinematic"))
            RageUI.ButtonWithStyle("Coordinates","",{RightLabel = "→→→"},true,function(a1, a2, a3)
                if a3 then
                    tRIFT.clientPrompt("Enter coordinates:","",function(aa)
                        local ab = {}
                        ...
                        tRIFT.addCameraFocus(n, q, "coord", vector3(ad, ae, af))
                    end)
                end
            end,RMenu:Get("camera_manager", "cinematic"))
        end)
    ...
    if RageUI.Visible(RMenu:Get('weather_time_manager', 'cinematic')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            local function a4()
                B = true
                tRIFT.overrideTime(C[D], E[F], G[H])
            end
            local function a5()
                B = false
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if (o or p) and noclipActive and tRIFT.getPlayerVehicle() ~= 0 then
            tRIFT.notify("~r~You may not noclip when in a vehicle.")
            tRIFT.toggleNoclip()
        end
            ...
            if a0.focusType ~= "" then
                if a0.focusType == "player" then
                    ...
                    if aj then
                        ...
                        print("[RIFT Cinematic] Failed to point cam at player, could not get entity.")
                    end
...
Citizen.CreateThread(function()
    while true do
        local al, am = tRIFT.getPlayerVehicle()
        ...
        if an ~= 0 and IsEntityAPed(an) and IsPedAPlayer(an) and not IsEntityVisible(an) then
            ...
            if ao ~= -1 then
                ...
                if ap > 0 then
                    if not tRIFT.clientGetPlayerIsStaff(tRIFT.clientGetUserIdFromSource(ap)) then
