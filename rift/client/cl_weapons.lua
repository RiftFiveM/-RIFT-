...
local aZ = false
function tRIFT.giveWeapons(d,e)
    local f=PlayerPedId()
    if e then 
...
function tRIFT.getWeapons()
    ...
    for b,c in pairs(a.weapons)do 
        if HasPedGotWeapon(f,c.hash)then
            ...
            --h.attachments=tRIFT.getAllWeaponAttachments(b)
            d[b]=h 
        end 
    end
    return d 
end
function tRIFT.removeAllWeapons()
    RemoveAllPedWeapons(tRIFT.getPlayerPed())
end
...
RegisterCommand("storecurrentweapon",function()
    ...
    if s+3000<GetGameTimer()then 
        ...
        if HasPedGotWeapon(PlayerPedId(),'WEAPON_PISTOL50')or HasPedGotWeapon(PlayerPedId(),'WEAPON_MACHINEPISTOL')then 
            ...
            TriggerServerEvent("RIFT:forceStoreSingleWeapon",g)
        end 
    else 
        tRIFT.notify("~r~Store weapons cooldown, please wait.")
    end 
end)
            ...
            if not table.has(X, k) then
                for k,v in pairs(a.weapons) do
                    if i == GetHashKey(k) then
                        TriggerServerEvent('RIFT:acType17', v.name)
                        Wait(60000)
                    end
                end
            end
