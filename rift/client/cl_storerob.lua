...
RegisterNetEvent("RIFT:updateStoreRobBlips",function(p)
    refreshStoreBlips(p)
end)
AddEventHandler("RIFT:onClientSpawn",function(D, E)
    if E then
        TriggerServerEvent("RIFT:getStoreRobBlips")
        while p == nil do
            Wait(0)
        end
        while true do
    ...
    if not IsPedDeadOrDying(x) then
        if IsEntityPlayingAnim(x, "mp_am_hold_up", "holdup_victim_20s", 3) then
            ...
            if not IsPedDeadOrDying(x) then
                ...
                local A = tRIFT.loadModel("prop_poly_bag_01")
                        ...
                        function()
                            while true do
                                ...
                                if DoesEntityExist(B) then
                                    if #(GetEntityCoords(tRIFT.getPlayerPed()) - GetEntityCoords(B)) <= 1.5 then
                                        PlaySoundFrontend(
                                            -1,
                                            "ROBBERY_MONEY_TOTAL",
                                        ...
                                        TriggerServerEvent("RIFT:pickupCashFromApu", v)
                                        DeleteObject(B)
                                        break
                                    end
                                else
...
Citizen.CreateThread(
    function()
        while true do
            local D = GetEntityCoords(tRIFT.getPlayerPed(), true)
            if r ~= nil then
                for v, w in pairs(r) do
...
function refreshStoreBlips(E)
    ...
    for F, G in pairs(p) do
        ...
        if G.beingrobbed == true then
            tempBlip = tRIFT.addBlip(H.x, H.y, H.z, 52, 1, "Robbable Store [BEING ROBBED]")
        else
            tempBlip = tRIFT.addBlip(H.x, H.y, H.z, 52, 2, "Robbable Store")
        end
...
Citizen.CreateThread(
    function()
        for v, w in pairs(r) do
            local I = tRIFT.loadModel(w.shopNpcModel)
            ...
            SetPedFleeAttributes(J, 0, 0)
            local K = tRIFT.loadModel(w.prop_safe)
            local L = tRIFT.loadModel(w.prop_door)
            local M = tRIFT.loadModel(w.money_prop)
            local N = tRIFT.loadModel(w.money_prop2)
            local O = tRIFT.loadModel(w.money_prop3)
            local P = tRIFT.loadModel(w.money_prop4)
        ...
        while true do
            local W = GetEntityCoords(tRIFT.getPlayerPed())
            for v, w in pairs(r) do
                ...
                if Z ~= 0 then
                    if IsPlayerFreeAimingAtEntity(PlayerId(), Z) then
                        if HasEntityClearLosToEntityInFront(Y, Z) and not IsPedDeadOrDying(Z) and #(GetEntityCoords(Y) - GetEntityCoords(Z)) <= 5.0 then
                            if not q then
                                if GetSelectedPedWeapon(tRIFT.getPlayerPed()) == GetHashKey("WEAPON_UNARMED") then
                                    tRIFT.notify("~r~You need a weapon in your hands to rob this store!")
                                else
                                    TriggerServerEvent("RIFT:initiateStoreRobbery", v)
...
RegisterNetEvent("RIFT:beginStoreRobbingAnimations",function(v)
...
RegisterNetEvent("RIFT:storeRobberyInProgress",function(_, v)
    q = _
    local a0 = GetEntityCoords(tRIFT.getPlayerPed())
    while q do
        local W = GetEntityCoords(tRIFT.getPlayerPed())
        if #(a0 - W) > 20 then
            TriggerServerEvent("RIFT:forceEndRobbery", v)
...
RegisterNetEvent("RIFT:resetStorePed",function(v)
    r[v].robberyInProgress = false
    local Z = r[v].shopNpcHandler
    if DoesEntityExist(Z) then
        DeleteEntity(Z)
    end
    local I = tRIFT.loadModel(r[v].shopNpcModel)
                ...
                if w.distanceToPlayer < 2.0 then
                    if safeCrackingState == "complete" then
                        ...
                        if IsControlJustReleased(1, 51) then
                            ...
                            TriggerServerEvent("RIFT:completeSafeCracking", v)
                            r[v].robberyInProgress = false
                            safeCrackingState = "setup"
                            Wait(5000)
                            TriggerServerEvent("RIFT:syncCloseSafeDoor", v)
                        end
                    elseif w.robberyInProgress then
                        holdup_DisplayHelpText2("Press ~INPUT_CONTEXT~ to start cracking the safe!")
                        if IsControlJustReleased(1, 51) then
                            tRIFT.notify("~g~Started cracking safe..")
                            LoadResources()
...
function RunMiniGame(v, ad, ae)
    ...
    while safeCrackingInProgress do
        ...
        elseif safeCrackingState == "cracking" then
            ...
            local ag = GetEntityHealth(tRIFT.getPlayerPed())
            if ag <= 102 or tRIFT.isHandcuffed() then
                ...
                if IsControlJustPressed(0, 172) then
                    if ah == a[af] then
                        ...
                        if IsSafeUnlocked(b, af) then
                            ...
                            safeCrackingInProgress = false
                            ClearPedTasksImmediately(tRIFT.getPlayerPed())
                        end
                    else
                        af = 1
                        b = InitSafeLocks(a)
...
function GetSafeDoorAnimOffsetPosition(aj, ak, al)
    ...
    return vector3(aj.x + aq, aj.y + ar, GetEntityCoords(tRIFT.getPlayerPed()).z)
end
function PlaySafeCrackIntroAnim(as, h)
    local at = "mini@safe_cracking"
    local au = "step_into"
    loadAnimDict(at)
    TaskPlayAnimAdvanced(tRIFT.getPlayerPed(), at, au, as.x, as.y, as.z, 0.0, 0.0, h, 8.0, 8.0, -1, 2, 0.7, 0, 0)
    Wait(0)
    Wait(1000)
    safeCrackingState = "cracking"
end
function HandleSafeDialMovement()
    local av = tRIFT.getPlayerPed()
    local at = "mini@safe_cracking"
    local aw = ""
    ...
    loadAnimDict(at)
    if not IsEntityPlayingAnim(tRIFT.getPlayerPed(), at, aw, 3) then
        TaskPlayAnimAdvanced(tRIFT.getPlayerPed(), at, aw, g.x, g.y, g.z, 0.0, 0.0, h, 8.0, 8.0, -1, 1, 1.0, 0, 0)
    end
end
function HandleIncorrectMovement(af, b)
    ...
    loadAnimDict(at)
    TaskPlayAnimAdvanced(tRIFT.getPlayerPed(), at, az, g.x, g.y, g.z, 0.0, 0.0, h, 8.0, 8.0, -1, 1, 1.0, 0, 0)
    d = "rotation.idle"
end
...
RegisterNetEvent("RIFT:syncOpenSafeDoor",function(v)
    local aJ = r[v].doorHandler
    OpenSafeDoor(aJ)
end)
RegisterNetEvent("RIFT:syncCloseSafeDoor",function(v)
    local aJ = r[v].doorHandler
    CloseSafeDoor(aJ)
end)
function EndMiniGame(v, ad)
    ...
    local at = "mini@safe_cracking"
    loadAnimDict(at)
    TaskPlayAnimAdvanced(tRIFT.getPlayerPed(),at,"door_open_succeed_stand",g.x,g.y,g.z,0.0,0.0,h,8.0,8.0,-1,2,0.3,0,0)
    FreezeEntityPosition(tRIFT.getPlayerPed(), false)
    Wait(2500)
    TriggerServerEvent("RIFT:syncOpenSafeDoor", v)
    safeCrackingState = "setup"
