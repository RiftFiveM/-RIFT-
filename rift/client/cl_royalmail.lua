...
local b = 15
RegisterNetEvent("RIFT:beginRoyalMailJob",function()
    a.tempVehicle = tRIFT.spawnVehicle("boxville2",a.spawnVehicleVector.x,a.spawnVehicleVector.y,a.spawnVehicleVector.z,343,true,true,true)
    a.onJob = true
    tRIFT.notify("~g~Royal Mail Job started, exit the car park on your left and head to your first drop off point.")
    while a.onJob do
...
RegisterNetEvent("RIFT:royalMailJobEnd",function()
...
RegisterNetEvent("RIFT:royalMailReachedNextStop",function(c, d)
    ...
    if a.tempMarker then
        tRIFT.removeMarker(a.tempMarker)
    end
    ...
    local f = tRIFT.loadModel("prop_drug_package")
...
RegisterNetEvent("RIFT:royalMailJobSetNextBlip",function(g)
    a.tempBlip = AddBlipForCoord(g.x, g.y, g.z)
    SetBlipSprite(a.tempBlip, 1)
    SetBlipRoute(a.tempBlip, true)
    a.tempMarker = tRIFT.addMarker(g.x, g.y, g.z - 1, 2.0, 2.0, 1.0, 200, 20, 0, 50, 50)
end)
AddEventHandler("RIFT:onClientSpawn",function(h, i)
    if i then
        local j = function(k)
        ...
        local m = function(k)
            if IsControlJustReleased(1, 38) and not a.onJob then
                TriggerServerEvent("RIFT:attemptBeginRoyalMailJob")
            end
        end
        tRIFT.addBlip(a.startVector.x, a.startVector.y, a.startVector.z, 67, 1, "Royal Mail Job")
        tRIFT.addMarker(a.startVector.x,a.startVector.y,a.startVector.z,1.0,1.0,1.0,255,0,0,70,50,39,false,false,true)
        tRIFT.createArea("royalmail", a.startVector, 1.5, 6, j, l, m, {})
    end
end)