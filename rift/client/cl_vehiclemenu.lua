...
Citizen.CreateThread(function()
    while true do
        if m and tRIFT.getPlayerVehicle() ~= k.limitingVehicle then
            m = false
            SetVehicleMaxSpeed(tRIFT.getPlayerVehicle(), k.predictedMax)
            predictedMax = nil
            tRIFT.notify("~r~Vehicle Changed, stopping limiter")
        end
        Wait(500)
    end
end)
...
function limiter()
    Citizen.CreateThread(function()
        while k.limiter do
            local z = tRIFT.getPlayerVehicle()
            ...
            if z ~= 0 then
                ...
                tRIFT.notify("~r~You are not in a vehicle!")
                k.limiter = false
            end
            Wait(100)
        end
...
RMenu.Add("vehicle_menu", "main", RageUI.CreateMenu("", "", tRIFT.getRageUIMenuWidth(), tRIFT.getRageUIMenuHeight(), "banners", "vehiclemenu"))
    ...
    if RageUI.Visible(RMenu:Get("vehicle_menu", "main")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.Button("Toggle Door Lock", "Select to toggle between doors locked and unlocked",{RightLabel=""},true,function(o,p,q)
                if q then
                    local L = tRIFT.getPlayerVehicle()
                    if L ~= 0 then
                        if GetVehicleDoorLockStatus(L) ~= 2 then
                            SetVehicleDoorsLocked(L, 2)
                            tRIFT.notify("~w~Doors are now ~r~Locked")
                        else
                            SetVehicleDoorsLocked(L, 0)
                            tRIFT.notify("~w~Doors are now ~g~Unlocked")
                        end
                    end
                end
            end)
                ...
                if K then
                    ...
                    if L ~= 0 then
                        ...
                        if M == 9 then
                            ...
                            tRIFT.notify("~r~Closed ~w~all doors.")
                        elseif M == 8 then
                            if r then
                                r = false
                                RollDownWindow(L, 0)
                                RollDownWindow(L, 1)
                                tRIFT.notify("Rolled windows ~r~Down")
                            else
                                r = true
                                RollUpWindow(L, 0)
                                RollUpWindow(L, 1)
                                tRIFT.notify("Rolled windows ~g~Up")
                            end
                        elseif IsVehicleDoorDamaged(L, N) then
                            tRIFT.notify("~r~Cannot shut this door when the door is damaged")
                        elseif GetVehicleDoorAngleRatio(L, N) == 0 then
                            SetVehicleDoorOpen(L, N, false, false)
                            tRIFT.notify(string.format("The ~b~%s ~w~door is now ~g~Open", h[M]))
                        else
                            SetVehicleDoorShut(L, N, false)
                            tRIFT.notify(string.format("The ~b~%s ~w~door is now ~r~Closed", h[M]))
                        end
                    end
                end
                k.door = M
            ...
            RageUI.Checkbox("Activate Speed Limiter","Select to turn on Speed Limiter",k.limiter,{RightBadge = RageUI.CheckboxStyle.Tick},function(I, K, J, O)
                if K then
                    if k.limiter then
                        ...
                        k.cruise = false
                        if not p then
                            local speed = GetEntitySpeed(tRIFT.getPlayerVehicle())
                            if convert(speed) < 10.0 then
                                p = true
                                limiter()
                            else
                                p = false
                                tRIFT.notify("~r~Alert~w~: Please slow down to enable the speed limiter.")
                            end
                        end
                    else
                ...
                if K then
                    ...
                    if GetIsVehicleEngineRunning(L) then
                        if L ~= 0 then
                            ...
                            tRIFT.notify("You've turned the ignition into the ~r~off ~w~position.")
                        end
                    else
                        if L then
                            SetVehicleEngineOn(L, true, false, true)
                            tRIFT.notify("You've turned the ignition to the ~g~on ~w~position.")
                        end
                    end
                end
            end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustPressed(1, 244) then
            if IsPedInAnyVehicle(tRIFT.getPlayerPed(), false) or RageUI.Visible(RMenu:Get("vehicle_menu", "main")) then
