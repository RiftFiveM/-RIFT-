...
RMenu.Add("policeloadouts","main",RageUI.CreateMenu("","Please Select Division",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"banners","gunstore"))
RMenu.Add("policeloadouts","confirm",RageUI.CreateSubMenu(RMenu:Get("policeloadouts", "main"),"","Confirm Selection",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight()))
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false, x = 0.875, y = 0.20 }, function()
            ...
            RageUI.Button("Confirm", nil, { RightBadge = "→→→" }, true, function(Hovered, Active, Selected)
                if Selected and selectedLoadout ~= nil then
                    TriggerServerEvent("RIFT:selectLoadout", selectedLoadout)
                    RageUI.CloseAll()
                end
            end)
        end)
...
RegisterNetEvent('RIFT:gotLoadouts')
AddEventHandler('RIFT:gotLoadouts', function(loadoutsTable)
    ...
    if q then
        ...
        local u = function(s)
            if IsControlJustPressed(1, 38) then
                TriggerServerEvent('RIFT:getPoliceLoadouts')
        ...
        for k,v in pairs(loadoutCoords) do
            tRIFT.createArea("police_loadouts_"..k, v, 1.5, 6, r, t, u, {locationId = k})
        end
    end
end)