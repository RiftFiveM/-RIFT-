...
local h
local i = false
RMenu.Add("scubadiving","main",RageUI.CreateMenu("", "", tRIFT.getRageUIMenuWidth(), tRIFT.getRageUIMenuHeight(), "banners", "scubajob"))
RMenu:Get("scubadiving", "main"):SetSubtitle("RIFT | ~b~Scuba Diving")
local function j()
...
AddEventHandler("RIFT:onClientSpawn",function(l, m)
    if m then
        local n = tRIFT.createDynamicPed(a.jobPedModel,a.jobPedPosition,a.jobPedHeading,true,"mini@strip_club@idles@bouncer@base","base",100,false,function(o)
        ...
        tRIFT.addBlip(a.jobPosition.x, a.jobPosition.y, a.jobPosition.z, 308, 0, "Scuba Diving")
        tRIFT.addMarker(a.jobPosition.x,a.jobPosition.y,a.jobPosition.z - 0.2,0.5,0.5,0.5,0,50,255,170,50,20,false,false,true)
        local q = function(r)
            j()
        end
        local s = function(r)
        ...
        tRIFT.createArea("scubajob", a.jobPosition, 1.5, 6, q, s, t, {})
    end
end)
        ...
        RageUI.DrawContent({header=true,glare=false,instructionalButton=true},function()
            if not e then
                RageUI.Button("Request Job",nil,{RightLabel = "→→→"},true,function(u, v, w)
                    if w then
                        TriggerServerEvent("RIFT:requestScubaJob")
                    end
                end)
            end
            if d ~= 0 then
                RageUI.Button("Claim Reward",nil,{RightLabel = "→→→"},true,function(u, v, w)
                    if w then
                        TriggerServerEvent("RIFT:claimScubaReward", c)
                        d = 0
                        c = nil
                        e = false
                        i = false
                        ...
                        tRIFT.setCustomization(h)
                        local x = PlayerPedId()
                        SetEnableScuba(x, false)
                        SetPedMaxTimeUnderwater(x, 10.0)
                        tRIFT.setMaxUnderWaterUITimenewTime(10.0)
                    end
                end)
            end
        end)
...
local function y()
    for z, A in pairs(b) do
        tRIFT.removeMarker(A.objectMarker)
        tRIFT.removeArea(A.objectArea)
...
function startScubaDiving()
    tRIFT.loadAnimDict("missheistchem2")
    local x = PlayerPedId()
    ...
    tRIFT.drawNativeNotification("Press ~INPUT_PICKUP~ to start Scuba Diving.")
    while not F do
    ...
    TriggerServerEvent("RIFT:finishScubaJob", c, d)
end
RegisterNetEvent("RIFT:createScubaJob",function(J, K)
    c = K
    e = true
    tRIFT.disableCustomizationSave(e)
    DoScreenFadeOut(500)
    Wait(600)
    ...
    SetBlipAlpha(L, 180)
    local q = function(r)
        tRIFT.drawNativeNotification("Press ~INPUT_DETONATE~ to begin diving!")
    end
    local s = function(r)
    end
    local t = function(r)
        if IsControlJustReleased(1, 47) then
            local x = tRIFT.getPlayerPed()
            local M = tRIFT.getPlayerVehicle()
            if M ~= 0 then
                if GetEntityModel(M) ~= GetHashKey("dinghy") then
                    tRIFT.notify("~r~You must be in a dinghy to start scuba diving!")
                else
                    tRIFT.removeArea("scubajob_" .. J.name)
                    f = "~y~Search for treasures and return to your boat when finished"
                    createRewardObjects(J)
                    CreateThread(startScubaDiving)
                end
            else
                tRIFT.notify("~r~You must be in a boat to start scuba diving!")
            end
        end
    end
    SetWaypointOff()
    local N = tRIFT.addBlip(J.position.x, J.position.y, J.position.z, J.blipId, J.blipColour, "Scuba Diving")
    SetNewWaypoint(J.position.x, J.position.y)
    tRIFT.createArea("scubajob_" .. J.name, J.position, 250, 15, q, s, t, {})
    h = tRIFT.getCustomization()
    local p = math.random(1, 15)
    local O = PlayerPedId()
    ...
    g = tRIFT.spawnVehicle("dinghy",J.dinghySpawnPositions[P].x,J.dinghySpawnPositions[P].y,J.dinghySpawnPositions[P].z,J.dinghySpawnHeading,true,true,true)
    ...
    TriggerServerEvent("RIFT:scubaSetVehicle", c, R)
    DoScreenFadeIn(2000)
    SetPedScubaGearVariation(PlayerPedId())
    Citizen.CreateThread(function()
        while e do
            if i then
                ...
                SetWaypointOff()
                TriggerServerEvent("RIFT:finishScubaJob", c, 0)
                TriggerServerEvent("RIFT:claimScubaReward", c)
                d = 0
                c = nil
                e = false
...
RegisterNetEvent("RIFT:ScubaBoatDestroyed",function(S)
    if e and S == c then
        i = true
    end
end)
function createRewardObjects(J)
    for z, A in pairs(J.rewardObjects) do
        local T = tRIFT.loadModel(A.objectModel)
        ...
        local V =
            tRIFT.addMarker(A.objectPosition.x,A.objectPosition.y,A.objectPosition.z + 0.6,0.5,0.5,0.5,255,223,0,150,50.0,0,false,true,true)
        local q = function(r)
            tRIFT.drawNativeNotification("Press ~INPUT_PICKUP~ to collect!")
        end
        local s = function(r)
        end
        local t = function(r)
            if IsControlJustReleased(1, 38) and e then
                tRIFT.notify("~g~Item collected.")
                d = d + 1
                tRIFT.removeMarker(b[r.objectId].objectMarker)
                tRIFT.removeArea(b[r.objectId].objectArea)
        ...
        tRIFT.createArea("scubajob_" .. J.name .. "_obj_" .. z, A.objectPosition, 3.0, 3.0, q, s, t, {objectId = z})
        b[z] = {}
        b[z].objectId = U
        b[z].objectMarker = V
