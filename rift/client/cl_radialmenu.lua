...
local function u()
    ...
    if v ~= 1 then
        ...
        local z = tPolar.rotationToDirection(x)
...
local function G(H)
    ...
    if noclipActive and tPolar.getStaffLevel() == 0 then
        return
    end
    inventoryType = nil
    D = true
    local I = GetGameTimer()
    tPolar.loadAnimDict("anim@gangops@facility@servers@bodysearch@")
    local t = PlayerPedId()
    ...
    ForcePedAiAndAnimationUpdate(t, false, false)
    if not tPolar.inEvent() and tPolar.globalOnPoliceDuty() then
        ...
        while D do
            if not IsEntityPlayingAnim(PlayerPedId(), "anim@gangops@facility@servers@bodysearch@", "player_search", 3) then
                tPolar.stopCircularProgressBar()
                D = false
                        ...
                        if not g then
                            if GetHashKey("xs_prop_arena_bag_01") == L then
                                ...
                                if B() then
                                    ...
                                    G(
                                        function()
                                            TriggerServerEvent("Polar:LootBag", N)
                                        end
                                    )
                                    Wait(1000)
                                end
                            elseif GetHashKey("prop_poly_bag_money") == L then
                                ...
                                if B() then
                                    ...
                                    local P = ObjToNet(O)
                                    TriggerEvent("Polar:startCombatTimer", false)
                                    TriggerServerEvent("Polar:Moneydrop", P)
                                    Wait(1000)
                                end
                            ...
                            elseif GetHashKey("xs_prop_arena_crate_01a") == L then
                                ...
                                if B() then
                                    ...
                                    G(
                                        function()
                                            TriggerServerEvent("Polar:openCrate", Q)
                                            inventoryType = "Crate"
                                        end
                                    )
                                    Wait(1000)
...
local function aa(ab)
    TriggerEvent("Polar:lockNearestVehicle")
end
local ac
local function ad(ab)
    ac = ab
    SetVehicleDoorOpen(ab, 5, true, false)
    TriggerEvent("Polar:clOpenTrunk")
    trunkStatus = true
    SendNUIMessage({closeRadialMenu = true})
    local ae = GetSoundId()
...
RegisterNetEvent("Polar:clCloseTrunk")
AddEventHandler(
    "Polar:clCloseTrunk",
    function()
        if ac then
...
local function ag(ab)
    TriggerEvent("Polar:verifyLockpick", ab)
end
local function ah(ai)
    local aj = GetPedInVehicleSeat(ai, -1)
    if aj ~= 0 then
        return tPolar.notify("~r~Can not repair vehicle with a person in the driver seat.")
    end
    if NetworkGetEntityIsNetworked(ai) then
        local ak = NetworkGetNetworkIdFromEntity(ai)
        if ak ~= 0 then
            TriggerServerEvent("Polar:attemptRepairVehicle", ai, ak)
        end
    end
end
local al = false
...
local function an(ab)
    if f then
        TriggerEvent("Polar:searchClient", ab)
    end
end
local function ao(ab)
    if f then
        ...
        if ap > 0 then
            exports.Polar:impound(ap, GetEntityModel(ab), VehToNet(ab), ab)
        else
            TriggerEvent("Polar:Notify", "~r~Vehicle is not owned by anyone")
            if GetPedInVehicleSeat(ab, -1) == 0 and GetPedInVehicleSeat(ab, 0) == 0 and NetworkGetEntityIsNetworked(ab) then
                TriggerServerEvent("Polar:impoundDeleteVehicle", NetworkGetNetworkIdFromEntity(ab))
            end
        end
    end
end
local function aq(ab)
    ...
    if as > 0 then
        if GetSelectedPedWeapon(PlayerPedId()) ~= GetHashKey("WEAPON_UNARMED") then
            TriggerServerEvent("Polar:robPlayer", as)
        else
            TriggerEvent("Polar:Notify", "~r~You need a weapon in your hands.")
        end
    end
end
local function at(ab)
    ...
    if aj ~= 0 then
        ...
        if as > 0 then
            TriggerServerEvent("Polar:askId", as)
        end
    end
end
local au = 0
...
local function aw(ab)
    ...
    local as = GetPlayerServerId(ar)
    if as > 0 then
        TriggerServerEvent("Polar:giveCashToPlayer", as)
    end
end
local function ax(ab)
    if not tPolar.canAnim() then
        return
    end
    if GetEntityHealth(ab) <= 102 then
        TriggerEvent("Polar:Notify", "~r~You can not search a player who is dead.")
        return
    end
    local ar = GetPlayerByEntityID(ab)
        ...
        if ay ~= nil then
            if
             ...
             then
                local as = GetPlayerServerId(ar)
                if as > 0 then
                    TriggerServerEvent("Polar:searchPlayer", as)
                end
            else
                TriggerEvent("Polar:Notify", "~r~Player must have their hands up or be on their knees!")
            end
        end
    else
        local as = GetPlayerServerId(ar)
...
local function az(ab)
    ...
    if as > 0 then
        if g then
            TriggerServerEvent("Polar:nhsRevive", as)
        else
            TriggerServerEvent("Polar:attemptCPR", as)
        end
    end
end
local function aA(ab)
...
local function aB(ab)
    if f or h then
        ...
        local as = GetPlayerServerId(ar)
        if as > 0 then
            TriggerServerEvent("Polar:dragPlayer", as)
        end
    end
end
local function aC(ab)
    if f or h then
        ...
        local as = GetPlayerServerId(ar)
        if as > 0 then
            TriggerServerEvent("Polar:putInVehicle", as)
        end
    end
end
local function aD(ab)
    ...
    local as = GetPlayerServerId(ar)
    if as > 0 then
        TriggerServerEvent("Polar:gunshotTest", as)
    end
end
local function aE(ab)
    if f or h then
        ...
        local as = GetPlayerServerId(ar)
        if as > 0 then
            TriggerServerEvent("Polar:seizeWeapons", as)
            TriggerServerEvent("Polar:jailPlayer", as)
        end
    end
end
local function aF(ab)
    if f or h then
        ...
        local as = GetPlayerServerId(ar)
        if as > 0 then
            TriggerServerEvent("Polar:requestTransport", as)
        end
    end
end
local function aG(ab)
...
local function aH(ab)
    if f or h then
        ...
        local as = GetPlayerServerId(ar)
        if as > 0 then
            TriggerServerEvent("Polar:seizeIllegals", as)
        end
    end
end
RegisterNUICallback(
    ...
    function(o)
        ...
        elseif aI == "leaveRadio" then
            TriggerEvent("Polar:clientLeaveRadioChannel")
        elseif aI == "toggleMute" then
            ExecuteCommand("toggleradiomute")
        elseif aI == "radioConfig" then
            TriggerEvent("Polar:openRadioConfig")
        elseif string.match(aI, "radioChannel") then
            local aJ = string.sub(aI, 13, #aI)
            TriggerEvent("Polar:clientJoinRadioChannel", tonumber(aJ))
        end
    end
)
RegisterNetEvent("PolarUI5:globalOnPoliceDuty")
AddEventHandler(
    "PolarUI5:globalOnPoliceDuty",
    function(aK)
        f = aK
    end
)
RegisterNetEvent("PolarUI5:globalOnNHSDuty")
AddEventHandler(
    "PolarUI5:globalOnNHSDuty",
    function(aK)
        g = aK
    end
)
RegisterNetEvent(
    "PolarUI5:globalOnPrisonDuty",
    function(aL)
        h = aL
    end
)
...
function tPolar.getSelectedEntity()
    return b, c
end
AddEventHandler(
    "Polar:showRadioWheel",
    function(aN)
        if k then
            return
        end
...
RegisterCommand(
    "lootclosestbag",
    function()
        if not playerIsAlive() or tPolar.getPlayerVehicle() ~= 0 or GetRenderingCam() ~= -1 then
            return
        end
        local t = tPolar.getPlayerPed()
        local aO = tPolar.getPlayerCoords()
        local aP = 0
        local aQ = 3.0
        ...
        if aP ~= 0 then
            ...
            if not g then
                ...
                elseif aS == GetHashKey("prop_box_ammo03a") then
                    ...
                    G(
                        function()
                            TriggerServerEvent("Polar:openCrate", aT)
                        end
                    )
