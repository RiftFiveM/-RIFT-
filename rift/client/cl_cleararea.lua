...
RegisterNetEvent("Polar:clearVehicles",function()
    for b, c in pairs(GetGamePool("CVehicle")) do
        if NetworkHasControlOfEntity(c) then
            if GetPedInVehicleSeat(c, -1) == 0 and c ~= GetVehiclePedIsIn(tPolar.getPlayerPed(), true) then
...
RegisterNetEvent("Polar:clearBrokenVehicles",function()
    for b, c in pairs(GetGamePool("CVehicle")) do
        if NetworkHasControlOfEntity(c) then
            if GetPedInVehicleSeat(c, -1) == 0 and c ~= GetVehiclePedIsIn(tPolar.getPlayerPed(), true) then
