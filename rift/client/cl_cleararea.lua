...
RegisterNetEvent("RIFT:clearVehicles",function()
    for b, c in pairs(GetGamePool("CVehicle")) do
        if NetworkHasControlOfEntity(c) then
            if GetPedInVehicleSeat(c, -1) == 0 and c ~= GetVehiclePedIsIn(tRIFT.getPlayerPed(), true) then
...
RegisterNetEvent("RIFT:clearBrokenVehicles",function()
    for b, c in pairs(GetGamePool("CVehicle")) do
        if NetworkHasControlOfEntity(c) then
            if GetPedInVehicleSeat(c, -1) == 0 and c ~= GetVehiclePedIsIn(tRIFT.getPlayerPed(), true) then
