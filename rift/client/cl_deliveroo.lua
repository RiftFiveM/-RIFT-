...
RegisterNetEvent("Polar:beginDeliverooJob",function()
    local e = d[math.random(1, #d)]
    b.tempVehicle = tPolar.spawnVehicle(e,b.spawnVehicleVector.x,b.spawnVehicleVector.y,b.spawnVehicleVector.z,343,true,true,true)
    b.onJob = true
    tPolar.disableCustomizationSave(b.onJob)
    b.previousCustomisation = tPolar.getCustomization()
    local f = PlayerPedId()
    SetPedComponentVariation(f, 5, 126, 0, 0)
    SetPedComponentVariation(f, 6, 0, 0, 0)
    SetPedComponentVariation(f, 11, 449, 0, 0)
    tPolar.notify("~g~Deliveroo Job started, exit the restaurant and head to the first drop off.")
    while b.onJob do
...
RegisterNetEvent("Polar:deliverooJobEnd",function(g)
    if g then
        ...
        tPolar.removeMarker(b.tempMarker)
    end
    tPolar.setCustomization(b.previousCustomisation)
    b.onJob = false
    tPolar.disableCustomizationSave(b.onJob)
    b.tempMarker = 0
    b.tempBlip = 0
...
RegisterNetEvent("Polar:deliverooJobReachedNextStop",function(i)
...
RegisterNetEvent("Polar:deliverooJobSetNextBlip",function(k)
    b.tempBlip = AddBlipForCoord(k.x, k.y, k.z)
    SetBlipSprite(b.tempBlip, 1)
    SetBlipRoute(b.tempBlip, true)
    b.tempMarker = tPolar.addMarker(k.x, k.y, k.z - 1, 2.0, 2.0, 1.0, 200, 20, 0, 50, 50)
    local l = a.random_first_names
    b.customerName = l[math.random(1, #l)] .. " " .. l[math.random(1, #l)]
end)
AddEventHandler("Polar:onClientSpawn",function(m, n)
    if n then
        local o = function(p)
        ...
        local r = function(p)
            if IsControlJustReleased(1, 38) and not b.onJob then
                TriggerServerEvent("Polar:attemptBeginDeliverooJob")
            end
        end
        tPolar.addBlip(b.startVector.x, b.startVector.y, b.startVector.z, 106, 1, "Deliveroo Job")
        tPolar.addMarker(b.startVector.x,b.startVector.y,b.startVector.z - 1,1.0,1.0,1.0,255,0,0,70,50,38,false,false,true)
        tPolar.createArea("deliveroo", b.startVector, 1.5, 6, o, q, r, {})
    end
end)
