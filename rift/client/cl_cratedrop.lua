...
local d
RegisterNetEvent("RIFT:crateDrop",function(e, f, g)
    -- e = vector3
    -- f = crate id
    -- g (if it's rig)
    ...
    DecorSetInt(a[f].crate, "RIFTACVeh", 955)
    ...
    DecorSetInt(a[f].parachute, "RIFTACVeh", 955)
    ...
    --AddOwnedExplosion(tRIFT.getPlayerPed(), y.x, y.y, y.z, 1, 0.0, true, false, 3.0)
    FreezeEntityPosition(a[f].crate, true)
    for h, i in pairs(c) do
...
RegisterNetEvent("RIFT:removeLootcrate",function(f)
    if a[f] then
...
RegisterNetEvent("RIFT:addCrateDropRedzone",function(f, e)
    tRIFT.loadModel("xs_prop_arena_crate_01a")
    a[f] = {}
...
RegisterNetEvent("RIFT:removeCrateRedzone",function()
    SetTimeout(300000,function()
        RemoveBlip(radiusBlip)
    end)
end)
