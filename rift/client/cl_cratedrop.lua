...
local d
RegisterNetEvent("Polar:crateDrop",function(e, f, g)
    -- e = vector3
    -- f = crate id
    -- g (if it's rig)
    ...
    DecorSetInt(a[f].crate, "PolarACVeh", 955)
    ...
    DecorSetInt(a[f].parachute, "PolarACVeh", 955)
    ...
    --AddOwnedExplosion(tPolar.getPlayerPed(), y.x, y.y, y.z, 1, 0.0, true, false, 3.0)
    FreezeEntityPosition(a[f].crate, true)
    for h, i in pairs(c) do
...
RegisterNetEvent("Polar:removeLootcrate",function(f)
    if a[f] then
...
RegisterNetEvent("Polar:addCrateDropRedzone",function(f, e)
    tPolar.loadModel("xs_prop_arena_crate_01a")
    a[f] = {}
...
RegisterNetEvent("Polar:removeCrateRedzone",function()
    SetTimeout(300000,function()
        RemoveBlip(radiusBlip)
    end)
end)
