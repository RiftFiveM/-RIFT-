RMenu.Add("policedog","main",RageUI.CreateMenu("Dog Support Unit", "~b~Dog Management", tRIFT.getRageUIMenuWidth(), tRIFT.getRageUIMenuHeight()))
AddEventHandler("RIFT:setPoliceOnDuty",function(a)
    if a then
        RegisterKeyMapping("k9attack", "Police Dog Attack", "keyboard", "INSERT")
    end
end)
AddEventHandler("RIFT:setPrisonGuardOnDuty",function(b)
    if b then
        RegisterKeyMapping("k9attack", "Police Dog Attack", "keyboard", "INSERT")
    end
end)
...
local i = "K9"
local j = "K9TARGET"
RegisterNetEvent("RIFT:policeDogMenu",function()
    ...
    if globalOnPrisonDuty and tRIFT.hasHmpCallsign() then
        RMenu:Get("riftscenemenu", "main"):SetSubtitle("~b~HMP~w~: " .. tRIFT.getHmpCallsign() .. " - " .. tRIFT.getHmpRank() .. " - " .. tRIFT.getHmpName())
    end
    RageUI.Visible(RMenu:Get("policedog", "main"), true)
end)
RegisterNetEvent("RIFT:sendClientRagdoll",function()
    SetPedToRagdoll(tRIFT.getPlayerPed(), 12000, 12000, 0, 0, 0, 0)
    showNotification("~y~~h~Alert~h~~s~: " .. "You have been bitten by a police dog.")
end)
RegisterNetEvent("RIFT:policeDogAttack",function()
...
local m = {breed = 1}
local function n()
    local o = GetResourceKvpString("rift_policedog_customisations")
...
local function r(s)
    ...
    local q = k[m.breed]
    p[q] = s
    SetResourceKvp("rift_policedog_customisations", json.encode(p))
end
...
function tpDog()
    local z = GetEntityCoords(tRIFT.getPlayerPed(), true)
                                    ...
                                    if M < 2.0 then
                                        ...
                                        for O, P in pairs(N) do
                                            if I == GetPlayerPed(P) then
                                                ...
                                                TriggerServerEvent("RIFT:performDogLog","Instructed dog to attack ID: " .. Q)
                                                TriggerServerEvent("RIFT:serverDogAttack", Q)
                                                H = true
                                                Wait(1000)
    ...
    while f.currentAction == d.SearchNearby and DoesEntityExist(f.handle) do
        ...
        for W, X in pairs(GetActivePlayers()) do
            ...
            if GetVehiclePedIsUsing(Y) == 0 and IsEntityVisible(Y) and #(GetEntityCoords(Y, true) - L) < 5.0 then
                ...
                local _ = tRIFT.clientGetUserIdFromSource(Z)
                if tRIFT.getJobType(_) == "" then
        ...
        if #V > 0 then
            ...
            TriggerServerEvent("RIFT:policeDogSniffPlayer", a0.source)
        end
        Citizen.Wait(5000)
    end
end
function putInVehicle()
    local a2 = tRIFT.getNearestVehicle(7.0)
    Citizen.Trace(a2)
...
function removeFromVehicle()
    if IsPedInAnyVehicle(tRIFT.getPlayerPed(), true) then
...
function newDog(q, a4)
    local a5 = tRIFT.getPlayerPed()
    if not IsPedInAnyPoliceVehicle(a5) then
        local a6 = tRIFT.loadModel(q)
        ...
        if DoesEntityExist(f.handle) then
            ...
            TriggerServerEvent("RIFT:performDogLog", "Created a new police dog")
        end
    end
end
function deleteDog()
    ...
    f.active = false
    f.handle = 0
    TriggerServerEvent("RIFT:performDogLog", "Deleted dog")
end
function dogFollow()
    if not f.active then
        return
    ...
    if f.currentAction ~= d.Follow then
        ClearPedTasks(f.handle)
        TaskFollowToOffsetOfEntity(f.handle, tRIFT.getPlayerPed(), 0.0, 0.0, 0.0, 7.0, -1, 10.0, true)
        f.currentAction = d.Follow
    else
        ClearPedTasks(f.handle)
        f.currentAction = d.Follow
...
RegisterNetEvent("RIFT:policeDogIndicate",function(ad)
