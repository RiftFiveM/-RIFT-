RegisterCommand("wheelchair",function()
    if globalNHSOnDuty then
        local a = tRIFT.loadModel("prop_wheelchair_01")
        local b = CreateObject(a, GetEntityCoords(tRIFT.getPlayerPed()), true)
        SetModelAsNoLongerNeeded(a)
    end
end,false)
RegisterCommand("removewheelchair",function()
    if globalNHSOnDuty then
        local b = GetClosestObjectOfType(GetEntityCoords(tRIFT.getPlayerPed()), 10.0, GetHashKey("prop_wheelchair_01"))
...
Citizen.CreateThread(function()
    while true do
        ...
        local f = tRIFT.getPlayerVehicle()
        if f == 0 then
...
function SitOnWheelchair(x)
    ...
    AttachEntityToEntity(tRIFT.getPlayerPed(), x, 0, 0, 0.0, 0.4, 0.0, 0.0, 180.0, 0.0, false, false, false, false, 2)
    local z = GetEntityHeading(x)
    while IsEntityAttachedToEntity(tRIFT.getPlayerPed(), x) do
        Citizen.Wait(5)
        if IsPedDeadOrDying(tRIFT.getPlayerPed()) then
            DetachEntity(tRIFT.getPlayerPed(), true, true)
        end
        if not IsEntityPlayingAnim(tRIFT.getPlayerPed(), "missfinale_c2leadinoutfin_c_int", "_leadin_loop2_lester", 3) then
            TaskPlayAnim(tRIFT.getPlayerPed(),"missfinale_c2leadinoutfin_c_int","_leadin_loop2_lester",8.0,8.0,-1,69,1,false,false,false)
        end
        local A = false
        ...
        if IsControlJustPressed(0, 73) or IsControlJustPressed(0, 177) then
            ...
            SetEntityCoords(tRIFT.getPlayerPed(), B, C, D)
            A = true
        end
        if A and not NetworkHasControlOfEntity(x) and GetGameTimer() - w >= 200 then
            local E = NetworkGetNetworkIdFromEntity(x)
            if E > 0 then
                TriggerServerEvent("RIFT:syncWheelchairPosition", E, GetEntityCoords(x, true), GetEntityHeading(x))
                w = GetGameTimer()
            end
        end
    end
...
function PickUpWheelchair(x)
    ...
    TriggerServerEvent("RIFT:wheelchairAttachPlayer", F)
    while not NetworkHasControlOfEntity(x) or not IsEntityAttachedToEntity(x, tRIFT.getPlayerPed()) do
        Wait(0)
    end
    LoadAnim("anim@heists@box_carry@")
    Wait(0)
    while IsEntityAttachedToEntity(x, tRIFT.getPlayerPed()) do
        Citizen.Wait(5)
        if not IsEntityPlayingAnim(tRIFT.getPlayerPed(), "anim@heists@box_carry@", "idle", 3) then
            TaskPlayAnim(tRIFT.getPlayerPed(), "anim@heists@box_carry@", "idle", 8.0, 8.0, -1, 50, 0, false, false, false)
        end
        if IsPedDeadOrDying(tRIFT.getPlayerPed()) then
...
RegisterNetEvent("RIFT:wheelchairAttachPlayer",function(F, G)
