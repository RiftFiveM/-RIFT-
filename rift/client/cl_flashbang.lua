...
RegisterNetEvent("RIFT:flashbangExplode",function(coords)
    if #(coords - tRIFT.getPlayerCoords()) <= b then
...
function explodeFlashbang(coords)
    ...
    TaskPlayAnim(tRIFT.getPlayerPed(),"anim@heists@ornate_bank@thermal_charge","cover_eyes_intro",-8.0,-8.0,1000,50,8.0,0,0,0)
...
function func_checkForFlashbang()
    if not a then
        if GetSelectedPedWeapon(tRIFT.getPlayerPed()) == GetHashKey("WEAPON_FLASHBANG") then
            a = true
        else
            a = false
        end
    else
        if IsPedShooting(tRIFT.getPlayerPed()) and not c then
            c = true
            Citizen.CreateThread(function()
                ...
                if e ~= 0 then
                    ...
                    AddExplosion(coords, 25, 0.0, 1.0, nil, true, true)
                    TriggerServerEvent("RIFT:flashbangThrown", f)
                end
            end)
...
tRIFT.createThreadOnTick(func_checkForFlashbang)
