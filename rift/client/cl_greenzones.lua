local a=module("rift-vehicles","cfg_vehiclemaxspeeds")
isInGreenzone = false
local cityCap = true
local b = false
local c = false
...
function tRIFT.areGreenzonesDisabled()
    return i
end
function tRIFT.setGreenzonesDisabled(p)
    i = p
end
n(
    function()
        for q, r in pairs(m) do
            if not r[5] and not tRIFT.isPurge() or r[5] and tRIFT.isPurge() then
...
n(
    function()
        local purgeActive = tRIFT.isPurge()
        while true do
            local t = tRIFT.getPlayerPed()
            local u = tRIFT.getPlayerCoords()
            for v, w in pairs(l) do
                if not w.isPurge and not purgeActive or w.isPurge and purgeActive then
                    ...
                    SetPlayerInvincible(tRIFT.getPlayerId(), false)
                    ...
                    SetNetworkVehicleAsGhost(tRIFT.getPlayerVehicle(), false)
                end
            end
            o(250)
        end
...
AddEventHandler("RIFT:onClientSpawn",function(y, z)
    if z then
        local A = function(B)
            inCityZone = true
        end
        ...
        tRIFT.createArea("cityzone",vector3(-225.30703735352, -916.74755859375, 31.216938018799),750.0,100,A,C,D,{})
    end
end)
local function E()
    local F = tRIFT.getPlayerCoords()
    local G = nil
    for H = 1, 25 do
    ...
    if G then
        local K, L = tRIFT.getPlayerVehicle()
        if K ~= 0 then
    ...
    while true do
        ...
        if not tRIFT.areGreenzonesDisabled() then
            ...
            if b or c then
                ...
                ClearPedBloodDamage(t)
                if usingDelgun then
                    tRIFT.setWeapon(t, "WEAPON_STAFFGUN", true)
                else
                    tRIFT.setWeapon(t, "WEAPON_UNARMED", true)
                end
                ResetPedVisibleDamage(t)
                ClearPedLastWeaponDamage(t)
            ...
            if b then
                ...
                local U, V = tRIFT.getPlayerCombatTimer()
                if U > 0 and V then
                    E()
                end
            end
...
RegisterCommand('togglecitycap', function()
    if tRIFT.getUserId() == 1 then
        cityCap = not cityCap
        tRIFT.notify('City Cap: '..(cityCap and "~g~Enabled" or "~r~Disabled"))
    end
end)