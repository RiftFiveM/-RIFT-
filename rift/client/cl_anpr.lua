...
RegisterCommand("flagf",function(o, p)
    if c ~= "" then
        ...
        if table.count(p) ~= 0 then
            q = table.concat(p, " ")
            TriggerServerEvent("RIFT:flagVehicleAnpr", string.upper(c), q)
        else
            tRIFT.notify("~r~Error~w~: You must provide a valid flag reason.")
        end
    end
end)
RegisterCommand("flagr",function(o, p)
    if d ~= "" then
        ...
        if table.count(p) ~= 0 then
            q = table.concat(p, " ")
            TriggerServerEvent("RIFT:flagVehicleAnpr", string.upper(d), q)
        else
            tRIFT.notify("~r~Error~w~: You must provide a valid flag reason.")
        end
    end
...
tRIFT.createThreadOnTick(func_drawFullAnpr)
function anprGetVehicle(u)
    ...
    local w = GetVehiclePedIsIn(tRIFT.getPlayerPed(), false)
    ...
    if B > 0 and IsEntityAVehicle(B) then
        ...
        if C ~= nil then
            if m[C] ~= nil then
                ...
                tRIFT.notify(string.format("~r~ANPR Alert~w~: Plate ~b~%s ~w~is flagged for:~b~~n~%s", C, D))
                n = true
...
function isPedInPoliceVehicle()
    if IsPedInAnyVehicle(tRIFT.getPlayerPed(), false) then
        if GetVehicleClass(tRIFT.getPlayerVehicle()) == 18 and globalOnPoliceDuty then
            return true
        end
    end
    return false
...
RegisterNetEvent("RIFT:setFlagVehicles",function(H)
    m = H
end)