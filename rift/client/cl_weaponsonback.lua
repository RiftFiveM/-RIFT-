local a = module("rift-weapons", "cfg/cfg_weaponsonback")
...
local function g()
    ...
    local i = tRIFT.getWeapons()
    local j = false
    local k = globalOnPoliceDuty and tRIFT.getPlayerVehicle() ~= 0
    for l in pairs(i) do
        local m = GetHashKey(l)
        local n = a.weapons[m]
        if n then
            local o = (not k or n.type ~= "Heavy") and (tRIFT.isEmergencyService() or not isInGreenzone) and not tRIFT.isPlayerInAnimalForm()
