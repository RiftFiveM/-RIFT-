...
RegisterCommand("gangblips", function()
    if not blipon then
        blipon = true
        TriggerServerEvent("Polar:EnableGangBlips")
        notify("Gang blips Enabled")
        Citizen.CreateThread(function()
            while blipon do
                Citizen.Wait(5000)
                TriggerServerEvent("Polar:GetGangBlipsData")
            end
        end)
    else
        DeleteAllBlips()
...
RegisterNetEvent("Polar:ReceiveGangBlipsData")
AddEventHandler("Polar:ReceiveGangBlipsData", function(gangBlipsData)
    DeleteAllBlips()
