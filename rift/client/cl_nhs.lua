RMenu.Add("nhs","main",RageUI.CreateMenu("NHS", "Status: ~b~Available", tRIFT.getRageUIMenuWidth(), tRIFT.getRageUIMenuHeight()))
RMenu.Add("nhs","equipment",RageUI.CreateSubMenu(RMenu:Get("nhs", "main"),"Equipment","Status: ~b~Available",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight()))
RMenu.Add("nhs","observations",RageUI.CreateSubMenu(RMenu:Get("nhs", "main"),"Observations","Status: ~b~Available",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight()))
RMenu.Add("nhs","lifepak",RageUI.CreateSubMenu(RMenu:Get("nhs", "main"),"Observations","Status: ~b~Available",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight()))
RMenu.Add("nhs","objects",RageUI.CreateSubMenu(RMenu:Get("nhs", "main"),"Objects","Status: ~b~Available",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight()))
RegisterKeyMapping("nhs", "NHS Menu", "keyboard", "U")
TriggerEvent("chat:addSuggestion", "/nhs", "Toggle the NHS Menu")
RegisterNetEvent("RIFT:beginRevive",function(a, b, c, d)
    print("nuser_id", b)
...
RegisterNetEvent("RIFT:attachLifepak",function(a, b, c, d)
            ...
            if e.active then
                ...
                if e.readyToRevive then
                    RageUI.Button("Shock Patient",nil,{RightLabel = ""},true,function(g, h, i)
                        if i then
                            TriggerServerEvent("RIFT:finishRevive", e.perm_id)
                            e.readyToRevive = false
                            TriggerServerEvent("RIFT:playNhsSound", "shockAdvisedCharging")
                        end
                    end)
                else
                    RageUI.Button("Detach from patient",nil,{RightLabel = ""},true,function(g, h, i)
                        if i then
                            ...
                            TriggerServerEvent("RIFT:playNhsSound", "threeBeeps")
                        end
                    end)
                end
            else
    ...
    if RageUI.Visible(RMenu:Get('nhs', 'lifepak')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                ...
                if e.readyToRevive then
                    RageUI.Button("Shock Patient",nil,{RightLabel = ""},true,function(g, h, i)
                        if i then
                            TriggerServerEvent("RIFT:finishRevive", e.perm_id)
                            e.readyToRevive = false
                            TriggerServerEvent("RIFT:playNhsSound", "carryOutShock")
                        end
                    end)
                end
            end
            RageUI.Button("Enable Metronome",nil,{RightLabel = "100-120bpm"},true,function(g, h, i)
                if i then
                    TriggerServerEvent("RIFT:playNhsSound", "metronome")
                end
            end)
...
function carryLifePak()
    ...
    f.active = true
    local l = tRIFT.loadModel("prop_lifepak")
...
function changeUpObs()
    Citizen.CreateThread(function()
        ...
        if not e.dead then
            ...
            if n == 3 then
                Wait(3000)
                TriggerServerEvent("RIFT:playNhsSound", "checkPatientAlarm")
            end
...
function beginObs(a, u, v, w, d)
    TriggerServerEvent("RIFT:playNhsSound", "threeBeeps")
    Wait(3000)
    TriggerServerEvent("RIFT:playNhsSound", "analysingHeartRythm")
    e = {}
    ...
    if e.coma then
        ...
        if IsEntityDead(GetPlayerPed(u)) then
            ...
            if e.readyToRevive then
                ...
                5000)
                TriggerServerEvent("RIFT:playNhsSound", "pushToShock")
            end
        end
    end
end
function connectObsToPatient()
    TriggerServerEvent("RIFT:attachLifepakServer")
end
RegisterNetEvent("RIFT:clientPlayNhsSound",function(x, y)
    local z = GetEntityCoords(tRIFT.getPlayerPed())
    local A = #(z - x)
    if A <= 15 then
...
RegisterNetEvent("RIFT:returnRevive",function()
    TriggerServerEvent("RIFT:playNhsSound", "checkPatientAlarm")
    e.bloodPressure1 = math.random(20, 199)
    e.bloodPressure2 = math.floor(e.bloodPressure1 - e.bloodPressure1 / 2.9)
    if e.bloodPressure2 > e.bloodPressure1 then
    ...
    e.coma = false
    Wait(3000)
    TriggerServerEvent("RIFT:playNhsSound", "safeToTouch")
    Wait(12000)
    TriggerServerEvent("RIFT:playNhsSound", "threeBeeps")
end)
    ...
    if globalNHSOnDuty or globalLFBOnDuty then
        ...
        if E ~= GetHashKey("WEAPON_UNARMED") then
            if not table.has(B, E) then
                tRIFT.setWeapon(D, "WEAPON_UNARMED", true)
            end
        end
    end
end
tRIFT.createThreadOnTick(C)

local K = false
RegisterNetEvent("RIFT:attemptCPR",function()
    K = true
    tRIFT.loadAnimDict("missheistfbi3b_ig8_2")
    ...
    while K do
        if not IsEntityPlayingAnim(PlayerPedId(), "missheistfbi3b_ig8_2", "cpr_loop_paramedic", 3) then
            K = false
            TriggerServerEvent("RIFT:cancelCPRAttempt")
            break
        end
        Citizen.Wait(0)
    end
end)
RegisterNetEvent("RIFT:cancelCPRAttempt",function()
    K = false
    if tRIFT.canAnim() then
