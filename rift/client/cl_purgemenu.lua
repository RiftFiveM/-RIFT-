...
local function u()
    if IsControlJustPressed(0, 167) then
        if a then
            ...
            a = true
            TriggerServerEvent("RIFT:getTopFraggers")
        end
    end
    if a then
        DrawRect(0.50, 0.222, 0.223, 0.075, 255, 0, 0, 255)
        DrawAdvancedText(0.595, 0.213, 0.005, 0.0028, 1.0, "RIFT PURGE", 255, 255, 255, 255, 1, 0)
...
RegisterNetEvent("RIFT:gotTopFraggers")
AddEventHandler("RIFT:gotTopFraggers",function(x)
    b = x
end)
RegisterNetEvent("RIFT:incrementPurgeKills")
AddEventHandler("RIFT:incrementPurgeKills",function()
    if b[12] and b[12][2] then
        b[12][2] = b[12][2] + 1
    else
        b[12] = {[1] = "test", 1}
