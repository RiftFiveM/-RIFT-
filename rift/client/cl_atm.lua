...
local b = false
RMenu.Add("riftatm","mainmenu",RageUI.CreateMenu("", "", tRIFT.getRageUIMenuWidth(), tRIFT.getRageUIMenuHeight(), "banners", "atm"))
RMenu:Get("riftatm", "mainmenu"):SetSubtitle("~b~ATM")
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('riftatm', 'mainmenu')) then
                ...
                if Selected then
                    ...
                    if tonumber(e) then
                        if GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
                            if a then
                                tRIFT.playAnim(false, {{"amb@prop_human_atm@male@exit", "exit"}}, false)
                                TriggerServerEvent('RIFT:Deposit', tonumber(e))
                            else
                                tRIFT.notify("~r~Not near ATM.")
                            end
                        else
                            tRIFT.notify("~r~Get out your vehicle to use the ATM")
                        end
                    else
                ...
                if Selected then
                    ...
                    if tonumber(e) then
                        if GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
                            if a then
                                tRIFT.playAnim(false, {{"amb@prop_human_atm@male@exit", "exit"}}, false)
                                TriggerServerEvent('RIFT:Withdraw', tonumber(e))
                            else
                                tRIFT.notify("~r~Not near ATM.")
                            end
                        else
            ...
            RageUI.Button("Deposit All", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected) 
                if Selected then
                    if GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
                        if a then
                            tRIFT.playAnim(false, {{"amb@prop_human_atm@male@exit", "exit"}}, false)
                            TriggerServerEvent('RIFT:DepositAll')
                        else
                            tRIFT.notify("~r~Not near ATM.")
                        end
                    else
            ...
            RageUI.Button("Withdraw All", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected) 
                if Selected then
                    if GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
                        if a then
                            tRIFT.playAnim(false, {{"amb@prop_human_atm@male@exit", "exit"}}, false)
                            TriggerServerEvent('RIFT:WithdrawAll')
                        else
                            tRIFT.notify("~r~Not near ATM.")
                        end
                    else
...
local function g()
    RageUI.CloseAll()
    RageUI.Visible(RMenu:Get("riftatm", "mainmenu"), true)
end
local function h()
    RageUI.CloseAll()
    RageUI.Visible(RMenu:Get("riftatm", "mainmenu"), false)
end
Citizen.CreateThread(function()
    ...
    for m, n in pairs(a.atms) do
        tRIFT.createArea("atm_" .. m, n, 1.5, 6, i, k, l, {atmId = m})
        local o = tRIFT.addBlip(n.x, n.y, n.z, 108, 4, "ATM", 0.8, true)
        tRIFT.addMarker(n.x, n.y, n.z, 0.7, 0.7, 0.5, 0, 255, 125, 125, 50, 29, false, false, true)
...
function tRIFT.createAtm(t, u)
    ...
    tRIFT.createArea(v,u,1.5,6,i,k,function()end)
    local w = tRIFT.addMarker(u.x, u.y, u.z, 0.7, 0.7, 0.5, 0, 255, 125, 125, 50, 29, false, false, true)
    s[t] = {area = v, marker = w}
end
function tRIFT.deleteAtm(t)
    local x = s[t]
    if x then
        tRIFT.removeMarker(x.marker)
        tRIFT.removeArea(x.area)
        s[t] = nil
    end
end
local y = false
local function z(A)
    ...
    for p = 1, math.random(3, 4) do
        ...
        local E = true
        tRIFT.minigameCircularProgressBar({Difficulty = D, Timeout = 25000, onComplete = function(F)
            B = F
            E = false
        ...
        if not B then
            ...
            tRIFT.startCircularProgressBar("",2000,nil,function()end)
            TriggerServerEvent("RIFT:atmWireCutSparks", A, false)
            Citizen.Wait(2000)
        end
    end
    C = true
...
RegisterNetEvent("RIFT:atmWireCuttingSuccessSync",function(A)
    ...
    tRIFT.loadPtfx("scr_xs_celebration")
...
RegisterNetEvent("RIFT:atmWireCuttingSuccess",function()
...
RegisterNetEvent("RIFT:atmWireCutSparks",function(A)
    local L = a.robberyAtms[A]
    local M = G(L)
    tRIFT.loadPtfx("core")
...
RegisterNetEvent("RIFT:startAtmWireCutting",function(A)
    y = true
    local V = PlayerPedId()
    local L = a.robberyAtms[A]
    tRIFT.setCanAnim(false)
    tRIFT.setPlayerCombatTimer(30, false)
    tRIFT.setWeapon(V, "WEAPON_UNARMED", true)
    ...
    tRIFT.loadClipSet("move_ped_crouched")
...
RegisterNetEvent("RIFT:atmInkArea",function(A)
    local L = a.robberyAtms[A]
    tRIFT.loadPtfx("veh_xs_vehicle_mods")
    for p = 1, 10 do
...
RegisterNetEvent("RIFT:atmGenericAlarm",function(A)
    local L = a.robberyAtms[A]
...
local function a1(j)
    Y = math.random(6, 12)
    a0 = 0
    TriggerServerEvent("RIFT:getAtmHasBeenRobbed", j.robberyId)
end
local function a2(j)
    if y then
        TriggerServerEvent("RIFT:atmStopWireCutting", j.robberyId)
    end
    Z = 0
end
local function a3()
    ...
    Citizen.Wait(500)
    local a4 = tRIFT.getPlayerCoords()
...
local function a7()
    ...
    local a4 = tRIFT.getPlayerCoords() + GetEntityForwardVector(PlayerPedId()) * 1.0
    ...
    if select(2, GetCurrentPedWeapon(V)) == "WEAPON_CROWBAR" then
        ...
        if IsDisabledControlJustPressed(0, 24) and not _ then
            Citizen.CreateThreadNow(function()
                ...
                tRIFT.loadAnimDict("melee@small_wpn@streamed_core")
                V = PlayerPedId()
                ...
                Z = Z + 1
                if Z >= Y then
                    TriggerServerEvent("RIFT:startAtmWireCutting", j.robberyId)
                end
...
Citizen.CreateThread(function()
    for A, L in pairs(a.robberyAtms) do
        tRIFT.createArea("atmrobbery_" .. A, L.xyz, 1.5, 6, a1, a2, a9, {robberyId = A})
    end
end)
RegisterNetEvent("RIFT:setAtmHasBeenRobbed",function(ac)
    a0 = ac
end)
