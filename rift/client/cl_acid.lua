...
local m = -20.0
local n = 0.01
RegisterNetEvent("RIFT:doAcid",function()
    local p = "mp_player_inteat@burger"
    local q = "mp_player_int_eat_burger"
    local r = tRIFT.loadModel("a_c_hen")
    RequestAnimDict(p)
    while not HasAnimDictLoaded(p) do
        Wait(0)
    end
    TaskPlayAnim(tRIFT.getPlayerPed(), p, q, 2.0, 2.0, -1, 51, 0, false, false, false)
    RemoveAnimDict(p)
...
DoAcid = function(x)
    ...
    while GetGameTimer() - A < x do
        local D = GetEntityCoords(tRIFT.getPlayerPed())
        ...
        if E > 1.0 then
            ...
            if z == 5 then
                z = 0
                local F = C - D
                local G = GetEntityVelocity(tRIFT.getPlayerPed())
                SetEntityCoordsNoOffset(tRIFT.getPlayerPed(), D.x + F.x, D.y + F.y, D.z)
                ForcePedMotionState(tRIFT.getPlayerPed(), -1115154469, 1, 1, 0)
                SetEntityVelocity(tRIFT.getPlayerPed(), G.x, G.y, G.z)
            end
            C = GetEntityCoords(tRIFT.getPlayerPed())
        end
        DrawToons()
        DrawCubes()
        if e and not f then
    ...
    SetPedMotionBlur(tRIFT.getPlayerPed(), false)
...
InitPed = function()
    ...
    while GetScreenCoordFromWorldCoord(L.x, L.y, L.z) and not IsPointOnRoad(L.x, L.y, L.z) do
        J = math.random(0, 359)
        K = math.random(50, 80)
        L = GetEntityCoords(tRIFT.getPlayerPed()) + PointOnSphere(0.0, J)
        Citizen.Wait(0)
    end
...
TrackEnt = function()
    while true do
        local E = GetVecDist(GetEntityCoords(tRIFT.getPlayerPed()), GetEntityCoords(g))
        if E > 5.0 then
            TaskGoToEntity(g, tRIFT.getPlayerPed(), -1, 4.0, 100.0, 1073741824, 0)
            Wait(1000)
        else
            ...
            if not d or GetGameTimer() - d > 1000 then
                d = GetGameTimer()
                TaskTurnPedToFaceEntity(g, tRIFT.getPlayerPed(), -1)
            end
...
InitCubes = function()
    ...
    SetPedMotionBlur(tRIFT.getPlayerPed(), true)
    local R = 4000
    local S = 0
    while S < R do
        S = S + 1
...
DrawCubes = function()
    local X = GetEntityCoords(tRIFT.getPlayerPed())
    local T = 10
    for U, V in pairs(b) do
        local Y = 0.1
        local Z = 0.1
...
local ae = false
local af = false
RegisterNetEvent("RIFT:cocaineEffect",function()
    if ae then
        af = true
        local t = 0
        while t < 10 do
            tRIFT.varyHealth(-10)
            tRIFT.notify("~r~I don't feel too good..")
            Wait(5000)
            t = t + 1
        end
...
RegisterNetEvent("RIFT:heroinEffect",function()
    ...
    tRIFT.enablePunching(true)
    Wait(30000)
    ClearTimecycleModifier()
    tRIFT.enablePunching(false)
