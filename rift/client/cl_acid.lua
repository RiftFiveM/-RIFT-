...
local m = -20.0
local n = 0.01
RegisterNetEvent("Polar:doAcid",function()
    local p = "mp_player_inteat@burger"
    local q = "mp_player_int_eat_burger"
    local r = tPolar.loadModel("a_c_hen")
    RequestAnimDict(p)
    while not HasAnimDictLoaded(p) do
        Wait(0)
    end
    TaskPlayAnim(tPolar.getPlayerPed(), p, q, 2.0, 2.0, -1, 51, 0, false, false, false)
    RemoveAnimDict(p)
...
DoAcid = function(x)
    ...
    while GetGameTimer() - A < x do
        local D = GetEntityCoords(tPolar.getPlayerPed())
        ...
        if E > 1.0 then
            ...
            if z == 5 then
                z = 0
                local F = C - D
                local G = GetEntityVelocity(tPolar.getPlayerPed())
                SetEntityCoordsNoOffset(tPolar.getPlayerPed(), D.x + F.x, D.y + F.y, D.z)
                ForcePedMotionState(tPolar.getPlayerPed(), -1115154469, 1, 1, 0)
                SetEntityVelocity(tPolar.getPlayerPed(), G.x, G.y, G.z)
            end
            C = GetEntityCoords(tPolar.getPlayerPed())
        end
        DrawToons()
        DrawCubes()
        if e and not f then
    ...
    SetPedMotionBlur(tPolar.getPlayerPed(), false)
...
InitPed = function()
    ...
    while GetScreenCoordFromWorldCoord(L.x, L.y, L.z) and not IsPointOnRoad(L.x, L.y, L.z) do
        J = math.random(0, 359)
        K = math.random(50, 80)
        L = GetEntityCoords(tPolar.getPlayerPed()) + PointOnSphere(0.0, J)
        Citizen.Wait(0)
    end
...
TrackEnt = function()
    while true do
        local E = GetVecDist(GetEntityCoords(tPolar.getPlayerPed()), GetEntityCoords(g))
        if E > 5.0 then
            TaskGoToEntity(g, tPolar.getPlayerPed(), -1, 4.0, 100.0, 1073741824, 0)
            Wait(1000)
        else
            ...
            if not d or GetGameTimer() - d > 1000 then
                d = GetGameTimer()
                TaskTurnPedToFaceEntity(g, tPolar.getPlayerPed(), -1)
            end
...
InitCubes = function()
    ...
    SetPedMotionBlur(tPolar.getPlayerPed(), true)
    local R = 4000
    local S = 0
    while S < R do
        S = S + 1
...
DrawCubes = function()
    local X = GetEntityCoords(tPolar.getPlayerPed())
    local T = 10
    for U, V in pairs(b) do
        local Y = 0.1
        local Z = 0.1
...
local ae = false
local af = false
RegisterNetEvent("Polar:cocaineEffect",function()
    if ae then
        af = true
        local t = 0
        while t < 10 do
            tPolar.varyHealth(-10)
            tPolar.notify("~r~I don't feel too good..")
            Wait(5000)
            t = t + 1
        end
...
RegisterNetEvent("Polar:heroinEffect",function()
    ...
    tPolar.enablePunching(true)
    Wait(30000)
    ClearTimecycleModifier()
    tPolar.enablePunching(false)
