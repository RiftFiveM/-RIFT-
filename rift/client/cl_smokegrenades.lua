...
local function b()
    ...
    TriggerServerEvent("RIFT:startThrowSmokeGrenade", c)
    Citizen.Wait(100)
    ...
    TriggerServerEvent("RIFT:endThrowSmokeGrenade", c, g)
end
local function h(i)
...
RegisterNetEvent("RIFT:displaySmokeGrenade",function(c, g)
    local l = {coords = g, isDeleted = false}
    tRIFT.createArea("smoke_" .. tostring(c),g,250.0,1000.0,k,m,function()end,l)
    Citizen.Wait(60000)
    tRIFT.removeArea("smoke_" .. tostring(c))
    if l.handle then
        StopParticleFxLooped(l.handle, false)
        l.handle = nil
        l.isDeleted = true
