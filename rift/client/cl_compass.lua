...
function tRIFT.isCompassEnabled()
    return showCompass
end
function tRIFT.setCompassEnabled(f)
    showCompass = f
    SetResourceKvp("rift_compass", tostring(f))
end
Citizen.CreateThread(function()
	local g = GetResourceKvpString("rift_compass") or "false"
	if g == "false" then
		showCompass = false
	else
		showCompass = true
...
local function h(i, j, k, l)
    tRIFT.DrawText(j, k, i, l.size, 0, 0, {l.colour.r, l.colour.g, l.colour.b, l.colour.a}, true)
end
local function m()
    if showCompass then
        ...
        if a.compass.followGameplayCam then
            ...
            o = 360.0 - GetEntityHeading(tRIFT.getPlayerPed())
        end
