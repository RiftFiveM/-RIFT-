local aK = false
RegisterNetEvent("RIFT:adminTicketFeedback",function(aL)
    print(aL)
    local aM, aN = tRIFT.getPlayerVehicle()
    if aM ~= 0 and aN and GetEntitySpeed(aM) > 25.0 or tRIFT.getPlayerCombatTimer() > 0 then
        return
    end
    if aK then
        return
    ...
    if aO then
        local aP = false
        tRIFT.clientPrompt("Attached Message","",function(aQ)
            TriggerServerEvent("RIFT:adminTicketFeedback", aL, aO, aQ)
            aP = true
        end)
        while not aP do
            for a1 = 0, 6 do
        ...
        TriggerServerEvent("RIFT:adminTicketNoFeedback", aL)
    end
    tRIFT.showUI()
