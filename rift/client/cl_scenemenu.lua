RMenu.Add("Polarscenemenu","main",RageUI.CreateMenu("","~b~Polar Traffic Scene Menu",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners","scenemenu"))
RMenu.Add("Polarscenemenu","objects",RageUI.CreateSubMenu(RMenu:Get("Polarscenemenu", "main"),"","~b~Spawn Objects",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners","scenemenu"))
RMenu.Add("Polarscenemenu","speedZone",RageUI.CreateSubMenu(RMenu:Get("Polarscenemenu", "main"),"","~b~Set a speed zone",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners","scenemenu"))
RMenu.Add("Polarscenemenu","spikes",RageUI.CreateSubMenu(RMenu:Get("Polarscenemenu", "main"),"","~b~Set a spike trap",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners","scenemenu"))
local b = {}
local c = {}
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('Polarscenemenu', 'main')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            E()
            RageUI.ButtonWithStyle("Object Menu",nil,{RightLabel = "→→→"},true,function(K, L, M)
            end,RMenu:Get("Polarscenemenu", "objects"))
            RageUI.ButtonWithStyle("Speed Zone",nil,{RightLabel = "→→→"},true,function(K, L, M)
            end,RMenu:Get("Polarscenemenu", "speedZone"))
            if tPolar.globalOnPoliceDuty() then
                RageUI.ButtonWithStyle("Spike Strip Menu",nil,{RightLabel = "→→→"},true,function(K, L, M)
                end,RMenu:Get("Polarscenemenu", "spikes"))
            end
            RageUI.ButtonWithStyle("Close Menu",nil,{RightLabel = "→→→"},true,function(K, L, M)
                if M then
                    RageUI.Visible(RMenu:Get("Polarscenemenu", "main"), false)
                end
            end)
        end)
    end
    if RageUI.Visible(RMenu:Get('Polarscenemenu', 'objects')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
    ...
    if RageUI.Visible(RMenu:Get('Polarscenemenu', 'speedZone')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.ButtonWithStyle("Create Speedzone",nil,{},true,function(K, L, M)
                if M then
                    if p == 0 then
                        tPolar.notify("~r~Please set a radius")
                    elseif q == 0 then
                        tPolar.notify("~r~Please set a speed")
                    else
                        TriggerServerEvent("Polar:createSpeedZone", tPolar.getPlayerCoords(), q + 0.0, p + 0.0)
                        tPolar.notify("~b~Speed zone created")
                    end
                end
            end,nil)
            RageUI.ButtonWithStyle("Delete Speedzone",nil,{},true,function(K, L, M)
                if M then
                    local Q = tPolar.getPlayerCoords()
                    local R = nil
                    local S = 250.0
                    ...
                    if R then
                        TriggerServerEvent("Polar:deleteSpeedZone", R)
                        tPolar.notify("~r~Speed zone removed")
                    end
                end
            end,nil)
        end)
    end
    if RageUI.Visible(RMenu:Get('Polarscenemenu', 'spikes')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
...
RegisterNetEvent("Polar:toggleTrafficMenu",function()
    RageUI.Visible(RMenu:Get("Polarscenemenu", "main"), not RageUI.Visible(RMenu:Get("Polarscenemenu", "main")))
    if globalOnPoliceDuty and tPolar.hasPoliceCallsign() then
        RMenu:Get("Polarscenemenu", "main"):SetSubtitle("~b~MPD~w~: " .. tPolar.getPoliceCallsign() .. " - " .. tPolar.getPoliceRank() .. " - " .. tPolar.getPoliceName())
    end
end)
...
function deleteObject()
    for n, o in pairs(k) do
        ...
        local Y, Z, _ = table.unpack(GetEntityCoords(tPolar.getPlayerPed(), true))
        if DoesObjectOfTypeExistAtCoords(Y, Z, _, 0.9, X, true) then
            ...
            if NetworkGetEntityIsNetworked(a0) and not NetworkHasControlOfEntity(a0) then
                local a1 = tPolar.getNetId(a0, "deleteObject()")
                if a1 ~= 0 then
                    TriggerServerEvent("Polar:requestSceneObjectDelete", a1)
                end
            else
                DeleteObject(a0)
            end
        ...
        for n, a2 in pairs(b) do
            if DoesEntityExist(a2) then
                if NetworkGetEntityIsNetworked(a2) then
                    ...
                    if a3 ~= 0 then
                        TriggerServerEvent("Polar:requestSceneObjectDelete", a3)
                    end
                else
                    DeleteEntity(a2)
                end
...
function deleteSpike()
    local Q = GetEntityCoords(tPolar.getPlayerPed())
    local a8 = getClosetSpike(Q)
    if a8 then
        TriggerServerEvent("Polar:removeSpike", a8[2])
    end
end
RegisterNetEvent("Polar:addSpike",function(Q, a7)
    c[a7] = {Q, a7}
end)
RegisterNetEvent("Polar:deleteSpike",function(a7)
    c[a7] = nil
end)
local a9 = 1
Citizen.CreateThread(function()
...
Citizen.CreateThread(function()
    while true do
        local a4, ah = tPolar.getPlayerVehicle()
        if a4 ~= 0 and ah then
            local ai = false
            local aj = 1000.0
...
RegisterNetEvent("Polar:createSpeedZone",function(T, ak, q, p)
...
RegisterNetEvent("Polar:deleteSpeedZone",function(T)
    local U = j[T]
    if U then
        RemoveBlip(U.blip)
