RMenu.Add("riftscenemenu","main",RageUI.CreateMenu("","~b~RIFT Traffic Scene Menu",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"banners","scenemenu"))
RMenu.Add("riftscenemenu","objects",RageUI.CreateSubMenu(RMenu:Get("riftscenemenu", "main"),"","~b~Spawn Objects",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"banners","scenemenu"))
RMenu.Add("riftscenemenu","speedZone",RageUI.CreateSubMenu(RMenu:Get("riftscenemenu", "main"),"","~b~Set a speed zone",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"banners","scenemenu"))
RMenu.Add("riftscenemenu","spikes",RageUI.CreateSubMenu(RMenu:Get("riftscenemenu", "main"),"","~b~Set a spike trap",tRIFT.getRageUIMenuWidth(),tRIFT.getRageUIMenuHeight(),"banners","scenemenu"))
local b = {}
local c = {}
...
RageUI.CreateWhile(1.0, true, function()
    ...
    if RageUI.Visible(RMenu:Get('riftscenemenu', 'speedZone')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.ButtonWithStyle("Create Speedzone",nil,{},true,function(K, L, M)
                if M then
                    if p == 0 then
                        tRIFT.notify("~r~Please set a radius")
                    elseif q == 0 then
                        tRIFT.notify("~r~Please set a speed")
                    else
                        TriggerServerEvent("RIFT:createSpeedZone", tRIFT.getPlayerCoords(), q + 0.0, p + 0.0)
                        tRIFT.notify("~b~Speed zone created")
                    end
                end
            end,nil)
            RageUI.ButtonWithStyle("Delete Speedzone",nil,{},true,function(K, L, M)
                if M then
                    local Q = tRIFT.getPlayerCoords()
                    local R = nil
                    local S = 250.0
                    ...
                    if R then
                        TriggerServerEvent("RIFT:deleteSpeedZone", R)
                        tRIFT.notify("~r~Speed zone removed")
                    end
                end
            end,nil)
        end)
    end
    if RageUI.Visible(RMenu:Get('riftscenemenu', 'spikes')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
...
RegisterNetEvent("RIFT:toggleTrafficMenu",function()
    RageUI.Visible(RMenu:Get("riftscenemenu", "main"), not RageUI.Visible(RMenu:Get("riftscenemenu", "main")))
    if globalOnPoliceDuty and tRIFT.hasPoliceCallsign() then
        RMenu:Get("riftscenemenu", "main"):SetSubtitle("~b~MPD~w~: " .. tRIFT.getPoliceCallsign() .. " - " .. tRIFT.getPoliceRank() .. " - " .. tRIFT.getPoliceName())
    end
end)
...
function deleteObject()
    for n, o in pairs(k) do
        ...
        local Y, Z, _ = table.unpack(GetEntityCoords(tRIFT.getPlayerPed(), true))
        if DoesObjectOfTypeExistAtCoords(Y, Z, _, 0.9, X, true) then
            ...
            if NetworkGetEntityIsNetworked(a0) and not NetworkHasControlOfEntity(a0) then
                local a1 = tRIFT.getNetId(a0, "deleteObject()")
                if a1 ~= 0 then
                    TriggerServerEvent("RIFT:requestSceneObjectDelete", a1)
                end
            else
                DeleteObject(a0)
            end
        ...
        for n, a2 in pairs(b) do
            if DoesEntityExist(a2) then
                if NetworkGetEntityIsNetworked(a2) then
                    ...
                    if a3 ~= 0 then
                        TriggerServerEvent("RIFT:requestSceneObjectDelete", a3)
                    end
                else
                    DeleteEntity(a2)
                end
...
function deleteSpike()
    local Q = GetEntityCoords(tRIFT.getPlayerPed())
    local a8 = getClosetSpike(Q)
    if a8 then
        TriggerServerEvent("RIFT:removeSpike", a8[2])
    end
end
RegisterNetEvent("RIFT:addSpike",function(Q, a7)
    c[a7] = {Q, a7}
end)
RegisterNetEvent("RIFT:deleteSpike",function(a7)
    c[a7] = nil
end)
local a9 = 1
Citizen.CreateThread(function()
...
Citizen.CreateThread(function()
    while true do
        local a4, ah = tRIFT.getPlayerVehicle()
        if a4 ~= 0 and ah then
            local ai = false
            local aj = 1000.0
...
RegisterNetEvent("RIFT:createSpeedZone",function(T, ak, q, p)
...
RegisterNetEvent("RIFT:deleteSpeedZone",function(T)
    local U = j[T]
    if U then
        RemoveBlip(U.blip)
