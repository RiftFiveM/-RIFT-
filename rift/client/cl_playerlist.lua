...
function func_playerlistControl()
    if IsUsingKeyboard(2) then
        if IsControlJustPressed(0, 212) then
            a = not a
            TriggerServerEvent("RIFT:getPlayerListData")
            Wait(100)
...
tRIFT.createThreadOnTick(func_playerlistControl)
RegisterNUICallback("closeRIFTPlayerList",function(j, k)
    SetNuiFocus(false, false)
    TriggerScreenblurFadeOut(100.0)
end)
AddEventHandler("RIFT:onClientSpawn",function(h, l)
    if l then
        TriggerServerEvent("RIFT:getPlayerListData")
    end
end)

RegisterNetEvent("RIFT:gotFullPlayerListData",function(staff, police, nhs, lfb, hmp, civillians)
...
local u,v,x
RegisterNetEvent("RIFT:playerListMetaUpdate",function(t)
...
function sendFullPlayerListData()
    ...
    SendNUIMessage({setServerMetaData = '<img src="playerlist_images/rift.png" align="top" width="20px",height="20px"><span class="staff">' ..tostring(H) .. "</span>"})
...
Citizen.CreateThread(function()
    while true do
        ...
        if u ~= nil and v ~= nil and x ~= nil then
            local A = tRIFT.getUserId()
            if tRIFT.isDev() then
                A = 0.5
            end
            SetDiscordAppId(1152397969312722984)
            SetDiscordRichPresenceAsset("rift")
            SetDiscordRichPresenceAssetText("discord.gg/rift")
            SetDiscordRichPresenceAssetSmall("rift")
            SetDiscordRichPresenceAssetSmallText("RIFT British Roleplay")
            SetRichPresence("[ID:" .. tostring(A) .."] | " .. tostring(v) .. "/" .. tostring(x) .. "\n" .. tRIFT.getPlayerName(PlayerId()))
        end
        Wait(15000)
    end
end)