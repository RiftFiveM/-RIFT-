...
RegisterNetEvent(
    "RIFT:playGrindingScenario",
    function(c, d)
        if not a then
            a = true
            local e = GetGameTimer()
            TaskStartScenarioInPlace(tRIFT.getPlayerPed(), c, 0, true)
            local f
            if d then
...
RegisterNetEvent(
    ...
    function()
        if not a then
            ...
            local j = tRIFT.loadModel("prop_tool_pickaxe")
            ...
            tRIFT.notify("~r~Mining currently in progress.")
        end
    end
)
local l = false
...
function func_drawGrindUI()
    if p then
        ...
        DrawAdvancedText(0.558, 0.303, 0.005, 0.0028, 0.539, "RIFT Grinding", 255, 255, 255, 255, 7, 0)
        ...
        local t, u = tRIFT.getNearestOwnedVehicle(8)
        if t then
            s = u
        else
            s = "None"
        ...
        if CursorInArea(0.35, 0.415, 0.415, 0.46) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                m()
                TriggerServerEvent("RIFT:requestGrinding", q, r)
                Wait(500)
                p = false
                setCursor(0)
                inGUIRIFT = false
        ...
        if CursorInArea(0.435, 0.51, 0.415, 0.46) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if s ~= "None" then
                    m()
                    TriggerServerEvent("RIFT:requestGrinding", q, r, s)
                    Wait(500)
                    p = false
                    setCursor(0)
                    inGUIRIFT = false
                else
                    tRIFT.notify("~r~No owned vehicle nearby.")
                end
            end
        else
    ...
    if a and s ~= "None" then
        if IsControlJustPressed(0, 73) then
            s = "None"
            TriggerServerEvent("RIFT:stopGrinding")
            tRIFT.notify("~r~Grinding cancelled.")
        end
    end
end
tRIFT.createThreadOnTick(func_drawGrindUI)
AddEventHandler(
    "RIFT:onClientSpawn",
    function(v, w)
        if w then
            ...
            local A = function(y)
                if y.nearby then
                    if IsControlJustReleased(0, 38) and GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
                        if not a then
                            if not IsNuiFocused() and not inGUIRIFT then
                                p = true
                                setCursor(1)
                                inGUIRIFT = true
                                q = y.drug
                            ...
                            tRIFT.notify("~r~Action in progress, please wait.")
                        end
                    end
                end
            end
...
Citizen.CreateThread(
    function()
        ...
        local G = tRIFT.loadModel("a_c_hen")
