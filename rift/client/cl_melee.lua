...
RegisterCommand("togglemelee",function()
    if tRIFT.getUserId()==1 or tRIFT.getUserId()==2 then 
        c = not c
        tRIFT.notify('Melee Combat: '..(c and "~g~Enabled" or "~r~Disabled"))
    end 
end)
function tRIFT.enablePunching(d)
    c = d
end
...
AddEventHandler("RIFT:onClientSpawn",function(h, i)
    if i then
        ...
        for n, o in pairs(a) do
            tRIFT.createArea("boxing_" .. n, o.pos, o.radius, 6, j, l, m, {})
        end
    end
end)
...
AddEventHandler("RIFT:onClientSpawn",function(h, i)
    if i then
        ...
        for n, u in pairs(t) do
            tRIFT.addMarker(u.x, u.y, u.z, 1.0, 1.0, 1.0, 0, 0, 255, 100, 50, 27, false, false, true)
            tRIFT.createArea(n .. "_boxingGloves", u, 1.5, 6, j, l, m, {})
        end
    end
end)
local v = {}
function putGloves()
    local w = tRIFT.getPlayerPed()
    local x = tRIFT.loadModel("prop_boxing_glove_01")
