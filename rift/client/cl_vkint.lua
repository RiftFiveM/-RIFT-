...
RMenu.Add("vkint","mainmenu",RageUI.CreateMenu("Teleporter", "Main Menu", tRIFT.getRageUIMenuWidth(), tRIFT.getRageUIMenuHeight()))
local b = nil
local function c(d)
    if tRIFT.getPlayerCombatTimer() > 0 then
        tRIFT.notify("~r~You can not teleport whilst you have a combat timer.")
        return
    end
...
Citizen.CreateThread(function()
    for f, d in pairs(a) do
        tRIFT.addMarker(d.position.x, d.position.y, d.position.z - 1, 1.0, 1.0, 1.0, 255, 255, 255, 170, 50, 27)
        tRIFT.createArea("vkint_" .. d.id,d.position,2.0,10.0,l,m,function()end,d)
    end
end)
