...
local u = {}
RMenu.Add("RIFTjackpot","instructions",RageUI.CreateMenu("", "", tRIFT.getRageUIMenuWidth(), tRIFT.getRageUIMenuHeight(), "banners", "casino_jackpot"))
RMenu:Get("RIFTjackpot", "instructions"):SetSubtitle("~b~JACKPOT")
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('RIFTjackpot', 'instructions')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            end,RMenu:Get("RIFTjackpot", "instructions"))
        end)
    end
end)
Citizen.CreateThread(function()
    while true do
        if insideDiamondCasino then
            b = 1000
            a = -1
            local y = GetEntityCoords(tRIFT.getPlayerPed())
            for z = 0, 15, 1 do
                local A = t[z]
                local B = #(y - A)
                if B < b then
...
RegisterNetEvent("RIFT:sitAtJackpotChair",function(E)
    e = E
    sittingAtInsideTrack = true
    goToJackpotChair(E)
end)
RegisterNetEvent("RIFT:updatePlayerWinChance",function(F)
    k = F
end)
RegisterNetEvent("RIFT:updateTotalPot",function(G)
    r = G
end)
function goToJackpotChair(E)
    ...
    PlayFacialAnim(tRIFT.getPlayerPed(), "enter_left_readyidle_facial", H)
    RemoveAnimDict(H)
    local O
    local P
            ...
            if j then
                ...
                if u ~= nil then
                    for R, S in pairs(u) do
                        if S.user_id == tRIFT.getUserId() then
    ...
    while true do
        if f then
            ...
            if IsControlJustPressed(0, 201) then
                if h then
                    TriggerServerEvent("RIFT:setJackpotBet", p)
                else
...
function showHowToJackpot(a4)
    ...
    if a ~= -1 then
        RageUI.Visible(RMenu:Get("RIFTjackpot", "instructions"), a4)
    end
end
Citizen.CreateThread(function()
    while true do
        if a ~= -1 and b < 2 then
            if IsControlJustPressed(0, 38) then
                if d[a] == false then
                    TriggerServerEvent("RIFT:requestSitAtJackpot", a)
                end
            end
        end
        Wait(0)
...
AddEventHandler("RIFT:onClientSpawn",function(a5, a6)
    if a6 then
        TriggerServerEvent("RIFT:requestJackpotChairData")
    end
end)
RegisterNetEvent("RIFT:sendJackpotChairData",function(a7)
    d = a7
end)
Citizen.CreateThread(function()
    while true do
        if sittingAtInsideTrack and f then
            ...
            if IsControlJustPressed(0, 202) and not g then
                ...
                PlayFacialAnim(tRIFT.getPlayerPed(), "exit_left_facial", H)
                ...
                f = false
                showHowToJackpot(false)
                TriggerServerEvent("RIFT:leaveJackpotChair")
                e = -1
...
RegisterNetEvent("RIFT:rollJackpot",function(a8, a9, aa, ab, ac)
                                        ...
                                        SetTimeout(1500,function()
                                            SetTimeout(1000,function()
                                                SetTimeout(750,function()
                                                    ...
                                                    o = ab
                                                    if al then
                                                        if ac == tRIFT.getUserId() then
                                                            SendNUIMessage({transactionType = "playCasinoWin"})
                                                            TriggerServerEvent("RIFT:waitingOnWinConfirm")
                                                        else
                                                            if e ~= -1 then
...
function getTrackGenderAnimDict()
    local ax = GetEntityModel(tRIFT.getPlayerPed())
...
RegisterNetEvent("RIFT:successJackpotBet",function()
    f = false
    h = false
    j = true
    PlaySoundFrontend(-1, "Hack_Success", "DLC_HEIST_BIOLAB_PREP_HACKING_SOUNDS", 1)
end)
RegisterNetEvent("RIFT:beginJackpot",function()
    s = false
    l = 60
    m = true
...
RegisterNetEvent("RIFT:newJackpotBet",function(ay)
    table.insert(u, ay)
end)
RegisterNetEvent("RIFT:cleanupJackpot",function()
    u = {}
    if e ~= -1 then
        f = true
        local H = getTrackGenderAnimDict()
        ...
        NetworkAddPedToSynchronisedScene(tRIFT.getPlayerPed(), az, H, I, 2.0, -2.0, 5, 0, 1148846080, 0)
