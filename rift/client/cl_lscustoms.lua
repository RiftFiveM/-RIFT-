...
local function DriveOutOfGarage(pos)
	Citizen.CreateThread(function()
		...
		local ok, veh_type, model = tRIFT.getNearestOwnedVehicle(1)
		myveh.model = model
...
AddEventHandler("RIFT:onClientSpawn",function(D, E)
    if E then
        ...
        for q,h in pairs(garages)do 
            tRIFT.addBlip(h.inside.position.x,h.inside.position.y,h.inside.position.z,72, nil, "LS Customs")
        end 
	end
end)
...
function tRIFT.isInsideLsCustoms()
    return inside
end
