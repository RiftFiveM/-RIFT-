...
RegisterCommand("leaderboardtest",function()
    if tRIFT.isDev() then
        local b = tRIFT.createLeaderboard(a)
        tRIFT.notify("~y~Leaderboard will close in 5 secconds")
        Wait(5000)
        tRIFT.closeLeaderboard(b)
    end
end,false)
local b = 1
local c = {}
...
function tRIFT.closeLeaderboard(b)
    c[b] = false
end
function tRIFT.createLeaderboard(m)
    local n = Scaleform("SC_LEADERBOARD")
    n.RunFunction("SET_DISPLAY_TYPE", {1})
    n.RunFunction("SET_MULTIPLAYER_TITLE", {"RIFT RP Event"})
...
function tRIFT.updateScoreboard(u, v, w)
    local x
    ...
    if u == -1 then
        ...
        if x then
            ...
            print("error: RIFT:updateScoreboard couldn't find row with values", u, v, w)
        end
    end
end
RegisterNetEvent(
    "RIFT:updateScoreboard",
    function(u, v, w)
        tRIFT.updateScoreboard(u, v, w)
    end
)
function tRIFT.setPlayerCanOpenLeaderboard(F)
    e = F
end
function tRIFT.clearLeaderboardData()
    d = {columns = {}, rows = {}}
end
function tRIFT.isLeaderboardSetup()
    return table.count(d.rows) > 0
end
function tRIFT.removePlayerFromLeaderboard(u)
