...
AddEventHandler("RIFT:onClientSpawn",function(p, q)
    if q then
        tRIFT.addMarker(b.x, b.y, b.z, 0.7, 0.7, 0.5, 0, 125, 255, 125, 150, 20, false, false, true)
        tRIFT.addBlip(b.x, b.y, b, 378, 3, "Organ Heist Police")
        tRIFT.addMarker(c.x, c.y, c.z, 0.7, 0.7, 0.5, 255, 0, 0, 125, 150, 20, false, false, true)
        tRIFT.addBlip(c.x, c.y, c.z, 378, 1, "Organ Heist Criminals")
    end
end)
...
Citizen.CreateThread(function()
    while true do
        if f then
            ...
            if IsControlJustPressed(0, 38) then
                if not globalOnPoliceDuty then
                    if not g then
                        TriggerServerEvent("RIFT:joinOrganHeist")
                        Wait(100)
                    else
                        tRIFT.notify("~r~Already joined the Organ Heist!")
                    end
                else
                    tRIFT.notify("~r~You are police, please use the other entrance!")
                end
            end
        elseif e then
            ...
            if IsControlJustPressed(0, 38) then
                if globalOnPoliceDuty then
                    ...
                    tRIFT.notify("~r~You are a civillian, please use the other entrance!")
                end
            end
        end
        ...
        if inGamePhase or inWaitingStage then
            ...
            RemoveWeaponFromPed(tRIFT.getPlayerPed(), "WEAPON_MOLOTOV")
            RemoveWeaponFromPed(tRIFT.getPlayerPed(), "WEAPON_FLASHBANG")
            RemoveWeaponFromPed(tRIFT.getPlayerPed(), "WEAPON_SMOKEGRENADE")
            RemoveWeaponFromPed(tRIFT.getPlayerPed(), "WEAPON_BZGAS")
            RemoveWeaponFromPed(tRIFT.getPlayerPed(), "WEAPON_STUNGUN")
            local r = PlayerPedId()
...
RegisterNetEvent("RIFT:teleportToOrganHeist",function(v, w, x, y)
    local r = PlayerPedId()
    d = w
    canPlayAnim = false
    h = x
    ...
    if x == "civ" then
        ...
        tRIFT.createAtm("Organ Heist", k.atmLocation)
    elseif x == "police" then
        SetPedRelationshipGroupHash(PlayerPedId(), m)
    end
    for l, z in pairs(k.gunStores[h]) do
        tRIFT.createGunStore(z[1], z[2], z[3])
    end
    tRIFT.setFriendlyFire(false)
    DoScreenFadeOut(1000)
...
RegisterNetEvent("RIFT:addOrganHeistPlayer",function(A, x)
    if x == "civ" then
        j[A] = true
    elseif x == "police" then
        i[A] = true
...
RegisterNetEvent("RIFT:organHeistKillConfirmed",function(B)
    PlaySoundFrontend(-1, "Weapon_Upgrade", "DLC_GR_Weapon_Upgrade_Soundset", true)
    tRIFT.playScreenEffect("MP_Celeb_Win", 0.25)
    tRIFT.notify("~g~Killed " .. B .. " received £25,000")
end)
    ...
    while true do
        if inWaitingStage then
            ...
            if #(s - k.pastGate) < 3.0 then
                ...
                NetworkFadeInEntity(PlayerPedId(), 0)
                DoScreenFadeIn(1000)
                tRIFT.notify("~r~You got too far from the organ heist and have been teleported back.")
            end
        end
        if inGamePhase then
            local s = GetEntityCoords(PlayerPedId())
...
RegisterNetEvent("RIFT:startOrganHeist",function()
...
RegisterNetEvent("RIFT:removeFromOrganHeist",function(F)
    if i[F] then
        i[F] = nil
    end
    if j[F] then
...
RegisterNetEvent("RIFT:endOrganHeist",function()
    ...
    inGamePhase = false
    d = 600
    tRIFT.deleteAtm("Organ Heist")
    for l, z in pairs(k.gunStores[h]) do
        tRIFT.deleteGunStore(z[1])
    end
    h = ""
    k = nil
...
RegisterNetEvent("RIFT:endOrganHeistWinner",function(x)
    Wait(10000)
    winOrganHeist(x)
    local r = PlayerPedId()
    tRIFT.setFriendlyFire(true)
...
function tRIFT.setDeathInOrganHeist()
    inOrganHeist = false
    inWaitingStage = false
    inGamePhase = false
end
