...
local b = {}
local c = 0
function tPolar.isInRadioChannel()
    return c ~= 0
end
local function d()
...
RegisterNetEvent("Polar:radiosAddPlayer",function(q, j, r)
    local s = b[q]
    if not s then
        return
    end
...
RegisterNetEvent("Polar:radiosRemovePlayer",function(q, j)
    local s = b[q]
    if not s then
        return
    end
...
RegisterNetEvent("Polar:radiosClearAll",function()
    t()
    b = {}
end)
...
RMenu.Add("radios","mainmenu",RageUI.CreateMenu("Radios", "Main Menu", tPolar.getRageUIMenuWidth(), tPolar.getRageUIMenuHeight()))
RMenu.Add("radios","advancedeffects",RageUI.CreateSubMenu(RMenu:Get("radios", "mainmenu"),"Radios","Advanced Effects",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight()))
RMenu.Add("radios","channelinfo",RageUI.CreateSubMenu(RMenu:Get("radios", "mainmenu"),"Radios","Channel Information",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight()))
    ...
    if RageUI.Visible(RMenu:Get('radios', 'advancedeffects')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            RageUI.ButtonWithStyle("Frequency Range In (Low)","",{RightLabel = tostring(E["freq_low"])},true,function(S, T, U)
                if U then
                    tPolar.clientPrompt("Frequency Range In (Low)","",function(W)
                        local w = tonumber(W)
                        if not w then
            ...
            RageUI.ButtonWithStyle("Frequency Range In (High)","",{RightLabel = tostring(E["freq_hi"])},true,function(S, T, U)
                if U then
                    tPolar.clientPrompt("Frequency Range In (High)","",function(W)
                        local w = tonumber(W)
                        if not w then
                            notify("~r~Inputted text could not be parsed as a number.")
                        elseif w < 1000 then
            ...
            RageUI.ButtonWithStyle("Ring Modulator (Frequency)","",{RightLabel = tostring(E["rm_mod_freq"])},true,function(S, T, U)
                if U then
                    tPolar.clientPrompt("Ring Modulator (Frequency)","",function(W)
                        local w = tonumber(W)
                        if not w then
                            notify("~r~Inputted text could not be parsed as a number.")
                        elseif w < 0 then
            ...
            RageUI.ButtonWithStyle("Ring Modulator (Mix)","",{RightLabel = tostring(E["rm_mix"])},true,function(S, T, U)
                if U then
                    tPolar.clientPrompt("Ring Modulator (Mix)","",function(W)
                        local w = tonumber(W)
                        if not w then
                            notify("~r~Inputted text could not be parsed as a number.")
                        elseif w < 0 then
            ...
            RageUI.ButtonWithStyle("Destruction (Fudge)","",{RightLabel = tostring(E["fudge"])},true,function(S, T, U)
                if U then
                    tPolar.clientPrompt("Destruction (Fudge)","",function(W)
                        local w = tonumber(W)
                        if not w then
                            notify("~r~Inputted text could not be parsed as a number.")
                        elseif w < 0 then
            ...
            RageUI.ButtonWithStyle("Frequency Range Out (Low)","",{RightLabel = tostring(E["o_freq_lo"])},true,function(S, T, U)
                if U then
                    tPolar.clientPrompt("Frequency Range Out (Low)","",function(W)
                        local w = tonumber(W)
                        if not w then
                            notify("~r~Inputted text could not be parsed as a number.")
                        elseif w < 60 then
            ...
            RageUI.ButtonWithStyle("Frequency Range Out (High)","",{RightLabel = tostring(E["o_freq_hi"])},true,function(S, T, U)
                if U then
                    tPolar.clientPrompt("Frequency Range Out (High)","",function(W)
                        local w = tonumber(W)
                        if not w then
                            notify("~r~Inputted text could not be parsed as a number.")
                        elseif w < 1000 then
...
Citizen.CreateThread(function()
    ...
    exports["pma-voice"]:setRadioVolume(u("radios_volume", 10) * 10)
    B = CreateAudioSubmix("PolarRadio")
...
RegisterNetEvent("Polar:radiosCreateChannel",function(q, h, L, X)
    if not b[q] then
...
RegisterCommand("toggleradiomute",function()
    if c ~= 0 then
        if exports["pma-voice"]:getRadioVolume() == 0 then
            exports["pma-voice"]:setRadioVolume(u("radios_volume", 10) * 10)
            TriggerServerEvent("Polar:radiosSetIsMuted", false)
        else
            exports["pma-voice"]:setRadioVolume(0)
            TriggerServerEvent("Polar:radiosSetIsMuted", true)
        end
    end
end,false)
RegisterKeyMapping("toggleradiomute", "Toggle Radio Volume", "KEYBOARD", "")
RegisterNetEvent("Polar:radiosSetPlayerIsMuted",function(q, j, p)
    local s = b[q]
    if not s then
        return
    end
