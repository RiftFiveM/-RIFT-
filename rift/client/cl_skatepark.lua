...
local cd = 0
AddEventHandler("Polar:onClientSpawn",function(b, c)
    if c then
        local d = function(e)
        ...
        local g = function(e)
            ...
            if IsControlJustPressed(1, 38)  then
                if GetGameTimer() - cd > 60000 then
                    local h = tPolar.loadModel(a[e.skateparkId].carID)
                    ...
                    DecorSetInt(i, "PolarACVeh", 955)
                    SetVehicleOnGroundProperly(i)
                    SetEntityInvincible(i, false)
                    SetPedIntoVehicle(tPolar.getPlayerPed(), i, -1)
                    SetModelAsNoLongerNeeded(h)
                    cd = GetGameTimer()
                else
                    tPolar.notify('~r~Please wait before spawning another BMX.')
                end
            end
        end
        for j, k in pairs(a) do
            tPolar.createArea("skatepark_" .. j, k.position, 1.5, 6, d, f, g, {skateparkId = j})
            tPolar.addMarker(k.position.x,k.position.y,k.position.z,1.0,1.0,1.0,255,0,0,170,50,38,false,false,true)
        end
    end
end)
