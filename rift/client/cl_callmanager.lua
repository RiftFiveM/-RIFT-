...
RegisterNetEvent("RIFT:addEmergencyCall",function(o, p, q, r, s, t)
    local u = 0
    if t == "admin" and tRIFT.getStaffLevel() > 0 then
        tRIFT.notify("~b~Admin ticket received.")
        PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
        local v = #(r - GetEntityCoords(tRIFT.getPlayerPed()))
        table.insert(d, {o, p, q, v, s, t, u})
        j()
        e[o] = r
    elseif t == "met" and tRIFT.globalOnPoliceDuty() then
        tRIFT.notify("~b~MET Police call received.")
        PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
        local v = #(r - GetEntityCoords(tRIFT.getPlayerPed()))
        table.insert(d, {o, p, q, v, s, t, u})
        j()
        e[o] = r
    elseif t == "nhs" and tRIFT.globalNHSOnDuty() then
        tRIFT.notify("~b~NHS call received.")
        PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
        local v = #(r - GetEntityCoords(tRIFT.getPlayerPed()))
        table.insert(d, {o, p, q, v, s, t, u})
        j()
...
RegisterNetEvent("RIFT:removeEmergencyCall",function(o)
    for m, n in pairs(d) do
        if n[1] == o then
            table.remove(d, m)
        end
...
Citizen.CreateThread(function()
    while true do
        if a then
            ...
            DrawAdvancedText(0.339, 0.529, 0.005, 0.0028, 0.51, "RIFT Call Manager", 255, 255, 255, 255, 7, 0)
            local w = 0
            ...
            if CursorInArea(0.4234, 0.4614, 0.9444, 0.9759) then
                ...
                if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                    if h ~= nil then
                        TriggerServerEvent("RIFT:TakeTicket", h)
                        a = not a
                        ...
                        if C ~= nil then
                            TriggerServerEvent("RIFT:TakeTicket", C)
                            a = not a
                            SetNewWaypoint(e[h].x, e[h].y)
                            TriggerEvent("RIFT:showHUD", true)
                            globalHideEmergencyCallUI = false
                            ...
                            tRIFT.notify("~r~No calls available.")
                        end
                    end
                end
            else
