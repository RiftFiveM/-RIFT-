...
local wbody_choices = function(args)
  ...
  choices["Equip"] = {function(player,choice)
    ...
    if user_id ~= nil then
      if RIFT.tryGetInventoryItem(user_id, fullidname, 1, true) then
        local weapons = {}
          ...
          if k == args[2] then
            if v.policeWeapon then
              if RIFT.hasPermission(user_id, 'police.onduty.permission') then
                RIFTclient.giveWeapons(player, {weapons, false})
              else
                RIFTclient.notify(player, {'~r~You cannot equip this weapon.'})
              end
            else
              RIFTclient.giveWeapons(player, {weapons, false})
            end
...
local wammo_choices = function(args)
  ...
  choices["Load"] = {function(player,choice)
    local user_id = RIFT.getUserId(player)
    if user_id ~= nil then
      local amount = RIFT.getInventoryItemAmount(user_id, fullidname)
      if string.find(fullidname, "Police") and not RIFT.hasPermission(user_id, 'police.onduty.permission') then
        RIFTclient.notify(player, {'~r~You cannot load this ammo.'})
        local bulletAmount = RIFT.getInventoryItemAmount(user_id, fullidname)
        RIFT.tryGetInventoryItem(user_id, fullidname, bulletAmount, false)
        return
      end
      RIFT.prompt(player, "Amount to load ? (max "..amount..")", "", function(player,ramount)
        ramount = parseInt(ramount)
        RIFTclient.getWeapons(player, {}, function(uweapons) -- gets current weapons
            ...
            for c,d in pairs(uweapons) do -- goes through current weapons
              if k == c then  -- if weapon in new cfg is the same as in current weapons
                if fullidname == v.ammo then -- check if ammo being loaded is the same as the ammo for that gun
                  if RIFT.tryGetInventoryItem(user_id, fullidname, ramount, true) then -- take ammo from inv
                    local weapons = {}
  ...
  choices["LoadAll"] = {function(player,choice)
    local user_id = RIFT.getUserId(player)
    if user_id ~= nil then
      ramount = parseInt(RIFT.getInventoryItemAmount(user_id, fullidname))
...
for i,v in pairs(RIFTAmmoTypes) do
