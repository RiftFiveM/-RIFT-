...
local wbody_choices = function(args)
  ...
  choices["Equip"] = {function(player,choice)
    ...
    if user_id ~= nil then
      if Polar.tryGetInventoryItem(user_id, fullidname, 1, true) then
        local weapons = {}
          ...
          if k == args[2] then
            if v.policeWeapon then
              if Polar.hasPermission(user_id, 'police.onduty.permission') then
                Polarclient.giveWeapons(player, {weapons, false})
              else
                Polarclient.notify(player, {'~r~You cannot equip this weapon.'})
              end
            else
              Polarclient.giveWeapons(player, {weapons, false})
            end
...
local wammo_choices = function(args)
  ...
  choices["Load"] = {function(player,choice)
    local user_id = Polar.getUserId(player)
    if user_id ~= nil then
      local amount = Polar.getInventoryItemAmount(user_id, fullidname)
      if string.find(fullidname, "Police") and not Polar.hasPermission(user_id, 'police.onduty.permission') then
        Polarclient.notify(player, {'~r~You cannot load this ammo.'})
        local bulletAmount = Polar.getInventoryItemAmount(user_id, fullidname)
        Polar.tryGetInventoryItem(user_id, fullidname, bulletAmount, false)
        return
      end
      Polar.prompt(player, "Amount to load ? (max "..amount..")", "", function(player,ramount)
        ramount = parseInt(ramount)
        Polarclient.getWeapons(player, {}, function(uweapons) -- gets current weapons
            ...
            for c,d in pairs(uweapons) do -- goes through current weapons
              if k == c then  -- if weapon in new cfg is the same as in current weapons
                if fullidname == v.ammo then -- check if ammo being loaded is the same as the ammo for that gun
                  if Polar.tryGetInventoryItem(user_id, fullidname, ramount, true) then -- take ammo from inv
                    local weapons = {}
  ...
  choices["LoadAll"] = {function(player,choice)
    local user_id = Polar.getUserId(player)
    if user_id ~= nil then
      ramount = parseInt(Polar.getInventoryItemAmount(user_id, fullidname))
...
for i,v in pairs(PolarAmmoTypes) do
