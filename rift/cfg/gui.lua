...
-- it is not recommended to modify the RIFT core files outside the cfg/ directory, create a new resource instead
-- you can load external images/fonts/etc using the NUI absolute path: nui://my_resource/myfont.ttf
-- example, changing the gui font (suppose a rift_mod resource containing a custom font)
cfg.css = [[
@font-face {
  font-family: "Custom Font";
  src: url(nui://rift_mod/customfont.ttf) format("truetype");
}
...
-- static menus are menu with choices defined by RIFT.addStaticMenuChoices(name, choices)
