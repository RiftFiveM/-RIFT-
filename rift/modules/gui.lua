...
function Polar.openMenu(source,menudef)
  local menudata = {}
  menudata.choices = {}
  ...
  -- openmenu
  Polarclient.openMenuData(source,{menudata})
end

-- force close player menu
function Polar.closeMenu(source)
  Polarclient.closeMenu(source,{})
end
...
function Polar.prompt(source,title,default_text,cb_result)
  prompts[source] = cb_result

  Polarclient.prompt(source,{title,default_text})
end
...
function Polar.request(source,text,time,cb_ok)
  local id = request_ids:gen()
  local request = {source = source, cb_ok = cb_ok, done = false}
  requests[id] = request

  Polarclient.request(source,{id,text,time}) -- send request to client
...
function Polar.registerMenuBuilder(name, builder)
...
function Polar.buildMenu(name, data, cbr)
...
function Polar.openMainMenu(source)
  Polar.buildMenu("main", {player = source}, function(menudata)
    menudata.name = "Main menu"
    menudata.css = {top="75px",header_color="rgba(0,125,255,0.75)"}
    Polar.openMenu(source,menudata) -- open the generated menu
  end)
end

-- SERVER TUNNEL API

function tPolar.closeMenu(id)
...
function tPolar.validMenuChoice(id,choice,mod)
  local menu = client_menus[id]
  if menu and menu.source == source then
...
function tPolar.promptResult(text)
  if text == nil then
    text = ""
  end
...
function tPolar.requestResult(id,ok)
...
function tPolar.openMainMenu()
  Polar.openMainMenu(source)
end
...
function Polar.addStaticMenuChoices(name, choices)
...
-- build static menus
local static_menus = {}
SetTimeout(10000,function() -- wait for Polar.addStaticMenuChoices calls
...
AddEventHandler("Polar:playerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    ...
    Polarclient.setDiv(source,{"additional_css",".div_additional_css{ display: none; }\n\n"..cfg.css,""})
