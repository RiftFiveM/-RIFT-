...
function RIFT.openMenu(source,menudef)
  local menudata = {}
  menudata.choices = {}
  ...
  -- openmenu
  RIFTclient.openMenuData(source,{menudata})
end

-- force close player menu
function RIFT.closeMenu(source)
  RIFTclient.closeMenu(source,{})
end
...
function RIFT.prompt(source,title,default_text,cb_result)
  prompts[source] = cb_result

  RIFTclient.prompt(source,{title,default_text})
end
...
function RIFT.request(source,text,time,cb_ok)
  local id = request_ids:gen()
  local request = {source = source, cb_ok = cb_ok, done = false}
  requests[id] = request

  RIFTclient.request(source,{id,text,time}) -- send request to client
...
function RIFT.registerMenuBuilder(name, builder)
...
function RIFT.buildMenu(name, data, cbr)
...
function RIFT.openMainMenu(source)
  RIFT.buildMenu("main", {player = source}, function(menudata)
    menudata.name = "Main menu"
    menudata.css = {top="75px",header_color="rgba(0,125,255,0.75)"}
    RIFT.openMenu(source,menudata) -- open the generated menu
  end)
end

-- SERVER TUNNEL API

function tRIFT.closeMenu(id)
...
function tRIFT.validMenuChoice(id,choice,mod)
  local menu = client_menus[id]
  if menu and menu.source == source then
...
function tRIFT.promptResult(text)
  if text == nil then
    text = ""
  end
...
function tRIFT.requestResult(id,ok)
...
function tRIFT.openMainMenu()
  RIFT.openMainMenu(source)
end
...
function RIFT.addStaticMenuChoices(name, choices)
...
-- build static menus
local static_menus = {}
SetTimeout(10000,function() -- wait for RIFT.addStaticMenuChoices calls
...
AddEventHandler("RIFT:playerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    ...
    RIFTclient.setDiv(source,{"additional_css",".div_additional_css{ display: none; }\n\n"..cfg.css,""})
