...
RegisterNetEvent("Polar:toggleShiftG4S")
AddEventHandler("Polar:toggleShiftG4S", function(Status)
    local source=source
    local user_id=Polar.getUserId(source)
    if Polar.hasGroup(user_id,"G4S Driver")then
        if Status then
            currentUsersOnG4SJob[user_id]={currentJob=""}
            TriggerClientEvent("Polar:startShiftG4S",source,math.random(1000,9999))
            jobsCompleted[user_id] = 0
            Wait(10000)
            TriggerClientEvent("Polar:updateJobInformation",source,GetCollection(user_id))
        else
            if currentUsersOnG4SJob[user_id]~=nil then
                currentUsersOnG4SJob[user_id]=nil
                jobsCompleted[user_id] = nil
                TriggerClientEvent("Polar:endShiftG4S",source)
            end
        end
    else
        Polarclient.notify(source,{"~r~You are not clocked on as currentUsersOnG4SJob G4S Driver."})
    end
end)

RegisterNetEvent("Polar:acceptJob")
AddEventHandler("Polar:acceptJob",function()
    local source=source
    local user_id=Polar.getUserId(source)
    if Polar.hasGroup(user_id,"G4S Driver")then
        for k,v in pairs(currentUsersOnG4SJob)do
            if k == user_id then    
                ...
                TriggerClientEvent("Polar:updateJobInformation",source,v.currentJob)
            end
        end
    else
        Polarclient.notify(source,{"~r~You are not clocked on as currentUsersOnG4SJob G4S Driver."})
...
RegisterNetEvent("Polar:updateMoneyInVehicle")
AddEventHandler("Polar:updateMoneyInVehicle", function()
    local source=source
    local user_id=Polar.getUserId(source)
    if Polar.hasGroup(user_id,"G4S Driver")then
        for k,v in pairs(currentUsersOnG4SJob)do
...
RegisterNetEvent("Polar:updateMoneyOutVehicle")
AddEventHandler("Polar:updateMoneyOutVehicle", function()
    local source=source
    local user_id=Polar.getUserId(source)
    if Polar.hasGroup(user_id,"G4S Driver")then
        for k,v in pairs(currentUsersOnG4SJob)do
    ...
    while true do
        ...
        for k,v in pairs(currentUsersOnG4SJob)do
            if v.currentJob.jobActive then
                if Polar.getUserSource(k)then
                    if not v.currentJob.collected then
                        if #(GetEntityCoords(GetPlayerPed(Polar.getUserSource(k)))-v.currentJob.collectionCoords)<1.5 then
                            v.currentJob.collected=true
                            currentUsersOnG4SJob[k].currentJob=v.currentJob
                            TriggerClientEvent("Polar:updateJobInformation",Polar.getUserSource(k),v.currentJob)
                            TriggerClientEvent("Polar:requestMoneyInVehicle",Polar.getUserSource(k))
                        end
                    elseif v.currentJob.moneyInVehicle and not v.currentJob.depositing and not v.currentJob.moneyOutVehicle then
                        if #(GetEntityCoords(GetPlayerPed(Polar.getUserSource(k)))-v.currentJob.depositCoords)<20 then
                            v.currentJob.depositing=true
                            currentUsersOnG4SJob[k].currentJob=v.currentJob
                            TriggerClientEvent("Polar:updateJobInformation",Polar.getUserSource(k),v.currentJob)
                            TriggerClientEvent("Polar:requestMoneyOutVehicle",Polar.getUserSource(k))
                        end
                    elseif not v.currentJob.deposited and v.currentJob.moneyOutVehicle then
                        if #(GetEntityCoords(GetPlayerPed(Polar.getUserSource(k)))-v.currentJob.depositCoords)<1.5 then
                            v.currentJob.deposited=true
                            v.currentJob.depositing=false
                            TriggerClientEvent("Polar:updateJobInformation",Polar.getUserSource(k),GetCollection(k))
                            local pay = grindBoost*math.random(8000,12000)
                            Polar.giveBankMoney(k,pay)
                        end
                    end
                end
            end
...
RegisterCommand('g4s', function(source, args)
    ...
    for k,v in pairs(currentUsersOnG4SJob) do
        if k == user_id then
            TriggerClientEvent("Polar:openG4SMenu", source)
        end
    end
end)