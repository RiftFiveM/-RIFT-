...
function RIFT.getUserGroups(user_id)
...
function RIFT.addUserGroup(user_id, group)
    if not RIFT.hasGroup(user_id, group) then
        local user_groups = RIFT.getUserGroups(user_id)
        ...
        if ngroup then
            if ngroup._config and ngroup._config.gtype ~= nil then
                ...
                for k, v in pairs(_user_groups) do -- remove all groups with the same gtype
                    ...
                    if kgroup and kgroup._config and ngroup._config and kgroup._config.gtype == ngroup._config.gtype then
                        RIFT.removeUserGroup(user_id, k)
                    end
                end
            end
            ...
            TriggerEvent("RIFT:playerJoinGroup", user_id, group, gtype)
        end
    end
end
...
function RIFT.getUserGroupByType(user_id, gtype)
    local user_groups = RIFT.getUserGroups(user_id)
    for k, v in pairs(user_groups) do
        local kgroup = groups[k]
        if kgroup then
...
function RIFT.getUsersByGroup(group)
    local users = {}

    for k, v in pairs(RIFT.rusers) do
        if RIFT.hasGroup(tonumber(k), group) then
...
function RIFT.getUsersByPermission(perm)
    local users = {}

    for k, v in pairs(RIFT.rusers) do
        if RIFT.hasPermission(tonumber(k), perm) then
            table.insert(users, tonumber(k))
        end
    end
...
function RIFT.removeUserGroup(user_id, group)
    local user_groups = RIFT.getUserGroups(user_id)
    ...
    TriggerEvent("RIFT:playerLeaveGroup", user_id, group, gtype)

    user_groups[group] = nil -- remove reference
end

-- check if the user has a specific group
function RIFT.hasGroup(user_id, group)
    local user_groups = RIFT.getUserGroups(user_id)
...
-- check if the user has a specific permission
function RIFT.hasPermission(user_id, perm)
    local user_groups = RIFT.getUserGroups(user_id)
    ...
    if fchar == "@" then -- special aptitude permission
        ...
        if #parts == 3 then -- decompose group.aptitude.operator
            ...
            local arift = math.floor(RIFT.expToLevel(RIFT.getExp(user_id, group, aptitude)))

            local fop = string.sub(op, 1, 1)
            if fop == "<" then -- less (group.aptitude.<x)
                local rift = parseInt(string.sub(op, 2, string.len(op)))
                if arift < rift then
                    return true
                end
            elseif fop == ">" then -- greater (group.aptitude.>x)
                ...
                local rift = parseInt(string.sub(op, 1, string.len(op)))
                if arift == rift then
                    return true
                end
            end
    ...
    elseif fchar == "#" then -- special item permission
        ...
        if #parts == 2 then -- decompose item.operator
            ...
            local amount = RIFT.getInventoryItemAmount(user_id, item)
...
function RIFT.hasPermissions(user_id, perms)
