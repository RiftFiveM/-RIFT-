...
function Polar.getUserGroups(user_id)
...
function Polar.addUserGroup(user_id, group)
    if not Polar.hasGroup(user_id, group) then
        local user_groups = Polar.getUserGroups(user_id)
        ...
        if ngroup then
            if ngroup._config and ngroup._config.gtype ~= nil then
                ...
                for k, v in pairs(_user_groups) do -- remove all groups with the same gtype
                    ...
                    if kgroup and kgroup._config and ngroup._config and kgroup._config.gtype == ngroup._config.gtype then
                        Polar.removeUserGroup(user_id, k)
                    end
                end
            end
            ...
            TriggerEvent("Polar:playerJoinGroup", user_id, group, gtype)
        end
    end
end
...
function Polar.getUserGroupByType(user_id, gtype)
    local user_groups = Polar.getUserGroups(user_id)
    for k, v in pairs(user_groups) do
        local kgroup = groups[k]
        if kgroup then
...
function Polar.getUsersByGroup(group)
    local users = {}

    for k, v in pairs(Polar.rusers) do
        if Polar.hasGroup(tonumber(k), group) then
...
function Polar.getUsersByPermission(perm)
    local users = {}

    for k, v in pairs(Polar.rusers) do
        if Polar.hasPermission(tonumber(k), perm) then
            table.insert(users, tonumber(k))
        end
    end
...
function Polar.removeUserGroup(user_id, group)
    local user_groups = Polar.getUserGroups(user_id)
    ...
    TriggerEvent("Polar:playerLeaveGroup", user_id, group, gtype)

    user_groups[group] = nil -- remove reference
end

-- check if the user has a specific group
function Polar.hasGroup(user_id, group)
    local user_groups = Polar.getUserGroups(user_id)
...
-- check if the user has a specific permission
function Polar.hasPermission(user_id, perm)
    local user_groups = Polar.getUserGroups(user_id)
    ...
    if fchar == "@" then -- special aptitude permission
        ...
        if #parts == 3 then -- decompose group.aptitude.operator
            ...
            local aPolar = math.floor(Polar.expToLevel(Polar.getExp(user_id, group, aptitude)))

            local fop = string.sub(op, 1, 1)
            if fop == "<" then -- less (group.aptitude.<x)
                local Polar = parseInt(string.sub(op, 2, string.len(op)))
                if aPolar < Polar then
                    return true
                end
            elseif fop == ">" then -- greater (group.aptitude.>x)
                ...
                local Polar = parseInt(string.sub(op, 1, string.len(op)))
                if aPolar == Polar then
                    return true
                end
            end
    ...
    elseif fchar == "#" then -- special item permission
        ...
        if #parts == 2 then -- decompose item.operator
            ...
            local amount = Polar.getInventoryItemAmount(user_id, item)
...
function Polar.hasPermissions(user_id, perms)
