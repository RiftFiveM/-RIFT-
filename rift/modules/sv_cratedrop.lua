...
AddEventHandler("RIFT:playerSpawn", function(user_id, source, first_spawn)
    if first_spawn then
       if #activeCrates > 0 then
            for k,v in pairs(activeCrates) do
                TriggerClientEvent('RIFT:addCrateDropRedzone', source, v, crateLocations[v])
            end
       end
    end
end)

RegisterServerEvent('RIFT:openCrate', function(crateID)
    local source = source
    local user_id = RIFT.getUserId(source)
    if activeCrates[crateID] == nil then return end
    if activeCrates[crateID].timeTillOpen > 0 then
        RIFTclient.notify(source, {'~r~Loot crate unlocking in '..activeCrates[crateID].timeTillOpen..' seconds.'})
    else
        if (#(GetEntityCoords(GetPlayerPed(source)) - crateLocations[crateID]) < 2.0) or (#(GetEntityCoords(GetPlayerPed(source)) - rigLocations[crateID]) < 2.0) then
            TriggerClientEvent("RIFT:removeLootcrate", -1, crateID)
            FreezeEntityPosition(GetPlayerPed(source), true)
            RIFTclient.startCircularProgressBar(source, {"", 15000, nil})
            local anims = {
                {'amb@medic@standing@kneel@base', 'base', 1},
                {'anim@gangops@facility@servers@bodysearch@', 'player_search', 1},
            }
            RIFTclient.playAnim(source,{true,anims,false})
            Wait(15000)
            ...
            while lootAmount > 0 do
                ...
                for k,v in pairs(availableItems) do
                    if k == randomItem then
                        RIFT.giveInventoryItem(user_id, v[1], v[2], true)
                    end
                end
                lootAmount = lootAmount - 1
            end
            activeCrates[crateID] = nil
            RIFT.giveMoney(user_id,math.random(50000,150000))
...
Citizen.CreateThread(function()
    while true do
        ...
        TriggerClientEvent('RIFT:crateDrop', -1, crateCoords, crateID, false)
...
Citizen.CreateThread(function()
    while true do
        ...
        TriggerClientEvent('RIFT:crateDrop', -1, crateCoords, crateID, true)
