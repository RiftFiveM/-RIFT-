...
AddEventHandler("RIFT:playerSpawn", function(user_id, source, first_spawn)
    if first_spawn then
        TriggerClientEvent('RIFT:policeSyncAllDoors', source, doors, doorStates)
    end
end)

RegisterNetEvent("RIFT:policeSetDoorState")
AddEventHandler("RIFT:policeSetDoorState", function(doorHash, state)
    local source = source
    local user_id = RIFT.getUserId(source)
    if RIFT.hasPermission(user_id, 'police.onduty.permission') then
        doorStates[doorHash] = state
        TriggerClientEvent('RIFT:policeSyncDoor', -1, doorHash, state)
    end
end)

RegisterNetEvent("RIFT:enterPoliceSyncDoors")
AddEventHandler("RIFT:enterPoliceSyncDoors", function()
    local source = source
    TriggerClientEvent('RIFT:gotAllDoorStates', source, doorStates)
end)