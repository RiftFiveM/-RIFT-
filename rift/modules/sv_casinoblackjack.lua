...
MySQL.createCommand("casinochips/remove_chips", "UPDATE Polar_casino_chips SET chips = (chips - @amount) WHERE user_id = @user_id")
    ...
    if source ~= nil then
        ...
        if chairId > 7 then
            if Polar.hasGroup(user_id,"Highroller") then
                ...
                Polarclient.notify(source,{'~r~You need to be a Highroller to sit at this table.'})
                return
            end
        end
        if chairId <= 7 then
                    ...
                    MySQL.query("casinochips/get_chips", {user_id = user_id}, function(rows, affected)
                        ...
                        if chips >= betAmount then
                            if chairId < 8 and betAmount > 100000 then
                                Polarclient.notify(source,{'~r~Maximum bet at this table is £100,000.'})
                                return
                            end
                            ...
                            tPolar.sendWebhook('blackjack-bet',"Polar Blackjack Logs", "> Player Name: **"..GetPlayerName(source).."**\n> Player TempID: **"..source.."**\n> Player PermID: **"..user_id.."**\n> Amount: **"..getMoneyStringFormatted(tostring(betAmount)).."**")
                        else 
