...
RegisterNetEvent("RIFT:attemptBeginBusJob")
AddEventHandler("RIFT:attemptBeginBusJob",function()
    local source=source
    local user_id=RIFT.getUserId(source)
    local BusJobTable={}
    if RIFT.hasGroup(user_id,"Bus Driver")then
        if not usersInBusJob[user_id] then
            ...
            TriggerClientEvent("RIFT:beginBusJob",source)
            Wait(100)
            TriggerClientEvent("RIFT:setNextBusJobBlip",source,busJobStops[BusJobTable.stopNumber].stopPosition)
        else
            RIFTclient.notify(source,{"~r~You are already a Bus Driver."})
        end
    else
        RIFTclient.notify(source,{"~r~You are not clocked on as a Bus Driver."})
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(busJobStops)do
                    if RIFT.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(RIFT.getUserSource(k)))-busJobStops[v.currentJob.stopNumber].stopPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            RIFT.giveBankMoney(k,pay)
                            TriggerClientEvent("RIFT:nextStopReachedBusJob",RIFT.getUserSource(k),pay)
                            if v.currentJob.stopNumber==#busJobStops then
                                TriggerClientEvent("RIFT:endBusJob", RIFT.getUserSource(k))
                                v.currentJob.jobActive=false
                                usersInBusJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("RIFT:setNextBusJobBlip",RIFT.getUserSource(k),busJobStops[v.currentJob.stopNumber].stopPosition)
                            end
                        end
                    end
                end
