...
function getGroupInGroups(id, type)
    if type == 'Staff' then
        for k,v in pairs(RIFT.getUserGroups(id)) do
...
local hiddenUsers = {}
RegisterNetEvent("RIFT:setUserHidden")
AddEventHandler("RIFT:setUserHidden",function(state)
    local source=source
...
Citizen.CreateThread(function()
    while true do
        ...
        uptime = uptime + 1
        TriggerClientEvent('RIFT:playerListMetaUpdate', -1, playerListMetaUpdates())
        TriggerClientEvent('RIFT:setHiddenUsers', -1, hiddenUsers)
        if os.date('%A') == 'Sunday' and tonumber(time["hour"]) == 23 and tonumber(time["min"]) == 0 and tonumber(time["sec"]) == 0 then
            exports['ghmattimysql']:execute("UPDATE rift_police_hours SET weekly_hours = 0")
            exports['ghmattimysql']:execute("UPDATE rift_staff_tickets SET ticket_count = 0")
        end
        Citizen.Wait(1000)
    end
end)

RegisterNetEvent('RIFT:getPlayerListData')
AddEventHandler('RIFT:getPlayerListData', function()
    local source = source
    local user_id = RIFT.getUserId(source)
    local staff = {}
    local police = {}
    ...
    for k,v in pairs(RIFT.getUsers()) do
        if not hiddenUsers[k] then
            ...
            if name ~= nil then
                local minutesPlayed = RIFT.getUserDataTable(k).PlayerTime or 0
                ...
                if RIFT.hasPermission(k, 'police.onduty.permission') and not RIFT.hasPermission(k, 'police.undercover') then
                    police[k] = {name = name, rank = string.gsub(getGroupInGroups(k, 'Police'), ' Clocked', ''), hours = hours}
                elseif RIFT.hasPermission(k, 'nhs.onduty.permission') then
                    nhs[k] = {name = name, rank = string.gsub(getGroupInGroups(k, 'NHS'), ' Clocked', ''), hours = hours}
                elseif RIFT.hasPermission(k, 'lfb.onduty.permission') then
                    lfb[k] = {name = name, rank = string.gsub(getGroupInGroups(k, 'LFB'), ' Clocked', ''), hours = hours}
                elseif RIFT.hasPermission(k, 'prisonguard.onduty.permission') then
                    hmp[k] = {name = name, rank = string.gsub(getGroupInGroups(k, 'HMP'), ' Clocked', ''), hours = hours}
                end
                if (not RIFT.hasPermission(k, "police.onduty.permission") or RIFT.hasPermission(k, 'police.undercover')) and not RIFT.hasPermission(k, "nhs.onduty.permission") and not RIFT.hasPermission(k, "lfb.onduty.permission") and not RIFT.hasPermission(k, "prisonguard.onduty.permission") then
                    civillians[k] = {name = name, rank = getGroupInGroups(k, 'Default'), hours = hours}
                end
            end
        end
    end
    TriggerClientEvent('RIFT:gotFullPlayerListData', source, staff, police, nhs, lfb, hmp, civillians)
    TriggerClientEvent('RIFT:gotJobTypes', source, nhsGroups, pdGroups, lfbGroups, hmpGroups, tridentGroups)
end)
...
local function paycheck(tempid, permid, money)
    ...
    RIFT.giveBankMoney(permid, pay)
    RIFTclient.notifyPicture(tempid, {'CHAR_BANK_MAZE', 'CHAR_BANK_MAZE', 'Payday: ~g~£'..getMoneyStringFormatted(tostring(pay)), "", 'PAYE', '', 1})
end
