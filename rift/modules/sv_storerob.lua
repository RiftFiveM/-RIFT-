...
RegisterNetEvent('RIFT:getStoreRobBlips')
AddEventHandler('RIFT:getStoreRobBlips', function()
    local source = source
    TriggerClientEvent('RIFT:updateStoreRobBlips', -1, stores)
end)


RegisterNetEvent('RIFT:initiateStoreRobbery')
AddEventHandler('RIFT:initiateStoreRobbery', function(store)
    local source = source
    local user_id = RIFT.getUserId(source)
    if RIFT.hasPermission(user_id, "police.onduty.permission") then
        TriggerClientEvent('RIFT:resetStorePed', source, store)
    else
        if #RIFT.getUsersByPermission('police.onduty.permission') > 2 then
            for k,v in pairs(stores) do
                if k == store then
                    if v.cooldown == 0 then
                        ...
                        v.cooldown = 300
                        TriggerClientEvent('RIFT:updateStoreRobBlips', -1, stores)
                        TriggerClientEvent('RIFT:beginStoreRobbingAnimations', -1, store)
                        TriggerClientEvent('RIFT:storeRobberyInProgress', source, true, store)
                        ...
                        TriggerEvent('RIFT:PDRobberyCall', source, v.storename, v.position)
                    else
            ...
            RIFTclient.notify(source, {'~r~There are not enough police on duty to rob a store.'})
            TriggerClientEvent('RIFT:resetStorePed', source, store)
        end
    end
end)

RegisterNetEvent('RIFT:completeSafeCracking')
AddEventHandler('RIFT:completeSafeCracking', function(store)
    local source = source
    local user_id = RIFT.getUserId(source)
    RIFT.giveMoney(user_id, math.random(85000, 150000))
    TriggerClientEvent('RIFT:syncCloseSafeDoor', -1, store)
    TriggerClientEvent('RIFT:resetStorePed', -1, store)
    for k,v in pairs(stores) do
        if k == store then
            v.beingrobbed = false
        end
    ...
    TriggerClientEvent('RIFT:updateStoreRobBlips', -1, stores)
    TriggerClientEvent('RIFT:storeRobberyInProgress', source, false, store)
end)

RegisterNetEvent('RIFT:forceEndRobbery')
AddEventHandler('RIFT:forceEndRobbery', function(store)
    local source = source
    local user_id = RIFT.getUserId(source)
    TriggerClientEvent('RIFT:resetStorePed', -1, store)
    for k,v in pairs(stores) do
...
RegisterNetEvent('RIFT:syncOpenSafeDoor')
AddEventHandler('RIFT:syncOpenSafeDoor', function(store)
    local source = source
    TriggerClientEvent('RIFT:syncOpenSafeDoor', -1, store)
end)
