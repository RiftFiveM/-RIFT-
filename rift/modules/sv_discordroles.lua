...
local function Get_Client_Discord_ID(source)
	return exports['ghmattimysql']:executeSync("SELECT discord_id FROM `Polar_verification` WHERE user_id = @user_id", {user_id = Polar.getUserId(source)})[1].discord_id
end
	...
	if discord_roles then
		...
		for _, role_id in pairs(does_not_have_roles) do
			for k,v in pairs(cfg.Guild_Roles[guild_name]) do
                if v == role_id and Polar.hasGroup(user_id, k) then
		...
		for _, role_id in pairs(has_roles) do
            for k,v in pairs(cfg.Guild_Roles[guild_name]) do
                if v == role_id and not Polar.hasGroup(user_id, k) then
                    Polar.addUserGroup(user_id, k)
                end
            end
		end
	end
	Polar.getJobSelectors(Polar.getUserSource(user_id))
end

local tracked = {}
RegisterNetEvent('Polar:getFactionWhitelistedGroups')
AddEventHandler('Polar:getFactionWhitelistedGroups', function()
	local source = source
	tPolar.getFactionGroups(source)
end)

function tPolar.getFactionGroups(source)
    local source = source
...
function tPolar.checkForRole(user_id, role_id)
	local discord_id = exports['ghmattimysql']:executeSync("SELECT discord_id FROM `Polar_verification` WHERE user_id = @user_id", {user_id = user_id})[1].discord_id
