...
local function Get_Client_Discord_ID(source)
	return exports['ghmattimysql']:executeSync("SELECT discord_id FROM `rift_verification` WHERE user_id = @user_id", {user_id = RIFT.getUserId(source)})[1].discord_id
end
	...
	if discord_roles then
		...
		for _, role_id in pairs(does_not_have_roles) do
			for k,v in pairs(cfg.Guild_Roles[guild_name]) do
                if v == role_id and RIFT.hasGroup(user_id, k) then
		...
		for _, role_id in pairs(has_roles) do
            for k,v in pairs(cfg.Guild_Roles[guild_name]) do
                if v == role_id and not RIFT.hasGroup(user_id, k) then
                    RIFT.addUserGroup(user_id, k)
                end
            end
		end
	end
	RIFT.getJobSelectors(RIFT.getUserSource(user_id))
end

local tracked = {}
RegisterNetEvent('RIFT:getFactionWhitelistedGroups')
AddEventHandler('RIFT:getFactionWhitelistedGroups', function()
	local source = source
	tRIFT.getFactionGroups(source)
end)

function tRIFT.getFactionGroups(source)
    local source = source
...
function tRIFT.checkForRole(user_id, role_id)
	local discord_id = exports['ghmattimysql']:executeSync("SELECT discord_id FROM `rift_verification` WHERE user_id = @user_id", {user_id = user_id})[1].discord_id
