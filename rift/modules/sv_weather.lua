...
RegisterServerEvent('RIFT:requestSync')
AddEventHandler('RIFT:requestSync', function()
    TriggerClientEvent('RIFT:updateWeather', -1, CurrentWeather, blackout)
    TriggerClientEvent('RIFT:updateTime', -1, baseTime, timeOffset, freezeTime)
end)
...
RegisterCommand('freezetime', function(source, args)
    if source ~= 0 then
        if isAllowedToChange(source) then
            ...
            if freezeTime then
                TriggerClientEvent('RIFT:notify', source, 'Time is now ~b~frozen~s~.')
            else
                TriggerClientEvent('RIFT:notify', source, 'Time is ~y~no longer frozen~s~.')
            end
        else
    ...
    if source ~= 0 then
        if isAllowedToChange(source) then
            ...
            if not DynamicWeather then
                TriggerClientEvent('RIFT:notify', source, 'Dynamic weather changes are now ~r~disabled~s~.')
            else
                TriggerClientEvent('RIFT:notify', source, 'Dynamic weather changes are now ~b~enabled~s~.')
            end
        else
            TriggerClientEvent('chatMessage', source, '', {255,255,255}, '^8Error: ^1You are not allowed to use this command.')
        end
    ...
    if source == 0 then
        ...
        if isAllowedToChange(source) then
            ...
            if args[1] == nil then
                ...
                if validWeatherType then
                    TriggerClientEvent('RIFT:notify', source, 'Weather will change to: ~y~' .. string.lower(args[1]) .. "~s~.")
                    CurrentWeather = string.upper(args[1])
                    newWeatherTimer = 10
                    TriggerEvent('RIFT:requestSync')
                else
...
RegisterCommand('blackout', function(source)
    if source == 0 then
        ...
        if isAllowedToChange(source) then
            blackout = not blackout
            if blackout then
                TriggerClientEvent('RIFT:notify', source, 'Blackout is now ~b~enabled~s~.')
            else
                TriggerClientEvent('RIFT:notify', source, 'Blackout is now ~r~disabled~s~.')
            end
            TriggerEvent('RIFT:requestSync')
        end
    end
...
RegisterCommand('morning', function(source)
    ...
    if isAllowedToChange(source) then
        ShiftToMinute(0)
        ShiftToHour(9)
        TriggerClientEvent('RIFT:notify', source, 'Time set to ~y~morning~s~.')
        TriggerEvent('RIFT:requestSync')
    end
end)
RegisterCommand('noon', function(source)
    ...
    if isAllowedToChange(source) then
        ShiftToMinute(0)
        ShiftToHour(12)
        TriggerClientEvent('RIFT:notify', source, 'Time set to ~y~noon~s~.')
        TriggerEvent('RIFT:requestSync')
    end
end)
RegisterCommand('evening', function(source)
    ...
    if isAllowedToChange(source) then
        ShiftToMinute(0)
        ShiftToHour(18)
        TriggerClientEvent('RIFT:notify', source, 'Time set to ~y~evening~s~.')
        TriggerEvent('RIFT:requestSync')
    end
end)
RegisterCommand('night', function(source)
    ...
    if isAllowedToChange(source) then
        ShiftToMinute(0)
        ShiftToHour(23)
        TriggerClientEvent('RIFT:notify', source, 'Time set to ~y~night~s~.')
        TriggerEvent('RIFT:requestSync')
    end
end)
    ...
    elseif source ~= 0 then
        if isAllowedToChange(source) then
            if tonumber(args[1]) ~= nil and tonumber(args[2]) ~= nil then
                ...
                TriggerClientEvent('RIFT:notify', source, 'Time was changed to: ~y~' .. newtime .. "~s~!")
                TriggerEvent('RIFT:requestSync')
            else
...
function NextWeatherStage()
    ...
    TriggerEvent("RIFT:requestSync")
    if debugprint then
        print("[RIFT] New random weather type has been generated: " .. CurrentWeather .. ".\n")
        print("[RIFT] Resetting timer to 10 minutes.\n")
    end
end
