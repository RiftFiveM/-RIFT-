...
RegisterNetEvent("RIFT:attemptBeginDeliverooJob")
AddEventHandler("RIFT:attemptBeginDeliverooJob",function()
    local source=source
    local user_id=RIFT.getUserId(source)
    local deliverooJobTable={}
    if RIFT.hasGroup(user_id,"Deliveroo")then
        if not usersInDeliverooJob[user_id] then
            ...
            TriggerClientEvent("RIFT:beginDeliverooJob",source)
            Wait(100)
            TriggerClientEvent("RIFT:deliverooJobSetNextBlip",source,deliverooStops[deliverooJobTable.stopNumber].stopPosition)
        else
            RIFTclient.notify(source,{"~r~You are already a Deliveroo Driver."})
        end
    else
        RIFTclient.notify(source,{"~r~You are not clocked on as a Deliveroo Driver."})
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(deliverooStops)do
                    if RIFT.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(RIFT.getUserSource(k)))-deliverooStops[v.currentJob.stopNumber].stopPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            RIFT.giveMoney(userid,amount)(k,pay)
                            TriggerClientEvent("RIFT:deliverooJobReachedNextStop",RIFT.getUserSource(k),pay)
                            if v.currentJob.stopNumber==#deliverooStops then
                                TriggerClientEvent("RIFT:deliverooJobEnd", RIFT.getUserSource(k), true)
                                v.currentJob.jobActive=false
                                usersInDeliverooJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("RIFT:deliverooJobSetNextBlip",RIFT.getUserSource(k),deliverooStops[v.currentJob.stopNumber].stopPosition)
                            end
                        end
                    end
                end
