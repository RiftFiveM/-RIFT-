...
RegisterNetEvent("Polar:attemptBeginRoyalMailJob")
AddEventHandler("Polar:attemptBeginRoyalMailJob",function()
    local source=source
    local user_id=Polar.getUserId(source)
    local RoyalMailJobTable={}
    if Polar.hasGroup(user_id,"Royal Mail Driver")then
        if not usersInRoyalMailJob[user_id] then
            ...
            TriggerClientEvent("Polar:beginRoyalMailJob",source)
            Wait(100)
            TriggerClientEvent("Polar:royalMailJobSetNextBlip",source,RoyalMailDrops[RoyalMailJobTable.stopNumber].dropPosition)
        else
            Polarclient.notify(source,{"~r~You are already a Royal Mail Driver."})
        end
    else
        Polarclient.notify(source,{"~r~You are not clocked on as a Royal Mail Driver."})
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(RoyalMailDrops)do
                    if Polar.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(Polar.getUserSource(k)))-RoyalMailDrops[v.currentJob.stopNumber].dropPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            Polar.giveBankMoney(k,pay)
                            TriggerClientEvent("Polar:royalMailReachedNextStop",Polar.getUserSource(k),RoyalMailDrops[v.currentJob.stopNumber].dropPosition,pay)
                            if v.currentJob.stopNumber==#RoyalMailDrops then
                                TriggerClientEvent("Polar:royalMailJobEnd", Polar.getUserSource(k))
                                v.currentJob.jobActive=false
                                usersInRoyalMailJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("Polar:royalMailJobSetNextBlip",Polar.getUserSource(k),RoyalMailDrops[v.currentJob.stopNumber].dropPosition)
                            end
                        end
                    end
                end
