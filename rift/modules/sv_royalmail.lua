...
RegisterNetEvent("RIFT:attemptBeginRoyalMailJob")
AddEventHandler("RIFT:attemptBeginRoyalMailJob",function()
    local source=source
    local user_id=RIFT.getUserId(source)
    local RoyalMailJobTable={}
    if RIFT.hasGroup(user_id,"Royal Mail Driver")then
        if not usersInRoyalMailJob[user_id] then
            ...
            TriggerClientEvent("RIFT:beginRoyalMailJob",source)
            Wait(100)
            TriggerClientEvent("RIFT:royalMailJobSetNextBlip",source,RoyalMailDrops[RoyalMailJobTable.stopNumber].dropPosition)
        else
            RIFTclient.notify(source,{"~r~You are already a Royal Mail Driver."})
        end
    else
        RIFTclient.notify(source,{"~r~You are not clocked on as a Royal Mail Driver."})
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(RoyalMailDrops)do
                    if RIFT.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(RIFT.getUserSource(k)))-RoyalMailDrops[v.currentJob.stopNumber].dropPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            RIFT.giveBankMoney(k,pay)
                            TriggerClientEvent("RIFT:royalMailReachedNextStop",RIFT.getUserSource(k),RoyalMailDrops[v.currentJob.stopNumber].dropPosition,pay)
                            if v.currentJob.stopNumber==#RoyalMailDrops then
                                TriggerClientEvent("RIFT:royalMailJobEnd", RIFT.getUserSource(k))
                                v.currentJob.jobActive=false
                                usersInRoyalMailJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("RIFT:royalMailJobSetNextBlip",RIFT.getUserSource(k),RoyalMailDrops[v.currentJob.stopNumber].dropPosition)
                            end
                        end
                    end
                end
