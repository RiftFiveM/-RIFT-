...
MySQL.createCommand("Polar/get_weapons", "SELECT weapon_info FROM Polar_weapon_whitelists WHERE user_id = @user_id")
MySQL.createCommand("Polar/set_weapons", "UPDATE Polar_weapon_whitelists SET weapon_info = @weapon_info WHERE user_id = @user_id")
MySQL.createCommand("Polar/add_user", "INSERT IGNORE INTO Polar_weapon_whitelists SET user_id = @user_id")
MySQL.createCommand("Polar/get_all_weapons", "SELECT * FROM Polar_weapon_whitelists")
MySQL.createCommand("Polar/create_weapon_code", "INSERT IGNORE INTO Polar_weapon_codes SET user_id = @user_id, spawncode = @spawncode, weapon_code = @weapon_code")
MySQL.createCommand("Polar/remove_weapon_code", "DELETE FROM Polar_weapon_codes WHERE weapon_code = @weapon_code")
MySQL.createCommand("Polar/get_weapon_codes", "SELECT * FROM Polar_weapon_codes")

AddEventHandler("playerJoining", function()
    local user_id = Polar.getUserId(source)
    MySQL.execute("Polar/add_user", {user_id = user_id})
end)
...
RegisterNetEvent("Polar:getCustomWeaponsOwned")
AddEventHandler("Polar:getCustomWeaponsOwned",function()
    local source = source
    local user_id = Polar.getUserId(source)
    ...
    MySQL.query("Polar/get_weapons", {user_id = user_id}, function(weaponWhitelists)
        if weaponWhitelists[1]['weapon_info'] ~= nil then
            ...
            TriggerClientEvent('Polar:gotCustomWeaponsOwned', source, ownedWhitelists)
        end
    end)
end)

RegisterNetEvent("Polar:requestWhitelistedUsers")
AddEventHandler("Polar:requestWhitelistedUsers",function(spawncode)
    local source = source
    local user_id = Polar.getUserId(source)
    local whitelistOwners = {}
    MySQL.query("Polar/get_all_weapons", {}, function(weaponWhitelists)
        for k,v in pairs(weaponWhitelists) do
            if v['weapon_info'] ~= nil then
                data = json.decode(v['weapon_info'])
                for a,b in pairs(data) do
                    if b[spawncode] then
                        whitelistOwners[v['user_id']] = (exports['ghmattimysql']:executeSync("SELECT username FROM Polar_users WHERE id = @user_id", {user_id = v['user_id']})[1]).username
                    end
                end
            end
        end
        TriggerClientEvent('Polar:getWhitelistedUsers', source, whitelistOwners)
    end)
end)

RegisterNetEvent("Polar:generateWeaponAccessCode")
AddEventHandler("Polar:generateWeaponAccessCode",function(spawncode, id)
    local source = source
    local user_id = Polar.getUserId(source)
    ...
    for a,b in pairs(whitelistedGuns) do
        for c,d in pairs(b) do
            if b[spawncode] and d[6]== user_id then
                MySQL.execute("Polar/create_weapon_code", {user_id = id, spawncode = spawncode, weapon_code = code})
                TriggerClientEvent('Polar:generatedAccessCode', source, code)
            end
        end
    end
end)

RegisterNetEvent("Polar:requestNewGunshopData")
AddEventHandler("Polar:requestNewGunshopData",function()
    local source = source
    local user_id = Polar.getUserId(source)
    MySQL.query("Polar/get_weapons", {user_id = user_id}, function(weaponWhitelists)
        ...
        tPolar.getSubscriptions(user_id, function(cb, plushours, plathours)
            if cb then
                if plathours > 0 and Polar.hasPermission(user_id, "vip.gunstore") then
                    for k,v in pairs(VIPWithPlat) do
                        gunstoreData["VIP"][k] = v
                    end
                end
            end
            if Polar.hasPermission(user_id, 'advancedrebel.license') then
                for k,v in pairs(RebelWithAdvanced) do
                    gunstoreData["Rebel"][k] = v
                end
            end
            TriggerClientEvent('Polar:recieveFilteredGunStoreData', source, gunstoreData)
        end)
    end)
end)
...
function gunStoreLogs(weaponshop, webhook, title, text)
    if weaponshop == 'policeLargeArms' or weaponshop == 'policeSmallArms' then
        tPolar.sendWebhook('pd-armoury', 'Polar Police Armoury Logs', text)
    elseif weaponshop == 'NHS' then
        tPolar.sendWebhook('nhs-armoury', 'Polar NHS Armoury Logs', text)
    elseif weaponshop == 'prisonArmoury' then
        tPolar.sendWebhook('hmp-armoury', 'Polar HMP Armoury Logs', text)
    elseif weaponshop == 'LFB' then
        tPolar.sendWebhook('lfb-armoury', 'Polar LFB Armoury Logs', text)
    end
    tPolar.sendWebhook(webhook,title,text)
end

RegisterNetEvent("Polar:buyWeapon")
AddEventHandler("Polar:buyWeapon",function(spawncode, price, name, weaponshop, purchasetype, vipstore)
    local source = source
    local user_id = Polar.getUserId(source)
    local hasPerm = false
    local gunstoreData = deepcopy(cfg.GunStores)
            ...
            if k == '_config' then
                ...
                if json.encode(v[5]) ~= '[""]' then
                    ...
                    for a,b in pairs(v[5]) do
                        if Polar.hasPermission(user_id, b) then
                    ...
                    for c,d in pairs(gunstoreData[weaponshop]) do
                        if c ~= '_config' then
                            if hasPerm then
                                    ...
                                    if name == d[1] then
                                        if purchasetype == 'armour' then
                                            if string.find(spawncode, "fillUp") then
                                                ...
                                                if Polar.tryPayment(user_id,price) then
                                                    Polarclient.notify(source, {'~g~You bought '..name..' for £'..getMoneyStringFormatted(price)..'.'})
                                                    TriggerClientEvent("Polar:PlaySound", source, 1)
                                                    Polarclient.setArmour(source, {100, true})
                                                    gunStoreLogs(weaponshop, 'weapon-shops',"Polar Weapon Shop Logs", "> Player Name: **"..GetPlayerName(source).."**\n> Player TempID: **"..source.."**\n> Player PermID: **"..user_id.."**\n> Purchased: **"..name.."**\n> Price: **£"..getMoneyStringFormatted(price).."**\n> Weapon Shop: **"..weaponshop.."**\n> Purchase Type: **"..purchasetype.."**")
                                                    return
                                                end
                                            elseif GetPedArmour(GetPlayerPed(source)) >= (price/1000) then
                                                Polarclient.notify(source, {'~r~You already have '..GetPedArmour(GetPlayerPed(source))..'% armour.'})
                                                return
                                            end
                                            if Polar.tryPayment(user_id,d[2]) then
                                                Polarclient.notify(source, {'~g~You bought '..name..' for £'..getMoneyStringFormatted(price)..'.'})
                                                TriggerClientEvent("Polar:PlaySound", source, 1)
                                                Polarclient.setArmour(source, {price/1000, true})
                                                gunStoreLogs(weaponshop, 'weapon-shops',"Polar Weapon Shop Logs", "> Player Name: **"..GetPlayerName(source).."**\n> Player TempID: **"..source.."**\n> Player PermID: **"..user_id.."**\n> Purchased: **"..name.."**\n> Price: **£"..getMoneyStringFormatted(price).."**\n> Weapon Shop: **"..weaponshop.."**\n> Purchase Type: **"..purchasetype.."**")
                                                if weaponshop == 'LargeArmsDealer' then
                                                    Polar.turfSaleToGangFunds(price, 'LargeArms')
                                                end
                                            else
                                                Polarclient.notify(source, {'~r~You do not have enough money for this purchase.'})
                                                TriggerClientEvent("Polar:PlaySound", source, 2)
                                            end
                                        elseif purchasetype == 'weapon' then
                                            Polarclient.hasWeapon(source, {spawncode}, function(hasWeapon)
                                                if hasWeapon then
                                                    Polarclient.notify(source, {'~r~You must store your current '..name..' before purchasing another.'})
                                                else
                                                    if Polar.tryPayment(user_id,d[2]) then
                                                        if price > 0 then
                                                            Polarclient.notify(source, {'~g~You bought a '..name..' for £'..getMoneyStringFormatted(price)..'.'})
                                                            if weaponshop == 'LargeArmsDealer' then
                                                                Polar.turfSaleToGangFunds(price, 'LargeArms')
                                                            end
                                                        else
                                                            Polarclient.notify(source, {'~g~'..name..' purchased.'})
                                                        end
                                                        TriggerClientEvent("Polar:PlaySound", source, 1)
                                                        Polarclient.giveWeapons(source, {{[spawncode] = {ammo = 250}}, false})
                                                        gunStoreLogs(weaponshop, 'weapon-shops',"Polar Weapon Shop Logs", "> Player Name: **"..GetPlayerName(source).."**\n> Player TempID: **"..source.."**\n> Player PermID: **"..user_id.."**\n> Purchased: **"..name.."**\n> Price: **£"..getMoneyStringFormatted(price).."**\n> Weapon Shop: **"..weaponshop.."**\n> Purchase Type: **"..purchasetype.."**")
                                                    else
                                                        Polarclient.notify(source, {'~r~You do not have enough money for this purchase.'})
                                                        TriggerClientEvent("Polar:PlaySound", source, 2)
                                        ...
                                        elseif purchasetype == 'ammo' then
                                            ...
                                            if Polar.tryPayment(user_id,price) then
                                                if price > 0 then
                                                    Polarclient.notify(source, {'~g~You bought 250x Ammo for £'..getMoneyStringFormatted(price)..'.'})
                                                    if weaponshop == 'LargeArmsDealer' then
                                                        Polar.turfSaleToGangFunds(price, 'LargeArms')
                                                    end
                                                else
                                                    Polarclient.notify(source, {'~g~250x Ammo purchased.'})
                                                end
                                                TriggerClientEvent("Polar:PlaySound", source, 1)
                                                Polarclient.giveWeapons(source, {{[spawncode] = {ammo = 250}}, false})
                                                gunStoreLogs(weaponshop, 'weapon-shops',"Polar Weapon Shop Logs", "> Player Name: **"..GetPlayerName(source).."**\n> Player TempID: **"..source.."**\n> Player PermID: **"..user_id.."**\n> Purchased: **"..name.."**\n> Price: **£"..getMoneyStringFormatted(price).."**\n> Weapon Shop: **"..weaponshop.."**\n> Purchase Type: **"..purchasetype.."**")
                                ...
                                if weaponshop == 'policeLargeArms' or weaponshop == 'policeSmallArms' then
                                    Polarclient.notify(source, {"~r~You shouldn't be in here, ALARM TRIGGERED!!!"})
                                else
                                    Polarclient.notify(source, {"~r~You do not have permission to access this store."})
                                end
                            end
                        end
                    end
