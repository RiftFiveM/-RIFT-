...
function module(rsc, path) -- load a LUA resource file as module
    if path == nil then -- shortcut for rift, can omit the resource parameter
        path = rsc
        rsc = "rift"
    end
    local key = rsc .. path
    ...
    if modules[key] then -- cached module
        ...
        if f then
            ...
            if ar[1] then
                ...
                modules[key] = nil
                print("[RIFT] error loading module " .. rsc .. "/" .. path .. ":" .. ar[2])
            end
        else
            print("[RIFT] error parsing module " .. rsc .. "/" .. path .. ":" .. err)
        end
    end
end
