...
async function RegisterVueComponent(component, path) {
    const response = await fetch("nui://Polar/ui/pnc/components/"+path);
        ...
        methods: {
            addMarker() {
                if(!this.doesPlayerHaveWarningMarkerOfType(this.selectedWarningMarkerType)) {
                    $.post('https://Polar/addmarker', JSON.stringify({
            ...
            wipeAllMarkers() {
                $.post('https://Polar/wipeallmarkers', JSON.stringify({
            ...
            addPoint() {
                $.post('https://Polar/addpoint', JSON.stringify({
            ...
            switchPage(page) {
                if(this.personPages.includes(page)){
                    if (this.selectedPerson.id){
                        ...
                        console.log("[Polar PNC] Person needs to be selected to enter this page")
                    }
                }else {
                    this.page = page;
                }
            ...
            closePNC() {
                $.post('https://Polar/exit', JSON.stringify({}));
            },
            searchPerson(){
                let first = this.personFirstName;
                let last = this.personLastName;
                if (first == "" || last == "") {
                    console.log("[Polar PNC] Full name needs to be provided")
                }else{
                    $.post('https://Polar/personsearch', JSON.stringify({
                        firstname: first,
                        lastname: last
                    }));
                }
            ...
            searchPlate(){
                ...
                $.post("https://Polar/platesearch", JSON.stringify({
                    plate: this.plate
                }))
            },
            selectPerson(person){
            ...
            submitFine() {
                ...
                $.post('https://Polar/submitfine', JSON.stringify({
            ...
            addAD(){
                ...
                if (this.ADInput.length === 0) {
                    ...
                    $.post('https://Polar/addattentiondrawn', JSON.stringify({
                        ad: this.ADInput
                    }));
                }
            ...
            removeAD(index, ad) {
                if(this.askedIfSure) {
                    ...
                    $.post('https://Polar/removeattentiondrawn', JSON.stringify({
            ...
            saveNotes() {
                $.post('https://Polar/savenotes', JSON.stringify({
            ...
            savePersonNotes() {
                $.post("https://Polar/savepersonnotes", JSON.stringify({
                    user_id: this.selectedPerson.id,
                    notes: this.selectedPerson.notes
                }))
            },
            GetWarrant() {
                $.post("https://Polar/generatewarrant", JSON.stringify({}))
            }
            },
