...
RMenu.Add('PolarDJ','main',RageUI.CreateMenu("","~b~DJ Mixer",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners", "dj"))
RMenu.Add('PolarDJ','admin',RageUI.CreateSubMenu(RMenu:Get("PolarDJ","main"),"","~b~DJ Admin Menu",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners", "dj"))
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('PolarDJ', 'main')) then
            ...
            elseif not a.active then 
                if h then 
                    RageUI.Button("Start Session", "Start a new DJ Session", {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                        if Selected then 
                            if tPolar.getPlayerCombatTimer() == 0 and not tPolar.isPlayerInRedZone() and tPolar.getPlayerVehicle() == 0 then
                                setupDj()
                            else
                                tPolar.notify('~r~You can not set up a DJ deck right now.')
                            end
                        end 
                    end)
                end 
...
RegisterNetEvent("Polar:playDjSong")
AddEventHandler("Polar:playDjSong",function(s,coords,t,u)
...
RegisterNetEvent("Polar:requestCurrentProgress")
AddEventHandler("Polar:requestCurrentProgress",function(s,coords)
...
RegisterNetEvent("Polar:toggleDjMenu")
AddEventHandler("Polar:toggleDjMenu",function()
    RageUI.Visible(RMenu:Get('PolarDJ','main'),true)
    h=true 
end)
RegisterNetEvent("Polar:toggleDjAdminMenu")
AddEventHandler("Polar:toggleDjAdminMenu",function(x)
    RageUI.Visible(RMenu:Get('PolarDJ','admin'),true)
    i=x 
end)
RegisterNetEvent("Polar:finaliseSong")
AddEventHandler("Polar:finaliseSong",function(y)
    if a.active then 
        TriggerServerEvent("Polar:playDjSongServer",y,a.coords)
    end 
end)
RegisterNetEvent("Polar:updateDjVolume")
AddEventHandler("Polar:updateDjVolume",function(coords,z)
    if specificSongPlaying(coords)then 
        SendNUIMessage({type='djVolume',volume=z})
    end 
end)
RegisterNetEvent("Polar:stopSong")
AddEventHandler("Polar:stopSong",function(coords)
    local A=false
    for o,p in pairs(c)do 
        if p[1]==coords then 
            A=true
...
RegisterNetEvent("Polar:skipDj")
AddEventHandler("Polar:skipDj",function(coords,B)
    if specificSongPlaying(coords)then 
        if B then 
...
function skipDj(B)
    if songPlaying()then 
        TriggerServerEvent("Polar:skipServer",a.coords,B)
    end 
end
function setupDj()
    a.active=true
    a.coords=tPolar.getPlayerCoords()
    a.spawnedDecks=true
...
function createDjObject(C,D,E,F,G)
    local F=tPolar.loadModel(F)
    coords=GetOffsetFromEntityInWorldCoords(tPolar.getPlayerPed(),C,D,E)
    ...
    local I=GetEntityHeading(tPolar.getPlayerPed())
    SetEntityHeading(H,I+G)
end
function stopDj()
    if songPlaying()then 
        TriggerServerEvent("Polar:stopSongServer",a.coords)
    end
    a.active=false
...
function updateVolume()
    if songPlaying()then 
        TriggerServerEvent("Polar:updateVolumeServer",a.coords,b.volume)
    end 
end
...
function func_checkDjSongs(J)
    for o,p in pairs(c)do 
        if#(J.playerCoords-p[1])>45.0 then 
            ...
            if not c[o][3]then 
                g=true
                TriggerServerEvent('Polar:requestCurrentProgressServer',o,p[1])
                c[o][3]=true 
            end 
        end 
    end 
end
tPolar.createThreadOnTick(func_checkDjSongs)
RegisterNUICallback('returnProgress',function(K,L)
    if K.progress~=nil then 
        if K.progress~=0 then 
            TriggerServerEvent("Polar:returnProgressServer",d,e,K.progress)
        end 
    end
    L('return')
end)
RegisterNetEvent("Polar:returnProgress")
AddEventHandler("Polar:returnProgress",function(s,coords,M,N)
    if g then 
        if specificSongPlaying(coords)then 
            c[s]=nil
            c[s]={coords,s,true}
