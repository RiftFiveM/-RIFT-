...
function tPolar.toggleNoclip()
    noclipActive = not noclipActive
    if IsPedInAnyVehicle(tPolar.getPlayerPed(), false) then
        c = GetVehiclePedIsIn(tPolar.getPlayerPed(), false)
    else
        c = tPolar.getPlayerPed()
    end
    ...
    if noclipActive then
        SetEntityVisible(tPolar.getPlayerPed(), false, false)
        tPolar.setRedzoneTimerDisabled(true)
    else
        SetEntityVisible(tPolar.getPlayerPed(), true, false)
...
Citizen.CreateThread(function()
    ...
    while true do
        if noclipActive then
            ...
            local o = 0.0
            local p = 0.0
            local r, s, t = table.unpack(tPolar.getPosition())
            local u, v, w = tPolar.getCamDirection()
...
RegisterCommand('staffmode', function()
    if tPolar.getStaffLevel() > 0 then
        tPolar.staffMode(not staffMode)
    end
end)
...
local a = {}
function tPolar.staffMode(status)
    if tPolar.getStaffLevel()>0 then
        if staffMode ~= status then
            ...
            if staffMode then 
                ...
                tPolar.setCustomization(a)
                tPolar.notify('~g~Staff Powerz Deactivated.')
            end 
        end
    end
end
...
RegisterNetEvent('Polar:sendTicketInfo')
AddEventHandler('Polar:sendTicketInfo', function(permid, name)
...
RegisterCommand("fix", function()
    if tPolar.isStaffedOn() or tPolar.getStaffLevel() >= 6 then
...
AddEventHandler("wk:fixVehicle", function()
    ...
    if IsPedInAnyVehicle(p) then
        ...
        SetVehicleFixed(q)
        tPolar.notify('~g~Fixed Vehicle')
    end
end)


function tPolar.staffBlips(P)
    if tPolar.getStaffLevel() >= 6 then
        d = P
        if d then
            tPolar.notify("~g~Blips enabled")
        else
            tPolar.notify("~r~Blips disabled")
            for Q, R in ipairs(GetActivePlayers()) do
                local S = GetPlayerPed(R)
                if GetPlayerPed(R) ~= tPolar.getPlayerPed() then
                    S = GetPlayerPed(R)
        ...
        if d then
            for Q, R in ipairs(GetActivePlayers()) do
                ...
                if I ~= PlayerPedId() then
                    ...
                    local U = tPolar.clientGetUserIdFromSource(T)
                    if not DoesBlipExist(blip) and not tPolar.isUserHidden(U) then
...
function tPolar.hasStaffBlips()
    return d
end

globalIgnoreDeathSound = false
RegisterNetEvent("Polar:deathSound",function(E)
    local F = GetEntityCoords(tPolar.getPlayerPed())
    local G = #(F - E)
    if not globalIgnoreDeathSound and G <= 15 then
        SendNUIMessage({transactionType = tPolar.getDeathSound()})
    end
end)