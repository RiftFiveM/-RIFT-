...
function tPolar.isPlayerInDrone()
    return c.active
end
local function d()
    ...
    if c.cameraEnabled then
        ...
        local f = 500 * math.ceil(#(tPolar.getPlayerCoords() - GetEntityCoords(c.vehicleHandle)) / 10)
        ...
        local g = tPolar.loadModel("prop_cs_tablet")
        c.tablet = CreateObject(g, tPolar.getPlayerCoords(), true)
        ...
        tPolar.loadAnimDict("amb@code_human_in_bus_passenger_idles@female@tablet@idle_a")
...
RegisterNetEvent("toggleDrone",function()
    if not c.active then
        if tPolar.getPlayerVehicle() == 0 and not tPolar.isInComa() then
            TriggerServerEvent("Polar:performDroneLog", "Powered on their DJI Inspire Drone")
            newDrone()
        else
            tPolar.notify("You are not able to use a drone right now.")
        end
    else
        TriggerServerEvent("Polar:performDroneLog", "Powered off their DJI Inspire Drone")
        deleteDrone()
    end
end)
Citizen.CreateThread(function()
    ...
    while true do
        if c.active then
            ...
            if not NetworkHasControlOfEntity(c.driverHandle) or not NetworkHasControlOfEntity(c.vehicleHandle) then
                tPolar.notify("You have ~b~lost control ~w~of the drone, after going out of range")
                deleteDrone()
            end
...
Citizen.CreateThread(function()
    while true do
        local p = tPolar.getPlayerVehicle()
        if p ~= 0 then
            local q = GetEntityModel(p)
            if q == GetHashKey(a) then
                o = o + 1
                if o > 3 then
                    tPolar.notify("~r~Access Denied~w~: Engaging defences in 3, 2, 1...")
                    Wait(500)
                    local r = tPolar.getPlayerCoords()
                    AddOwnedExplosion(tPolar.getPlayerPed(), r.x, r.y, r.z, 4, 17.0, true, false, 10.0)
                    ...
                    if tPolar.getPlayerVehicle() ~= nil and DoesEntityExist(tPolar.getPlayerVehicle()) then
                        DeleteEntity(tPolar.getPlayerVehicle())
                    end
                end
            end
        end
...
function newDrone()
    local g = tPolar.loadModel(a)
    local v = tPolar.loadModel("s_m_m_bouncer_01")
    local e = tPolar.getPlayerPed()
    local r = GetOffsetFromEntityInWorldCoords(e, 0.0, 4.0, 0.0)
    local w = GetEntityHeading(e)
    c.vehicleHandle = tPolar.spawnVehicle(a, r, w, true)
    SetModelAsNoLongerNeeded(g)
    SetEntityNoCollisionEntity(tPolar.getPlayerPed(), c.vehicleHandle, 1)
    Wait(1000)
    ...
    c.active = true
    tPolar.notify("Drone ~b~created~w~.")
    Wait(1000)
    if globalNHSOnDuty then
        tPolar.notify("~y~Reminder~w~: These drones are reserved for usage by HEMS, Specialised Off Road Unit")
        tPolar.notify("and Senior Pilots only. If you are not in these divisions, you ~r~should not ~r~be using this.")
    end
    tPolar.notify("To remove your drone, use ~b~/drone")
end
function deleteDrone()
    ...
    tPolar.notify("Drone ~b~removed~w~.")
end
...
local function B()
    ...
    if p == 0 then
        if z then
            if #(z - tPolar.getPlayerCoords()) > 20.0 then
                local C = PlayerPedId()
    ...
    if #(z - D) > 20.0 then
        ...
        if IsDisabledControlJustPressed(0, 23) or IsDisabledControlJustPressed(0, 75) then
            tPolar.notify("You must be near the ~y~start position~w~ to exit.")
        end
    end
end
tPolar.createThreadOnTick(B)
