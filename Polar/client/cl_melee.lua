...
RegisterCommand("togglemelee",function()
    if tPolar.getUserId()==1 or tPolar.getUserId()==2 then 
        c = not c
        tPolar.notify('Melee Combat: '..(c and "~g~Enabled" or "~r~Disabled"))
    end 
end)
function tPolar.enablePunching(d)
    c = d
end
...
AddEventHandler("Polar:onClientSpawn",function(h, i)
    if i then
        ...
        for n, o in pairs(a) do
            tPolar.createArea("boxing_" .. n, o.pos, o.radius, 6, j, l, m, {})
        end
    end
end)
...
AddEventHandler("Polar:onClientSpawn",function(h, i)
    if i then
        ...
        for n, u in pairs(t) do
            tPolar.addMarker(u.x, u.y, u.z, 1.0, 1.0, 1.0, 0, 0, 255, 100, 50, 27, false, false, true)
            tPolar.createArea(n .. "_boxingGloves", u, 1.5, 6, j, l, m, {})
        end
    end
end)
local v = {}
function putGloves()
    local w = tPolar.getPlayerPed()
    local x = tPolar.loadModel("prop_boxing_glove_01")
