...
RMenu.Add('dvsa','main',RageUI.CreateMenu("","Driver & Vehicle Standards Agency", tPolar.getRageUIMenuWidth(), tPolar.getRageUIMenuHeight(), "banners", "dvsa"))
RMenu.Add('dvsa','licence',RageUI.CreateSubMenu(RMenu:Get('dvsa','main'),"","DVSA: ~b~Driving Licence", tPolar.getRageUIMenuWidth(), tPolar.getRageUIMenuHeight()))
RMenu.Add('dvsa','tests',RageUI.CreateSubMenu(RMenu:Get('dvsa','main'),"","DVSA: ~b~Driving Tests", tPolar.getRageUIMenuWidth(), tPolar.getRageUIMenuHeight()))
RMenu.Add('dvsa','alerts',RageUI.CreateSubMenu(RMenu:Get('dvsa','main'),"","DVSA: ~b~Alerts", tPolar.getRageUIMenuWidth(), tPolar.getRageUIMenuHeight()))
local a={}
local b={}
local c={}
local d={}
...
tPolar.addMarker(i.test.reception.x,i.test.reception.y,i.test.reception.z-0.96,1.2,1.2,1.2,0,255,125,125,50,27,true,false,false,nil,nil,0.0,0.0,0.0)
local j=tPolar.addBlip(i.test.reception.x,i.test.reception.y,i.test.reception.z,523,47,"DVSA Test Centre",1.0,false)
local k=function()
...
tPolar.createArea("dvsaTestCentre_",i.test.reception,1.5,6,k,l,m,{})
RegisterNetEvent("Polar:dvsaData",function(n,o,p,q)
    h=true
    a=n
    b=o
    c=p
    ...
    for r,s in pairs(i.peds)do 
        if not s.eup then 
            ...
            local ped=tPolar.createDynamicPed(s.model,t,s.heading,true,"mini@strip_club@idles@bouncer@base","base",30,false,nil)
        end 
    end
    tPolar.loadModel(i.models.camera)
...
RegisterNetEvent("Polar:updateDvsaData",function(n,o,p,q)
    if n~=nil then 
        a=n 
    end
    if o~=nil then 
...
RegisterNetEvent("Polar:dvsaMessage",function(w,x,y)
    tPolar.notifyPicture(i.images.dict,i.images.govLarge,y,w,x)
end)
RegisterNetEvent("Polar:beginTestClient",function(z,A)
    if z then 
        TriggerServerEvent("Polar:dvsaBucket",false)
        currentTest={active=false,ped=0,vehicle=0,parkingSpace=0,route=0,waypoint=0,blip=0,serious=0,minors=0,minorsReason={},seriousReason={},subtitle=""}
        ...
        Wait(1000)
        currentTest.vehicle=tPolar.spawnVehicle(i.test.modelName,i.test.parkingSpaces[A].coords.x,i.test.parkingSpaces[A].coords.y,i.test.parkingSpaces[A].coords.z,i.test.parkingSpaces[A].heading,true,true)
        ...
        local B=tPolar.loadModel(i.test.examinerModel)
        ...
        Wait(9000)
        TriggerEvent("Polar:PlaySound", "welcomedvsa")
        currentTest.subtitle="Hey! I'm ~y~Lloyd Shettle"
        tPolar.notifyPicture(i.images.dict,i.images.govLarge,"Your assigned examiner for the test is Lloyd.","DVSA","UK Government")
        tPolar.loadAnimDict("amb@medic@standing@tendtodead@base")
        ...
        Wait(4000)
        tPolar.notifyPicture(i.images.dict,i.images.govLarge,"You'll now be given information about the test, listen carefully","DVSA","UK Government")
        initialMoveOff()
    else 
        tPolar.notifyPicture(i.images.dict,i.images.govLarge,"We currently have no test availability, please try again shortly.","Callum","Driving Examiner")
...
function initialMoveOff()
    TriggerEvent("Polar:PlaySound", "testExplained")
...
function handleTestRoute()
    local C=false
    currentTest.blip=tPolar.addBlip(i.test.routes[currentTest.route][currentTest.waypoint+1].coords.x,i.test.routes[currentTest.route][currentTest.waypoint+1].coords.y,i.test.routes[currentTest.route][currentTest.waypoint+1].coords.z,0,38,"Waypoint",1.0,false)
    ...
    currentTest.marker=tPolar.addMarker(i.test.routes[currentTest.route][currentTest.waypoint+1].coords.x,i.test.routes[currentTest.route][currentTest.waypoint+1].coords.y,i.test.routes[currentTest.route][currentTest.waypoint+1].coords.z,1.6,1.6,1.6,0,89,255,200,50,36,true,true)
    ...
    while currentTest.active do 
        if not C then 
            ...
            local E=#(tPolar.getPlayerCoords()-i.test.routes[currentTest.route][currentTest.waypoint+1].coords)
            ...
            if F>i.test.routes[currentTest.route][currentTest.waypoint+1].limit+19.0 then 
                if not i.test.routes[currentTest.route][currentTest.waypoint+1].speeding then 
                    ...
                    if F>i.test.routes[currentTest.route][currentTest.waypoint+1].limit+85.0 then 
                        ...
                        TriggerEvent("Polar:PlaySound", "slowDownOrTermination")
            ...
            if E<3.5 then 
                ...
                if currentTest.waypoint>=#(i.test.routes[currentTest.route])then 
                    ...
                    if currentTest.blip~=0 then 
                        tPolar.removeBlip(currentTest.blip)
                    end
                    if currentTest.marker~=0 then 
                        tPolar.removeMarker(currentTest.marker)
                    end
                    returnToTestCentre()
                else 
...
function policeChase()
    TriggerEvent("Polar:PlaySound", "policePursuitContinue")
    tPolar.notifyPicture(i.images.dict,i.images.govLarge,"You are required to ~b~move over ~w~to allow a police pursuit to continue.","DVSA","UK Government")
    currentTest.subtitle="Move ~y~over"
    local G='blista'
    tPolar.loadModel(G)
    local H=vector3(113.0901,-1226.426,37.60364)
    local I=tPolar.spawnVehicle(G,H,270.93,false,true)
    ...
    tPolar.loadModel(M)
    local N=tPolar.spawnVehicle(M,H,270.93,false,true)
...
function returnToTestCentre()
    ...
    currentTest.blip=tPolar.addBlip(i.test.parkingSpaces[currentTest.parkingSpace].coords.x,i.test.parkingSpaces[currentTest.parkingSpace].coords.y,i.test.parkingSpaces[currentTest.parkingSpace].coords.z,0,81,"Waypoint",1.0,false)
    currentTest.marker=tPolar.addMarker(i.test.parkingSpaces[currentTest.parkingSpace].coords.x,i.test.parkingSpaces[currentTest.parkingSpace].coords.y,i.test.parkingSpaces[currentTest.parkingSpace].coords.z,1.2,1.2,1.2,0,255,125,125,50,0,true,true)
    SetBlipRoute(currentTest.blip,true)
    SetBlipRouteColour(currentTest.blip,38)
    TriggerEvent("Polar:PlaySound", "newDestinationSet")
    currentTest.subtitle="I've set a ~y~new sat nav destination~w~, please follow it"
    Wait(4000)
    currentTest.subtitle="Follow the ~y~sat nav"
    local T=false
    while true do 
        local E=#(tPolar.getPlayerCoords()-i.test.parkingSpaces[currentTest.parkingSpace].coords)
        if E<15.0 then 
...
function finishTest(S)
    ...
    useTablet()
    TriggerEvent("Polar:PlaySound", "completePaperwork")
    ...
    local V=""
    if S then 
        TriggerEvent("Polar:PlaySound", "testPassed")
        tPolar.notifyPicture(i.images.dict,i.images.govLarge,"You ~b~passed your test with ~y~"..currentTest.minors.." ~w~minors","UK Government","DVSA")
        ...
        Wait(4000)
        TriggerEvent("Polar:PlaySound", "testPassedGoodbye")
        ...
        TriggerEvent("Polar:PlaySound", "testFailed")
        tPolar.notifyPicture(i.images.dict,i.images.govLarge,"Unfortunately you have ~r~failed your test","UK Government","DVSA")
        currentTest.subtitle="Unfortunately you have ~y~failed ~w~your driving test"
        Wait(6000)
        tPolar.notifyPicture(i.images.dict,i.images.govLarge,"Your ~r~serious ~w~faults were:","UK Government","DVSA")
        Wait(3000)
        TriggerEvent("Polar:PlaySound", "seriousFaults")
        for r,s in pairs(currentTest.seriousReason)do 
            V=V..", "..currentTest.seriousReason[r]
            tPolar.notify("~r~Serious Fault~w~: "..currentTest.seriousReason[r])
            Wait(500)
        end
        tPolar.notifyPicture(i.images.dict,i.images.govLarge,"Your ~y~minor ~w~faults were:","UK Government","DVSA")
        Wait(3000)
        TriggerEvent("Polar:PlaySound", "minorFaults")
        for r,s in pairs(currentTest.minorsReason)do 
            U=U..", "..currentTest.minorsReason[r]
            tPolar.notify("~y~Minor Fault~w~: "..currentTest.minorsReason[r])
            Wait(500)
        end
        Wait(4000)
        TriggerEvent("Polar:PlaySound", "testFailedGoodbye")
    ...
    if tPolar.teleport~=nil then 
        tPolar.teleport(i.test.finishTestTpCoords.x,i.test.finishTestTpCoords.y,i.test.finishTestTpCoords.z)
    end
    Wait(3000)
    SetTimeout(20000,function()
        currentTest.requested=false 
    end)
    TriggerServerEvent("Polar:dvsaBucket",true)
    if S then 
        TriggerServerEvent("Polar:candidatePassed",currentTest.serious,currentTest.minors,U)
        currentTest.active=false 
    else 
        TriggerServerEvent("Polar:candidateFailed",currentTest.serious,currentTest.minors,V,U)
        currentTest.active=false 
    end
    if currentTest.blip~=0 then 
        tPolar.removeBlip(currentTest.blip)
...
RegisterNetEvent("Polar:phoneToggledDvsa",function()
...
function pullUpOnRight()
    ...
    tPolar.notifyPicture(i.images.dict,i.images.govLarge,"Listen carefully for directions","Controlled Stop","DVSA")
    local T=false
    local Y=false
    TriggerEvent("Polar:PlaySound", "controlledStop")
    ...
    TriggerEvent("Polar:PlaySound", "moveOffWhenReady")
    Wait(2000)
    currentTest.subtitle="Follow the ~y~sat nav"
    useTablet()
end
function operateHeadlights()
    PlaySoundFrontend(-1,"Out_Of_Bounds_Timer","DLC_HEISTS_GENERAL_FRONTEND_SOUNDS",1)
    tPolar.notifyPicture(i.images.dict,i.images.govLarge,"Listen carefully for directions","Show Me Question","DVSA")
    currentTest.subtitle="I will now ask you one ~y~show me question ~w~while driving"
    TriggerEvent("Polar:PlaySound", "askShowMeQuestion")
    Wait(6000)
    local T=false
    local Z=false
    TriggerEvent("Polar:PlaySound", "operateMainBeamHeadlights")
    currentTest.subtitle="When it is ~y~safe ~w~to do so, show me how you'd ~y~operate the main beam headlights."
    tPolar.notifyPicture(i.images.dict,i.images.govLarge,"Turn on your headlights as requested","Show Me Question","DVSA")
    SetTimeout(20000,function()
        T=true 
    end)
    local _=false
    ...
    TriggerEvent("Polar:PlaySound", "continueToFollow")
    currentTest.subtitle="Thank you, please continue to follow the sat nav"
    Wait(2000)
    tPolar.notifyPicture(i.images.dict,i.images.govLarge,"Thank you, the show me question is complete","Show Me Question","DVSA")
    currentTest.subtitle="Follow the ~y~sat nav"
end
function stopSignDetection()
    PlaySoundFrontend(-1,"Out_Of_Bounds_Timer","DLC_HEISTS_GENERAL_FRONTEND_SOUNDS",1)
    TriggerEvent("Polar:PlaySound", "stopSign")
    tPolar.notifyPicture(i.images.dict,i.images.govLarge,"You are legally required to stop at this sign","Stop Sign","DVSA")
    local a2=false
    local a3=false
    Citizen.CreateThread(function()
...
function emergencyStop()
    PlaySoundFrontend(-1,"Out_Of_Bounds_Timer","DLC_HEISTS_GENERAL_FRONTEND_SOUNDS",1)
    TriggerEvent("Polar:PlaySound", "emergencyStopIntroduction")
    tPolar.notifyPicture(i.images.dict,i.images.govLarge,"Listen carefully for directions","Test Instruction","DVSA")
    ...
    TriggerEvent("Polar:PlaySound", "stopNowMessage")
    currentTest.subtitle="~r~STOP ~w~- Perform an emergency stop"
    tPolar.notifyPicture(i.images.dict,i.images.govLarge,"Perform an emergency stop immediately","STOP","DVSA")
    Wait(3000)
    if not GetEntitySpeed(currentTest.vehicle)==0.0 then 
...
function useTablet()
    local B=tPolar.loadModel('prop_cs_tablet')
...
function cameraTransition(t)
    ...
    local ped=tPolar.getPlayerPed()
    FreezeEntityPosition(ped,true)
    local a5=tPolar.getPlayerCoords()
                ...
                if not currentTest.active and not a.full and a.active then 
                    if not currentTest.requested then 
                        RageUI.Button("Begin driving test","Begin your driving test",{RightBadge=RageUI.BadgeStyle.Alert,RightLabel="£"..getMoneyStringFormatted(i.test.price)},true,function(a8,a9,aa)
                            if aa then 
                                if a.banned then 
                                    tPolar.notifyPicture(i.images.dict,i.images.govLarge,"Your licence has been suspended","UK Government","DVSA")
                                else 
                                    if not currentTest.requested then 
                                        TriggerServerEvent("Polar:beginTest")
                        ...
                        RageUI.Button("No tests available","Try again in a few minutes",{RightBadge=RageUI.BadgeStyle.Alert},true,function(a8,a9,aa)
                            if aa then 
                                tPolar.notifyPicture(i.images.dict,i.images.govLarge,"We have no tests available, please try again in a few minutes.","UK Government","Driving Test")
                            end 
                        end)
                    end 
                else 
                    if not currentTest.active and (a.full or a.active) then 
                        RageUI.Button("Surrender your licence","Surrender your driving licence to the DVSA",{RightBadge=RageUI.BadgeStyle.Alert},true,function(a8,a9,aa)
                            if aa then 
                                a.full=false
                                a.active=false
                                tPolar.notifyPicture(i.images.dict,i.images.govLarge,"You've surrendered your licence. Your points and offences will remain held.","UK Government","Licence Surrendered")
                                TriggerServerEvent("Polar:surrenderLicence")
                            end 
                        end)
                    end 
                end 
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            if a.full then 
                RageUI.Button("Licence Type:","This indiates if you hold a full licence",{RightLabel="Full",RightBadge=RageUI.BadgeStyle.Car},true,function(a8,a9,aa)
                    if aa then 
                        tPolar.notifyPicture(i.images.dict,i.images.govLarge,"Your licence is full. You have passed your driving test.","UK Government","DVSA")
                    end 
                end)
            else 
                if a.banned then 
                    ...
                    if a.active then 
                        RageUI.Button("Licence Type:","This indiates if you hold a full licence",{RightLabel="Provisional",RightBadge=RageUI.BadgeStyle.Alert},true,function(a8,a9,aa)
                            if aa then 
                                tPolar.notifyPicture(i.images.dict,i.images.govLarge,"Your licence is currently provisional, take a test at the DVSA test centre","UK Government","DVSA")
                            end 
                        end)
                    else 
                        RageUI.Button("Licence Type:","This indiates if you hold a full licence",{RightLabel="No licence",RightBadge=RageUI.BadgeStyle.Alert},true,function(a8,a9,aa)
                            if aa then 
                                tPolar.notifyPicture(i.images.dict,i.images.govLarge,"You do not hold a UK Driving licence.","UK Government","DVSA")
                            end 
                        end)
                        if not f then 
                            RageUI.Button("Apply for a provisional licence","Apply for a provisional licence",{RightBadge=RageUI.BadgeStyle.Car},true,function(a8,a9,aa)
                                if aa then 
                                    TriggerServerEvent("Polar:activateLicence")
                                    f=true
                                    tPolar.notifyPicture(i.images.dict,i.images.govLarge,"We will process your application shortly. Thank you for submitting.","UK Government","DVSA")
                                end 
                            end)
                        end 
                    end 
            ...
            if a.active then 
                RageUI.Button("Penalty Points:","This indicates your amount of licence points",{RightLabel=a.points},true,function(a8,a9,aa)
                    if aa then 
                        tPolar.notifyPicture(i.images.dict,i.images.govLarge,"Your licence currently has "..a.points.." penalty points.","UK Government","DVSA")
                    end 
                end)
                RageUI.Button("Licence Number:","This indicates your licence number",{RightLabel=a.id},true,function(a8,a9,aa)
                    if aa then 
                        tPolar.notifyPicture(i.images.dict,i.images.govLarge,"Your licence number is "..a.id..", this is issued with your licence.","UK Government","DVSA")
                    end 
                end)
                RageUI.Button("Licence Issued:","This indicates the date and time of issue",{RightLabel=a.date},true,function(a8,a9,aa)
                    if aa then 
                        tPolar.notifyPicture(i.images.dict,i.images.govLarge,"Your licence was issued at "..a.date..".","UK Government","DVSA")
                    end 
                end)
                if next(b) then 
                    ...
                    for r,s in pairs(b)do 
                        RageUI.Button(s.offence,"Date: "..s.date.." | Type: "..s.type,{RightBadge=RageUI.BadgeStyle.Alert},true,function(a8,a9,aa)
                            if aa then 
                                tPolar.notifyPicture(i.images.dict,i.images.govLarge,"You were given "..s.points.." penalty points on "..s.date..".","UK Government","Offence: "..s.offence)
                            end 
                        end)
                    end 
                end 
            ...
            if c~=nil and next(c) then 
                for r,s in pairs(c)do 
                    ...
                    RageUI.Button(s.date.." | Result: "..ac,"~r~Serious Faults~w~: "..s.serious.." | ~b~Minor Faults~w~: "..s.minor,{RightBadge=RageUI.BadgeStyle.Alert},true,function(a8,a9,aa)
                        if aa then 
                            tPolar.notifyPicture(i.images.dict,i.images.govLarge,"Your test result was a "..ac.." ~w~with ~b~"..s.serious.." ~w~serious faults and ~b~"..s.minor.." ~w~minor faults.","UK Government","Driving Test Result")
                            if s.minor~=nil and s.pass and s.minor>0 then 
                                tPolar.notifyPicture(i.images.dict,i.images.govLarge,"You received ~b~"..s.minor.." ~w~minors","UK Government","Driving Test Result")
                            elseif not s.pass then 
                                if s.seriousReason~=nil then 
                                    tPolar.notifyPicture(i.images.dict,i.images.govLarge,"Your serious faults were: ~b~"..s.seriousReason,"~w~UK Government","Driving Test Result")
                                end
                                if s.minor>0 then 
                                    if s.minorsReason~=nil then 
                                        tPolar.notifyPicture(i.images.dict,i.images.govLarge,"You received minors for ~b~"..s.minorsReason,"UK Government","Driving Test Result")
                                    end 
                                end 
                            end 
                        end 
            ...
            if next(d) then
                for r,s in pairs(d)do 
                    ...
                    RageUI.Button(s.title.." "..s.date,"Press to read this message",{RightBadge=RageUI.BadgeStyle.Alert},true,function(a8,a9,aa)
                        if aa then 
                            tPolar.notifyPicture(i.images.dict,i.images.govLarge,s.message,"UK Government",s.title)
                        end 
                    end)
                end
            else
...
Citizen.CreateThread(function()
    while true do 
        ...
        if not a.full and not a.banned and not currentTest.active then 
            local vehicle=tPolar.getPlayerVehicle()
            if vehicle~=0 and GetPedInVehicleSeat(vehicle,-1)==tPolar.getPlayerPed()then 
...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        if vehicle~=0 and GetPedInVehicleSeat(vehicle,-1)==tPolar.getPlayerPed()then 
            ...
            local t=tPolar.getPlayerCoords()
            if t~=nil then 
                for r,s in pairs(i.cameras)do 
...
RegisterNetEvent("Polar:speedCameraFlash",function(t,aj)
    for ak=1,2 do 
        local T=false
        Citizen.SetTimeout(450,function()
            T=true 
...
function flashCamera(al)
    ...
    TriggerServerEvent("Polar:speedCameraFlashServer",math.ceil(GetEntitySpeed(GetVehiclePedIsIn(tPolar.getPlayerPed(), false))*2.2369))
...
RegisterNetEvent("Polar:deletePlate",function(ao)
    local ap=tPolar.getObjectId(ao)
...
function createPlate(vehicle)
    e.active=true
    e.vehicle=vehicle
    local B=tPolar.loadModel('prop_lplate')
    local t=tPolar.getPlayerCoords()
    ...
    tPolar.notifyPicture(i.images.dict,i.images.lPlate,i.notifications.lPlatesAdded,"DVSA","Licence Services")
    tPolar.notifyPicture(i.images.dict,i.images.lPlate,i.notifications.lPlatesAdded2,"DVSA","Licence Services")
end
function makeVehicleCrash(as,J,H,L,at)
    tPolar.loadModel(as)
    local I=tPolar.spawnVehicle(as,H,at,true,true)
    while not DoesEntityExist(I)do 
        Wait(0)
    end
