...
RegisterCommand("flagf",function(o, p)
    if c ~= "" then
        ...
        if table.count(p) ~= 0 then
            q = table.concat(p, " ")
            TriggerServerEvent("Polar:flagVehicleAnpr", string.upper(c), q)
        else
            tPolar.notify("~r~Error~w~: You must provide a valid flag reason.")
        end
    end
end)
RegisterCommand("flagr",function(o, p)
    if d ~= "" then
        ...
        if table.count(p) ~= 0 then
            q = table.concat(p, " ")
            TriggerServerEvent("Polar:flagVehicleAnpr", string.upper(d), q)
        else
            tPolar.notify("~r~Error~w~: You must provide a valid flag reason.")
        end
    end
...
tPolar.createThreadOnTick(func_drawFullAnpr)
function anprGetVehicle(u)
    ...
    local w = GetVehiclePedIsIn(tPolar.getPlayerPed(), false)
    ...
    if B > 0 and IsEntityAVehicle(B) then
        ...
        if C ~= nil then
            if m[C] ~= nil then
                ...
                tPolar.notify(string.format("~r~ANPR Alert~w~: Plate ~b~%s ~w~is flagged for:~b~~n~%s", C, D))
                n = true
...
function isPedInPoliceVehicle()
    if IsPedInAnyVehicle(tPolar.getPlayerPed(), false) then
        if GetVehicleClass(tPolar.getPlayerVehicle()) == 18 and globalOnPoliceDuty then
            return true
        end
    end
    return false
...
RegisterNetEvent("Polar:setFlagVehicles",function(H)
    m = H
end)