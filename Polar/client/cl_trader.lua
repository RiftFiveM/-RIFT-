...
RegisterNetEvent("Polar:updateTraderCommissions",function(d, e, f, g, h, i, j)
...
Citizen.CreateThread(function()
    local k = tPolar.loadAnimDict("mini@strip_club@idles@bouncer@base")
    for l, m in pairs(b.trader) do
        tPolar.addMarker(m.position.x,m.position.y,m.position.z,0.7,0.7,0.5,m.colour.r,m.colour.g,m.colour.b,125,50,29,true,true)
        tPolar.createDynamicPed(m.dealerModel,m.dealerPos + vector3(0.0, 0.0, -1.0),m.dealerHeading,true,"mini@strip_club@idles@bouncer@base","base",100,false)
    end
end)

RMenu.Add("trader","seller",RageUI.CreateMenu("","~b~Sell your goods!",tPolar.getRageUIMenuWidth(),tPolar.getRageUIMenuHeight(),"banners","seller",0,0,255,255))
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if a == "Legal" then
                RageUI.ButtonWithStyle("Sell Copper","£" .. getMoneyStringFormatted(c["Copper"]),{RightLabel = "→→→"},true,function(m, n, o)
                    if o then
                        if GetVehiclePedIsIn(tPolar.getPlayerPed(), false) == 0 then
                            TriggerServerEvent("Polar:sellCopper")
                        else
                            tPolar.notify("~r~Exit your vehicle.")
                        end
                    end
                ...
                RageUI.ButtonWithStyle("Sell Limestone","£" .. getMoneyStringFormatted(c["Limestone"]),{RightLabel = "→→→"},true,function(m, n, o)
                    if o then
                        if GetVehiclePedIsIn(tPolar.getPlayerPed(), false) == 0 then
                            TriggerServerEvent("Polar:sellLimestone")
                        else
                            tPolar.notify("~r~Exit your vehicle.")
                        end
                    end
                ...
                RageUI.ButtonWithStyle("Sell Gold","£" .. getMoneyStringFormatted(c["Gold"]),{RightLabel = "→→→"},true,function(m, n, o)
                    if o then
                        if GetVehiclePedIsIn(tPolar.getPlayerPed(), false) == 0 then
                            TriggerServerEvent("Polar:sellGold")
                        else
                            tPolar.notify("~r~Exit your vehicle.")
                        end
                    end
                ...
                RageUI.ButtonWithStyle("Sell Diamond","£" .. getMoneyStringFormatted(c["Diamond"]),{RightLabel = "→→→"},true,function(m, n, o)
                    if o then
                        if GetVehiclePedIsIn(tPolar.getPlayerPed(), false) == 0 then
                            TriggerServerEvent("Polar:sellDiamond")
                        else
                            tPolar.notify("~r~Exit your vehicle.")
                        end
                    end
                ...
                RageUI.ButtonWithStyle("Sell All","",{RightLabel = "→→→"},true,function(m, n, o)
                    if o then
                        if GetVehiclePedIsIn(tPolar.getPlayerPed(), false) == 0 then
                            TriggerServerEvent("Polar:sellAll")
                        else
                            tPolar.notify("~r~Exit your vehicle.")
                        end
                    end
            ...
            elseif a == "Weed" then
                RageUI.ButtonWithStyle("Sell Weed","£" .. getMoneyStringFormatted(c["Weed"] or 0),{RightLabel = "→→→"},true,function(m, n, o)
                    if o then
                        if GetVehiclePedIsIn(tPolar.getPlayerPed(), false) == 0 then
                            TriggerServerEvent("Polar:sellWeed")
                        else
                            tPolar.notify("~r~Exit your vehicle.")
                        end
                    end
            ...
            elseif a == "Cocaine" then
                RageUI.ButtonWithStyle("Sell Cocaine","£" .. getMoneyStringFormatted(c["Cocaine"] or 0),{RightLabel = "→→→"},true,function(m, n, o)
                    if o then
                        if GetVehiclePedIsIn(tPolar.getPlayerPed(), false) == 0 then
                            TriggerServerEvent("Polar:sellCocaine")
                        else
                            tPolar.notify("~r~Exit your vehicle.")
                        end
                    end
            ...
            elseif a == "Meth" then
                RageUI.ButtonWithStyle("Sell Meth","£" .. getMoneyStringFormatted(c["Meth"] or 0),{RightLabel = "→→→"},true,function(m, n, o)
                    if o then
                        if GetVehiclePedIsIn(tPolar.getPlayerPed(), false) == 0 then
                            TriggerServerEvent("Polar:sellMeth")
                        else
                            tPolar.notify("~r~Exit your vehicle.")
                        end
                    end
            ...
            elseif a == "Heroin" then
                RageUI.ButtonWithStyle("Sell Heroin","£" .. getMoneyStringFormatted(c["Heroin"] or 0),{RightLabel = "→→→"},true,function(m, n, o)
                    if o then
                        if GetVehiclePedIsIn(tPolar.getPlayerPed(), false) == 0 then
                            TriggerServerEvent("Polar:sellHeroin")
                        else
                            tPolar.notify("~r~Exit your vehicle.")
                        end
                    end
            ...
            elseif a == "LSDNorth" then
                RageUI.ButtonWithStyle("Sell LSD","£" .. getMoneyStringFormatted(c["LSDNorth"] or 0),{RightLabel = "→→→"},true,function(m, n, o)
                    if o then
                        if GetVehiclePedIsIn(tPolar.getPlayerPed(), false) == 0 then
                            TriggerServerEvent("Polar:sellLSDNorth")
                        else
                            tPolar.notify("~r~Exit your vehicle.")
                        end
                    end
            ...
            elseif a == "LSDSouth" then
                RageUI.ButtonWithStyle("Sell LSD","£" .. getMoneyStringFormatted(c["LSDSouth"] or 0),{RightLabel = "→→→"},true,function(m, n, o)
                    if o then
                        if GetVehiclePedIsIn(tPolar.getPlayerPed(), false) == 0 then
                            TriggerServerEvent("Polar:sellLSDSouth")
                        else
                            tPolar.notify("~r~Exit your vehicle.")
                        end
                    end
...
AddEventHandler("Polar:onClientSpawn",function(p, q)
    if q then
        ...
        for y, l in pairs(b.trader) do
            tPolar.createArea("trader_" .. y, l.position, 1.5, 6, r, t, u, {traderId = y, traderName = l.type})
        end
    end
end)

RegisterNetEvent("Polar:updateTraderPrices",function(z, A, B, C, i, j, D, E, F, G)
    c["Weed"] = z or 0
    c["Cocaine"] = A or 0
    c["Meth"] = B or 0
    c["Heroin"] = C or 0
