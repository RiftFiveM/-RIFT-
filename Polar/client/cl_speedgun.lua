    ...
    if tPolar.globalOnPoliceDuty() then 
        if m[1]~=nil then 
            if tonumber(m[1])>100 then 
                d=tonumber(m[1])
                tPolar.notify(string.format("~g~Maximum speed set to %smph",tonumber(m[1])))
            else 
                d=101
                tPolar.notify("~r~Minimum speed you can set is 101mph!")
            end 
        end 
    else 
        tPolar.notify("~r~Speed gun is not enabled!")
    end 
end)
...
tPolar.createThreadOnTick(func_drawRadar)
Citizen.CreateThread(function()
    while true do 
        if tPolar.globalOnPoliceDuty() and not usingDelgun then
            if GetSelectedPedWeapon(tPolar.getPlayerPed())==a then 
                b=true 
            else 
                b=false 
            end
            if b and tPolar.getPlayerVehicle()==0 then 
                local n=tPolar.getPlayerId()
                if IsPlayerFreeAiming(n)then 
                    ...
                    if p~=0 and GetPedInVehicleSeat(p,-1)==e and not IsPedInAnyPlane(e) and not IsPedInAnyHeli(e) then 
                        ...
                        if k[f]~=nil then 
                            ...
                            tPolar.notify(string.format("~h~~r~Vehicle Flagged:~s~~n~Plate %s is flagged for:~n~%s",f,r))
                            soundPlayed=true
                        ...
                        if h>d and h>101 then 
                            ...
                            if not j[q]then 
                                ...
                                TriggerServerEvent("Polar:speedGunFinePlayer",GetPlayerServerId(NetworkGetPlayerIndexFromPed(e)),h-d)
                                Citizen.Wait(3000)
                            else 
                                tPolar.notify("~r~This vehicle has been fined recently!")
                            end 
                            else i="~w~"
                        end 
                    end 
...
RegisterNetEvent("Polar:speedGunPlayerFined")
AddEventHandler("Polar:speedGunPlayerFined",function()
    PlaySoundFrontend(-1,"ScreenFlash","MissionFailedSounds",1)
    StartScreenEffect("FocusOut",0,false)
    Wait(2000)
    StopScreenEffect("FocusOut")
end)
AddEventHandler("Polar:setFlagVehicles",function(s)
    k=s 
end)