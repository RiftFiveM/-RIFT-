...
local function GetVehicleMods1()	
	local player = GetPlayerPed(-1)
	local veh = GetVehiclePedIsIn(player,false)

	if veh ~= nil then
		for i=0,48, 1 do
		Citizen.Trace("Number of mods for " .. i .. " Are :" .. GetNumVehicleMods(veh,i) .."\n")
		end
	end
...
local function DriveInGarage()
	...
	if DoesEntityExist(veh) then
		...
		myveh.engineSounds = DecorGetInt(veh, "lsAudioId")
		for i = 0, 48 do
...
local function DriveOutOfGarage(pos)
	Citizen.CreateThread(function()
		...
		local ok, veh_type, model = tPolar.getNearestOwnedVehicle(1)
		myveh.model = model
		myveh.spawncode = veh_type
...
function CheckPurchases(m)
	...
	elseif name == "tank" or name == "ornaments" or name == "arch cover" or name == "aerials" or name == "roof scoops" or name == "doors" or name == "roll cage" or name == "engine block" or name == "cam cover" or name == "strut brace" or name == "trim design" or name == "dashboard" or name == "dials" or name == "seats" or name == "steering wheels" or name == "plate holder" or name == "vanity plates" or name == "shifter leavers" or name == "plaques" or name == "speakers" or name == "trunk" or name == "headlights" or name == "turbo" or  name == "hydraulics" or name == "liveries" or name == "horn" then
		for i,b in pairs(m.buttons) do
			if myveh.mods[b.modtype].mod == b.mod then
				b.sprite = "garage"
			else
...
AddEventHandler("Polar:onClientSpawn",function(D, E)
    if E then
        ...
        for q,h in pairs(garages)do 
            tPolar.addBlip(h.inside.position.x,h.inside.position.y,h.inside.position.z,72, nil, "LS Customs")
        end 
	end
end)
...
function tPolar.isInsideLsCustoms()
    return inside
end
