    ...
    while true do
        if a.active and a.id ~= 0 then
            ...
            if h ~= GetHashKey("WEAPON_UNARMED") and (not b[a.id].allowPistol or not table.has(c, h)) then
                tPolar.setWeapon(g, "WEAPON_UNARMED", true)
            end
...
RMenu.Add("policeshields","main",RageUI.CreateMenu("Police Shields", "~b~Shield Management", tPolar.getRageUIMenuWidth(), tPolar.getRageUIMenuHeight()))
...
RegisterNetEvent("Polar:toggleShieldMenu",function(l)
...
function removeShield(m)
    if DoesEntityExist(a.prop) then
        ...
        if m then
            tPolar.notify("~g~Success~w~: Shield removed")
        end
    end
end
function createShield(n)
    ...
    local o = false
    local g = tPolar.getPlayerPed()
    local p = tPolar.getPlayerCoords()
    if a.active then
        removeShield(false)
    end
    a.id = n
    local q = tPolar.loadModel(b[n].model)
    ...
    tPolar.loadAnimDict(b[n].animDict)
