...
local function checkVisibleOrStaff(n, o)
    return IsEntityVisible(n) or not tPolar.clientGetPlayerIsStaff(o)
end
...
RegisterCommand("blipson",function()
    if tPolar.globalOnPoliceDuty() or tPolar.globalNHSOnDuty() then 
        tPolar.notify('~g~Emergency blips enabled.')
        a=true 
    end 
end,false)
...
RegisterCommand("blipsoff",function()
    if tPolar.globalOnPoliceDuty() or tPolar.globalNHSOnDuty() then 
        tPolar.notify('~r~Emergency blips disabled.')
        a=false
        d()
        g()
    end 
...
RegisterNetEvent("Polar:disableFactionBlips",function()
    a=false
    tPolar.setPolice(false)
    tPolar.setHMP(false)
    tPolar.setNHS(false)
    d()
    g()
...
function tPolar.copBlips()
    return a
end
Citizen.CreateThread(function()
    while true do 
        if a or tPolar.isInComa() then 
            ...
            for e,f in ipairs(GetActivePlayers())do
                ...
                if j~=p then 
                    ...
                    if q~=-1 then 
                        local r=tPolar.clientGetUserIdFromSource(q)
                        local s=tPolar.getJobType(r)
                        if s~="" and r ~= tPolar.getUserId() then 
...
RegisterNetEvent("Polar:sendFarBlips",function(w)
    if not u and a then
        g()
        for e,x in pairs(w)do 
            if x.source~=v and GetPlayerFromServerId(x.source)== -1 and x.bucket == tPolar.getPlayerBucket() then 
...
RegisterNetEvent("Polar:Revive",function()
    if not a then
        Citizen.Wait(1000)
        d()
    end
