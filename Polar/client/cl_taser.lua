...
RegisterNetEvent("Polar:receiveBarbs",function(e)
    c = true
    d = e
end)
RegisterNetEvent("Polar:receiveActivation",function()
    if not tPolar.isStaffedOn() then
        displayTaserNotification("You are currently being ~b~activated~w~.")
        SetPedToRagdoll(tPolar.getPlayerPed(), 5000, 5000, 0, 0, 0, 0)
    end
end)
RegisterNetEvent("Polar:arcSuccessful",function()
    ...
    TriggerServerEvent("Polar:playTaserSound", GetEntityCoords(tPolar.getPlayerPed()), "arcsound")
    b = false
...
RegisterNetEvent("playTaserSoundClient",function(f, g)
    local h = GetEntityCoords(tPolar.getPlayerPed(), true)
    local i = #(h - f)
    if i <= 15 then
        SendNUIMessage({transactionType = g})
    end
end)
RegisterNetEvent("Polar:barbsNoLonger",function()
    c = false
    d = 0
    displayTaserNotification("Your taser ~b~barbs ~w~have been ~b~disconnected~w~.")
end)
RegisterNetEvent("Polar:barbsRippedOut",function(e)
...
RegisterNetEvent("Polar:reloadTaser",function(j)
    if GetSelectedPedWeapon(tPolar.getPlayerPed()) == GetHashKey(a.hash) then
        MakePedReload(tPolar.getPlayerPed())
        if a.cartridges == 0 then
            a.cartridges = 2
            if a.reactivate1.active then
                TriggerServerEvent("Polar:barbsNoLongerServer", a.reactivate1.serverId)
            end
            if a.reactivate2.active then
                TriggerServerEvent("Polar:barbsNoLongerServer", a.reactivate2.serverId)
            end
            a.reactivate1 = {active = false, ped = 0, serverId = 0}
            a.reactivate2 = {active = false, ped = 0, serverId = 0}
        ...
        if k.weapon == GetHashKey(a.hash) then
            if IsControlJustPressed(1, 308) then
                if a.reactivate1.active == true then
                    ...
                    TriggerServerEvent("Polar:playTaserSound", k.playerCoords, "reactivate")
                    TriggerServerEvent("Polar:reactivatePed", a.reactivate1.serverId)
                end
            elseif IsControlJustPressed(1, 307) then
                if a.reactivate2.active == true then
                    ...
                    TriggerServerEvent("Polar:playTaserSound", k.playerCoords, "reactivate")
                    TriggerServerEvent("Polar:reactivatePed", a.reactivate2.serverId)
                end
            elseif IsControlJustPressed(1, 121) and b then
                TriggerServerEvent("Polar:arcTaser")
            end
            if a.cartridges < 1 then
                ...
                if IsPedShooting(k.playerPed) then
                    TriggerServerEvent("Polar:playTaserSound", k.playerCoords, "taser")
                    a.cartridges = a.cartridges - 1
                    local l, m = GetEntityPlayerIsFreeAimingAt(k.playerId)
                    if l and IsEntityAPed(k.playerPed) then
                        local n = tPolar.getPedServerId(m)
                        TriggerServerEvent("Polar:receiveBarbsServer", n)
                        if n ~= nil then
                            if a.cartridges == 1 then
...
tPolar.createThreadOnTick(func_taserHandler)
...
Citizen.CreateThread(function()
    SetPedMinGroundTimeForStungun(tPolar.getPlayerPed(), 15000)
    while true do
        if IsPedBeingStunned(tPolar.getPlayerPed()) then
            stunGun()
        end
        Wait(100)
    end
        ...
        if c then
            while c do
                ...
                if IsControlJustPressed(1, 38) then
                    c = false
                    if d and d > 0 then
                        TriggerServerEvent("Polar:barbsRippedOutServer", d)
                    end
                    d = 0
...
function tPolar.setStunExitsEarly()
    p = true
end
