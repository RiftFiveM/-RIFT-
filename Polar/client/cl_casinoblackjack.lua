...
RegisterCommand(
    "getcasinotable",
    function()
        local K = GetEntityCoords(tPolar.getPlayerPed())
...
Citizen.CreateThread(function()
    ...
    for O = 0, #G.blackjackTables, 1 do
        ...
        tPolar.createDynamicPed(dealerModel,P.dealerPos,P.dealerHeading,true,nil,nil,75.0,nil,function(Q)
            f[O + 1] = Q
            ...
            tPolar.loadAnimDict(dealerAnimDict)
...
function resetDealerIdle(U)
    ...
    if DoesEntityExist(U) then
        ...
        TaskPlayAnim(tPolar.getPlayerPed(),"anim_casino_b@amb@casino@games@shared@player@","idle_cardgames",1.0,1.0,-1,0)
        RemoveAnimDict(dealerAnimDict)
    end
end
local V = false
                ...
                if tonumber(W) then
                    ...
                    if W > 0 then
                        if not isChairHighRollers(F) and z > B then
                            tPolar.notify("~r~Maximum bet is " ..tostring(getMoneyStringFormatted(B)) .." please purchase a membership for higher bets.")
                        else
                            z = W
                        end
                    end
            ...
            if IsControlJustPressed(0, 201) then
                if tonumber(z) >= 0 then
                    ...
                    elseif isChairHighRollers(F) and z < B then
                        tPolar.notify("~r~Minimum bet is " .. tostring(getMoneyStringFormatted(B)))
                    elseif z > 250000000 then
                        tPolar.notify("~r~Maximum bet is £250,000,000")
                    else
...
Citizen.CreateThread(function()
    while true do
        if sittingAtBlackjackTable and A then
            SetPedCapsule(tPolar.getPlayerPed(), 0.2)
            if IsControlJustPressed(0, 202) and not i then
                ...
                TaskPlayAnim(tPolar.getPlayerPed(), blackjackAnimDictToLoad, "sit_exit_left", 1.0, 1.0, 2500, 0)
...
RMenu.Add('Polarblackjack', 'instructions', RageUI.CreateMenu("", "test",1300, 50,"casinoui_cards_blackjack", "casinoui_cards_blackjack"))
RMenu:Get('Polarblackjack', 'instructions'):SetSubtitle("Blackjack Table")
RMenu.Add('Polarblackjack_high', 'instructions', RageUI.CreateMenu("", "test",1300, 50,"casinoui_cards_blackjack_high", "casinoui_cards_blackjack_high"))
RMenu:Get('Polarblackjack_high', 'instructions'):SetSubtitle("High Rollers Table")


RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('Polarblackjack', 'instructions')) then
        RageUI.DrawContent({ header = true, glare=  false, instructionalButton = true }, function()           
            ...
            end, RMenu:Get('Polarblackjack', 'instructions'))            
        end, function()
        end)
    end
    if RageUI.Visible(RMenu:Get('Polarblackjack_high', 'instructions')) then
        RageUI.DrawContent({ header = true, glare=  false, instructionalButton = true }, function()           
            RageUI.FakeButton("test", "The aim of Blackjack is to beat the Dealer's hand without going over 21. This game uses four 52           card decks, which are shuffled at the start of every hand.                                                                                    The dealer will stand on soft 17.", { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
            end, RMenu:Get('Polarblackjack_high', 'instructions'))            
        end, function()
            ---Panels
        end)
    end
...
function showHowToBlackjack(flag)
    ...
    if not isChairHighRollers(a) then
        RageUI.Visible(RMenu:Get('Polarblackjack', 'instructions'), flag)
    else 
        RageUI.Visible(RMenu:Get('Polarblackjack_high', 'instructions'), flag)
    end
end
...
function goToBlackjackSeat(an)
    ...
    c = an
    fVar3 = blackjack_func_217(tPolar.getPlayerPed(), blackjack_func_218(c, 0), 1)
    fVar4 = blackjack_func_217(tPolar.getPlayerPed(), blackjack_func_218(c, 1), 1)
    fVar5 = blackjack_func_217(tPolar.getPlayerPed(), blackjack_func_218(c, 2), 1)
    ...
    TaskGoStraightToCoord(tPolar.getPlayerPed(), ao.x, ao.y, ao.z, 1.0, 5000, ap, 0.01)
...
function getNewCardFromMachine(nextCard, Y, ab)
    local aR = tPolar.loadModel(nextCard)
...
function requestCard()
    ...
    local aW = blackjack_func_215(F)
    local aX = NetworkCreateSynchronisedScene(aV.x, aV.y, aV.z, aW.x, aW.y, aW.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(tPolar.getPlayerPed(),aX,"anim_casino_b@amb@casino@games@blackjack@player","request_card",4.0,-2.0,13,16,1148846080,0)
    NetworkStartSynchronisedScene(aX)
...
function putBetOnTable()
    ...
    local aW = blackjack_func_215(F)
    local aX = NetworkCreateSynchronisedScene(aV.x, aV.y, aV.z, aW.x, aW.y, aW.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(tPolar.getPlayerPed(),aX,"anim_casino_b@amb@casino@games@blackjack@player",getAnimNameFromBet(100),4.0,-2.0,13,16,1148846080,0)
    NetworkStartSynchronisedScene(aX)
    SetTimeout(3000,function()
        E = true
    end)
...
function angryIBust()
    ...
    local aW = blackjack_func_215(F)
    local aX = NetworkCreateSynchronisedScene(aV.x, aV.y, aV.z, aW.x, aW.y, aW.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(tPolar.getPlayerPed(),aX,"anim_casino_b@amb@casino@games@shared@player@","reaction_terrible_var_01",4.0,-2.0,13,16,1148846080,0)
    NetworkStartSynchronisedScene(aX)
    SetTimeout(5000,function()
        E = true
    end)
...
function angryILost()
    ...
    local aW = blackjack_func_215(F)
    local aX = NetworkCreateSynchronisedScene(aV.x, aV.y, aV.z, aW.x, aW.y, aW.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(tPolar.getPlayerPed(),aX,"anim_casino_b@amb@casino@games@shared@player@","reaction_bad_var_01",4.0,-2.0,13,16,1148846080,0)
    NetworkStartSynchronisedScene(aX)
    SetTimeout(5000,function()
        E = true
    end)
...
function annoyedIPushed()
    ...
    local aW = blackjack_func_215(F)
    local aX = NetworkCreateSynchronisedScene(aV.x, aV.y, aV.z, aW.x, aW.y, aW.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(tPolar.getPlayerPed(),aX,"anim_casino_b@amb@casino@games@shared@player@","reaction_impartial_var_01",4.0,-2.0,13,16,1148846080,0)
    NetworkStartSynchronisedScene(aX)
    SetTimeout(5000,function()
        E = true
    end)
...
function happyIWon()
    ...
    local aW = blackjack_func_215(F)
    local aX = NetworkCreateSynchronisedScene(aV.x, aV.y, aV.z, aW.x, aW.y, aW.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(tPolar.getPlayerPed(),aX,"anim_casino_b@amb@casino@games@shared@player@","reaction_good_var_01",4.0,-2.0,13,16,1148846080,0)
    NetworkStartSynchronisedScene(aX)
    SetTimeout(5000,function()
        E = true
    end)
