...
local function b()
    ...
    TriggerServerEvent("Polar:startThrowSmokeGrenade", c)
    Citizen.Wait(100)
    ...
    TriggerServerEvent("Polar:endThrowSmokeGrenade", c, g)
end
local function h(i)
...
RegisterNetEvent("Polar:displaySmokeGrenade",function(c, g)
    local l = {coords = g, isDeleted = false}
    tPolar.createArea("smoke_" .. tostring(c),g,250.0,1000.0,k,m,function()end,l)
    Citizen.Wait(60000)
    tPolar.removeArea("smoke_" .. tostring(c))
    if l.handle then
        StopParticleFxLooped(l.handle, false)
        l.handle = nil
        l.isDeleted = true
