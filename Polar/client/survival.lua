...
local n = 0
WeaponNames = {}
local o = module("Polar-weapons", "cfg/weapons")
...
local y = -1
RegisterNetEvent("Polar:setNHSCallerId",function(z)
    y = z
end)

AddEventHandler("Polar:countdownEnded",function()
    f = true
end)
    ...
    while true do
        if IsDisabledControlJustPressed(0, 38) then
            if b and not i then
                ...
                if not tPolar.isPlayerInRedZone() and not tPolar.isPlayerInTurf() then -- and not tPolar.isInPaintball() and not globalInBoxingZone then
                    PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
                    tPolar.notify("~g~NHS have been notified.")
                    TriggerServerEvent('Polar:NHSComaCall')
                    TriggerEvent("Polar:DEATH_SCREEN_NHS_CALLED")
                end
                i = true
            elseif f and b then
                PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
                TriggerEvent("Polar:CLOSE_DEATH_SCREEN")
                tPolar.respawnPlayer()
                i = false
                if y ~= -1 then
                    TriggerServerEvent("Polar:endNHSCall", y)
                end
                TriggerEvent("Polar:respawnKeyPressed")
                TriggerServerEvent('Polar:SendSpawnMenu')
            end
            Wait(1000)
        end
        Wait(0)
    ...
    while true do
        ...
        if IsEntityDead(GetPlayerPed(-1)) and not b then
            ...
            if currentBackpack then
                TriggerEvent('Polar:removeBackpack')
            end
            if not tPolar.isPurge() then
                TriggerServerEvent("Polar:forceStoreWeapons")
            end
            tPolar.ejectVehicle()
            b = true
            j = I
            TriggerServerEvent("Polar:getNumOfNHSOnline")
            local x,y,z = table.unpack(I)
            Polarserver.updatePos({x,y,z})
            Polarserver.updateHealth({0})
            tPolar.setArmour(0)
            Wait(250)
        end
        if h <= 0 then
    ...
    while true do
        if b then
            ...
            if not IsEntityDead(L) then
                ...
                if not IsEntityPlayingAnim(L, a.dict, a.anim, 3) and not tPolar.isUsingStretcher() then
                    if a.dict ~= nil then
                        tPolar.loadAnimDict(a.dict)
...
RegisterNetEvent("Polar:getNumberOfDocsOnline",function(O)
    if not tPolar.isPurge() then
        if tPolar.isPlayerInRedZone() or tPolar.isPlayerInTurf() then
    ...
    if not tPolar.isPurge() then
        if GetVehiclePedIsIn(PlayerPedId(), false) ~= 0 then
            ...
            TriggerEvent('Polar:IsInMoneyComa', true)
            ExecuteCommand('storeallweapons')
            if not tPolar.globalOnPoliceDuty() then
                TriggerServerEvent('Polar:InComa')
            end
            Polarserver.MoneyDrop()
        end
    end
    CreateThread(function()
        local Q = GetGameTimer()
        while tPolar.getKillerInfo().ready == nil do
            Wait(0)
        end
        local R = tPolar.getKillerInfo()
        local S = false
        if R.name == nil then
            S = true
        end
        f = false
        TriggerEvent("Polar:SHOW_DEATH_SCREEN", e, R.name or "N/A", R.user_id or "N/A", R.weapon or "N/A", S)
    end)
    if not tPolar.isPurge() then
...
local T = 0
RegisterNetEvent("Polar:OpenSpawnMenu",function(U)
    DoScreenFadeIn(1000)
    TriggerScreenblurFadeIn(100.0)
    ExecuteCommand("hideui")
    SetPlayerControl(PlayerId(), false, 0)
    local L = PlayerPedId()
    local V = tPolar.getPlayerCoords()
    ...
    TriggerEvent("PolarDEATHUI:openSpawnMenu", W)
end)

AddEventHandler("Polar:respawnButtonClicked",function(X, Y)
    if Y and Y > 0 then
        TriggerServerEvent("Polar:takeAmount", Y)
    end
    local Z = p.spawnLocations[X].coords
    TriggerEvent("Polar:playGTAIntro")
    tPolar.ClearWeapons()
    if tPolar.isHandcuffed() then
...
function tPolar.respawnPlayer()
    ...
    TriggerServerEvent('Polar:playerRespawned')
end

function tPolar.disableComa()
    b = false
    i = false
end

function tPolar.isInComa()
    return b
end

local a0 = false
function tPolar.isTazed(a1)
    return a0 or IsPedBeingStunned(PlayerPedId(), 0) or tPolar.isPedBeingTackled() or isInGreenzone and not globalOnPoliceDuty and not a1
end
function tPolar.isTazedByRevive()
    return a0
end
RegisterNetEvent("TriggerTazer",function()
    ...
    local L = PlayerPedId()
    tPolar.loadClipSet("move_m@drunk@verydrunk")
...
Citizen.CreateThread(function()
    ...
    while true do
        Citizen.Wait(0)
        if IsEntityDead(PlayerPedId()) and not tPolar.isPedScriptGuidChanging() then
            Citizen.Wait(500)
            ...
            local ab = tPolar.getPedServerId(aa)
            if inOrganHeist then
                TriggerServerEvent("Polar:diedInOrganHeist", ab)
                tPolar.setDeathInOrganHeist()
            end
            local ac = false
            local ad = 0
            ...
            R.source = ab
            R.user_id = tPolar.getUserId(ab)
            R.weapon = tostring(a9)
            R.ready = true
            if GetGameTimer() < k + 10000 then
                local ab = tPolar.getPedServerId(aa)
                TriggerServerEvent("Polar:onPlayerKilled", "killed", ab, a9, ac, ad)
                n = 0
            elseif GetGameTimer() < k + l then
                local ab = tPolar.getPedServerId(aa)
                l = 0
                TriggerServerEvent("Polar:onPlayerKilled", "finished off", ab, a9)
                n = 0
            end
            a7 = nil
            a6 = nil
