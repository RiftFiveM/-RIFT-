...
local j = false
function tPolar.isPlayerHidingInBoot()
    return j
end
local function k(vehicle, l)
    if tPolar.isHandcuffed() or tPolar.isTazed() then
        return false
    end
    ...
    if l and #(tPolar.getPlayerCoords() - l) > 1.0 then
        return false
    end
    return true
end
Citizen.CreateThread(
    function()
        ...
        while true do
            ...
            local q = tPolar.getPlayerVehicle()
            local r = tPolar.getClosestVehicle(7.0)
            local s = GetVehicleClass(r)
            local t = GetVehicleDoorLockStatus(r)
            if q == 0 and a[s] and GetEntityHealth(tPolar.getPlayerPed()) > 102 and not noclipActive then
                if r and r ~= 0 then
                    o = 0
                    ...
                    if w < 2 then
                        ...
                        if IsDisabledControlJustReleased(1, 137) and k(vehicle) then
                            if not b(vehicle) then
                                ...
                                tPolar.startCircularProgressBar("",2000,nil,function()
                                end)
                                if k(vehicle, l) then
                                    if t <= 1 then
                                        tPolar.setCanAnim(false)
                                        tPolar.setWeapon(p, "WEAPON_UNARMED", true)
                                        j = true
                                        ...
                                        Wait(100)
                                        tPolar.loadAnimDict("timetable@floyd@cryingonbed@base")
                                        ...
                                        while z and j do
                                            ...
                                            if IsDisabledControlPressed(0, 177) then
                                                if GetVehicleDoorLockStatus(r) <= 1 then
                                                    ...
                                                    tPolar.notify("~r~Vehicle is locked, cannot get in boot.")
                                                end
                                            end
                                        ...
                                        tPolar.notify("~r~Vehicle is locked, cannot get out of boot.")
                                    end
                                else
                                    tPolar.notify("~r~You and the vehicle must remain stationary to get in.")
                                end
                            else
                                tPolar.notify("~r~Someone is already in this boot.")
                            end
                        end
                    else
                        ClearHelp(true)
...
RegisterNetEvent("Polar:removeHiddenInBoot",function()
    j = false
end)
