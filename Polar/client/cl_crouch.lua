...
local function _()
    ...
    local o = PlayerPedId()
    if not n(o) or IsEntityInWater(o) or not tPolar.canAnim() then
        return
    end
    i = true
    if Pointing then
    ...
    tPolar.loadAnimDict("move_crawl")
    tPolar.loadAnimDict("move_crawlprone2crawlfront")
    if M(o) then
